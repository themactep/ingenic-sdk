...
static int sensor_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip) {
	...
	if (reset_gpio != -1) {
		...
		if (!ret) {
			...
			ISP_WARNING("gpio request fail %d\n", reset_gpio);
		}
	}
	if (pwdn_gpio != -1) {
...
static int sensor_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	if (data_interface == TX_SENSOR_DATA_INTERFACE_DVP) {
		...
		wsize = &sensor_win_sizes[1];
		sensor_attr.max_integration_time_native = 749, sensor_attr.integration_time_limit = 749,
		sensor_attr.total_width = 2304, sensor_attr.total_height = 750, sensor_attr.max_integration_time = 749,
