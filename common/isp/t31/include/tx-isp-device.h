...
struct tx_isp_subdev_video_ops {
	...
	int (*link_setup)(const struct tx_isp_subdev_pad *local, const struct tx_isp_subdev_pad *remote, u32 flags);
};
...
#define tx_isp_call_module_notify(ent, args...)                                                                        \
	(!(ent) ? -ENOENT : (((ent)->notify) ? (ent)->notify(((ent)->parent), ##args) : -ENOIOCTLCMD))

#define tx_isp_call_subdev_notify(ent, args...)                                                                        \
	(!(ent) ? -ENOENT : (((ent)->module.notify) ? ((ent)->module.notify(&((ent)->module), ##args)) : -ENOIOCTLCMD))

#define tx_isp_call_subdev_event(ent, args...)                                                                         \
	(!(ent) ? -ENOENT : (((ent)->event) ? (ent)->event((ent), ##args) : -ENOIOCTLCMD))

#define tx_isp_subdev_call(sd, o, f, args...)                                                                          \
	(!(sd) ? -ENODEV : (((sd)->ops->o && (sd)->ops->o->f) ? (sd)->ops->o->f((sd), ##args) : -ENOIOCTLCMD))
...
#define tx_isp_sd_readl(sd, reg) tx_isp_readl(((sd)->base), reg)
#define tx_isp_sd_writel(sd, reg, value) tx_isp_writel(((sd)->base), reg, value)
