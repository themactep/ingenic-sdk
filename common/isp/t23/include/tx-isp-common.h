...
enum tx_sensor_csi_fmt {
    ...
    TX_SENSOR_RAW10,
    TX_SENSOR_RAW12,
    TX_SENSOR_YUV422 = 7, /**> This mode requires the use of specific sensor drivers(such as TP2850) and bin files*/
};
...
enum tx_isp_notification {
    ...
    TX_ISP_EVENT_USER_SIWTCH_GPIO,
    TX_ISP_EVENT_MSCALER_ALL_DONE,
#endif
...
#ifdef CONFIG_MULTI_SENSOR
struct tx_isp_double_switch_attr {
    unsigned short switch_gpio;
    uint16_t Msensor_gstate;
    uint16_t Ssensor_gstate;
};

struct tx_isp_treble_switc_attr {
    unsigned short switch_gpio[2];
    uint16_t Msensor_gstate[2];
    uint16_t Ssensor_gstate[2];
    uint16_t Tsensor_gstate[2];
};

struct tx_isp_user_switch_gpio {
    uint16_t enable;
    uint16_t sensornum;
    union {
        struct tx_isp_double_switch_attr d;
        struct tx_isp_treble_switc_attr t;
    };
};
#endif
...
struct priv_pipe {
    ...
    int (*priv_clearbuf)(void *handle, void *buffer);
    int (*priv_s_stream)(void *handle, int onoff);
    void *priv_vic;
    void *priv_core;
};
