...
#define AUDIO_DRIVER_VERSION "V30-20220416a"
...
static int set_codec_mic_again(struct audio_aic_device *aic, void *data)
{
	...
	switch(again.channel){
		case 1:
			if(1 == mic_mono_channel){
				ret = aic->livingcodec->record->set_again(MONO_LEFT,again.gain);
				aic->codec_mic_info->alc_en_l = 0;
				aic->codec_mic_info->lagain   = again.gain;
			}
			else if (2 == mic_mono_channel){
				ret = aic->livingcodec->record->set_again(MONO_RIGHT,again.gain);
...
static int set_codec_mic_dgain(struct audio_aic_device *aic, void *data)
{
	...
	ret = aic->livingcodec->record->set_dgain(dgain.channel,dgain.gain);

	if (dgain.channel == MONO_LEFT)
		aic->codec_mic_info->ldgain = dgain.gain;
	else if (dgain.channel == MONO_RIGHT)
		aic->codec_mic_info->rdgain = dgain.gain;
...
static int set_codec_mic_alc_gain(struct audio_aic_device *aic, void *data)
{
	...
	if (aic->livingcodec->record->set_alc_threshold)
		aic->livingcodec->record->set_alc_threshold(alc.channel,alc.max_gain,alc.min_gain);

	if (alc.channel == MONO_LEFT) {
		aic->codec_mic_info->alc_en_l = 1;
		aic->codec_mic_info->lalc_maxgain = alc.max_gain;
		aic->codec_mic_info->lalc_mingain = alc.min_gain;
	} else if (alc.channel == MONO_RIGHT) {
		aic->codec_mic_info->alc_en_r = 1;
		aic->codec_mic_info->ralc_maxgain = alc.max_gain;
		aic->codec_mic_info->ralc_mingain = alc.min_gain;
	}
		...
		case AUDIO_CMD_GET_GAIN:
			{
				...
				if (vol->channel == MONO_LEFT)
					vol->gain = cur_codec->lagain;
				else if (vol->channel == MONO_RIGHT)
					vol->gain= cur_codec->ragain;
				break;
			}
		...
		case AUDIO_CMD_GET_VOLUME:
			{
				struct volume *vol= (struct volume*)data;
				if (vol->channel == MONO_LEFT)
					vol->gain = cur_codec->ldgain;
				else if (vol->channel == MONO_RIGHT)
					vol->gain = cur_codec->rdgain;
				break;
			}
		...
		case AUDIO_CMD_GET_ALC_GAIN:
			{
				...
				if (vol->channel == MONO_LEFT) {
					vol->max_gain = cur_codec->lalc_maxgain;
					vol->min_gain = cur_codec->lalc_mingain;
				} else if (vol->channel == MONO_RIGHT) {
					vol->max_gain = cur_codec->ralc_maxgain;
					vol->min_gain = cur_codec->ralc_mingain;
				}
				break;
			}
...
static int set_codec_spk_again(struct audio_aic_device *aic, void *data)
{
	...
	ret = aic->livingcodec->playback->set_again(again.channel,again.gain);

	if (again.channel == MONO_LEFT)
		aic->codec_spk_info->lagain = again.gain;
	else if (again.channel == MONO_RIGHT)
		aic->codec_spk_info->ragain = again.gain;
...
static int set_codec_spk_dgain(struct audio_aic_device *aic, void *data)
{
	...
	ret = aic->livingcodec->playback->set_dgain(dgain.channel,dgain.gain);

	if (dgain.channel == MONO_LEFT)
		aic->codec_spk_info->ldgain = dgain.gain;
	else if (dgain.channel == MONO_RIGHT)
		aic->codec_spk_info->rdgain = dgain.gain;
...
//	ret = aic->livingcodec->playback.set_again(&again);
	aic->codec_aec_info->lagain = gain.gain;
	return ret;
}
...
//	ret = aic->livingcodec->playback.set_dgain(dgain);
	aic->codec_aec_info->ldgain = dgain.gain;
	return ret;
}
...
#define EXTERNAL_CODEC 1
	...
	ret = request_irq(aic->aic_irq, aic_irq_handler, IRQF_ONESHOT, "aic_irq", aic);
	if (ret < 0) {
...
//	__i2s_send_rfirst(aic);
	__i2s_send_lfirst(aic);//first send left channel date
...
static int __exit audio_aic_remove(struct platform_device *pdev)
{
	...
	clk_disable(aic->ce_rclock);
	clk_disable(aic->ce_tclock);
