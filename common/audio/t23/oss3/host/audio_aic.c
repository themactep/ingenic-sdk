...
static bool dma_chan_filter(struct dma_chan *chan, void *filter_param)
{
	...
	// printk("%s: %d %d\n", __func__, pipe->dma_type, (int)chan->private);
...
static int set_codec_mic_datatype(struct audio_aic_device *aic, void *data)
{
	...
	//printk("%s: data_type.frame_vsize = %u\n", __func__, data_type.frame_vsize);
...
#else
#define FILE_NAME "/record_test.pcm"
...
static int set_codec_mic_start(struct audio_aic_device *aic, void *data)
{
	...
	loff_t *pos;
	int cnt = 0;
	int *pdata = kmalloc(frm_cnt * 4,GFP_KERNEL);
	if(!pdata)
		return -1;
	...
	vfs_write(fp, pdata, frm_cnt * 4, pos);
	filp_close(fp, NULL);
	kfree(pdata);
	return -1;
}
...
#define EXTERNAL_CODEC 1
	...
	if(excodec_addr != DEFAULT_EXCODEC_ADDR){
		...
		printk("excodec name=%s, excodec_addr=0x%x\n", excodec_name, excodec_addr);
