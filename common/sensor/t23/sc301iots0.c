...
struct tx_isp_sensor_attribute sc301iot_attr = {.name = "sc301iot",
	.chip_id = SENSOR_CHIP_ID,
...
static int sensor_s_stream(struct tx_isp_subdev *sd, int enable) {
	...
	if (enable) {
		if (data_interface == TX_SENSOR_DATA_INTERFACE_MIPI) {
			ret = sensor_write_array(sd, sc301iot_stream_on_mipi);
		} else {
		...
		if (data_interface == TX_SENSOR_DATA_INTERFACE_MIPI) {
			ret = sensor_write_array(sd, sc301iot_stream_off_mipi);
		} else {
			ISP_ERROR("Don't support this Sensor Data interface\n");
		}
...
static int sensor_fsync(struct tx_isp_subdev *sd, struct tx_isp_sensor_fsync *fsync) {
	...
	case 0:
		...
		case 3:
			ISP_INFO("[%s] -> mode 3\n", __func__);
			sensor_read(sd, 0x320e, &val);
			vts = val;
			sensor_read(sd, 0x320f, &val);
			vts = (((vts << 8) | val) << 1);
			sensor_write(sd, 0x320e, ((vts >> 8) & 0xff));
			sensor_write(sd, 0x320f, (vts & 0xff));
			sensor_write(sd, 0x3230, 0x0b);
			sensor_write(sd, 0x3231, 0x16);
			break;
		}
		break;
	}
