...
struct tx_isp_sensor_attribute jxh63p_attr = {.name = "jxh63p",
	.chip_id = SENSOR_CHIP_ID,
...
int jxh63p_read(struct tx_isp_subdev *sd, unsigned char reg, unsigned char *value) {
...
int jxh63p_write(struct tx_isp_subdev *sd, unsigned char reg, unsigned char value) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int jxh63p_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static const struct i2c_device_id jxh63p_id[] = {{"jxh63p", 0}, {}};
