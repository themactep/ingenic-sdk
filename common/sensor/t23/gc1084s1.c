...
static struct regval_list sensor_init_regs_1280_720_50fps_mipi[] = {
	{0x03fe, 0xf0},
	{0x03fe, 0xf0},
	{0x03fe, 0xf0},
	{0x03fe, 0x00},
...
static struct tx_isp_sensor_win_setting sensor_win_sizes[] = {
	{
		...
		.regs = sensor_init_regs_1280_720_50fps_mipi,
	},
};
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps) {
	...
	ret = sensor_read(sd, 0x0d05, &val);
	hts = val;
	ret += sensor_read(sd, 0x0d06, &val);
	hts = ((hts << 8) | val);
...
static int sensor_set_vflip(struct tx_isp_subdev *sd, int enable) {
	...
	switch (enable) {
	case 0:
		ret = sensor_write(sd, 0x0015, 0x00); /*normal*/
		ret = sensor_write(sd, 0x0d15, 0x00);
		break;
	case 1:
		ret = sensor_write(sd, 0x0015, 0x01); /*mirror*/
		ret = sensor_write(sd, 0x0d15, 0x01);
		break;
	case 2:
		ret = sensor_write(sd, 0x0015, 0x02); /*flip*/
		ret = sensor_write(sd, 0x0d15, 0x02);
		break;
	case 3:
		ret = sensor_write(sd, 0x0015, 0x03); /*mirror and flip*/
		ret = sensor_write(sd, 0x0d15, 0x03);
		break;
	}
	return ret;
}
...
static int sensor_fsync(struct tx_isp_subdev *sd, struct tx_isp_sensor_fsync *fsync) {
	...
	case 0:
		...
		case 3:
			...
			ret_val |= val;
			ret_val = ret_val * 2;
			sensor_write(sd, 0x0d41, ret_val >> 8);
...
static int sensor_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	sensor_attr.max_integration_time_native = sensor_attr.total_height - 16;
	sensor_attr.integration_time_limit = sensor_attr.total_height - 16;
	sensor_attr.max_integration_time = sensor_attr.total_height - 16;
