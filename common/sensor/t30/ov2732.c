...
static int sensor_set_vflip(struct tx_isp_subdev *sd, int enable) {
	...
	ret += sensor_read(sd, 0x3820, &val1);
	ret += sensor_read(sd, 0x373d, &val2);
	if (ret < 0) {
		ISP_INFO("set vflip read regs error!\n");
		return ret;
	}
	if (enable) {
		val1 = val1 & 0xcf;
	...
	ret += sensor_write(sd, 0x3820, val1);
	ret += sensor_write(sd, 0x373d, val2);
	ret += sensor_write(sd, 0x3208, 0x13);
	ret += sensor_write(sd, 0x320d, 0x00);
	ret += sensor_write(sd, 0x3208, 0xa3);
	if (ret < 0) {
		ISP_INFO("set vflip write regs error!\n");
		return ret;
	}
	return 0;
}
