...
unsigned int sensor_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again) {
	...
	// Limit Max gain
	ISP_INFO("isp_gain in = %d, again_reg = %d\n", isp_gain, again_reg);
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps) {
	...
	ISP_INFO("### hmax=%d,vmax=%d\n", hmax, vmax);
	sensor->video.attr->total_width = hmax;
	...
	sensor->video.attr->max_integration_time_native = vmax - 9;
	sensor->video.attr->integration_time_limit = vmax - 9;
...
static int sensor_set_vflip(struct tx_isp_subdev *sd, int enable) {
	...
	ISP_INFO("%s, err not support yet\n", __func__);
	return 0;
