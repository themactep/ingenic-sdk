...
struct again_lut sensor_again_lut[] = {{0x0, 0},
	{0x1, 2794},
	{0x2, 5509},
	{0x3, 9011},
	{0x4, 12388},
...
static int sensor_read_array(struct v4l2_subdev *sd, struct regval_list *vals) {
	...
	while (vals->reg_num != SENSOR_REG_END) {
		...
		/*ISP_INFO("read vals->reg_num:0x%02x, vals->value:0x%02x\n",vals->reg_num, val);*/
...
static int sensor_set_integration_time(struct v4l2_subdev *sd, int value) {
	...
	int ret = 0;
	int exposure_thresh = sensor_attr.max_integration_time * 7 / 10;
	unsigned char reg_value = 0xe0;
	if (value <= exposure_thresh)
		reg_value = 0xe0;
	else if (value < sensor_attr.max_integration_time)
		reg_value = 709 - (693 * value / (sensor_attr.max_integration_time));
	else
		reg_value = 0x10;

	sensor_write(sd, 0x331e, reg_value);
...
static int sensor_set_analog_gain(struct v4l2_subdev *sd, int value) {
	...
	ret = sensor_write(sd, 0x3e09, (unsigned char)value);
	if (ret < 0)
		return ret;
