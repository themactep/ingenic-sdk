...
#define SENSOR_CHIP_ID 0x0051
#define SENSOR_CHIP_ID_H (0x24)
...
struct again_lut sensor_again_dcg_lut[] = {
	{0x00, 0x21, 96713},
	{0x00, 0x22, 97770},
	{0x00, 0x23, 98814},
	{0x00, 0x24, 101881},
 ...
 */
/* struct tx_isp_sensor_board_info sensor_board = { */
...
static int sensor_read_array(struct v4l2_subdev *sd, struct regval_list *vals) {
	...
	while (vals->reg_num != SENSOR_REG_END) {
		...
		value = (val[0] << 8) | (val[1]);
		/*ISP_INFO("vals->reg_num:%x, vvals->value:0x%x\n",vals->reg_num, value);*/
		vals++;
	}
	return 0;
}
...
static int sensor_write_array(struct v4l2_subdev *sd, struct regval_list *vals) {
	...
	while (vals->reg_num != SENSOR_REG_END) {
		if (vals->reg_num == SENSOR_REG_DELAY) {
			...
			/*ISP_INFO("vals->reg_num=0x%x,vals->value =0x%x\n",vals->reg_num,vals->value);*/
...
unsigned int sensor_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again) {
	...
	struct again_lut *dcg_lut = sensor_again_dcg_lut;
	while ((lut->gain <= sensor_attr.max_again) || (dcg_lut->gain <= sensor_attr.max_again)) {
		if (isp_gain < 95645) {
			if (isp_gain == 0) {
				tmp_again = 0x1300;
				tmp_dgain = 0x20;
...
static int sensor_set_integration_time(struct v4l2_subdev *sd, int value) {
	sensor_write(sd, 0x3012, value);
	/*[15:0] = 0x16, 0x10 */
	return 0;
}

static int sensor_set_analog_gain(struct v4l2_subdev *sd, int value) {
	/* sensor_write(sd, 0x30B0, 0x1330); */
	/* sensor_write(sd, 0x305e, 0x40); */
	/* sensor_write(sd, 0x3100, 0x00 ); */
	/*ISP_INFO("aaa tmp_again=========0x%x final_again ===0x%x, tmp_dgain ========0x%x,final_dgain ===0x%x, tmp_dcg =0x%x\n",tmp_again,final_again,tmp_dgain,final_dgain,tmp_dcg); */
	if ((final_again != tmp_again) || (final_dgain != tmp_dgain)) {
		sensor_write(sd, 0x30B0, tmp_again);
		sensor_write(sd, 0x305e, tmp_dgain);
		sensor_write(sd, 0x3100, tmp_dcg);
...
static int sensor_g_chip_ident(struct v4l2_subdev *sd, struct v4l2_dbg_chip_ident *chip) {
	...
	if (pwdn_gpio != -1) {
		...
		if (!ret) {
			/* gpio_direction_output(sensor_board.pwdn_gpio, 1); */
			/* msleep(150); */
...
static int sensor_s_register(struct v4l2_subdev *sd, struct v4l2_dbg_register *reg) {
	struct i2c_client *client = v4l2_get_subdevdata(sd);
