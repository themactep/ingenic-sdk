...
static int sensor_set_integration_time(struct v4l2_subdev *sd, int value) {
	...
	ret += sensor_write(sd, 0x3212, 0x01);
	ret += sensor_write(sd, 0x3502, ((unsigned char)(expo & 0x0f)) << 4);
	ret += sensor_write(sd, 0x3501, (unsigned char)((expo >> 4) & 0xff));
	ret += sensor_write(sd, 0x3500, (unsigned char)((expo >> 12) & 0x0f));
	ret += sensor_write(sd, 0x3212, 0x11);
	ret += sensor_write(sd, 0x3212, 0xa1);
	if (ret < 0)
		return ret;
	return 0;
}
...
static int sensor_set_fps(struct tx_isp_sensor *sensor, int fps) {
	...
	ret += sensor_write(sd, 0x3212, 0x00);
	ret += sensor_write(sd, 0x380f, vts & 0xff);
	ret += sensor_write(sd, 0x380e, (vts >> 8) & 0xff);
	ret += sensor_write(sd, 0x3212, 0x10);
	ret += sensor_write(sd, 0x3212, 0xa0);
