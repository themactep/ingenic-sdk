...
#define SENSOR_VERSION "H20240327a"
...
static int sensor_write_array(struct tx_isp_subdev *sd, struct regval_list *vals) {
	...
	while (vals->reg_num != SENSOR_REG_END) {
		if (vals->reg_num == SENSOR_REG_DELAY) {
			...
			ret = sensor_write(sd, vals->reg_num, vals->value);
			ret = sensor_read(sd, vals->reg_num, &value);
			if (ret < 0)
				return ret;
		}
		vals++;
...
#ifdef SENSOR_WDR_2_FRAME
		sensor_attr.max_again_short = xxxx;
		sensor_attr.min_integration_time_short = xx;
		sensor_attr.max_integration_time_short = xx;
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps) {
	...
	case 0:
		sclk = 1500 * 1920 * 25 * 2; /**< HTS * VTS * FPS */
		max_fps = 25;
		break;
	default:
		ret = -1;
	...
	if (newformat > (max_fps << 8) || newformat < (SENSOR_OUTPUT_MIN_FPS << 8)) {
		ISP_ERROR("warn: fps(%d/%d) not in range\n", fps >> 16, fps & 0xffff);
		return -1;
	}
	...
	sensor_write(sd, 0x0340, (unsigned char)(vts >> 8));
	sensor_write(sd, 0x0341, (unsigned char)(vts & 0xff));
