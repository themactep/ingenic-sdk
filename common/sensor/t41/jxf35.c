...
static int sensor_set_expo(struct tx_isp_subdev *sd, int value) {
	...
	if (ag_last != again) {
		...
		/*black sun cancellation strategy*/
		if (value < 0x20) {
			ret += sensor_write(sd, 0x2f, reg_2f | 0x20);
			ret += sensor_write(sd, 0x0c, reg_0c | 0x40);
			ret += sensor_write(sd, 0x80, reg_80 | 0x01);
		} else {
			ret += sensor_write(sd, 0x2f, reg_2f & 0xdf);
			ret += sensor_write(sd, 0x0c, reg_0c & 0xbf);
			ret += sensor_write(sd, 0x80, reg_80 & 0xfe);
		}
	}
