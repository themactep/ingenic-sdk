...
struct tx_isp_sensor_attribute sensor_attr = {
	...
	.max_again = 393216, /* depend on sensor_again_lut sensor adc=0x3f0(64 again) -- 393216 */
	.max_dgain = 0,
...
static int sensor_setting_select(struct tx_isp_subdev *sd, int deboot) {
	...
	case 0:
		...
		sensor_attr.max_integration_time = SENSOR_INIT_30FPS_VTS - 2;
		...
		memcpy((void *)(&(sensor_attr.mipi)), (void *)(&sensor_mipi), sizeof(sensor_attr.mipi));
		break;
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			*((u32 *)0xb3380000) = 0x5;
			...
			ISP_INFO("mis40c1 stream on\n");
		}

	} else {
		ret = sensor_write_array(sd, sensor_stream_off);
		sensor->video.state = TX_ISP_MODULE_INIT;
		ISP_INFO("mis40c1 stream off\n");
	}
...
static int sensor_set_expo(struct tx_isp_subdev *sd, int value) {
	...
	ret += sensor_write(sd, 0x3103, (unsigned char)(gain >> 8 & 0x03));
	ret += sensor_write(sd, 0x3104, (unsigned char)(gain & 0xff));
	...
	if ((gain >= 0x3f0) && (vts >= 0xbb8)) {
		ret += sensor_write(sd, 0x3a13, 0x29);
	} else {
		ret += sensor_write(sd, 0x3a13, 0x3d);
	}
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps) {
	...
	val = 0;
	ret += sensor_read(sd, 0x3107, &val);
	hts = val << 8;
	val = 0;
	ret += sensor_read(sd, 0x3108, &val);
	hts = (hts | val);
	if (0 != ret) {
...
static int sensor_set_vflip(struct tx_isp_subdev *sd, int enable) {
	...
	case 0:
		...
		ret += sensor_write(sd, 0x310a, 0x04);
		ret += sensor_write(sd, 0x310c, 0xa3);
		ret += sensor_write(sd, 0x310e, 0x04);
		ret += sensor_write(sd, 0x3110, 0x03);
		ret += sensor_write(sd, 0x3006, 0x00);
		break;
	case 1:
		ret += sensor_write(sd, 0x3006, 0x02);
		...
		ret += sensor_write(sd, 0x310a, 0x04);
		ret += sensor_write(sd, 0x310c, 0xa3);
		ret += sensor_write(sd, 0x310e, 0x05);
		ret += sensor_write(sd, 0x3110, 0x04);
		ret += sensor_write(sd, 0x3006, 0x00);
		break;
	case 2:
		ret += sensor_write(sd, 0x3006, 0x02);
		...
		ret += sensor_write(sd, 0x310a, 0x05);
		ret += sensor_write(sd, 0x310c, 0xa4);
		ret += sensor_write(sd, 0x310e, 0x04);
		ret += sensor_write(sd, 0x3110, 0x03);
		ret += sensor_write(sd, 0x3006, 0x00);
		break;
