...
struct tx_isp_mipi_bus sensor_mipi_linear_60fps = {
	.mode = SENSOR_MIPI_OTHER_MODE,
...
static struct tx_isp_sensor_win_setting sensor_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code = TISP_VI_FMT_SGRBG12_1X12,
		.colorspace = TISP_COLORSPACE_SRGB,
		.regs = sensor_init_regs_1920_1080_dol2_mipi,
	},
	{
		.width = SENSOR_WIDTH,
		.height = SENSOR_HEIGHT,
...
static int sensor_setting_select(struct tx_isp_subdev *sd, int deboot) {
	...
	case 0:
		...
		sensor_attr.max_integration_time = SENSOR_INIT_30FPS_VTS - 4;
		sensor_attr.total_width = SENSOR_INIT_30FPS_HTS;
		sensor_attr.total_height = SENSOR_INIT_30FPS_VTS;
	...
	case 1:
		...
		sensor_attr.min_integration_time = 1;
		sensor_attr.max_integration_time = ((SENSOR_WDR_INIT_30FPS_VTS - 4) / 17) << 4;
		sensor_attr.total_width = SENSOR_WDR_INIT_30FPS_HTS;
		sensor_attr.total_height = SENSOR_WDR_INIT_30FPS_VTS;
		sensor_attr.integration_time_apply_delay = 2;
		sensor_attr.again_apply_delay = 2;
		sensor_attr.dgain_apply_delay = 0;
		sensor_attr.integration_time_limit = sensor_attr.max_integration_time;
		...
		sensor_attr.max_integration_time_short = (SENSOR_WDR_INIT_30FPS_VTS - 4) / 17;
#endif /* SENSOR_WDR_2_FRAME */
		break;
	case 2:
		wsize = &sensor_win_sizes[2];
		memcpy((void *)(&(sensor_attr.mipi)), (void *)(&sensor_mipi_linear_60fps), sizeof(sensor_attr.mipi));
		sensor_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sensor_attr.min_integration_time = 1;
		sensor_attr.max_integration_time = SENSOR_INIT_60FPS_VTS - 4;
		sensor_attr.total_width = SENSOR_INIT_60FPS_HTS;
		sensor_attr.total_height = SENSOR_INIT_60FPS_VTS;
		sensor_attr.integration_time_apply_delay = 2;
		sensor_attr.again_apply_delay = 2;
		sensor_attr.dgain_apply_delay = 0;
		sensor_attr.integration_time_limit = sensor_attr.max_integration_time;
...
static int mis20s1_temp_status(struct tx_isp_subdev *sd, int *again) {
	...
	int ret = ISP_SUCCESS;
	ret += sensor_read(sd, 0x3502, &temEn);
	if ((temEn & 0x01) == 1) {
		ret += sensor_read(sd, 0x3504, &tmper);
		temperature = tmper;
		ret += sensor_read(sd, 0x3505, &tmper);
		temperature = (temperature << 8) | tmper;

		if (temperature > 0xb05) {
			ret += sensor_write(sd, 0x410f, 0x3b);
		...
		if (temperature > 0xa38) { // 65 start DPC en
			if (*again < 0x380) {
				...
				ret += sensor_write(sd, 0x4402, 0x3f);
				ret += sensor_write(sd, 0x3008, 0x01);
			}
		} else {
			ret += sensor_write(sd, 0x4402, 0x00);
...
#else
		ret += sensor_write(sd, 0x310c, 0x01);
#endif
	} else {
		ret += sensor_write(sd, 0x310c, 0x00);
	}

	again = again & 0x0fff;
	mis20s1_temp_status(sd, &again);

	ret += sensor_write(sd, 0x3100, (unsigned char)((exp >> 8) & 0xff));
	ret += sensor_write(sd, 0x3101, (unsigned char)(exp & 0xff));
	ret += sensor_write(sd, 0x3106, (unsigned char)((again >> 8) & 0x03));
	ret += sensor_write(sd, 0x3107, (unsigned char)(again & 0xff));
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps) {
	...
	case 0:
		...
		break;
	case 1:
		sclk = SENSOR_WDR_INIT_30FPS_HTS * SENSOR_WDR_INIT_30FPS_VTS *
			SENSOR_WDR_OUTPUT_INIT_FPS; /**< HTS * VTS * FPS */
	...
	val = 0;
	ret += sensor_read(sd, 0x3110, &val);
	hts = val << 8;
	val = 0;
	ret += sensor_read(sd, 0x3111, &val);
	hts = (hts | val);
	if (0 != ret) {
	...
	ret += sensor_write(sd, 0x310f, (unsigned char)(vts & 0xff));
