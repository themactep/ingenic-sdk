...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps) {
	...
	sensor->video.attr->max_integration_time_native =
		(sensor->info.default_boot == 0) ? (2 * vts - 10) : (vts / 2 - short_time / 2 - 5);
	...
	sensor->video.attr->max_integration_time =
		(sensor->info.default_boot == 0) ? (2 * vts - 10) : (vts / 2 - short_time / 2 - 5);
	if (sensor->info.default_boot == 1)
		sensor->video.attr->max_integration_time_short = short_time / 2 - 4;
	ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
	if (ret) {
...
static int sensor_attr_check(struct tx_isp_subdev *sd) {
	...
	case 0:
		...
		sensor_attr.mipi.clk = 792, sensor_attr.min_integration_time = 3;
		sensor_attr.min_integration_time_native = 3, sensor_attr.total_width = 1556;
	...
	case 1:
		...
		sensor_attr.mipi.clk = 1080, sensor_attr.min_integration_time = 1;
		sensor_attr.min_integration_time_short = 1;
		sensor_attr.min_integration_time_native = 1, sensor_attr.total_width = 2400;
	...
	case 2:
		...
		memcpy(&sensor_attr.mipi, &sensor_mipi_linear, sizeof(sensor_mipi_linear));
		sensor_attr.mipi.clk = 1080, sensor_attr.min_integration_time = 3;
		sensor_attr.min_integration_time_native = 3, sensor_attr.total_width = 2400;
