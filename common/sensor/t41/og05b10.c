...
static int sensor_setting_select(struct tx_isp_subdev *sd, int deboot) {
	...
	case 0:
		...
		sensor_attr.min_integration_time = 6;
		sensor_attr.max_integration_time = 2128 - 30;
		sensor_attr.total_width = 752;
		sensor_attr.total_height = 2128;
...
#endif /* SENSOR_WDR_2_FRAME */
		memcpy((void *)(&(sensor_attr.mipi)), (void *)(&og05b10_30fps_mipi_linear), sizeof(sensor_attr.mipi));
		break;
	default:
...
static int sensor_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init) {
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			ISP_INFO("og05b10 stream on\n");
		}

	} else {
		ret = sensor_write_array(sd, sensor_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		ISP_INFO("og05b10 stream off\n");
	}
...
static int sensor_set_expo(struct tx_isp_subdev *sd, int value) {
	...
	ret += sensor_write(sd, 0x3509, (unsigned char)((again & 0x0f) << 4));
	ret += sensor_write(sd, 0x3508, (unsigned char)(((again >> 4) & 0x0f)));
