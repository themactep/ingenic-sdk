...
static int sensor_setting_select(struct tx_isp_subdev *sd, int deboot) {
	...
	case 0:
		...
		sensor_attr.max_again = 262080;
		sensor_attr.min_integration_time = 8;
		sensor_attr.max_integration_time = 2520 - 8;
		sensor_attr.total_width = 2976 * 2;
		sensor_attr.total_height = 2520;
...
static int sensor_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init) {
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			ISP_INFO("os08c10 stream on\n");
		}

	} else {
		ret = sensor_write_array(sd, sensor_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		ISP_INFO("os08c10 stream off\n");
	}
...
static int sensor_set_analog_gain(struct tx_isp_subdev *sd, int value) {
	...
	ret += sensor_write(sd, 0x3508, (unsigned char)((value >> 8) & 0x3f));
...
static int sensor_set_vflip(struct tx_isp_subdev *sd, int enable) {
	...
	switch (enable) {
	case 0:
		sensor_write(sd, 0x3218, 0x08);
		sensor_write(sd, 0x368b, 0xb0);
		sensor_write(sd, 0x3811, 0x0f);
		sensor_write(sd, 0x3813, 0x10);
		sensor_write(sd, 0x3820, 0x80);
		sensor_write(sd, 0x3821, 0x04);
		break;
	case 1:
		sensor_write(sd, 0x3218, 0x00);
		sensor_write(sd, 0x368b, 0x30);
		sensor_write(sd, 0x3811, 0x10);
		sensor_write(sd, 0x3813, 0x10);
		sensor_write(sd, 0x3820, 0x80);
		sensor_write(sd, 0x3821, 0x00);
		break;
	case 2:
		sensor_write(sd, 0x3218, 0x08);
		sensor_write(sd, 0x368b, 0xb0);
		sensor_write(sd, 0x3811, 0x0f);
		sensor_write(sd, 0x3813, 0x0f);
		sensor_write(sd, 0x3820, 0x84);
		sensor_write(sd, 0x3821, 0x04);
		break;
	case 3:
		sensor_write(sd, 0x3218, 0x00);
