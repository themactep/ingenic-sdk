...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps) {
	...
	sensor->video.attr->max_integration_time_native =
		(sensor->info.default_boot == 0) ? (2 * vts - 8) : (vts / 2 - short_time / 2 - 4);
	...
	sensor->video.attr->max_integration_time =
		(sensor->info.default_boot == 0) ? (2 * vts - 8) : (vts / 2 - short_time / 2 - 4);
	if (sensor->info.default_boot == 1)
		sensor->video.attr->max_integration_time_short = short_time / 2 - 3;
	ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
	if (ret) {
...
static int sensor_attr_check(struct tx_isp_subdev *sd) {
	...
	case 0:
		...
		sensor_attr.mipi.clk = 792, sensor_attr.min_integration_time = 1;
		sensor_attr.min_integration_time_native = 1, sensor_attr.total_width = 2200;
	...
	case 1:
		...
		sensor_attr.mipi.clk = 825, sensor_attr.min_integration_time = 1;
