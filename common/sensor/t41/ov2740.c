...
static int sensor_frame_sync(struct tx_isp_subdev *sd, struct tx_isp_frame_sync *sync)
{
	...
	private_msleep(1);
	sensor_write(sd, 0x3816, 0x00);
	private_msleep(1);
	sensor_write(sd, 0x3817, 0x00);
	private_msleep(1);
	sensor_write(sd, 0x3818, 0x00);
	private_msleep(1);
	sensor_write(sd, 0x3819, 0x01);
	private_msleep(1);
	sensor_write(sd, 0x3001, 0x04);
	private_msleep(1);
	sensor_write(sd, 0x3007, 0x44);
	private_msleep(1);
...
static int sensor_attr_check(struct tx_isp_subdev *sd) {
	...
	case 0:
		...
		sensor_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI, sensor_attr.max_integration_time_native = 1340,
		sensor_attr.integration_time_limit = 1340, sensor_attr.total_width = 2160,
		sensor_attr.total_height = 1344, sensor_attr.max_integration_time = 1340,
		sensor_attr.one_line_expr_in_us = 30;
		sensor_attr.again = 0x80;
		sensor_attr.integration_time = 0x4440;
		break;
	case 1:
		wsize = &sensor_win_sizes[1];
		memcpy(&sensor_attr.mipi, &sensor_mipi_linear_60fps, sizeof(sensor_mipi_linear_60fps));
		sensor_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sensor_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI, sensor_attr.max_integration_time_native = 1340,
		sensor_attr.integration_time_limit = 1340, sensor_attr.total_width = 0x538,
		sensor_attr.total_height = 1344, sensor_attr.max_integration_time = 1340,
		sensor_attr.one_line_expr_in_us = 30;
		sensor_attr.again = 0x80;
		sensor_attr.integration_time = 0x4440;
