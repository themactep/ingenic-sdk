...
struct tx_isp_mipi_bus sensor_1920_1080_mipi = {
...
struct tx_isp_mipi_bus sensor_1280_720_mipi = {
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 408,
	.lans = 2,
	.settle_time_apative_en = 0,
...
struct tx_isp_mipi_bus sensor_640_360_mipi = {
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 216,
	.lans = 2,
	.settle_time_apative_en = 0,
...
static int sensor_write_array(struct tx_isp_subdev *sd, struct regval_list *vals) {
	...
	while (vals->reg_num != SENSOR_REG_END) {
		if (vals->reg_num == SENSOR_REG_DELAY) {
			...
			ISP_INFO("Write:{0x%2x, 0x%2x}\n", vals->reg_num, vals->value);
			ret = sensor_read(sd, vals->reg_num, &val);
			ISP_INFO("Read :{0x%2x, 0x%2x}\n", vals->reg_num, val);
			if (ret < 0)
				return ret;
		}
		vals++;
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps) {
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
	unsigned int sclk = SENSOR_SUPPORT_1920_1080_60FPS_SCLK_MIPI;
	unsigned int hts = 0;
	unsigned int vts = 0;
	unsigned char val = 0;
	...
	case TX_SENSOR_RES_200:
		sclk = SENSOR_SUPPORT_1920_1080_60FPS_SCLK_MIPI;
		break;
	case TX_SENSOR_RES_100:
		sclk = SENSOR_SUPPORT_1280_720_60FPS_SCLK_MIPI;
		break;
	case TX_SENSOR_RES_30:
		sclk = SENSOR_SUPPORT_640_360_60FPS_SCLK_MIPI;
		break;
	default:
		ret = -1;
	...
	if (sensor_resolution == TX_SENSOR_RES_30)
		hts = ((hts << 8) | val) << 1;
	else
		hts = ((hts << 8) | val) << 2;
...
static int sensor_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	switch (sensor_resolution) {
	case TX_SENSOR_RES_200:
		memcpy((void *)(&(sensor_attr.mipi)), (void *)(&sensor_1920_1080_mipi), sizeof(sensor_1920_1080_mipi));
		wsize = &sensor_win_sizes[0];
		sensor_attr.max_integration_time_native = 1125 - 4;
		sensor_attr.integration_time_limit = 1125 - 4;
		sensor_attr.total_width = 640 * 4;
		sensor_attr.total_height = 1125;
		sensor_attr.max_integration_time = 1125 - 4;
		break;
	case TX_SENSOR_RES_100:
		memcpy((void *)(&(sensor_attr.mipi)), (void *)(&sensor_1280_720_mipi), sizeof(sensor_1280_720_mipi));
		wsize = &sensor_win_sizes[1];
		sensor_attr.max_integration_time_native = 1133 - 4;
		sensor_attr.integration_time_limit = 1133 - 4;
		sensor_attr.total_width = 600 * 4;
		sensor_attr.total_height = 1133;
		sensor_attr.max_integration_time = 1133 - 4;
		break;
	case TX_SENSOR_RES_30:
		memcpy((void *)(&(sensor_attr.mipi)), (void *)(&sensor_640_360_mipi), sizeof(sensor_640_360_mipi));
		wsize = &sensor_win_sizes[1];
		sensor_attr.max_integration_time_native = 400 - 4;
		sensor_attr.integration_time_limit = 400 - 4;
		sensor_attr.total_width = 1800 * 2;
		sensor_attr.total_height = 400;
		sensor_attr.max_integration_time = 400 - 4;
		break;
	default:
		ret = -1;
		ISP_ERROR("Now we do not support this sensor_resolution!!!\n");
