...
/* VGA@70fps: insmod sensor_jxf37s1_t40.ko data_type=0 data_interface=1 sensor_resolution=30 */
/* 480x270@110fps: insmod sensor_jxf37s1_t40.ko data_type=0 data_interface=1 sensor_resolution=13 */
/* 1080p@25fps: insmod sensor_jxf37s1_t40.ko data_type=0 data_interface=1 sensor_resolution=200  */
/* DOL 1080p@15fps: insmod sensor_jxf37s1_t40.ko data_interface=1 data_type=2  */
...
static int sensor_set_expo(struct tx_isp_subdev *sd, int value) {
	...
	if (ag_last != again) {
		...
		if ((((ag_last < 0x10) && (again >= 0x10)) || ((ag_last >= 0x10) && (again < 0x10))) ||
			(ag_last == -1)) {
			/* ret += sensor_write(sd, 0x2f, tmp1); */
			/* ret += sensor_write(sd, 0x0c, tmp2); */
			/* ret += sensor_write(sd, 0x82, tmp3); */
			...
			if (ret < 0)
				return -1;
			if ((val & 0x80) == 0) {
				sensor_write(sd, 0xc0, 0x2f);
				sensor_write(sd, 0xc1, tmp1);
				sensor_write(sd, 0xc2, 0x0c);
...
static int sensor_attr_check(struct tx_isp_subdev *sd) {
	...
	case 0:
		...
		sensor_attr.again = 0;
		sensor_attr.integration_time = 0xff02;
		break;
	case 1:
		memcpy((void *)(&(sensor_attr.mipi)), (void *)(&sensor_mipi), sizeof(sensor_mipi));
		wsize = &sensor_win_sizes[1];
