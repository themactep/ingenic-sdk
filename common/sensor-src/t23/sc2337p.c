...
static int sc2337p_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int sc2337p_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	if (sensor_max_fps == 30) {
		...
		sc2337p_attr.min_integration_time = 1, sc2337p_attr.min_integration_time_native = 1,
