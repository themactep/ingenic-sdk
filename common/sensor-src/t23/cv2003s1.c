...
static int sensor_set_expo(struct tx_isp_subdev *sd, int value) {
	...
	ret += sensor_write(sd, 0x3048, (unsigned char)((expo & 0x0ff)));
...
static int sensor_s_stream(struct tx_isp_subdev *sd, int enable) {
	...
	if (enable) {
		...
		ISP_INFO("cv2003s1 stream on\n");

	} else {
		ret = sensor_write_array(sd, sensor_stream_off_mipi);
		ISP_INFO("cv2003s1 stream off\n");
	}
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps) {
	...
	ret += sensor_write(sd, 0x3021, (unsigned char)(vts >> 8));
	if (0 != ret) {
...
static int sensor_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip) {
	...
	ISP_INFO("cv2003s1 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
...
static int sensor_fsync(struct tx_isp_subdev *sd, struct tx_isp_sensor_fsync *fsync) {
	...
	case 0:
		...
		case 3:
			...
			sensor_write(sd, 0x307a, 0x02);
			sensor_write(sd, 0x306d, 0x0f);
			sensor_write(sd, 0x3078, 0x00);
			sensor_write(sd, 0x3000, 0x00);
			break;
		}
		break;
	}
...
static int sensor_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	sensor_attr.max_integration_time_native = sensor_attr.total_height - 7;
	sensor_attr.integration_time_limit = sensor_attr.total_height - 7;
	sensor_attr.max_integration_time = sensor_attr.total_height - 7;
