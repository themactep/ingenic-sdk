...
static int sensor_set_expo(struct tx_isp_subdev *sd, int value) {
	...
	ret += sensor_write(sd, 0x0e, (unsigned char)((it >> 8) & 0xff));
	ret += sensor_write(sd, 0x0f, (unsigned char)(it & 0xff));
...
static int sensor_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sensor_write(sd, 0xfd, 0x01);
	ret += sensor_write(sd, 0x24, (unsigned char)(value & 0xff));
	ret += sensor_write(sd, 0x01, 0x01);
	if (ret < 0)
		return ret;
...
static int sensor_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init) {
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_RUNNING) {
			...
			ISP_INFO("os04j10 stream on\n");
		}
	} else {
		ret = sensor_write_array(sd, sensor_stream_off_mipi);
		ISP_INFO("os04j10 stream off\n");
	}
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps) {
	...
	ret += sensor_read(sd, 0x2a, &val);
	hts = val;
	ret += sensor_read(sd, 0x2b, &val);
	hts = ((hts << 8) | val) << 1;

	ret += sensor_read(sd, 0x2f, &val);
	vts_old = val;
	ret += sensor_read(sd, 0x30, &val);
	...
	ISP_INFO("-----hts=%d,vts=%d,vb=%d,fps=%d-----", hts, vts, vb, (((fps >> 16) & 0xffff) / (fps & 0xffff)));
	ret = sensor_write(sd, 0x15, (unsigned char)(vb & 0xff));
	ret += sensor_write(sd, 0x14, (unsigned char)(vb >> 8));
...
static int sensor_set_hvflip(struct tx_isp_subdev *sd, int enable) {
	...
	ret += tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
	ret += sensor_write(sd, 0x12, value);
	ret += sensor_write(sd, 0xfe, 0x02);
...
static int sensor_attr_check(struct tx_isp_subdev *sd) {
	...
	case 0:
		...
		sensor_attr.max_integration_time_native = 1489 - 8;
		sensor_attr.integration_time_limit = 1489 - 8;
		sensor_attr.total_width = 1208;
		sensor_attr.total_height = 1489;
		sensor_attr.max_integration_time = 1489 - 8;
		sensor_attr.again = 0x80;
		sensor_attr.integration_time = 0x640;
		break;
	default:
...
static int sensor_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip) {
	...
	ISP_INFO("os04j10 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
