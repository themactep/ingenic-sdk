...
int os04l10_write(struct tx_isp_subdev *sd, uint16_t reg, unsigned char value) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int os04l10_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init) {
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_RUNNING) {
			...
			ISP_INFO("os04l10 stream on\n");
		}
	} else {
		ret = os04l10_write_array(sd, os04l10_stream_off_mipi);
		ISP_INFO("os04l10 stream off\n");
	}
...
static int os04l10_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
	...
	ISP_INFO("os04l10 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
...
static int os04l10_probe(struct i2c_client *client, const struct i2c_device_id *id) {
...
static const struct i2c_device_id os04l10_id[] = {{"os04l10", 0}, {}};
