...
#ifdef SENSOR_I2C_REG_8BIT
int sc235ai_read(struct tx_isp_subdev *sd, unsigned char reg, unsigned char *value) {
...
int sc235ai_write(struct tx_isp_subdev *sd, unsigned char reg, unsigned char value) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int sc235ai_read(struct tx_isp_subdev *sd, uint16_t reg, unsigned char *value) {
	int ret;
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int sc235ai_write(struct tx_isp_subdev *sd, uint16_t reg, unsigned char value) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
#endif /* SENSOR_WDR_2_FRAME */
		memcpy((void *)(&(sc235ai_attr.mipi)),
			(void *)(&sc235ai_mipi_1920_1056_30fps_linear),
			sizeof(sc235ai_attr.mipi));
		break;
	case 1:
...
#endif /* SENSOR_WDR_2_FRAME */
		memcpy((void *)(&(sc235ai_attr.mipi)),
			(void *)(&sc235ai_mipi_1920_1080_30fps_linear),
			sizeof(sc235ai_attr.mipi));
		break;
	default:
...
static int sc235ai_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int sc235ai_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init) {
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			ISP_INFO("sc235ai stream on\n");
		}

	} else {
		ret = sc235ai_write_array(sd, sc235ai_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		ISP_INFO("sc235ai stream off\n");
	}
...
static int sc235ai_probe(struct i2c_client *client, const struct i2c_device_id *id) {
...
static const struct i2c_device_id sc235ai_id[] = {{"sc235ai", 0}, {}};
