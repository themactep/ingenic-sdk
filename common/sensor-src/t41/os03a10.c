...
#define SENSOR_VERSION "H20240129a"
...
unsigned int os03a10_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again) {
	...
	while (lut->gain <= os03a10_attr.max_again) {
		if (isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if ((lut->gain == os03a10_attr.max_again) && (isp_gain >= lut->gain)) {
				*sensor_again = lut->value;
				return lut->gain;
			}
		}
...
#ifdef SENSOR_I2C_REG_8BIT
int os03a10_read(struct tx_isp_subdev *sd, unsigned char reg, unsigned char *value) {
...
int os03a10_write(struct tx_isp_subdev *sd, unsigned char reg, unsigned char value) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int os03a10_read(struct tx_isp_subdev *sd, uint16_t reg, unsigned char *value) {
	int ret;
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int os03a10_write(struct tx_isp_subdev *sd, uint16_t reg, unsigned char value) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int os03a10_setting_select(struct tx_isp_subdev *sd, int deboot) {
	...
	case 0:
		...
		os03a10_attr.max_dgain = 0;
		os03a10_attr.max_again = 259142;
		os03a10_attr.min_integration_time = 2;
		os03a10_attr.max_integration_time = 1624 - 8;
		os03a10_attr.total_width = 1476;
		os03a10_attr.total_height = 1624;
		os03a10_attr.integration_time_apply_delay = 2;
		os03a10_attr.again_apply_delay = 2;
		os03a10_attr.dgain_apply_delay = 0;
		os03a10_attr.integration_time_limit = os03a10_attr.max_integration_time;
		os03a10_attr.max_integration_time_native = os03a10_attr.max_integration_time;
		os03a10_attr.min_integration_time_native = os03a10_attr.min_integration_time;
		os03a10_attr.expo_fs = 1;
#ifdef SENSOR_WDR_2_FRAME
		os03a10_attr.max_again_short = xxxx;
		os03a10_attr.min_integration_time_short = xx;
		os03a10_attr.max_integration_time_short = xx;
...
static int os03a10_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int os03a10_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init) {
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			ISP_INFO("os03a10 stream on\n");
		}

	} else {
		ret = os03a10_write_array(sd, os03a10_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		ISP_INFO("os03a10 stream off\n");
	}
...
static int os03a10_set_expo(struct tx_isp_subdev *sd, int value) {
	...
	ret += os03a10_write(sd, 0x3508, (unsigned char)((again >> 8) & 0xff));
...
static int os03a10_set_integration_time(struct tx_isp_subdev *sd, int value) {
	...
	ret += os03a10_write(sd, 0x3501, (unsigned char)((value >> 8) & 0xff));
	ret += os03a10_write(sd, 0x3502, (unsigned char)(value & 0xff));
...
static int os03a10_set_analog_gain(struct tx_isp_subdev *sd, int value) {
	...
	ret += os03a10_write(sd, 0x3508, (unsigned char)((value >> 8) & 0xff));
	ret += os03a10_write(sd, 0x3509, (unsigned char)(value & 0xff));
...
#ifdef SENSOR_MIR_FLIP
static int os03a10_set_vflip(struct tx_isp_subdev *sd, int enable) {
	int ret = ISP_SUCCESS;
	uint8_t val = 0;

	os03a10_write(sd, 0x0100, 0x00);
	os03a10_read(sd, 0x3820, &val);
	switch (enable) {
	case 0:
		val = ((val & 0xF9) | 0x02);
		break;
	...
	os03a10_write(sd, 0x3820, val);
	os03a10_write(sd, 0x0100, 0x01);
...
static int os03a10_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en) {
...
static int os03a10_set_wdr(struct tx_isp_subdev *sd, int wdr_en) {
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
	int ret = ISP_SUCCESS;
...
static int os03a10_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg) {
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if (arg)
			ret = os03a10_set_vflip(sd, sensor_val->value);
		break;
#endif /* SENSOR_MIR_FLIP */
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
	...
	case TX_ISP_EVENT_SENSOR_WDR:
		if (arg)
			ret = os03a10_set_wdr(sd, init->enable);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if (arg)
			ret = os03a10_set_wdr_stop(sd, init->enable);
		break;
#endif /* SENSOR_WDR_2_FRAME */
	default:
		break;
...
static int os03a10_probe(struct i2c_client *client, const struct i2c_device_id *id) {
...
static const struct i2c_device_id os03a10_id[] = {{"os03a10", 0}, {}};
