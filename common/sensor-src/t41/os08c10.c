...
#ifdef SENSOR_I2C_REG_8BIT
int os08c10_read(struct tx_isp_subdev *sd, unsigned char reg, unsigned char *value) {
...
int os08c10_write(struct tx_isp_subdev *sd, unsigned char reg, unsigned char value) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int os08c10_read(struct tx_isp_subdev *sd, uint16_t reg, unsigned char *value) {
	int ret;
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int os08c10_write(struct tx_isp_subdev *sd, uint16_t reg, unsigned char value) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int os08c10_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int os08c10_probe(struct i2c_client *client, const struct i2c_device_id *id) {
...
static const struct i2c_device_id os08c10_id[] = {{"os08c10", 0}, {}};
