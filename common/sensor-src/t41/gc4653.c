...
#ifdef SENSOR_I2C_REG_8BIT
int gc4653_read(struct tx_isp_subdev *sd, unsigned char reg, unsigned char *value) {
...
int gc4653_write(struct tx_isp_subdev *sd, unsigned char reg, unsigned char value) {
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int gc4653_write_array(struct tx_isp_subdev *sd, struct regval_list *vals) {
	...
	while (vals->reg_num != SENSOR_REG_END) {
		if (vals->reg_num == SENSOR_REG_DELAY) {
			...
			ret = gc4653_write(sd, vals->reg_num, vals->value);
			ret = gc4653_read(sd, vals->reg_num, &value);
			if (ret < 0)
				return ret;
		}
		vals++;
...
static int gc4653_setting_select(struct tx_isp_subdev *sd, int deboot) {
	...
	case 0:
		...
		gc4653_attr.max_dgain = 0;
		gc4653_attr.max_again = 409243;
		gc4653_attr.min_integration_time = 2;
		gc4653_attr.max_integration_time = 1920 - 1;
		gc4653_attr.total_width = 3000;
		gc4653_attr.total_height = 1920;
		gc4653_attr.integration_time_apply_delay = 2;
		gc4653_attr.again_apply_delay = 2;
		gc4653_attr.dgain_apply_delay = 0;
		gc4653_attr.integration_time_limit = gc4653_attr.max_integration_time;
		gc4653_attr.max_integration_time_native = gc4653_attr.max_integration_time;
		gc4653_attr.min_integration_time_native = gc4653_attr.min_integration_time;
		gc4653_attr.expo_fs = 1;
#ifdef SENSOR_WDR_2_FRAME
		gc4653_attr.max_again_short = xxxx;
		gc4653_attr.min_integration_time_short = xx;
		gc4653_attr.max_integration_time_short = xx;
...
static int gc4653_set_fps(struct tx_isp_subdev *sd, int fps) {
	...
	gc4653_write(sd, 0x0340, (unsigned char)(vts >> 8));
...
static int gc4653_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg) {
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if (arg)
			ret = gc4653_set_vflip(sd, sensor_val->value);
		break;
#endif /* SENSOR_MIR_FLIP */
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
...
static const struct i2c_device_id gc4653_id[] = {{"gc4653", 0}, {}};
