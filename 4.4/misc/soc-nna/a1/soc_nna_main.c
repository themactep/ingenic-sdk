...
#define SOC_NNA_VERSION "0x000000a1"
...
{
	long ret = 0;
	uint32_t soc_version_buf = 0x000000a1;
...
static int soc_nna_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	oram_base = 0x12600000 + l2cache_size * 1024;
	...
	/* 根据地址区分地址的类型 */
	if(paddr_start == 0x12500000){
		/* nna dma config address */
		mmap_type = 2;
		extend = 0;
	} else if(paddr_start == oram_base) {
		/* oram address, no extend*/
		mmap_type = 1;
		extend = 0;
	} else if((paddr_start < oram_base) && (paddr_end > oram_base)) {
		/* oram address, extend*/
		mmap_type = 1;
		extend = 1;
	} else {
	...
	if (extend == 0){
		...
		unsigned int real_paddr = (0x12600000 + l2cache_size * 1024);
		...
		/* 根据上层传过来扩充映射内存的大小和ORAM size，来做处理，
		 * 每次映射的区域不能超过实际的物理大小，不改变物理地址，
		 * 根据虚拟地址偏移多次映射来达到扩充虚拟地址的目的 */
		...
		for(n = 0 ; n < set_oram_nums; n++){
			if (io_remap_pfn_range(vma, vma->vm_start + n * real_size, real_paddr>>PAGE_SHIFT,
						(before_s % real_size)&&(n == (set_oram_nums -1)) ? before_s % real_size : real_size ,
						vma->vm_page_prot)) {
				dev_err(mdev->this_device, "%s(%d) [%d:%d]: io_remap_pfn_range failed\n",
						__func__, __LINE__, current->tgid, current->pid);
				return -EAGAIN;
		...
		for(n = 0; n < set_oram_nums ; n++){
			if (io_remap_pfn_range(vma, vma->vm_start+ before_s + real_size * (n + 1) ,
						real_paddr>>PAGE_SHIFT,(after_s % real_size)&&(n == (set_oram_nums -1)) ? after_s % real_size : real_size,
						vma->vm_page_prot)) {
				dev_err(mdev->this_device, "%s(%d) [%d:%d]: io_remap_pfn_range failed\n",
						__func__, __LINE__, current->tgid, current->pid);
				return -EAGAIN;
...
static int soc_nna_probe(struct platform_device *pdev)
{
	...
	*(volatile unsigned int *)0xb2200060 = oram_clk | (1 << 5);
	printk("@@@ SOC-NNA Driver Probe Success, Version:20230512a @@@\n");
