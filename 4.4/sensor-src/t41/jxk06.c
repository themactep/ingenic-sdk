...
static int sensor_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	if (value<0x10) {
		...
		/* ret += sensor_write(sd, 0x66, 0x41); */
		if (ret < 0)
			return ret;
	} else if (value>=0x10 && value <0x30) {
		ret += sensor_write(sd, 0x0C, 0x00);
		ret += sensor_write(sd, 0x66, 0x24);
		if (ret < 0)
			return ret;
	} else {
	...
	if (value <= 0x40) {
		...
		ret += sensor_write(sd, 0x99, (unsigned char)(val_99));
		ret += sensor_write(sd, 0x9b, (unsigned char)(val_9b));
	}
...
static int sensor_set_vflip(struct tx_isp_subdev *sd, int enable) {
	...
	switch (enable) {
		case 0:
			...
			val &= ~0x30;
			sensor_write(sd, 0x12, val | 0x30);
			sensor_read(sd, 0xaa, &val);
			val &= ~0x0f;
			sensor_write(sd, 0xaa, val | 0x04);
			sensor_write(sd, 0x27, dismode);
			break;
		case 1:
			sensor_read(sd, 0x12, &val);
			val &= ~0x30;
			sensor_write(sd, 0x12, val | 0x10);
			sensor_read(sd, 0xaa, &val);
			val &= ~0x0f;
			sensor_write(sd, 0xaa, val | 0x0b);
			sensor_write(sd, 0x27, dismode + 1);
			break;
		case 2:
			sensor_read(sd, 0x12, &val);
			val &= ~0x30;
			sensor_write(sd, 0x12, val | 0x20);
			sensor_read(sd, 0xaa, &val);
			val &= ~0x0f;
			sensor_write(sd, 0xaa, val | 0x04);
			sensor_write(sd, 0x27, dismode);
