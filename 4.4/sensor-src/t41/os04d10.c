...
#define SENSOR_VERSION "H20231107b"
...
static int sensor_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
	...
	if (ret < 0) {
		ISP_ERROR("Failed to write page register, ret=%d\n", ret);
		return ret;
	}

	ret = sensor_read(sd, 0x02, &v);
	ISP_WARNING("Read reg 0x02: ret=%d, value=0x%02x (expected 0x%02x)\n", ret, v, SENSOR_CHIP_ID_H);
	if (ret < 0)
		return ret;
	...
	*ident = v;
	...
	ISP_WARNING("Read reg 0x03: ret=%d, value=0x%02x (expected 0x%02x)\n", ret, v, SENSOR_CHIP_ID_M);
	if (ret < 0)
		return ret;
	...
	*ident = (*ident << 8) | v;
	...
	ISP_WARNING("Read reg 0x04: ret=%d, value=0x%02x (expected 0x%02x)\n", ret, v, SENSOR_CHIP_ID_L);
	if (ret < 0)
		return ret;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
    ...
    ISP_WARNING("Got sensor clock, current rate: %lu Hz\n", rate);

    ret = private_clk_set_rate(sensor->mclk, 24000000);
    if (ret < 0) {
        ISP_ERROR("Failed to set clock rate to 24MHz, ret=%d\n", ret);
    }

    ret = private_clk_prepare_enable(sensor->mclk);
    if (ret < 0) {
        ISP_ERROR("Failed to enable clock, ret=%d\n", ret);
    } else {
        ISP_WARNING("Successfully enabled sensor clock at 24MHz\n");
    }
				...
				struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("Initializing sensor with reset_gpio=%d, pwdn_gpio=%d\n", reset_gpio, pwdn_gpio);

	if (reset_gpio != -1) {
		ret = private_gpio_request(reset_gpio,"sensor_reset");
		if (!ret) {
			ISP_WARNING("Performing reset sequence on GPIO %d\n", reset_gpio);
	...
	if (pwdn_gpio != -1) {
		...
		if (!ret) {
			ISP_WARNING("Performing power-down sequence on GPIO %d\n", pwdn_gpio);
	...
	ISP_WARNING("Attempting to detect sensor...\n");
