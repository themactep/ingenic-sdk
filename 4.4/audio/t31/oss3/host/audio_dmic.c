...
#define DMIC_PORTC_PINS 0x3<<13 | 0x1<<20
...
//	if (0 != init_dma_config(dmic_pipe, DMA_FROM_DEVICE, dmic->io_res->start, GET_MAP_TYPE(dmic->dma_res->start), &pdev->dev)) {
	...
	if (dmic_gpio == 1) {
		...
		dmic->dmic_gpio_func.func = GPIO_FUNC_3;
