...
#include <asm/io.h>
#include <linux/fs.h>
#include "../devices/dmic/mic.h"

extern unsigned int snd_debug_level;
#define AUDIO_DRIVER_VERSION "V20-20200426a"
...
static int dmic_amic_sync = 0;
module_param(dmic_amic_sync, int, S_IRUGO);
MODULE_PARM_DESC(dmic_amic_sync, "sync flag of amic and dmic");

#ifdef CONFIG_JZ_TS_DMIC
extern struct mic_dev *g_mic_dev;
#endif
...
static int snd_reuqest_dma(struct dsp_pipe *dp, struct device *dev)
{
	...
	/* alloc memory */
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
	dp->reservesize = PAGE_ALIGN(SND_DSP_DMA_BUFFER_SIZE);
#endif

	dp->vaddr = dma_alloc_noncoherent(dev,
							  dp->reservesize,
							  &dp->paddr,
							  GFP_KERNEL | GFP_DMA);
...
\********************************************************/
#ifdef CONFIG_JZ_TS_DMIC
static int init_dmic_pipe(struct dsp_pipe *dp,struct device *dev)
{
	int ret = 0;
	if (NULL == dp)
		return -ENODEV;
	...
	dp->reservesize = PAGE_ALIGN(SND_DSP_DMA_BUFFER_SIZE) * 2;
...
/* dmic enable stream */
static inline long dsp_ioctl_enable_dmic_stream(struct file *file)
{
	long ret = 0;
	...
	dp = object->dp;
	if (dp->dp_dmic == NULL) {
		snd_debug_print("DMIC is not enabled!\n");
		return -EPERM;
	}
	...
	if(dp == NULL || ddata == NULL)
		return -EINVAL;
	attr = &dp->dp_dmic->living_dmic_attr;
	mutex_lock(&dp->dp_dmic->mutex);
	/* the route hasn't been opened */
	if(dp->dp_dmic->pipe_state == SND_DSP_STATE_CLOSE){
		snd_error_print("Please opened it firstly!\n");
		mutex_unlock(&dp->dp_dmic->mutex);
		return -EPERM;
	}

	if(dp->dp_dmic->pipe_state == SND_DSP_STATE_RUN){
	...
	if(file->f_mode & FMODE_READ){
		if ((16000 != object->rate) && (8000 != object->rate)) {
			snd_error_print("Dmic Don't support the samplerate:%d.\n", object->rate);
			ret = -EPERM;
			goto exit;
		}
		dp->dp_dmic->fragment_size = object->channel * sizeof(short) * object->rate / 100;
		ret = (int)ddata->dev_ioctl_2(ddata, MIC_SET_DMIC_SAMPLERATE, object->rate);
		if(ret){
			snd_error_print("Failed to set dmic samplerate\n");
			goto exit;
		}
		ret = (int)ddata->dev_ioctl_2(ddata, MIC_SET_DMIC_CHANNEL, object->channel);
		if(ret){
			snd_error_print("Failed to set dmic channel\n");
			goto exit;
		}
		ret = (int)ddata->dev_ioctl_2(ddata, MIC_SET_PERIODS_MS, 32);
		if(ret){
			snd_error_print("Failed to set dmic period\n");
			goto exit;
		}
		ret = (int)ddata->dev_ioctl_2(ddata, MIC_INIT, 1);
		if(ret){
			snd_error_print("Dmic init failed\n");
			goto exit;
		}
	}else{
	...
	/* */
	ret = jz_asoc_dma_prepare_and_submit(dp->dp_dmic);
	if (ret != 0) {
		snd_error_print("Failed to init dmic dma\n");
		goto exit;
	}

	spin_lock_irqsave(&dp->dp_dmic->pipe_lock, lock_flags);
	dp->dp_dmic->pipe_state = SND_DSP_STATE_RUN;
	spin_unlock_irqrestore(&dp->dp_dmic->pipe_lock, lock_flags);
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
			dp->dp_aec->fragment_size = dp->fragment_size;
#endif
		}else{
			snd_error_print("Don't support the fmode(%d)!\n",file->f_mode);
			ret = -EPERM;
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
		if(dp->dp_aec) {
			ret = jz_asoc_dma_prepare_and_submit(dp->dp_aec);
		}
#endif
...
static inline long dsp_ioctl_disable_dmic_stream(struct file *file)
{
	long ret = 0;
	int i = 0;
	struct dsp_route_object *object = NULL;
	...
	dp = object->dp;
	if (dp->dp_dmic == NULL) {
		snd_error_print("DMIC is not enabled!\n");
		return -EPERM;
	}
	ddata = dp ? dp->pddata: NULL;
...
#ifdef CONFIG_JZ_TS_DMIC
		jz_asoc_dma_stop_and_reset(dp->dp_dmic);
#endif

		/*mutex_lock(&dp->mutex);*/
		ret |= (int)ddata->dev_ioctl_2(ddata, MIC_DEINIT, 1);
		if (ret)
			ret = -EFAULT;

		/*spin_lock_irqsave(&dp->pipe_lock, lock_flags);*/
		while(!list_empty(&dp->dp_dmic->tasklist)){
			node = list_first_entry(&dp->dp_dmic->tasklist, typeof(*node), list);
			list_del(&node->list);
			if(node->flags != SND_TASK_NODE_FINISHED){
				node->flags = SND_TASK_NODE_FINISHED;
				wake_up_interruptible(&dp->dp_dmic->wq);
			}
		}
		/*spin_unlock_irqrestore(&dp->pipe_lock, lock_flags);*/
	}
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
		if(dp->dp_aec)
			jz_asoc_dma_stop_and_reset(dp->dp_aec);
#endif
		/*mutex_lock(&dp->mutex);*/
		if(file->f_mode & FMODE_WRITE){
...
#ifdef CONFIG_JZ_TS_DMIC
static inline long dsp_ioctl_dmic_stream(struct file *file, struct dmic_stream *stream)
{
	long ret = 0;
	struct dsp_route_object *object = NULL;
	struct dsp_route_attr *attr = NULL;
	struct dsp_pipe *dp = NULL;
	struct dsp_pipe *dp_dmic = NULL;

	/*unsigned long lock_flags;*/
	unsigned int dmic_totalsize = 0;
	unsigned int dmic_maxsize = 0;
	unsigned int dmic_size = 0;
	void* __user *dmic_buff = 0;
	void* __user *dmic_aecbuff = 0;

	if(stream->dmic_size == 0) {
		snd_debug_print("The ai stream size is 0!\n");
		return 0;
	}
	...
	dp = object->dp;
	dp_dmic = dp->dp_dmic;
	attr = &dp_dmic->living_dmic_attr;

	dmic_buff = stream->dmic_data;
	dmic_aecbuff = stream->dmic_aec;
	dmic_totalsize = stream->dmic_size;
	dmic_maxsize = 2 * SND_TASK_NODE_SIZE / dp_dmic->fragment_size * dp_dmic->fragment_size;

	while(dmic_totalsize) {
		dmic_size = dmic_totalsize > dmic_maxsize ? dmic_maxsize : dmic_totalsize;

		object->datanode.dmic_data = object->dmic_buffer;
		object->datanode.dmic_size = dmic_size;
		object->datanode.flags = SND_TASK_NODE_PREPARED;

		if((object->dmic_aec_enable == SND_AEC_ON) && stream->dmic_aec){
			object->datanode.dmic_aec = object->dmic_aecbuffer;
		} else {
			object->datanode.dmic_aec = NULL;
		}

		mutex_lock(&dp_dmic->mutex);
		list_add_tail(&object->datanode.list, &dp_dmic->tasklist);
		mutex_unlock(&dp_dmic->mutex);
		do{
			ret = wait_event_interruptible(dp_dmic->wq, object->datanode.flags == SND_TASK_NODE_FINISHED);
		} while(ret == -ERESTARTSYS);

		ret = copy_to_user(dmic_buff, object->dmic_buffer, dmic_size);
		if(ret){
			snd_error_print("failed to copy_to_user\n");
			ret = -EIO;
			break;
		}
		if(object->datanode.dmic_aec){
			ret = copy_to_user(dmic_aecbuff, object->dmic_aecbuffer, dmic_size/attr->channel);
			if(ret){
				snd_error_print("failed to copy_to_user\n");
				ret = -EIO;
				break;
		...
		dmic_totalsize -= dmic_size;
		dmic_buff += dmic_size;
		dmic_aecbuff += dmic_size/attr->channel;

	}

	stream->dmic_size -= dmic_totalsize;
	object->ioctrl_counter++;
	object->iodata_counter += stream->dmic_size;
...
static inline long dsp_ioctl_aec_ops(struct file *file, char onoff)
{
...
#elif (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
	fragsize = dp->fragment_size;
#else
	fragsize = dp->fragment_size;
#endif
	if(dp_dmic) dmic_fragsize = (dp_dmic->fragment_size)/attr->channel;
	mutex_lock(&dp->mutex);
...
#ifdef CONFIG_JZ_TS_DMIC
		if(dp_dmic) {
			for(index = 0; index < dp_dmic->fragment_cnt; index ++) {
				dp_dmic->aecfragments[index].vaddr = (dp_dmic->aec_buffer + index*dmic_fragsize);
				dp_dmic->aecfragments[index].size = dmic_fragsize;
				dp_dmic->aecfragments[index].index = index;
				list_add_tail(&dp_dmic->aecfragments[index].list, &dp_dmic->aecfragments_head);
			}
		}
#endif
	}
	...
	if(onoff){
		dp->aec_enable++;
		if(dp_dmic) dp_dmic->aec_enable++;
	}else{
		dp->aec_enable--;
		if(dp_dmic) dp_dmic->aec_enable--;
	}
	object->aec_enable = onoff;
	if(dp_dmic) object->dmic_aec_enable = onoff;
	mutex_unlock(&dp->mutex);
		      ...
		      struct snd_dev_data *ddata)
{
	...
	case SNDCTL_EXT_DISABLE_STREAM:
		...
		break;
	case SNDCTL_EXT_ENABLE_DMIC_STREAM:
		ret = dsp_ioctl_enable_dmic_stream(file);
		break;
	case SNDCTL_EXT_DISABLE_DMIC_STREAM:
		snd_debug_print(" disable dmic stream #########\n");
		ret = dsp_ioctl_disable_dmic_stream(file);
		break;
	...
	case SNDCTL_EXT_GET_AI_STREAM:
		...
		break;
#ifdef CONFIG_JZ_TS_DMIC
	case DMIC_GET_AI_STREAM:
		if(file->f_mode & FMODE_READ){
			struct dmic_stream stream;
			copy_from_user(&stream, (__user void*)arg, sizeof(stream));
			ret = dsp_ioctl_dmic_stream(file, &stream);
			copy_to_user((__user void*)arg, &stream, sizeof(stream));
		} else {
			snd_error_print("The fd can't record audio, please open it with O_RDONLY\n");
			ret = -EPERM;
	...
	case SNDCTL_EXT_DISABLE_AEC:
		...
		break;
#ifdef CONFIG_JZ_TS_DMIC
	case DMIC_SET_PERIODS_MS:
	case DMIC_GET_PERIODS_MS:
	case DMIC_ENABLE_RECORD:
	case DMIC_DISABLE_RECORD:
	case DMIC_SET_DMIC_GAIN:
	case DMIC_GET_DMIC_GAIN:
	case DMIC_GET_DMIC_GAIN_RANGE:
	case DMIC_SET_DMIC_CHANNEL:
	case DMIC_SET_DMIC_SAMPLERATE: {
		ret = mic_ioctl_2(g_mic_dev, cmd, arg);
		break;
	}
#endif
	default:
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
		dpi->dp_aec = endpoints->aec_endpoint;
#endif
...
#ifdef CONFIG_JZ_TS_DMIC
		object->dmic_buffer = dp_dmic->taskbuffer + i*SND_TASK_NODE_SIZE;
		object->dmic_aecbuffer = dp_dmic->taskaecbuffer + i*SND_TASK_NODE_SIZE;
		if(dpi->dp_dmic->pipe_state == SND_DSP_STATE_CLOSE){
			dpi->dp_dmic->pipe_state = SND_DSP_STATE_OPEN;
			dpi->dp_dmic->aec_enable = 0;
		}
#endif
		       ...
		       struct snd_dev_data *ddata)
{
	...
	if (file->f_mode & FMODE_READ) {
		...
		if(object->aec_enable == SND_AEC_ON)
			xb_snd_dsp_ioctl(file, SNDCTL_EXT_DISABLE_AEC, 0, ddata);
		if(object->state == SND_DSP_STATE_RUN) {
			xb_snd_dsp_ioctl(file, SNDCTL_EXT_DISABLE_STREAM, 0, ddata);
#ifdef CONFIG_JZ_TS_DMIC
			xb_snd_dsp_ioctl(file, SNDCTL_EXT_DISABLE_DMIC_STREAM, 0, ddata);
#endif
		}
		...
		if (dpi->dp_dmic) {
			dpi->dp_dmic->pipe_state = SND_DSP_STATE_CLOSE;
		}
		mutex_unlock(&dpi->mutex);
	}
next:
...
out:
	mutex_lock(&endpoints->mutex);
#ifdef CONFIG_JZ_TS_DMIC
	if((dpi->pipe_state == SND_DSP_STATE_CLOSE) && (dpo->pipe_state == SND_DSP_STATE_CLOSE) && (dpi->dp_dmic->pipe_state == SND_DSP_STATE_CLOSE)){
...
static inline void dmic_dmaaddr_sync_ioaddr(struct dsp_pipe *dp, unsigned int dma_current)  //add by sxzhang:2018.04.04
{
	unsigned int tmp_io = dp->io_tracer;
	if(dp->dma_tracer > tmp_io)
		tmp_io += dp->fragment_cnt;
...
static inline int ai_copy_to_user(struct dsp_pipe *dp, struct task_node *node)
{
	...
	struct list_head *list = NULL;
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
	struct list_head *aec_list = NULL;

	struct dsp_data_fragment *aec_io_frag = &dp->dp_aec->fragments[dp->dp_aec->io_tracer];
	struct dsp_data_fragment *aec_dma_frag = &dp->dp_aec->fragments[dp->dp_aec->dma_tracer];
#endif
...
#else
		if(node->aec){
			dma_sync_single_for_device(NULL, (dma_addr_t)(aec_io_frag->paddr), aec_io_frag->size, DMA_FROM_DEVICE);
			memcpy(node->aec, aec_io_frag->vaddr, size_inno);
			node->aec += size_inno;
			memset(aec_io_frag->vaddr, 0, aec_io_frag->size);
			dma_sync_single_for_device(NULL, (dma_addr_t)(aec_io_frag->paddr), aec_io_frag->size, DMA_TO_DEVICE);
		}
#endif
		list = &io_frag->list;
		if(list->next == &dp->fragments_head)
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
		if(node->aec){
			aec_list = &aec_io_frag->list;
			if(aec_list->next == &dp->dp_aec->fragments_head)
				aec_list = aec_list->next;
			aec_io_frag = list_entry(aec_list->next, struct dsp_data_fragment, list);
		}
#endif
	}
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
	if(node->aec){
		dp->dp_aec->io_tracer = aec_io_frag->index;
	}
#endif
	return node->size;
}
...
#ifdef CONFIG_JZ_TS_DMIC
static inline int dmic_copy_to_user(struct dsp_pipe *dp, struct task_node *node)
{
	int i = 0;

	struct dsp_pipe *dp_dmic = dp;
	struct dsp_route_attr *attr = NULL;
	int size_dmic = 0;
	int dmic_cnt = 0;
	struct dsp_data_fragment *dmic_io_frag;
	struct dsp_data_fragment *dmic_aec_frag;
	struct dsp_data_fragment *dmic_dma_frag;
	struct list_head *dmic_list = NULL;

	attr = &dp->living_dmic_attr;
	size_dmic = dp_dmic->fragment_size;
	dmic_io_frag = &dp_dmic->fragments[dp_dmic->io_tracer];
	dmic_aec_frag = &dp_dmic->aecfragments[dp_dmic->io_tracer];
	dmic_dma_frag = &dp_dmic->fragments[dp_dmic->dma_tracer];
	dmic_cnt = node->dmic_size/size_dmic;

	for(i = 0; i < dmic_cnt; i++)  {
		if(dmic_io_frag == dmic_dma_frag)
			break;
		dma_sync_single_for_device(NULL, (dma_addr_t)(dmic_io_frag->paddr), dmic_io_frag->size, DMA_FROM_DEVICE);
		memcpy(node->dmic_data, dmic_io_frag->vaddr, size_dmic);
		node->dmic_data += size_dmic;
		memset(dmic_io_frag->vaddr, 0, dmic_io_frag->size);
		dma_sync_single_for_device(NULL, (dma_addr_t)(dmic_io_frag->paddr), dmic_io_frag->size, DMA_TO_DEVICE);
		if(node->dmic_aec) {
			dmic_aec_frag = &dp_dmic->aecfragments[dmic_io_frag->index];
			copy_to_user(node->dmic_aec, dmic_aec_frag->vaddr, size_dmic/attr->channel);
			memset(dmic_aec_frag->vaddr, 0, size_dmic/attr->channel);
			node->dmic_aec += size_dmic/attr->channel;
		}
		dmic_list = &dmic_io_frag->list;
		if(dmic_list->next == &dp_dmic->fragments_head)
			dmic_list = dmic_list->next;
		dmic_io_frag = list_entry(dmic_list->next, struct dsp_data_fragment, list);
	}

	dp_dmic->io_tracer = dmic_io_frag->index;
	node->dmic_size -= i*size_dmic;
	return node->dmic_size;
}
#endif
#ifndef CONFIG_JZ_TS_DMIC
static void dsp_workqueue_handle(struct work_struct *work)
{
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
	struct dsp_pipe *dp_aec = endpoints->aec_endpoint;
#endif
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
	unsigned int aec_current_index = 0;
#endif
	struct task_node *node = NULL;
	struct list_head *list = NULL;
	...
	if(dpi && dpi->is_trans){
		...
		if(node->aec){
			dma_chan = dpi->dp_aec->dma_chan;
			aec_currentaddr = dma_chan->device->get_current_trans_addr(dma_chan, NULL, NULL,
					dp_aec->dma_config.direction);
			aec_current_index = ((aec_currentaddr - dp_aec->paddr) / dp_aec->fragment_size) % dp_aec->fragment_cnt;

			/* sync the pointers of ai and aec */
			if(ai_current_index < aec_current_index){
				ai_current_index = ai_current_index + (dpi->fragment_cnt >> 1) < aec_current_index ? aec_current_index : ai_current_index;
			}else{
				ai_current_index = aec_current_index + (dpi->fragment_cnt >> 1) < ai_current_index ? ai_current_index : aec_current_index;
			}
		}
#endif
	}
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
		dp_aec->dma_tracer = dpi->dma_tracer;
#endif

		ai_dmaaddr_sync_ioaddr(dpi, ai_current_index);
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
		dp_aec->io_tracer = dpi->io_tracer;
#endif
		loop_cnt0 = 0;
		/* copy data */
...
#else
static void dsp_dmic_workqueue_handle(struct work_struct *work)
{
	int ret = 0;
	struct dsp_endpoints *endpoints = container_of(work, struct dsp_endpoints, workqueue);
	struct dsp_pipe *dpi = endpoints->in_endpoint;
	...
	struct dsp_pipe *dp_aec = endpoints->aec_endpoint;
#endif
	struct dsp_pipe *dp_dmic = dpi->dp_dmic;
	...
	unsigned int aec_current_index = 0;
#endif
	unsigned int dmic_current_index = 0;
	struct task_node *node = NULL;
	struct list_head *list = NULL;
	struct task_node *dmic_node = NULL;
	...
	spin_lock_irqsave(&dp_dmic->pipe_lock, lock_flags);
	/* get living dma address */
	if(dp_dmic && dp_dmic->is_trans) {
		if(dp_dmic) {
			dma_chan = dp_dmic->dma_chan;
			dmic_currentaddr = dma_chan->device->get_current_trans_addr(dma_chan, NULL, NULL,
				dp_dmic->dma_config.direction);
			dmic_current_index = ((dmic_currentaddr - dp_dmic->paddr) / dp_dmic->fragment_size) % dp_dmic->fragment_cnt;
		}
	}
	if(dpo && dpo->is_trans){
		dma_chan = dpo->dma_chan;
	...
	spin_unlock_irqrestore(&dp_dmic->pipe_lock, lock_flags);
	...
	loop_cnt0 = 0;
	/* copy dmic and aec to user */
	if(dp_dmic && dp_dmic->is_trans && dp_dmic->pipe_state == SND_DSP_STATE_RUN){
		mutex_lock(&dp_dmic->mutex);
#if (defined(CONFIG_SOC_T30) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
		/* sync aec data */
		if(dp_dmic->aec_enable && dpo->is_trans){
				ao_start = dpo->dma_tracer;
				ao_sfrag = &dpo->fragments[ao_start];
				aec_start = dp_dmic->dma_tracer;
				aec_end = dmic_current_index;
				aec_sfrag = &dp_dmic->aecfragments[aec_start];
				aec_efrag = &dp_dmic->aecfragments[aec_end];
				...
				while(aec_sfrag->index != aec_efrag->index){
					...
					/* move aec */
					list = &aec_sfrag->list;
					if(list->next == &dp_dmic->aecfragments_head)
						list = list->next;
					aec_sfrag = list_entry(list->next, struct dsp_data_fragment, list);
					/* move ao */
					list = &ao_sfrag->list;
				...
				if(aec_sfrag->index != dmic_current_index)
					printk(KERN_ERR"error:aec %d, ai %d sd %d\n", aec_sfrag->index, dmic_current_index, aec_efrag->index);
		}
#endif

		dp_dmic->dma_tracer = dp_dmic->fragments[dmic_current_index].index;

		ai_dmaaddr_sync_ioaddr(dp_dmic, dmic_current_index);

		loop_cnt0 = 0;
		/* copy data */
		while(!list_empty(&dp_dmic->tasklist)) {
			loop_cnt0++;
			if(loop_cnt0 > 1000)
				snd_error_print("LOOPS are too much\n");
			dmic_node = list_first_entry(&dp_dmic->tasklist, typeof(*dmic_node), list);
			ret = dmic_copy_to_user(dp_dmic, dmic_node);
			if(ret == 0){
				list_del(&dmic_node->list);
				dmic_node->flags = SND_TASK_NODE_FINISHED;
				wake_up_interruptible(&dp_dmic->wq);
			} else
				break;
		}
		mutex_unlock(&dp_dmic->mutex);
...
static enum hrtimer_restart jz_asoc_hrtimer_callback(struct hrtimer *hr_timer) {
	...
	if(dpi && dpi->pipe_state == SND_DSP_STATE_RUN) {
		...
		if(dpi->is_trans == false) {
			/* dmic_amic_sync:0 represent need not use amic and dmic at the same time */
			if (!dmic_amic_sync) {
				/* start to amic dma */
				dma_chan = dpi->dma_chan;
				dpi->dma_tracer = 0;
				...
				dp_aec->dma_tracer = 0;
				dp_aec->io_tracer = dpi->io_tracer;
				dma_async_issue_pending(dp_aec->dma_chan);
				dp_aec->is_trans = true;
#endif
			} else {
				/* dmic_amic_sync:1 represent need use amic and dmic at the same time */
				if (dp_dmic->pipe_state != SND_DSP_STATE_RUN) {
					/* dmic is not initialized */
				} else {
					local_irq_save(flags);
					/* start to dmic dma */
					dma_chan = dp_dmic->dma_chan;
					dp_dmic->dma_tracer = 0;
					dp_dmic->io_tracer = (dp_dmic->fragment_cnt - SND_DSP_PIPE_DAM2IO_CNT);
					dma_async_issue_pending(dma_chan);
					ddata->dev_ioctl_2(ddata, DMIC_ENABLE_DMA, 0);
					dp_dmic->is_trans = true;
	...
	if(dp_dmic && dp_dmic->pipe_state == SND_DSP_STATE_RUN) {
		ddata = dpi->pddata;

		if(dp_dmic->is_trans == false) {
			if (!dmic_amic_sync) {
				/* start to dmic dma */
				dma_chan = dp_dmic->dma_chan;
				if(dp_dmic->is_trans == false) {
				...
				if (dpi->pipe_state != SND_DSP_STATE_RUN) {
					/* amic is not initialized */
				} else {
					local_irq_save(flags);
					/* start to amic dma */
					dma_chan = dpi->dma_chan;
...
#ifdef CONFIG_JZ_TS_DMIC
		if (dp->dp_dmic != NULL) {
			ret = init_dmic_pipe(dp->dp_dmic, g_mic_dev->dev);
			if (ret)
				goto error3;
		}
#endif
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
	if((dp = endpoints->aec_endpoint) != NULL) {
		ret = init_pipe(dp , ddata->dev,DMA_FROM_DEVICE);
		if (ret)
			goto error2;
		dp->dsp = endpoints;
		endpoints->in_endpoint->dp_aec = endpoints->aec_endpoint;
	}
#endif
...
#else
	INIT_WORK(&endpoints->workqueue, dsp_dmic_workqueue_handle);
#endif
...
int xb_snd_dsp_remove(struct snd_dev_data *ddata)
{
	...
	deinit_pipe(endpoints->out_endpoint, ddata->dev);
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31) || defined(CONFIG_SOC_C100))
	deinit_pipe(endpoints->aec_endpoint, ddata->dev);
#endif
#ifdef CONFIG_JZ_TS_DMIC
	if(endpoints->dmic_endpoint)
		deinit_pipe(endpoints->dmic_endpoint, ddata->dev);
#endif
