...
int ov2735_read(struct tx_isp_subdev *sd, unsigned char reg,
		unsigned char *value)
{
...
static int ov2735_write(struct tx_isp_subdev *sd, unsigned char reg,
			unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int ov2735_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
static int ov2735_write_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != OV2735_REG_END) {
		if (vals->reg_num == OV2735_REG_DELAY) {
		...
		struct tx_isp_chip_ident *chip)
{
	...
	if (ret) {
		printk("chip found @ 0x%x (%s) is not an ov2735 chip.\n",
		...
		const struct i2c_device_id *id)
{
	...
	sensor->video.mbus.colorspace = wsize->colorspace;
	sensor->video.fps = wsize->fps;
	tx_isp_subdev_init(&sensor_platform_device, sd, &ov2735_ops);
...
static __init int init_ov2735(void)
{
	...
	if(ret){
		printk("Failed to init ov2735 dirver.\n");
		return -1;
	}
