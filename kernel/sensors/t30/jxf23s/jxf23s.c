...
#define JXF23S_SUPPORT_30FPS_SCLK (86400000)
#define JXF23S_SUPPORT_15FPS_SCLK (43200000)
...
static struct regval_list jxf23s_init_regs_1920_1080_15fps_dvp[] = {
	{0x0E,0x11},
	{0x0F,0x14},
	{0x10,0x40},
	{0x11,0x80},
...
static struct tx_isp_sensor_win_setting jxf23s_win_sizes[] = {
	/* 1920*1080 */
	{
		...
		.height		= 1080,
		.fps		= 25 << 16 | 1,
		.mbus_code	= V4L2_MBUS_FMT_SBGGI10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= jxf23s_init_regs_1920_1080_25fps_dvp,
	}
};

static enum v4l2_mbus_pixelcode jxf23s_mbus_code[] = {
...
static struct regval_list jxf23s_stream_on_dvp[] = {
	{0x12, 0x00},
	{JXF23S_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list jxf23s_stream_off_dvp[] = {
	{0x12, 0x40},
	{JXF23S_REG_END, 0x00},	/* END MARKER */
};
...
static int jxf23s_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int jxf23s_init(struct tx_isp_subdev *sd, int enable)
{
	...
	case TX_SENSOR_MAX_FPS_25:
		wsize->fps = 25 << 16 | 1;
		wsize->regs = jxf23s_init_regs_1920_1080_25fps_dvp;
		break;
	case TX_SENSOR_MAX_FPS_15:
		wsize->fps = 15 << 16 | 1;
		wsize->regs = jxf23s_init_regs_1920_1080_15fps_dvp;
		break;
	default:
	...
	ret += jxf23s_read(sd, 0x99, &val_99);
	ret += jxf23s_read(sd, 0x9b, &val_9b);
...
static int jxf23s_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int jxf23s_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	switch (sensor_max_fps) {
	case TX_SENSOR_MAX_FPS_25:
		sclk = JXF23S_SUPPORT_30FPS_SCLK;
		max_fps = SENSOR_OUTPUT_MAX_FPS;
		break;
	case TX_SENSOR_MAX_FPS_15:
		sclk = JXF23S_SUPPORT_15FPS_SCLK;
				   ...
				   struct tx_isp_chip_ident *chip)
{
	...
	int ret = ISP_SUCCESS;
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"jxf23s_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = private_gpio_request(pwdn_gpio,"jxf23s_pwdn");
		if(!ret){
...
static int jxf23s_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = jxf23s_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = jxf23s_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = jxf23s_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = jxf23s_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = jxf23s_set_mode(sd, *(int*)arg);
		break;
...
//	*(volatile unsigned int*)(0xB0010134) = 0xC0000000;
	...
	jxf23s_attr.dbus_type = data_interface;
	if (data_interface == TX_SENSOR_DATA_INTERFACE_DVP){
		wsize->regs = jxf23s_init_regs_1920_1080_25fps_dvp;
		memcpy((void*)(&(jxf23s_attr.dvp)),(void*)(&jxf23s_dvp),sizeof(jxf23s_dvp));
	} else if (data_interface == TX_SENSOR_DATA_INTERFACE_MIPI){
		wsize->regs = jxf23s_init_regs_1920_1080_25fps_mipi;
	...
	*/
	...
	case TX_SENSOR_MAX_FPS_25:
		break;
	case TX_SENSOR_MAX_FPS_15:
		jxf23s_attr.max_integration_time_native = 1121;
		jxf23s_attr.integration_time_limit = 1121;
		jxf23s_attr.total_width = 2560;
		jxf23s_attr.total_height = 1125;
		jxf23s_attr.max_integration_time = 1121;
		break;
	default:
		printk("Now we do not support this framerate!!!\n");
	}
	jxf23s_attr.max_again = 324678;
	jxf23s_attr.max_dgain = 0;
	sd = &sensor->sd;
	video = &sensor->video;
...
static __init int init_jxf23s(void)
{
	...
	if(ret){
		printk("Failed to init jxf23s dirver.\n");
		return -1;
	}
