...
struct tx_isp_sensor_attribute gc1054_attr;
...
unsigned int gc1054_gainone_to_reg(unsigned int gain_one, unsigned int *regs)
{
...
unsigned int gc1054_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
...
struct tx_isp_dvp_bus gc1054_dvp={
...
struct tx_isp_sensor_attribute gc1054_attr={
	...
	.sensor_ctrl.alloc_again = gc1054_alloc_again,
...
static struct regval_list gc1054_init_regs_1280_720_dvp[] = {
	...
	{GC1054_FLAG_END, 0x00},
};

/*
 * the order of the gc1054_win_sizes is [full_resolution, preview_resolution].
 */
static struct tx_isp_sensor_win_setting gc1054_win_sizes[] = {
	/* 1280*720 */
	{
		.width		= 1280,
		.height		= 720,
		...
		.regs 		= gc1054_init_regs_1280_720_dvp,
	},
	{
		.width		= 1280,
		.height		= 720,
		...
		.mbus_code	= V4L2_MBUS_FMT_SRGGB10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= gc1054_init_regs_1280_720_mipi,
	},
};
...
static struct regval_list gc1054_stream_off_dvp[] = {
	{GC1054_FLAG_END, 0x00},	/* END MARKER */
};
...
static int gc1054_write(struct tx_isp_subdev *sd, unsigned char reg,
			unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int gc1054_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
	while (vals->reg_num != GC1054_FLAG_END) {
		if(vals->reg_num == GC1054_FLAG_DELAY) {
				msleep(vals->value);
		} else {
			ret = gc1054_read(sd, vals->reg_num, &val);
			if (ret < 0)
				return ret;
			if (vals->reg_num == GC1054_PAGE_REG){
				val &= 0xf8;
				val |= (vals->value & 0x07);
				ret = gc1054_write(sd, vals->reg_num, val);
				ret = gc1054_read(sd, vals->reg_num, &val);
			}
		}
		vals++;
...
static int gc1054_write_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != GC1054_FLAG_END) {
		if (vals->reg_num == GC1054_FLAG_DELAY) {
...
static int gc1054_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}

static int gc1054_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
	unsigned char v;
	int ret;

	ret = gc1054_read(sd, 0xf0, &v);
	pr_debug("-----%s: %d ret = %d, v = 0x%02x\n", __func__, __LINE__, ret,v);
	if (ret < 0)
		return ret;
	if (v != GC1054_CHIP_ID_H)
		return -ENODEV;
	ret = gc1054_read(sd, 0xf1, &v);
	pr_debug("-----%s: %d ret = %d, v = 0x%02x\n", __func__, __LINE__, ret,v);
	if (ret < 0)
		return ret;
...
static int gc1054_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;

	ret = gc1054_write(sd, 0x4, value & 0xff);
	if (ret < 0) {
		ISP_ERROR("gc1054_write error  %d\n" ,__LINE__);
		return ret;
	}
	ret = gc1054_write(sd, 0x3, (value & 0x1f00)>>8);
	if (ret < 0) {
		ISP_ERROR("gc1054_write error  %d\n" ,__LINE__ );
		return ret;
	}
...
static int gc1054_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;

	ret = gc1054_write(sd, 0xfe, 0x01);
	ret += gc1054_write(sd, 0xb6, (value >> 12) & 0xf);
	ret += gc1054_write(sd, 0xb1, (value >> 8) & 0xf);
	ret += gc1054_write(sd, 0xb2, (value << 2) & 0xff);
	if (ret < 0) {
		ISP_ERROR("gc1054_write error  %d" ,__LINE__ );
		return ret;
	}
...
static int gc1054_set_digital_gain(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int gc1054_get_black_pedestal(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int gc1054_init(struct tx_isp_subdev *sd, int enable)
{
	...
	ret = gc1054_write_array(sd, wsize->regs);
	if (ret)
		return ret;
...
static int gc1054_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
	if (enable) {
		if (data_interface == TX_SENSOR_DATA_INTERFACE_DVP){
			ret = gc1054_write_array(sd, gc1054_stream_on_dvp);
		} else if (data_interface == TX_SENSOR_DATA_INTERFACE_MIPI){
			ret = gc1054_write_array(sd, gc1054_stream_on_mipi);
		}
...
static int gc1054_set_fps(struct tx_isp_subdev *sd, int fps)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	unsigned int pclk = 0;
	...
	case TX_SENSOR_DATA_INTERFACE_DVP:
		pclk = GC1054_SUPPORT_30FPS_SCLK_DVP;
		break;
	case TX_SENSOR_DATA_INTERFACE_MIPI:
		pclk = GC1054_SUPPORT_30FPS_SCLK_MIPI;
		break;
	default:
		ret = -1;
	...
	ret = gc1054_read(sd, 0x5, &tmp);
	hb = tmp;
	ret += gc1054_read(sd, 0x6, &tmp);
	hb = (hb << 8) + tmp;
	ret += gc1054_read(sd, 0xf, &tmp);
	win_width = tmp;
	ret += gc1054_read(sd, 0x10, &tmp);
	win_width = (win_width << 8) + tmp;
	ret += gc1054_read(sd, 0x2c, &tmp);
	if(ret < 0)
		return -1;
	sh_delay = tmp;
	hts=2*(hb+16)+((win_width+sh_delay)/2);
	ret = gc1054_read(sd, 0xd, &tmp);
	win_high = tmp;
	ret += gc1054_read(sd, 0xe, &tmp);
	if(ret < 0)
		return -1;
	win_high = (win_high << 8) + tmp;
	vts = pclk * (fps & 0xffff) / hts / ((fps & 0xffff0000) >> 16);
	vb = vts - win_high - 16;
	ret = gc1054_write(sd, 0x8, (unsigned char)(vb & 0xff));
	ret += gc1054_write(sd, 0x7, (unsigned char)(vb >> 8));
	if(ret < 0)
		return -1;
...
static int gc1054_g_chip_ident(struct tx_isp_subdev *sd,
		struct tx_isp_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"gc1054_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = private_gpio_request(pwdn_gpio,"gc1054_pwdn");
		if(!ret){
...
static int gc1054_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	switch(cmd){
		...
		case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
			if(arg)
				ret = gc1054_get_black_pedestal(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_RESIZE:
			if(arg)
				ret = gc1054_set_mode(sd, *(int*)arg);
			break;
...
static int gc1054_s_register(struct tx_isp_subdev *sd, const struct tx_isp_dbg_register *reg)
{
	int len = 0;
...
static struct tx_isp_subdev_core_ops gc1054_core_ops = {
	.g_chip_ident = gc1054_g_chip_ident,
	.reset = gc1054_reset,
	.init = gc1054_init,
...
static struct tx_isp_subdev_video_ops gc1054_video_ops = {
	.s_stream = gc1054_s_stream,
};

static struct tx_isp_subdev_sensor_ops	gc1054_sensor_ops = {
	.ioctl	= gc1054_sensor_ops_ioctl,
};

static struct tx_isp_subdev_ops gc1054_ops = {
	.core = &gc1054_core_ops,
	.video = &gc1054_video_ops,
	.sensor = &gc1054_sensor_ops,
};
		...
		const struct i2c_device_id *id)
{
	...
	gc1054_attr.dbus_type = data_interface;
	if(data_interface == TX_SENSOR_DATA_INTERFACE_DVP){
		ret = set_sensor_gpio_function(sensor_gpio_func);
		if (ret < 0)
			goto err_set_sensor_gpio;
		gc1054_attr.dvp.gpio = sensor_gpio_func;
		wsize = &gc1054_win_sizes[0];
		memcpy((void*)(&(gc1054_attr.dvp)),(void*)(&gc1054_dvp),sizeof(gc1054_dvp));
	}
	else if(data_interface == TX_SENSOR_DATA_INTERFACE_MIPI){
		wsize = &gc1054_win_sizes[1];
		gc1054_attr.max_integration_time_native =899;
		gc1054_attr.integration_time_limit = 899;
		gc1054_attr.total_width = 1726;
		gc1054_attr.total_height = 903;
		gc1054_attr.max_integration_time = 899;
		memcpy((void*)(&(gc1054_attr.mipi)),(void*)(&gc1054_mipi),sizeof(gc1054_mipi));
	 ...
	 */
	gc1054_attr.max_again = 262850;//private_log2_fixed_to_fixed(gc1054_attr.max_again, TX_ISP_GAIN_FIXED_POINT, LOG2_GAIN_SHIFT);
	gc1054_attr.max_dgain = gc1054_attr.max_dgain;
	sd = &sensor->sd;
	video = &sensor->video;
	sensor->video.attr = &gc1054_attr;
	...
	sensor->video.mbus.colorspace = wsize->colorspace;
	sensor->video.fps = wsize->fps;
	tx_isp_subdev_init(&sensor_platform_device, sd, &gc1054_ops);
...
static __init int init_gc1054(void)
{
	...
	if(ret){
		ISP_ERROR("Failed to init gc1054 dirver.\n");
		return -1;
	}
	return private_i2c_add_driver(&gc1054_driver);
}

static __exit void exit_gc1054(void)
{
	private_i2c_del_driver(&gc1054_driver);
}
