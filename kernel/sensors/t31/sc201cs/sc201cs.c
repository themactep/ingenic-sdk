...
static int sc201cs_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int sc201cs_init(struct tx_isp_subdev *sd, int enable)
{
...
static int sc201cs_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int sc201cs_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
	     	ret = sc201cs_set_expo(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
	//	if(arg)
	//		ret = sc201cs_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
	//	if(arg)
	//		ret = sc201cs_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = sc201cs_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = sc201cs_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = sc201cs_set_mode(sd, *(int*)arg);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = sc201cs_set_fps(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = sc201cs_set_vflip(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_LOGIC:
		if(arg)
			ret = sc201cs_set_logic(sd, *(int*)arg);
	default:
		break;
	}
...
static __init int init_sc201cs(void)
{
	...
	if(ret){
		ISP_ERROR("Failed to init sc201cs dirver.\n");
		return -1;
	}
