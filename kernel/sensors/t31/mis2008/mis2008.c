...
struct tx_isp_sensor_attribute mis2008_attr={
	...
	.max_integration_time_native = 1350 - 1,
	.integration_time_limit = 1350 - 1,
	.total_width = 2252,
	.total_height = 1350,
	.max_integration_time = 1350 - 1,
	.expo_fs = 1,
...
static struct regval_list mis2008_init_regs_1920_1080_25fps_mipi[] = {
	{0x300a,0x01},
	{0x3006,0x02},
	{0x3637,0x1e},
	{0x3c40,0x8d},
...
static struct regval_list mis2008_init_regs_1920_1080_25fps_dvp[] = {
	{0x300a,0x00},
	{0x3006,0x02},
	{0x3637,0x1e},
	{0x3c40,0x8d},
...
static int mis2008_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int mis2008_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret += mis2008_read(sd, 0x3100, &tmp);
	if (tmp < 1) {
		ret += mis2008_write(sd, 0x3a02, 0x0b);
		ret += mis2008_write(sd, 0x3a1c, 0x1f);
	} else {
		ret += mis2008_write(sd, 0x3a02, 0x0a);
		ret += mis2008_write(sd, 0x3a1c, 0x30);
	}
	if (ret != 0)
		return ret;
...
static int mis2008_init(struct tx_isp_subdev *sd, int enable)
{
...
static int mis2008_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
			      ...
			      struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("mis2008 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	if(chip){
...
static int mis2008_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = mis2008_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = mis2008_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = mis2008_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = mis2008_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = mis2008_set_mode(sd, *(int*)arg);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = mis2008_set_fps(sd, *(int*)arg);
		break;
...
static int mis2008_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	mis2008_attr.dbus_type = data_interface;
	...
	*/
	mis2008_attr.max_again = 260651;//259138;
...
static __init int init_mis2008(void)
{
	...
	if(ret){
		ISP_ERROR("Failed to init mis2008 dirver.\n");
		return -1;
	}
...
MODULE_DESCRIPTION("A low-level driver for ImageDesign mis2008 sensors");
MODULE_LICENSE("GPL");
