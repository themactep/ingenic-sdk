...
#define OV2740_SUPPORT_SCLK (0x438 * 0x8b0 * 30)
#define SENSOR_VERSION	"H20210524a"
...
struct tx_isp_sensor_attribute ov2740_attr={
	...
	.max_integration_time_native = 0x8b0 - 4,
	.integration_time_limit = 0x8b0 - 4,
	.total_width = 1080,
	.total_height = 2224,
	.max_integration_time = 0x8b0 - 4,
...
int ov2740_write(struct tx_isp_subdev *sd, uint16_t reg, unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int ov2740_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int ov2740_init(struct tx_isp_subdev *sd, int enable)
{
...
static int ov2740_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
			       ...
			       struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("ov2740 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
...
static int ov2740_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	return 0;
}
...
static int ov2740_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
	     	ret = ov2740_set_expo(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = ov2740_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = ov2740_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = ov2740_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = ov2740_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = ov2740_set_mode(sd, *(int*)arg);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = ov2740_set_fps(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = ov2740_set_vflip(sd, *(int*)arg);
		break;
	default:
		break;
	}
...
static int ov2740_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
static __init int init_ov2740(void)
{
	...
	if(ret){
		ISP_ERROR("Failed to init ov2740 dirver.\n");
		return -1;
	}
