...
#define SC450AI_SUPPORT_PCLK (144000000) /* mipiclk*lane_num/bit = pclk */
...
struct tx_isp_mipi_bus sc450ai_mipi1={
    .mode = SENSOR_MIPI_OTHER_MODE,
    .clk = 360,
    .lans = 4,
    .settle_time_apative_en = 0,
...
static struct regval_list sc450ai_init_regs_2696_1528_30fps_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x36f9,0x80},
...
static struct tx_isp_sensor_win_setting sc450ai_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= sc450ai_init_regs_2696_1528_30fps_mipi,
	}
};
...
static int sc450ai_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int sc450ai_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc450ai_write(sd, 0x3e08, (unsigned char)((value & 0xff00) >> 8));
	if (ret < 0)
		return ret;
...
static int sc450ai_init(struct tx_isp_subdev *sd, int enable)
{
...
static int sc450ai_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;

	if (enable) {
		ret = sc450ai_write_array(sd, sc450ai_stream_on_mipi);
		ISP_WARNING("sc450ai stream on\n");
...
/*
	sclk = SC450AI_SUPPORT_PCLK;

	ret += sc450ai_read(sd, 0x320c, &tmp);
	hts = tmp;
	ret += sc450ai_read(sd, 0x320d, &tmp);
	hts = ((hts << 8) + tmp);
	if (0 != ret) {
	...
	ret = sc450ai_write(sd, 0x320f, (unsigned char)(vts & 0xff));
	ret += sc450ai_write(sd, 0x320e, (unsigned char)(vts >> 8));
...
static int sc450ai_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = sc450ai_set_expo(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		//if(arg)
		//	ret = sc450ai_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		//if(arg)
		//	ret = sc450ai_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = sc450ai_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = sc450ai_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = sc450ai_set_mode(sd, *(int*)arg);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = sc450ai_set_fps(sd, *(int*)arg);
		break;
	default:
		break;
	}
			...
			const struct i2c_device_id *id)
{
	...
	switch(sensor_max_fps){
		...
		case TX_SENSOR_MAX_FPS_30:
			...
			memcpy(&(sc450ai_attr.mipi), &sc450ai_mipi1, sizeof(sc450ai_mipi1));
        	break;
		default:
...
MODULE_DESCRIPTION("A low-level driver for Smartsens sc450ai sensors");
MODULE_LICENSE("GPL");
