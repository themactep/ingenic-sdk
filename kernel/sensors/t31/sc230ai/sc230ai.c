...
#define SC230AI_SUPPORT_SCLK (81000000)
...
static struct regval_list sc230ai_stream_on_mipi[] = {
	{0x0100, 0x01},
	{SC230AI_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list sc230ai_stream_off_mipi[] = {
	{0x0100, 0x00},
	{SC230AI_REG_END, 0x00},	/* END MARKER */
};
...
static int sc230ai_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int sc230ai_set_expo(struct tx_isp_subdev *sd, int value)
{
     ...
     //integration time
//	 printk("------> it <---------\n");
     ret = sc230ai_write(sd, 0x3e00, (unsigned char)((it >> 12) & 0xf));
...
static int sc230ai_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc230ai_write(sd, 0x3e08, (unsigned char)((value & 0xff00) >> 8));
	if (ret < 0)
		return ret;
...
static int sc230ai_init(struct tx_isp_subdev *sd, int enable)
{
...
static int sc230ai_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;

	if (enable) {
		ret = sc230ai_write_array(sd, sc230ai_stream_on_mipi);
		ISP_WARNING("sc230ai stream on\n");

	}
	else {
		ret = sc230ai_write_array(sd, sc230ai_stream_off_mipi);
...
static int sc230ai_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = sc230ai_set_expo(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		//if(arg)
		//	ret = sc230ai_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		//if(arg)
		//	ret = sc230ai_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = sc230ai_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = sc230ai_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = sc230ai_set_mode(sd, *(int*)arg);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = sc230ai_set_fps(sd, *(int*)arg);
		break;
	default:
		break;
	}
