			      ...
			      struct tx_isp_chip_ident *chip)
{
	...
	printk("jxh63 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	if(chip){
...
static int jxh63_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = jxh63_set_vflip(sd, *(int*)arg);
		break;
	default:
		break;
	}
...
static int jxh63_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(data_interface == TX_SENSOR_DATA_INTERFACE_MIPI)
	{
		...
		jxh63_attr.total_height = 750;
		jxh63_attr.min_integration_time = 1;
		jxh63_attr.max_integration_time = 750 -1;
		jxh63_attr.min_integration_time_native = 1;
		jxh63_attr.max_integration_time_native = 750 - 1;
		jxh63_attr.integration_time_limit = 750 - 1;
		pr_debug("----->mipi\n");
	}
