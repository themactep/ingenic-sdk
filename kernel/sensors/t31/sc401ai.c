...
#define SENSOR_VERSION	"H20210125a"
...
struct tx_isp_mipi_bus sc401ai_1280_720_mipi={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 315,
	.lans = 2,
...
struct tx_isp_mipi_bus sc401ai_2560_1440_mipi={
		.mode = SENSOR_MIPI_OTHER_MODE,
		.clk = 630,
		.lans = 2,
		.settle_time_apative_en = 1,
...
static struct regval_list sc401ai_init_regs_2560_1440_25fps_mipi[] = {
	{0x0103, 0x01},
	{0x0100, 0x00},
	{0x36e9, 0x80},
	{0x36f9, 0x80},
...
static struct regval_list sc401ai_init_regs_1280_720_30fps_mipi[] = {
	{0x0103, 0x01},
	{0x0100, 0x00},
	{0x36e9, 0x80},
	{0x36f9, 0x80},
...
static struct tx_isp_sensor_win_setting sc401ai_win_sizes[] = {
	/* 2560*1920 */
	{
		...
		.regs 		= sc401ai_init_regs_2560_1440_25fps_mipi,
	},
	/* 1280*720 */
	{
		.width		= 1280,
		...
		.mbus_code	= V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= sc401ai_init_regs_1280_720_30fps_mipi,
	},
};
...
static int sc401ai_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int sc401ai_init(struct tx_isp_subdev *sd, int enable)
{
...
static int sc401ai_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int sc401ai_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = sc401ai_set_expo(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		//if(arg)
		//	ret = sc401ai_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		//if(arg)
		//	ret = sc401ai_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = sc401ai_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = sc401ai_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = sc401ai_set_mode(sd, *(int*)arg);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = sc401ai_set_fps(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = sc401ai_set_vflip(sd, *(int*)arg);
	default:
		break;
	}
			...
			const struct i2c_device_id *id)
{
	...
	if(sensor_resolution == TX_SENSOR_RES_400){
		wsize = &sc401ai_win_sizes[0];
		memcpy(&sc401ai_attr.mipi, &sc401ai_2560_1440_mipi, sizeof(sc401ai_2560_1440_mipi));
		sc401ai_attr.max_integration_time_native = 1796;
		sc401ai_attr.integration_time_limit = 1796;
		sc401ai_attr.total_width = 2800;
		sc401ai_attr.total_height = 1800;
		sc401ai_attr.max_integration_time = 1796;
	}else if(sensor_resolution == TX_SENSOR_RES_100){
		wsize = &sc401ai_win_sizes[1];
		memcpy(&sc401ai_attr.mipi, &sc401ai_1280_720_mipi, sizeof(sc401ai_1280_720_mipi));
		sc401ai_attr.max_integration_time_native = 1496;
		sc401ai_attr.integration_time_limit = 1496;
		sc401ai_attr.total_width = 2800;
		sc401ai_attr.total_height = 1500;
		sc401ai_attr.max_integration_time = 1496;
	}else{
		ISP_WARNING("Sensor Configuration not found.\n");
		return -ENODEV;
	}
...
static __init int init_sc401ai(void)
{
	...
	if(ret){
		ISP_ERROR("Failed to init sc401ai dirver.\n");
		return -1;
	}
