...
int jxf23_read(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *value)
{
...
int jxf23_write(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int jxf23_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
static int jxf23_write_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != JXF23_REG_END) {
		if (vals->reg_num == JXF23_REG_DELAY) {
...
static int jxf23_reset(struct v4l2_subdev *sd, u32 val)
{
	return 0;
}

static int jxf23_detect(struct v4l2_subdev *sd, unsigned int *ident)
{
	unsigned char v;
	int ret;
...
static int jxf23_set_integration_time(struct v4l2_subdev *sd, int value)
{
	int ret = 0;
...
static int jxf23_set_analog_gain(struct v4l2_subdev *sd, int value)
{
	int ret = 0;
	if(value<0x10){
...
static int jxf23_set_digital_gain(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int jxf23_get_black_pedestal(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int jxf23_init(struct v4l2_subdev *sd, u32 enable)
{
...
static int jxf23_s_stream(struct v4l2_subdev *sd, int enable)
{
	int ret = 0;
...
static int jxf23_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int jxf23_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int jxf23_set_fps(struct tx_isp_sensor *sensor, int fps)
{
...
static int jxf23_set_mode(struct tx_isp_sensor *sensor, int value)
{
	struct tx_isp_notify_argument arg;
	struct v4l2_subdev *sd = &sensor->sd;
...
static int jxf23_g_chip_ident(struct v4l2_subdev *sd,
		struct v4l2_dbg_chip_ident *chip)
{
	...
	int ret = ISP_SUCCESS;
	if(reset_gpio != -1){
		ret = gpio_request(reset_gpio,"jxf23_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = gpio_request(pwdn_gpio,"jxf23_pwdn");
		if(!ret){
	...
	if (ret) {
		v4l_err(client,
				"chip found @ 0x%x (%s) is not an jxf23 chip.\n",
				client->addr, client->adapter->name);
		return ret;
	}
	v4l_info(client, "jxf23 chip found @ 0x%02x (%s)\n",
			client->addr, client->adapter->name);
	return v4l2_chip_ident_i2c_client(client, chip, ident, 0);
}

static int jxf23_s_power(struct v4l2_subdev *sd, int on)
{

	return 0;
}
static long jxf23_ops_private_ioctl(struct tx_isp_sensor *sensor, struct isp_private_ioctl *ctrl)
{
	struct v4l2_subdev *sd = &sensor->sd;
	long ret = 0;
	switch(ctrl->cmd){
		case TX_ISP_PRIVATE_IOCTL_SENSOR_INT_TIME:
			ret = jxf23_set_integration_time(sd, ctrl->value);
			break;
		case TX_ISP_PRIVATE_IOCTL_SENSOR_AGAIN:
			ret = jxf23_set_analog_gain(sd, ctrl->value);
			break;
		case TX_ISP_PRIVATE_IOCTL_SENSOR_DGAIN:
			ret = jxf23_set_digital_gain(sd, ctrl->value);
			break;
		case TX_ISP_PRIVATE_IOCTL_SENSOR_BLACK_LEVEL:
			ret = jxf23_get_black_pedestal(sd, ctrl->value);
			break;
		case TX_ISP_PRIVATE_IOCTL_SENSOR_RESIZE:
			ret = jxf23_set_mode(sensor,ctrl->value);
			break;
		...
		case TX_ISP_PRIVATE_IOCTL_SENSOR_FPS:
			ret = jxf23_set_fps(sensor, ctrl->value);
			break;
		default:
...
static long jxf23_ops_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct tx_isp_sensor *sensor =container_of(sd, struct tx_isp_sensor, sd);
	int ret;
	switch(cmd){
		case VIDIOC_ISP_PRIVATE_IOCTL:
			ret = jxf23_ops_private_ioctl(sensor, arg);
			break;
		default:
			return -1;
			break;
...
static int jxf23_g_register(struct v4l2_subdev *sd, struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	unsigned char val = 0;
	int ret;
...
static int jxf23_s_register(struct v4l2_subdev *sd, const struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static const struct v4l2_subdev_core_ops jxf23_core_ops = {
	.g_chip_ident = jxf23_g_chip_ident,
	.reset = jxf23_reset,
	.init = jxf23_init,
	.s_power = jxf23_s_power,
...
static const struct v4l2_subdev_video_ops jxf23_video_ops = {
	.s_stream = jxf23_s_stream,
	.s_parm = jxf23_s_parm,
	.g_parm = jxf23_g_parm,
};

static const struct v4l2_subdev_ops jxf23_ops = {
	.core = &jxf23_core_ops,
	.video = &jxf23_video_ops,
};

static int jxf23_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	...
	switch(sensor_gpio_func){
		...
		case DVP_PA_HIGH_10BIT:
			mbus = jxf23_mbus_code[0];
			break;
		case DVP_PA_12BIT:
			mbus = jxf23_mbus_code[1];
			break;
		default:
			goto err_set_sensor_gpio;
	}

	for(i = 0; i < ARRAY_SIZE(jxf23_win_sizes); i++)
		jxf23_win_sizes[i].mbus_code = mbus;
	...
	v4l2_i2c_subdev_init(sd, client, &jxf23_ops);
...
{
	return i2c_add_driver(&jxf23_driver);
}

static __exit void exit_jxf23(void)
{
	i2c_del_driver(&jxf23_driver);
}
