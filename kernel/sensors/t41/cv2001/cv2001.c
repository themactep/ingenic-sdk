...
#define CV2001_CHIP_ID_L	0x01
#define CV2001_CHIP_ID_H	0x20
...
struct tx_isp_mipi_bus cv2001_mipi={
...
struct tx_isp_sensor_win_setting *wsize = &cv2001_win_sizes[0];
...
static int cv2001_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	return 0;
}
...
static int cv2001_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
	...
	return 0;
	ret = cv2001_read(sd, 0x3001, &v);
	ISP_WARNING("-----%s: %d ret = %d, v = 0x%02x\n", __func__, __LINE__, ret,v);
	if (ret < 0)
		return ret;
	if (v != CV2001_CHIP_ID_H)
		return -ENODEV;
	*ident = v;
...
static int cv2001_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
...
static int cv2001_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		memcpy(&(cv2001_attr.mipi), &cv2001_mipi, sizeof(cv2001_mipi));
		cv2001_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		cv2001_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		break;
	default:
	...
	case TISP_SENSOR_VI_MIPI_CSI0:
		cv2001_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		cv2001_attr.mipi.index = 0;
		break;
	case TISP_SENSOR_VI_DVP:
		cv2001_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
		break;
	default:
...
static int cv2001_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		//ret = cv2001_set_expo(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = cv2001_set_integration_time(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = cv2001_set_analog_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = cv2001_set_digital_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = cv2001_get_black_pedestal(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = cv2001_set_mode(sd, sensor_val->value);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		ret = cv2001_set_fps(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_VFLIP:
		ret = cv2001_set_vflip(sd, sensor_val->value);
		break;
	default:
		break;
	}
