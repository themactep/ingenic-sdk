...
#define JXH62_SUPPORT_30FPS_PCLK (36*1000*1000)
#define JXH62_SUPPORT_15FPS_PCLK (24*1000*1000)
...
static struct regval_list jxh62_init_regs_1280_720_15fps[] = {
	{0x12,0x40},
	{0x0E,0x10},
	{0x0F,0x01},
	{0x10,0x0A},
...
static struct tx_isp_sensor_win_setting jxh62_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= jxh62_init_regs_1280_720_15fps,
	},
...
static int jxh62_init(struct tx_isp_subdev *sd, int enable)
{
	...
	struct tx_isp_sensor_win_setting *wsize = NULL;// = &jxh62_win_sizes[0];
	int ret = 0;
...
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	unsigned int pclk = JXH62_SUPPORT_30FPS_PCLK;
	unsigned short hts;
	...
	switch (sensor_max_fps) {
	case TX_SENSOR_MAX_FPS_30:
		pclk = JXH62_SUPPORT_30FPS_PCLK;
		max_fps = SENSOR_OUTPUT_MAX_FPS;
		break;
	case TX_SENSOR_MAX_FPS_15:
		pclk = JXH62_SUPPORT_15FPS_PCLK;  //24MHz
...
static int jxh62_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	...
	case TX_SENSOR_MAX_FPS_15:
		wsize = &jxh62_win_sizes[1];
		jxh62_attr.max_integration_time_native = 799;
		jxh62_attr.integration_time_limit = 799;
		jxh62_attr.total_width = 2000;
		jxh62_attr.total_height = 800;
		jxh62_attr.max_integration_time = 799;
		break;
	default:
		printk("Now we do not support this framerate!!!\n");
	}
