...
#define SC1245_CHIP_ID_M	(0x45)
#define SC1245_CHIP_ID_L	(0x03)
...
#define SENSOR_VERSION	"H20181024b"
...
int sc1245_read(struct tx_isp_subdev *sd, uint16_t reg, unsigned char *value)
{
...
int sc1245_write(struct tx_isp_subdev *sd, uint16_t reg, unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int sc1245_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
static int sc1245_write_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != SC1245_REG_END) {
		if (vals->reg_num == SC1245_REG_DELAY) {
...
static int sc1245_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}

static int sc1245_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
	int ret;
	unsigned char v;
	...
	*ident = v;
	...
	if (ret < 0)
		return ret;
	if (v != SC1245_CHIP_ID_M)
		return -ENODEV;
	*ident = (*ident << 8) | v;

	ret = sc1245_read(sd, 0x3020, &v);
	pr_debug("-----%s: %d ret = %d, v = 0x%02x\n", __func__, __LINE__, ret,v);
	if (ret < 0)
		return ret;
...
static int sc1245_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
	...
	/* if (value < 160) { */
	/* 	ret += sc1245_write(sd, 0x3314, 0x14); */
	/* } */
	/* else if(value > 320) { */
	/* 	ret += sc1245_write(sd, 0x3314, 0x04); */
	/* } */
...
static int sc1245_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
...
static int sc1245_set_digital_gain(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int sc1245_get_black_pedestal(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int sc1245_init(struct tx_isp_subdev *sd, int enable)
{
...
static int sc1245_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int sc1245_set_fps(struct tx_isp_subdev *sd, int fps)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	unsigned int pclk = 0;
	unsigned short hts;
...
static int sc1245_set_mode(struct tx_isp_subdev *sd, int value)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
...
static int sc1245_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
	char val = 0;
	...
	val = enable ? 0x60 : 0;
	ret += sc1245_write(sd, 0x3221, val);
...
static int sc1245_g_chip_ident(struct tx_isp_subdev *sd,
		struct tx_isp_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"sc1245_reset");
		if(!ret){
	...
	if (pwdn_gpio != -1) {
		ret = private_gpio_request(pwdn_gpio, "sc1245_pwdn");
		if(!ret){
	...
	if (ret) {
		printk("chip found @ 0x%x (%s) is not an sc1245 chip.\n",
				client->addr, client->adapter->name);
		return ret;
	}
	printk("sc1245 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	if(chip){
		memcpy(chip->name, "sc1245", sizeof("sc1245"));
		chip->ident = ident;
...
static int sc1245_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	long ret = 0;
	...
	switch(cmd){
		case TX_ISP_EVENT_SENSOR_INT_TIME:
			if(arg)
				ret = sc1245_set_integration_time(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_AGAIN:
			if(arg)
				ret = sc1245_set_analog_gain(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_DGAIN:
			if(arg)
				ret = sc1245_set_digital_gain(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
			if(arg)
				ret = sc1245_get_black_pedestal(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_RESIZE:
			if(arg)
				ret = sc1245_set_mode(sd, *(int*)arg);
			break;
		...
		case TX_ISP_EVENT_SENSOR_FPS:
			if(arg)
				ret = sc1245_set_fps(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_VFLIP:
			if(arg)
				ret = sc1245_set_vflip(sd, *(int*)arg);
			break;
		default:
			break;;
	}
...
static int sc1245_g_register(struct tx_isp_subdev *sd, struct tx_isp_dbg_register *reg)
{
	unsigned char val = 0;
	int len = 0;
	int ret = 0;
...
static int sc1245_s_register(struct tx_isp_subdev *sd, const struct tx_isp_dbg_register *reg)
{
	int len = 0;
	len = strlen(sd->chip.name);
	if(len && strncmp(sd->chip.name, reg->name, len)){
...
static struct tx_isp_subdev_core_ops sc1245_core_ops = {
...
static struct tx_isp_subdev_video_ops sc1245_video_ops = {
	.s_stream = sc1245_s_stream,
};

static struct tx_isp_subdev_sensor_ops	sc1245_sensor_ops = {
	.ioctl	= sc1245_sensor_ops_ioctl,
};

static struct tx_isp_subdev_ops sc1245_ops = {
	.core = &sc1245_core_ops,
	.video = &sc1245_video_ops,
	.sensor = &sc1245_sensor_ops,
};
...
//	*(volatile unsigned int*)(0xB0010130) = 0xCAAAAAAA;
	...
	sensor->video.mbus.colorspace = wsize->colorspace;
	sensor->video.fps = wsize->fps;
	tx_isp_subdev_init(&sensor_platform_device, sd, &sc1245_ops);
...
static __init int init_sc1245(void)
{
	...
	if(ret){
		printk("Failed to init sc1245 dirver.\n");
		return -1;
	}

	return private_i2c_add_driver(&sc1245_driver);
}

static __exit void exit_sc1245(void)
{
	private_i2c_del_driver(&sc1245_driver);
}

module_init(init_sc1245);
module_exit(exit_sc1245);

MODULE_DESCRIPTION("A low-level driver for OmniVision sc1245 sensors");
MODULE_LICENSE("GPL");
