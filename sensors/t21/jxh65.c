...
struct again_lut jxh65_again_lut[] = {
	{0x0, 0},
	{0x1, 5731},
	{0x2, 11136},
	{0x3, 16248},
...
{
	struct again_lut *lut = jxh65_again_lut;
	while(lut->gain <= jxh65_attr.max_again) {
		if(isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == jxh65_attr.max_again) && (isp_gain >= lut->gain)) {
				*sensor_again = lut->value;
				return lut->gain;
			}
		}
...
int jxh65_read(struct tx_isp_subdev *sd, unsigned char reg,
	       unsigned char *value)
{
...
int jxh65_write(struct tx_isp_subdev *sd, unsigned char reg,
		unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int jxh65_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
static int jxh65_write_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != JXH65_REG_END) {
		if (vals->reg_num == JXH65_REG_DELAY) {
...
static int jxh65_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}

static int jxh65_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
	unsigned char v;
	int ret;
...
static int jxh65_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	unsigned int expo = value;
	int ret = 0;
	jxh65_write(sd, 0x01, (unsigned char)(expo & 0xff));
	jxh65_write(sd, 0x02, (unsigned char)((expo >> 8) & 0xff));
	if (ret < 0)
		return ret;

	return 0;
}
static int jxh65_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
...
static int jxh65_set_digital_gain(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int jxh65_get_black_pedestal(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int jxh65_init(struct tx_isp_subdev *sd, int enable)
{
...
static int jxh65_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int jxh65_set_fps(struct tx_isp_subdev *sd, int fps)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
...
static int jxh65_set_mode(struct tx_isp_subdev *sd, int value)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
...
static int jxh65_g_chip_ident(struct tx_isp_subdev *sd,
			      struct tx_isp_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"jxh65_reset");
		if(!ret){
	...
	if (pwdn_gpio != -1) {
		ret = private_gpio_request(pwdn_gpio, "jxh65_pwdn");
		if(!ret){
	...
	if (ret) {
		printk("chip found @ 0x%x (%s) is not an jxh65 chip.\n",
		       client->addr, client->adapter->name);
		return ret;
	}
	printk("jxh65 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	if(chip){
		memcpy(chip->name, "jxh65", sizeof("jxh65"));
		chip->ident = ident;
	}
...
static int jxh65_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	long ret = 0;
	...
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = jxh65_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = jxh65_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = jxh65_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = jxh65_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = jxh65_set_mode(sd, *(int*)arg);
		break;
...
static int jxh65_g_register(struct tx_isp_subdev *sd, struct tx_isp_dbg_register *reg)
{
	unsigned char val = 0;
	int len = 0;
	int ret = 0;
...
static int jxh65_s_register(struct tx_isp_subdev *sd, const struct tx_isp_dbg_register *reg)
{
	int len = 0;
...
static struct tx_isp_subdev_core_ops jxh65_core_ops = {
...
static struct tx_isp_subdev_video_ops jxh65_video_ops = {
	.s_stream = jxh65_s_stream,
};

static struct tx_isp_subdev_sensor_ops	jxh65_sensor_ops = {
	.ioctl	= jxh65_sensor_ops_ioctl,
};

static struct tx_isp_subdev_ops jxh65_ops = {
	.core = &jxh65_core_ops,
	.video = &jxh65_video_ops,
	.sensor = &jxh65_sensor_ops,
};
...
static int jxh65_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	...
	sensor->video.mbus.colorspace = wsize->colorspace;
	sensor->video.fps = wsize->fps;
	tx_isp_subdev_init(&sensor_platform_device, sd, &jxh65_ops);
...
static __init int init_jxh65(void)
{
	...
	if(ret){
		printk("Failed to init jxh65 dirver.\n");
		return -1;
	}
	return private_i2c_add_driver(&jxh65_driver);
}
