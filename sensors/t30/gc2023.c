...
static enum v4l2_mbus_pixelcode gc2023_mbus_code[] = {
...
static int gc2023_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
	unsigned char val = 0;

	ret = gc2023_write(sd, 0xfe, 0x00);
	ret += gc2023_read(sd, 0x17, &val);
	if (enable){
		val = val | 0x02;
...
static int gc2023_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	switch(cmd){
		...
		case TX_ISP_EVENT_SENSOR_VFLIP:
			if(arg)
				ret = gc2023_set_vflip(sd, *(int*)arg);
			break;
		default:
			break;;
	}
		...
		const struct i2c_device_id *id)
{
	...
	gc2023_attr.dvp.gpio = sensor_gpio_func;

	switch(sensor_gpio_func){
		case DVP_PA_LOW_10BIT:
		case DVP_PA_HIGH_10BIT:
			mbus = gc2023_mbus_code[0];
			break;
		case DVP_PA_12BIT:
			mbus = gc2023_mbus_code[1];
			break;
		default:
			goto err_set_sensor_gpio;
	}

	for(i = 0; i < ARRAY_SIZE(gc2023_win_sizes); i++)
		gc2023_win_sizes[i].mbus_code = mbus;
...
static __init int init_gc2023(void)
{
	...
	if(ret){
		printk("Failed to init gc2023 dirver.\n");
		return -1;
	}
