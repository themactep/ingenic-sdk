...
struct tx_isp_mipi_bus jxf22_mipi={
	.clk = 800,
	.lans = 1,
};
struct tx_isp_dvp_bus jxf22_dvp={
...
int jxf22_read(struct tx_isp_subdev *sd, unsigned char reg,
		unsigned char *value)
{
...
int jxf22_write(struct tx_isp_subdev *sd, unsigned char reg,
		unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int jxf22_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
static int jxf22_write_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != JXF22_REG_END) {
		if (vals->reg_num == JXF22_REG_DELAY) {
...
static int jxf22_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}

static int jxf22_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
	unsigned char v;
	int ret;
...
static int jxf22_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
...
static int jxf22_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
...
static int jxf22_set_digital_gain(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int jxf22_get_black_pedestal(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int jxf22_init(struct tx_isp_subdev *sd, int enable)
{
...
static int jxf22_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int jxf22_set_fps(struct tx_isp_subdev *sd, int fps)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
	unsigned int sclk = 0;
...
static int jxf22_set_mode(struct tx_isp_subdev *sd, int value)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
...
static int jxf22_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
	unsigned char val = 0;

	ret += jxf22_read(sd, 0x12, &val);
	if (enable){
		val = val | 0x10;
...
static int jxf22_g_chip_ident(struct tx_isp_subdev *sd,
		struct tx_isp_chip_ident *chip)
{
	...
	int ret = ISP_SUCCESS;
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"jxf22_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = private_gpio_request(pwdn_gpio,"jxf22_pwdn");
		if(!ret){
	...
	if (ret) {
		printk("chip found @ 0x%x (%s) is not an jxf22 chip.\n",
				client->addr, client->adapter->name);
		return ret;
	}
	printk("jxf22 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	if(chip){
		memcpy(chip->name, "jxf22", sizeof("jxf22"));
		chip->ident = ident;
...
static int jxf22_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	long ret = 0;
	...
	switch(cmd){
		case TX_ISP_EVENT_SENSOR_INT_TIME:
			if(arg)
				ret = jxf22_set_integration_time(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_AGAIN:
			if(arg)
				ret = jxf22_set_analog_gain(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_DGAIN:
			if(arg)
				ret = jxf22_set_digital_gain(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
			if(arg)
				ret = jxf22_get_black_pedestal(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_RESIZE:
			if(arg)
				ret = jxf22_set_mode(sd, *(int*)arg);
			break;
		...
		case TX_ISP_EVENT_SENSOR_FPS:
			if(arg)
				ret = jxf22_set_fps(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_VFLIP:
			if(arg)
				ret = jxf22_set_vflip(sd, *(int*)arg);
			break;
		default:
			break;;
	}
...
static int jxf22_g_register(struct tx_isp_subdev *sd, struct tx_isp_dbg_register *reg)
{
	unsigned char val = 0;
	int len = 0;
	int ret = 0;
...
static int jxf22_s_register(struct tx_isp_subdev *sd, const struct tx_isp_dbg_register *reg)
{
	int len = 0;
...
static struct tx_isp_subdev_core_ops jxf22_core_ops = {
...
static struct tx_isp_subdev_video_ops jxf22_video_ops = {
	.s_stream = jxf22_s_stream,
};

static struct tx_isp_subdev_sensor_ops	jxf22_sensor_ops = {
	.ioctl	= jxf22_sensor_ops_ioctl,
};

static struct tx_isp_subdev_ops jxf22_ops = {
	.core = &jxf22_core_ops,
	.video = &jxf22_video_ops,
	.sensor = &jxf22_sensor_ops,
};
...
static int jxf22_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (data_interface == TX_SENSOR_DATA_INTERFACE_DVP){
		...
		memcpy((void*)(&(jxf22_attr.dvp)),(void*)(&jxf22_dvp),sizeof(jxf22_dvp));
	} else if (data_interface == TX_SENSOR_DATA_INTERFACE_MIPI){
		wsize->regs = jxf22_init_regs_1920_1080_25fps_mipi;
		memcpy((void*)(&(jxf22_attr.mipi)),(void*)(&jxf22_mipi),sizeof(jxf22_mipi));
	} else{
	...
	sensor->video.mbus.colorspace = wsize->colorspace;
	sensor->video.fps = wsize->fps;
	tx_isp_subdev_init(&sensor_platform_device, sd, &jxf22_ops);
...
static __init int init_jxf22(void)
{
	...
	if(ret){
		printk("Failed to init jxf22 dirver.\n");
		return -1;
	}
	return private_i2c_add_driver(&jxf22_driver);
}

static __exit void exit_jxf22(void)
{
	private_i2c_del_driver(&jxf22_driver);
}
