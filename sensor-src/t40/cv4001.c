...
struct tx_isp_sensor_attribute cv4001_attr={
	...
	.max_integration_time = 3192, /* 3200 - 8 */
	.min_integration_time_native = 4,
	.max_integration_time_native = 3192,
	.integration_time_limit = 3192,
	.total_width = 743,
	.total_height = 3200,
...
static struct tx_isp_sensor_win_setting cv4001_win_sizes[] = {
	{
		...
		.height		= 1520,
		.fps		= 30 << 16 | 1,
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,//RAW
...
static int cv4001_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	...
	exp0 = vmax - it;
	exp0 = (exp0 >> 1) << 1;
	ret = cv4001_write(sd, 0x3060, (unsigned char)(exp0 & 0xff));
	ret += cv4001_write(sd, 0x3061, (unsigned char)((exp0 >> 8) & 0xff));
...
static int cv4001_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret = cv4001_write(sd, 0x3164, again);
...
static int cv4001_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	...
	ret = cv4001_write(sd, 0x3034, val);
	ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
	//if(!ret)
	//	ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
...
static int cv4001_g_chip_ident(struct tx_isp_subdev *sd, struct tx_isp_chip_ident *chip)
{
	...
	ret = cv4001_detect(sd, &ident);	//检查sensor id
	if (ret)
	{
