...
struct tx_isp_sensor_attribute jxq03_attr={
	...
	.max_integration_time_native = 1600 - 4,
	.integration_time_limit = 1600 - 4,
	.total_width = 3600,
	.total_height = 1600,
	.max_integration_time = 1600 - 4,
...
/* 59# */
static struct regval_list jxq03_init_regs_3m_30fps_mipi_wdr[] = {
	{0x12, 0x48},
	{0x48, 0x96},
	{0x48, 0x16},
	{0x0E, 0x11},
...
static struct tx_isp_sensor_win_setting jxq03_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= jxq03_init_regs_3m_30fps_mipi_wdr,
	}
};
...
static int jxq03_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	return 0;
}
...
static int jxq03_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	if(value < 0x10) {
		ret += jxq03_write(sd, 0x0c, tmp1);
		ret += jxq03_write(sd, 0x3b, 0x00);
		ret += jxq03_write(sd, 0x82, tmp2);
	} else {
		ret += jxq03_write(sd, 0x0c, tmp1);
		ret += jxq03_write(sd, 0x3b, 0x38);
		ret += jxq03_write(sd, 0x82, tmp2);
	}
	if (ret < 0)
		return ret;
...
static int jxq03_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
...
static int jxq03_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	int ret = 0;
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	...
	if (init->enable) {
	    if (sensor->video.state == TX_ISP_MODULE_DEINIT){
            ...
            ret += jxq03_read(sd, 0x2f, &reg_0c);
            ret += jxq03_read(sd, 0x82, &reg_82);
            if (ret)
                return ret;
...
#if 0
	ret += jxq03_write(sd, 0xc0, 0x22);
	ret += jxq03_write(sd, 0xc1, (unsigned char)(vts & 0xff));
	ret += jxq03_write(sd, 0xc2, 0x23);
	ret += jxq03_write(sd, 0xc3, (unsigned char)(vts >> 8));
	/*quick launch*/
	ret = jxq03_read(sd, 0x1f, &val);
	val |= 0xc0; /*set bit[7],  register group write function,  auto clean*/
	ret += jxq03_write(sd, 0x1f, val);
#else
	ret += jxq03_write(sd, 0x22, (unsigned char)(vts & 0xff));
	ret += jxq03_write(sd, 0x23, (unsigned char)(vts >> 8));
#endif
	if (0 != ret) {
...
static int jxq03_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
	...
	if(wdr_en == 1){
		...
		jxq03_attr.min_integration_time = 2;
		jxq03_attr.min_integration_time_native = 2;
		jxq03_attr.max_integration_time_native = 1600 - 4;
		jxq03_attr.integration_time_limit = 1600 - 4;
		jxq03_attr.total_width = 3600;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
    ...
    ISP_WARNING("***************>> %s[%d] <<***************", "default_boot", info->default_boot);
    ...
    switch (info->video_interface) {
        case TISP_SENSOR_VI_MIPI_CSI0:
            jxq03_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
            jxq03_attr.mipi.index = 0;
            break;
        ...
        case TISP_SENSOR_VI_DVP:
            jxq03_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
            break;
        default:
            ISP_ERROR("Have no this MCLK Source!!!\n");
    }
...
static int jxq03_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
	...
	ret += jxq03_write(sd, 0xc0, 0x12);
	ret += jxq03_write(sd, 0xc1, val);
	ret += jxq03_write(sd, 0xc2, 0x28);
	ret += jxq03_write(sd, 0xc3, vwinSt);
	ret = jxq03_read(sd, 0x1f, &valg);
	if(ret < 0)
		return -1;
	valg |= 0xc0; /*bit[7], register group write function,auto clean.bit[6] lanch immediately*/
	jxq03_write(sd, 0x1f, valg);
...
static int jxq03_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = jxq03_set_integration_time(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = jxq03_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = jxq03_set_analog_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = jxq03_set_analog_gain_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = jxq03_set_digital_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = jxq03_get_black_pedestal(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = jxq03_set_mode(sd, sensor_val->value);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = jxq03_set_fps(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = jxq03_set_vflip(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_WDR:
		ret = jxq03_set_wdr(sd, init->enable);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		ret = jxq03_set_wdr_stop(sd, init->enable);
		break;
	default:
		break;
	}
...
MODULE_DESCRIPTION("A low-level driver for SOI jxq03 sensors");
MODULE_LICENSE("GPL");
