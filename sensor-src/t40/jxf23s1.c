...
static int jxf23_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	/* printk("[ %s:%d ] vinum=%d, enable=%d\n", __func__, __LINE__,init->vinum, init->enable); */
	if (init->enable) {
...
static const struct i2c_device_id jxf23s1_id[] = {
	{ "jxf23s1", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, jxf23s1_id);

static struct i2c_driver jxf23s1_driver = {
	.driver = {
		.owner	= THIS_MODULE,
		.name	= "jxf23s1",
	},
...
static __init int init_jxf23s1(void)
{
	int ret = 0;
...
static __exit void exit_jxf23s1(void)
{
	private_i2c_del_driver(&jxf23s1_driver);
}

module_init(init_jxf23s1);
module_exit(exit_jxf23s1);
