...
/*
 * the order of the gc3003a_win_sizes is [full_resolution, preview_resolution].
 */
...
static int gc3003a_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret += gc3003a_write(sd, 0x00b8, val_lut[again].again_reg_val_2);
	ret += gc3003a_write(sd, 0x00b9, val_lut[again].again_reg_val_3);
	ret += gc3003a_write(sd, 0x0155, val_lut[again].again_reg_val_4);
	ret += gc3003a_write(sd, 0x0080, val_lut[again].again_reg_val_5);
	/*integration time*/
...
static int gc3003a_set_vflip(struct tx_isp_subdev *sd, int enable)
{
    ...
    ret = gc3003a_write (sd, 0x031d, 0x2d);
	ret += gc3003a_write(sd, 0x0d15, val0);
	ret += gc3003a_write(sd, 0x0015, val1);
	ret += gc3003a_write(sd, 0x031d, 0x28);
	if (0 != ret)
		ISP_ERROR("%s reg write err!!\n");
