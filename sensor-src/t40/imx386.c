...
#define IMX386_CHIP_ID_H	(0x03)
#define IMX386_CHIP_ID_L	(0x86)
#define IMX386_REG_END		0xffff
#define IMX386_REG_DELAY	0xfffe
#define IMX386_SUPPORT_SCLK     (399785760)
#define SENSOR_OUTPUT_MAX_FPS 30
#define SENSOR_OUTPUT_MIN_FPS 5
#define AGAIN_MAX_DB 0x64
#define DGAIN_MAX_DB 0x64
#define LOG2_GAIN_SHIFT 16
#define SENSOR_VERSION	"H20220930a"
...
static struct regval_list imx386_init_regs_3840_2160_30fps_mipi[] = {
	{0x0100, 0x00},
	{0x0101, 0x00},
	{0x0136, 0x18},
	{0x3A7D, 0x00},
...
/*
 * the order of the imx386_win_sizes is [full_resolution, preview_resolution].
 */
static struct tx_isp_sensor_win_setting imx386_win_sizes[] = {
	{
		...
		.regs 		= imx386_init_regs_3840_2160_30fps_mipi,
	}

};
static struct tx_isp_sensor_win_setting *wsize = &imx386_win_sizes[0];
...
int imx386_read(struct tx_isp_subdev *sd, uint16_t reg,	unsigned char *value)
{
	int ret;
...
int imx386_write(struct tx_isp_subdev *sd, uint16_t reg, unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int imx386_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if(sensor->video.state == TX_ISP_MODULE_INIT){
			...
			pr_debug("imx386 stream on\n");
		}
	} else {
		ret = imx386_write_array(sd, imx386_stream_off_mipi);
		sensor->video.state = TX_ISP_MODULE_INIT;
		pr_debug("imx386 stream off\n");
	}
...
static int imx386_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	ret += imx386_read(sd, 0x0342, &tmp);
	hts = tmp & 0x0f;
	ret += imx386_read(sd, 0x0343, &tmp);
	if(ret < 0)
		return -1;
	hts = (hts << 8) + tmp;
...
static int imx386_set_hvflip(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
	uint8_t val;

	/* 2'b01:mirror,2'b10:filp */
	val = imx386_read(sd, 0x0101, &val);
	switch(enable) {
	case 0://normal
		val &= 0xfc;
		break;
	...
	ret = imx386_write(sd, 0x0101, val);
			       ...
			       struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("imx386 chip found @ 0x%02x (%s) version %s\n", client->addr, client->adapter->name,SENSOR_VERSION);
	if(chip){
...
static int imx386_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = imx386_set_hvflip(sd, sensor_val->value);
		break;
	default:
		break;
	}
...
MODULE_DESCRIPTION("A low-level driver for Sony imx386 sensor");
MODULE_LICENSE("GPL");
