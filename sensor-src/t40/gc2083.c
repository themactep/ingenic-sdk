...
#define GC2083_SUPPORT_25FPS_SCLK (82462500)

#define SENSOR_OUTPUT_MIN_FPS 5
#define SENSOR_VERSION	"H20220808a"
...
unsigned int gc2083_alloc_integration_time(unsigned int it, unsigned char shift, unsigned int *sensor_it)
{
...
static struct regval_list gc2083_stream_on[] = {
	{GC2083_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list gc2083_stream_off[] = {
	{GC2083_REG_END, 0x00},	/* END MARKER */
};
...
static int gc2083_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			ret = gc2083_write_array(sd, gc2083_stream_on);
			sensor->video.state = TX_ISP_MODULE_RUNNING;
			pr_debug("gc2083 stream on\n");
			sensor->video.state = TX_ISP_MODULE_RUNNING;
		}
	} else {
		ret = gc2083_write_array(sd, gc2083_stream_off);
...
return 0;
	...
	switch(sensor->info.default_boot){
		case 0:
			wpclk = GC2083_SUPPORT_25FPS_SCLK;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		memcpy(&gc2083_attr.mipi, &gc2083_mipi_linear, sizeof(gc2083_mipi_linear));
		gc2083_attr.one_line_expr_in_us = 22;
		gc2083_attr.total_width = 2932;
		gc2083_attr.total_height = 1125;
		gc2083_attr.max_integration_time_native = 1125 - 8;
		gc2083_attr.integration_time_limit = 1125 - 8;
		gc2083_attr.max_integration_time = 1125 - 8;
                gc2083_attr.again = 0;
