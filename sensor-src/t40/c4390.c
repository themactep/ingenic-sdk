...
#define SENSOR_VERSION	"H20210522a"
...
static struct regval_list c4390_init_regs_2k_25fps_mipi[] = {
	...
	{0x3287,0x4c},  /*for avdd=3.0v*/
	{0x3290,0xa3},
	{0x3211,0x16},
	{0x3216,0x1c},

	{0x0101,0x01}, /* default mirror on */
	{0x3009,0x07}, /* default startx pos */
	{0x300b,0x10}, /* default starty pos */
...
int c4390_read(struct tx_isp_subdev *sd,  uint16_t reg, unsigned char *value)
{
...
int c4390_write(struct tx_isp_subdev *sd, uint16_t reg,unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int c4390_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	return 0;
}
...
static int c4390_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
...
static int c4390_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	int ret = 0;
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
    ...
    switch(info->video_interface){
        case TISP_SENSOR_VI_MIPI_CSI0:
            c4390_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
            c4390_attr.mipi.index = 0;
            break;
        case TISP_SENSOR_VI_MIPI_CSI1:
            c4390_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
            c4390_attr.mipi.index = 1;
            break;
        case TISP_SENSOR_VI_DVP:
            c4390_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
            break;
        default:
			       ...
			       struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("c4390 chip found @ 0x%02x (%s), version %s\n", client->addr, client->adapter->name,SENSOR_VERSION);
	if(chip){
...
static int c4390_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = c4390_set_integration_time(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = c4390_set_analog_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = c4390_set_digital_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = c4390_get_black_pedestal(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = c4390_set_mode(sd, sensor_val->value);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = c4390_set_fps(sd, sensor_val->value);
		break;
	default:
		break;
	}
