...
//	jxf23_read_array(sd, wsize->regs);
...
static int jxf23_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		ISP_WARNING("jxf23s0 stream off\n");
	}
...
static __init int init_jxf23s0(void)
{
...
static __exit void exit_jxf23s0(void)
{
	private_i2c_del_driver(&jxf23_driver);
}

module_init(init_jxf23s0);
module_exit(exit_jxf23s0);
