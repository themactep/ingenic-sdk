...
#define OS04D10_REG_END		0xff
#define OS04D10_REG_PAGE	0xfd
#define OS04D10_REG_DELAY	0xfe

#define OS04D10_SUPPORT_SCLK_FPS_30 (35946240)
...
unsigned int os04d10_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while(lut->gain <= os04d10_attr.max_again) {
		if(isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == os04d10_attr.max_again) && (isp_gain >= lut->gain)) {
				*sensor_again = lut->value;
				return lut->gain;
			}
		}
...
static struct regval_list os04d10_init_regs_2560_1440_30fps[] = {
	{0xfd,0x00},//P0
	{0x20,0x00},
	{0x20,0x01},
	{0x20,0x01},
...
static struct tx_isp_sensor_win_setting os04d10_win_sizes[] = {
	/* 5M @25fps*/
	{
		...
		.regs 		= os04d10_init_regs_2560_1440_30fps,
	},
};
...
static struct regval_list os04d10_stream_on[] = {
	{OS04D10_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list os04d10_stream_off[] = {
	{OS04D10_REG_END, 0x00},	/* END MARKER */
};
	...
	while (vals->reg_num != OS04D10_REG_END) {
		if (vals->reg_num == OS04D10_REG_DELAY) {
			...
			if (vals->reg_num == OS04D10_REG_PAGE){
				val &= 0xf8;
				val |= (vals->value & 0x07);
				ret = os04d10_write(sd, vals->reg_num, val);
				ret = os04d10_read(sd, vals->reg_num, &val);
			}
		}
...
static int os04d10_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret = os04d10_write(sd, 0xfd, 0x01);
	ret += os04d10_write(sd, 0x24, value);
	ret += os04d10_write(sd, 0x01, 0x01);
	if (ret < 0)
		return ret;
...
static int os04d10_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT){
            ret = os04d10_write_array(sd, os04d10_stream_on);
            pr_debug("os04d10 stream on\n");
            sensor->video.state = TX_ISP_MODULE_RUNNING;
		}
	}
	else {
		ret = os04d10_write_array(sd, os04d10_stream_off);
...
static int os04d10_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sclk = OS04D10_SUPPORT_SCLK_FPS_30;
	...
	/* get vb old */
	ret += os04d10_read(sd, 0x05, &val);
	vb = val<<8;
	ret += os04d10_read(sd, 0x06, &val);
	vb |= val;

	/* get vts old */
	ret += os04d10_read(sd, 0x35, &val);
	vts = val<<8;
	ret += os04d10_read(sd, 0x36, &val);
	vts |= val;
	...
	ret += os04d10_write(sd, 0xfd, 0x01);
	ret += os04d10_write(sd, 0x05, (vb >> 8) & 0xff);
	ret += os04d10_write(sd, 0x06, vb & 0xff);
	ret += os04d10_write(sd, 0x01, 0x01);
	if (0 != ret) {
		ISP_ERROR("err: %s os04d10_write err\n",__func__);
		return ret;
	}
...
static int os04d10_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
	unsigned char val = 0;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
    ...
    wsize = &os04d10_win_sizes[0];
    os04d10_attr.max_integration_time_native = 1463;
    os04d10_attr.integration_time_limit = 1463;
    os04d10_attr.total_width = 814;
    os04d10_attr.total_height = 1472;
    os04d10_attr.max_integration_time = 1463;
    os04d10_attr.one_line_expr_in_us = 19;
    os04d10_attr.integration_time = 4;
				...
				struct tx_isp_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"os04d10_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = private_gpio_request(pwdn_gpio,"os04d10_pwdn");
		if(!ret){
	...
	ISP_WARNING("os04d10 chip found @ 0x%02x (%s) version %s \n", client->addr, client->adapter->name, SENSOR_VERSION);
	if(chip){
...
static int os04d10_st(struct tx_isp_subdev *sd, int enable)
{
	return 0;
}
...
static int os04d10_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = os04d10_set_vflip(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_LOGIC:
		if(arg)
			ret = os04d10_st(sd, sensor_val->value);
	default:
		break;
	}
...
static int os04d10_s_register(struct tx_isp_subdev *sd, const struct tx_isp_dbg_register *reg)
{
	...
	if (!private_capable(CAP_SYS_ADMIN))
		return -EPERM;
	os04d10_write(sd, reg->reg & 0xff, reg->val & 0xff);
...
MODULE_DESCRIPTION("A low-level driver for OmniVision os04d10 sensors");
MODULE_LICENSE("GPL");
