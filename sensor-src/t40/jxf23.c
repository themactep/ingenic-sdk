...
static struct regval_list jxf23_init_regs_640_480_30fps_dvp[] = {
        {0x0E, 0x11},
        {0x0F, 0x14},
        {0x10, 0x40},
        {0x11, 0x80},
...
static struct regval_list jxf23_init_regs_640_480_15fps_dvp[] = {
        {0x0E, 0x11},
        {0x0F, 0x14},
        {0x10, 0x40},
        {0x11, 0x80},
...
static struct tx_isp_sensor_win_setting jxf23_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= jxf23_init_regs_640_480_30fps_dvp,
	},
	{
		.width		= 640,
		.height		= 480,
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= jxf23_init_regs_640_480_15fps_dvp,
	}
};
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
                ...
                jxf23_attr.total_height = 1350;
                jxf23_attr.max_integration_time = 1346;
                jxf23_attr.integration_time_limit = 1346;
	...
	case 1:
        ...
        case 5:
                ...
                jxf23_attr.total_width = 0x960;
                jxf23_attr.total_height = 0x258;
                jxf23_attr.max_integration_time = 0x258 - 4;
                jxf23_attr.integration_time_limit = 0x258 - 4;
			      ...
			      struct tx_isp_chip_ident *chip)
{
	...
	if(info->rst_gpio != -1){
		ret = private_gpio_request(info->rst_gpio,"jxf23_reset");
		if(!ret){
	...
	if(info->pwdn_gpio != -1){
		ret = private_gpio_request(info->pwdn_gpio,"jxf23_pwdn");
		if(!ret){
...
static int jxf23_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if(sensor->video.state == TX_ISP_MODULE_INIT){
			...
			if (jxf23_attr.dbus_type == TX_SENSOR_DATA_INTERFACE_DVP){
				/* ret += jxf23_write_array(sd, jxf23_stream_on_dvp); */
			} else if (jxf23_attr.dbus_type == TX_SENSOR_DATA_INTERFACE_MIPI){
