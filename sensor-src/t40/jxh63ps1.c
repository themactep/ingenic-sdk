...
#define JXH63P_SUPPORT_PCLK (43200*1000)
...
static struct regval_list jxh63p_init_regs_1280_720_30fps_mipi[] = {
	...
	{JXH63P_REG_DELAY,0x10},
	{JXH63P_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list jxh63p_init_regs_1280_720_30fps_dvp[] = {
...
static struct tx_isp_sensor_win_setting jxh63p_win_sizes[] = {
	/* [0] 1280*720 @30fps */
	{
		...
		.regs 		= jxh63p_init_regs_1280_720_30fps_dvp,
	},
	/* [1] 1280*720 @30fps */
	{
		.width		= 1280,
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= jxh63p_init_regs_1280_720_30fps_mipi,
	},
};
struct tx_isp_sensor_win_setting *wsize = &jxh63p_win_sizes[1];

/*
 * the part of driver was fixed.
 */

static struct regval_list jxh63p_stream_on[] = {
	{0x12, 0x00},
	{JXH63P_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list jxh63p_stream_off[] = {
	/* Sensor enter LP11*/
	{0x12, 0x40},
	{JXH63P_REG_END, 0x00},	/* END MARKER */
};
...
static int jxh63p_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	return 0;
}
...
static int jxh63p_set_expo(struct tx_isp_subdev* sd, int value)
{
	int ret = 0;
	...
	/*set sensor again */
	ret += jxh63p_write(sd, 0x00, (unsigned char)(again & 0x7f));
...
static int jxh63p_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	...
	int ret = 0;
	jxh63p_write(sd, 0x01, (unsigned char)(expo & 0xff));
	jxh63p_write(sd, 0x02, (unsigned char)((expo >> 8) & 0xff));
	if (ret < 0)
		return ret;
...
{
	/* 0x00 bit[6:0] */
	jxh63p_write(sd, 0x00, (unsigned char)(value & 0x7f));
	return 0;
}
#endif
...
static int jxh63p_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
...
static int jxh63p_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	int ret = 0;
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			ret = jxh63p_write_array(sd, jxh63p_stream_on);
			sensor->video.state = TX_ISP_MODULE_RUNNING;
			pr_debug("jxh63p stream on\n");
			sensor->video.state = TX_ISP_MODULE_RUNNING;
		}
	}
	else {
		ret = jxh63p_write_array(sd, jxh63p_stream_off);
		pr_debug("jxh63p stream off\n");
		sensor->video.state = TX_ISP_MODULE_DEINIT;
	}
	return ret;
}
...
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	unsigned int pclk = JXH63P_SUPPORT_PCLK;
	unsigned short hts;
	...
	if(newformat > (sensor_max_fps << 8) || newformat < (SENSOR_OUTPUT_MIN_FPS << 8))
		return -1;
	ret += jxh63p_read(sd, 0x21, &tmp);
	hts = tmp;
	ret += jxh63p_read(sd, 0x20, &tmp);
	if(ret < 0)
		return -1;
	hts = (hts << 8) + tmp;
	hts <<= 1;
	...
	jxh63p_write(sd, 0xc0, 0x22);
	jxh63p_write(sd, 0xc1, (unsigned char)(vts & 0xff));
	jxh63p_write(sd, 0xc2, 0x23);
	jxh63p_write(sd, 0xc3, (unsigned char)(vts >> 8));
	ret = jxh63p_read(sd, 0x1f, &tmp);
	if(ret < 0)
		return -1;
	tmp |= (1 << 7); //set bit[7],  register group write function,  auto clean
	jxh63p_write(sd, 0x1f, tmp);
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		jxh63p_attr.total_width = 1920;
		jxh63p_attr.total_height = 750;
		jxh63p_attr.min_integration_time = 1;
		jxh63p_attr.max_integration_time = 750 -1;
		jxh63p_attr.min_integration_time_native = 1;
		jxh63p_attr.max_integration_time_native = 750 - 1;
		jxh63p_attr.integration_time_limit = 750 - 1;
		jxh63p_attr.again = 0;
		jxh63p_attr.integration_time = 0x6f;
		pr_debug("----->dvp\n");
		break;
	case 1:
		wsize = &jxh63p_win_sizes[1];
		jxh63p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		memcpy((void*)(&(jxh63p_attr.mipi)),(void*)(&jxh63p_mipi),sizeof(jxh63p_mipi));
		jxh63p_attr.total_width = 1920;
		jxh63p_attr.total_height = 750;
		jxh63p_attr.min_integration_time = 1;
		jxh63p_attr.max_integration_time = 750 - 4;
		jxh63p_attr.min_integration_time_native = 1;
		jxh63p_attr.max_integration_time_native = 750 - 4;
		jxh63p_attr.integration_time_limit = 750 - 4;
		jxh63p_attr.again = 0;
		jxh63p_attr.integration_time = 0x6f;
		pr_debug("----->mipi\n");
		break;
	...
	case TISP_SENSOR_VI_MIPI_CSI0:
		jxh63p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		jxh63p_attr.mipi.index = 0;
		break;
	case TISP_SENSOR_VI_MIPI_CSI1:
		jxh63p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		jxh63p_attr.mipi.index = 1;
		break;
	case TISP_SENSOR_VI_DVP:
		jxh63p_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
		break;
	default:
			      ...
			      struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("jxh63ps1 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	if(chip){
...
static int jxh63p_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = jxh63p_set_expo(sd, sensor_val->value);
		break;
#if 0
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = jxh63p_set_integration_time(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = jxh63p_set_analog_gain(sd, sensor_val->value);
		break;
#endif
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = jxh63p_set_digital_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = jxh63p_get_black_pedestal(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = jxh63p_set_mode(sd, sensor_val->value);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = jxh63p_set_fps(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = jxh63p_set_hvflip(sd, sensor_val->value);
		break;
	default:
		break;
	}
...
MODULE_DESCRIPTION("A low-level driver for SOI jxh63p sensors");
MODULE_LICENSE("GPL");
