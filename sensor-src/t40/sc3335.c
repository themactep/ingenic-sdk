...
#define SC3335_SUPPORT_20FPS_SCLK (72000000)
...
static struct regval_list sc3335_init_regs_2304_1296_20fps_mipi[] = {
...
static struct tx_isp_sensor_win_setting sc3335_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= sc3335_init_regs_2304_1296_20fps_mipi,
	},
	{
		.width		= 2304,
		.height		= 1296,
...
static int sc3335_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	return 0;
}

static int sc3335_fsync(struct tx_isp_subdev *sd, struct tx_isp_frame_sync *sync)
{
	sc3335_write(sd, 0x3222, 0x02);
...
static int sc3335_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
...
static int sc3335_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	int ret = 0;
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
...
static int sc3335_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case TX_SENSOR_MAX_FPS_20:
		sclk = SC3335_SUPPORT_20FPS_SCLK;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		sensor_max_fps = TX_SENSOR_MAX_FPS_30;
	        sc3335_attr.again = 0;
                sc3335_attr.integration_time = 0xa82;
		break;
	case 1:
		wsize = &sc3335_win_sizes[1];
		memcpy((void*)(&(sc3335_attr.mipi)),(void*)(&sc3335_mipi),sizeof(sc3335_mipi));
		sc3335_attr.mipi.clk = 360;
		sc3335_attr.max_integration_time_native = 1440 - 5;
		sc3335_attr.integration_time_limit = 1440 - 5;
		sc3335_attr.total_width = 2500;
		sc3335_attr.total_height = 1440;
		sc3335_attr.max_integration_time = 1440 - 5;
		sc3335_attr.one_line_expr_in_us = 35;
		sensor_max_fps = TX_SENSOR_MAX_FPS_20;
	        sc3335_attr.again = 0;
                sc3335_attr.integration_time = 0xb36;
		break;
	default:
	...
	case TISP_SENSOR_VI_MIPI_CSI0:
		sc3335_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		sc3335_attr.mipi.index = 0;
		break;
	case TISP_SENSOR_VI_MIPI_CSI1:
		sc3335_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		sc3335_attr.mipi.index = 1;
		break;
	case TISP_SENSOR_VI_DVP:
		sc3335_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
		break;
	default:
...
static int sc3335_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = sc3335_set_expo(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		//		if(arg)
		//			ret = sc3335_set_integration_time(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		//		if(arg)
		//			ret = sc3335_set_analog_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = sc3335_set_digital_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = sc3335_get_black_pedestal(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = sc3335_set_mode(sd, sensor_val->value);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = sc3335_set_fps(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = sc3335_set_vflip(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_LOGIC:
		if(arg)
			ret = sc3335_set_logic(sd, sensor_val->value);
		break;
	default:
		break;
	}
