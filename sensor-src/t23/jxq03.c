...
static int jxq03_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	if(value < 0x10) {
		ret += jxq03_write(sd, 0x2f, r2f_val | 0x20);
		ret += jxq03_write(sd, 0x0c, r0c_val | 0x40);
		ret += jxq03_write(sd, 0x82, r82_val | 0x02);
	} else {
		ret += jxq03_write(sd, 0x2f, r2f_val & 0xdf);
		ret += jxq03_write(sd, 0x0c, r0c_val & 0xbf);
		ret += jxq03_write(sd, 0x82, r82_val & 0xfd);
	}
...
static int jxq03_init(struct tx_isp_subdev *sd, int enable)
{
	...
	ret += jxq03_read(sd, 0x2f, &r2f_val);
	ret += jxq03_read(sd, 0x0c, &r0c_val);
	ret += jxq03_read(sd, 0x82, &r82_val);
