...
static struct regval_list sensor_init_regs_3m_25fps_mipi_wdr[] = {
	{0x12, 0x48},
	{0x48, 0x96},
	{0x48, 0x16},
	{0x0E, 0x11},
...
static struct regval_list sensor_init_regs_3m_25fps_mipi[] = {
	{0x12, 0x40},
	{0x48, 0x96},
	{0x48, 0x16},
	{0x0E, 0x11},
...
static struct tx_isp_sensor_win_setting sensor_win_sizes[] = {
	/* 2304*1296 */
	{
		...
		.regs = sensor_init_regs_3m_25fps_mipi,
	},
	{
		.width = 2304,
		.height = 1296,
		...
		.mbus_code = V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace = V4L2_COLORSPACE_SRGB,
		.regs = sensor_init_regs_3m_25fps_mipi_wdr,
	}
};
...
static int sensor_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	if (value < 0x10) {
		...
		ret += sensor_write(sd, 0x82, r82_val | 0x02);
	} else {
...
#if 0
	if (value < 0x10) {
		ret += sensor_write(sd, 0xc0, 0x2f);
		ret += sensor_write(sd, 0xc1, 0x64);
		ret += sensor_write(sd, 0xc2, 0x0c);
		ret += sensor_write(sd, 0xc3, 0x40);
		ret += sensor_write(sd, 0xc4, 0x82);
		ret += sensor_write(sd, 0xc5, 0x02);
	} else {
		ret += sensor_write(sd, 0xc0, 0x2f);
		ret += sensor_write(sd, 0xc1, 0x44);
		ret += sensor_write(sd, 0xc2, 0x0c);
		ret += sensor_write(sd, 0xc3, 0x00);
		ret += sensor_write(sd, 0xc4, 0x82);
		ret += sensor_write(sd, 0xc5, 0x00);
	}
...
static int sensor_init(struct tx_isp_subdev *sd, int enable)
{
	...
	ret += sensor_read(sd, 0x82, &r82_val);
...
static int sensor_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
	...
	if (wdr_en == 1) {
		...
		sensor_attr.max_integration_time_short = 156;
		sensor_attr.min_integration_time = 2;
		sensor_attr.min_integration_time_native = 2;
		sensor_attr.max_integration_time_native = 2503;
		sensor_attr.integration_time_limit = 2503;
		sensor_attr.total_width = 3600;
		sensor_attr.total_height = 2666;
		sensor_attr.max_integration_time = 2503;
