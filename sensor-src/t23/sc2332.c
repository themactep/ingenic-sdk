...
#define SC2332_SUPPORT_30FPS_SCLK_MIPI (74250000)
#define SC2332_SUPPORT_30FPS_SCLK_DVP (78000000)
#define SENSOR_OUTPUT_MAX_FPS 30
#define SENSOR_OUTPUT_MIN_FPS 5
#define SENSOR_VERSION	"H20200825a"
...
static struct regval_list sc2332_init_regs_1920_1080_25fps_dvp[] = {
	{SC2332_REG_END, 0x00},	/* END MARKER */
};
...
static struct tx_isp_sensor_win_setting sc2332_win_sizes[] = {
	/* 1920*1080 */
	{
		...
		.regs 		= sc2332_init_regs_1920_1080_25fps_dvp,
	},
	{
		.width		= 1920,
		.height		= 1080,
...
static int sc2332_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	...
	if (value < 250) {
		ret += sc2332_write(sd, 0x3314, 0x14);
	} else if (value > 450){
		ret += sc2332_write(sd, 0x3314, 0x04);
	}
...
static int sc2332_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	/* denoise logic */
	sc2332_write(sd,0x3812,0x00);
	if (value < 0x720) {
		sc2332_write(sd,0x3634,0x64);
	}else if (value <= 0x720 && value < 0xf20){
		sc2332_write(sd,0x3634,0x64);
	} else if(value >= 0xf20 && value < 0x1f20){
		sc2332_write(sd,0x3634,0x44);
	} else if(value >= 0x1f20){
		sc2332_write(sd,0x3634,0x24);
	}
	sc2332_write(sd,0x3812,0x30);
...
static __init int init_sc2332(void)
{
	...
	if(ret){
		ISP_ERROR("Failed to init sc2332 driver.\n");
		return -1;
	}
