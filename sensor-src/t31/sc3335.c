...
#define SENSOR_SUPPORT_25FPS_SCLK (86375000)
...
static int sensor_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	if (gain_val != again) {
		ret = sensor_read(sd, 0x3109, &logic_trig);
		if (ret < 0)
			return ret;
		if (1 == logic_trig) {
			if (gain_val< 0x740) {
...
static int sensor_set_logic(struct tx_isp_subdev *sd, int value)
{
	...
	ret = sensor_read(sd, 0x3974, &temp_val);
	if (ret != 0) {
		return ret;
	...
	if ((temp_val >= 0x40) && (cur_lut_node >= 65)) {
		sensor_attr.max_again = lut[cur_lut_node - 2].gain;
		cur_lut_node -= 2;
		node_change = 1;
...
static int sensor_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	case TX_SENSOR_MAX_FPS_25:
		...
		sensor_attr.max_integration_time_native = 1382 - 5;
		sensor_attr.integration_time_limit = 1382 - 5;
		sensor_attr.total_width = 2200;
		sensor_attr.total_height = 1382;
		sensor_attr.max_integration_time = 1382 - 5;
		break;
	default:
		ISP_ERROR("do not support max framerate %d in mipi mode\n",sensor_max_fps);
	}
...
static __init int init_sensor(void)
{
	...
	if (ret) {
		ISP_ERROR("Failed to init sc3335 driver.\n");
		return -1;
	}
