...
#define SENSOR_CHIP_ID 0x530342
...
static struct regval_list sensor_init_regs_2304_1296_25fps[] = {
	{0xfd,0x00},
	{0x20,0x00},
	{0xfd,0x00},
	{0x36,0x01},
...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	ret += sensor_write(sd, 0xfd, 0x01);
	ret += sensor_write(sd, 0x4e, (unsigned char)(vts >> 8));
	ret += sensor_write(sd, 0x4f, (unsigned char)(vts & 0xff));
	ret += sensor_write(sd, 0x01, 0x01);
	if (0 != ret) {
...
static __init int init_sensor(void)
{
	...
	if (ret) {
		ISP_ERROR("Failed to init os03b10 driver.\n");
		return -1;
	}
