...
#define SENSOR_CHIP_ID 0x1034
#define SENSOR_BUS_TYPE TX_SENSOR_CONTROL_INTERFACE_I2C
#define SENSOR_I2C_ADDRESS 0x21
#define SENSOR_MAX_WIDTH 1726
#define SENSOR_MAX_HEIGHT 750
...
int gc1034_read(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *value)
{
...
static int gc1034_write(struct v4l2_subdev *sd, unsigned char reg,
			unsigned char value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int gc1034_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
static int gc1034_write_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != SENSOR_FLAG_END) {
		if (vals->reg_num == SENSOR_FLAG_DELAY) {
...
static int gc1034_reset(struct v4l2_subdev *sd, u32 val)
{
	return 0;
}

static int gc1034_detect(struct v4l2_subdev *sd, unsigned int *ident)
{
	unsigned char v;
	int ret;
...
static int gc1034_set_integration_time(struct v4l2_subdev *sd, int value)
{
	int ret = 0;

	ret = gc1034_write(sd, 0x4, value & 0xff);
	if (ret < 0) {
		printk("gc1034_write error  %d\n" ,__LINE__);
		return ret;
	}
...
static int gc1034_set_analog_gain(struct v4l2_subdev *sd, int value)
{
	int ret = 0;

	gc1034_write(sd, 0xfe, 0x01);
	ret = gc1034_write(sd, 0xb6, (value >> 12) & 0xf);
	if (ret < 0) {
		printk("gc1034_write error  %d" ,__LINE__ );
		return ret;
	}
	...
	gc1034_write(sd, 0xfe, 0x00);

	return 0;
}

static int gc1034_set_digital_gain(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int gc1034_get_black_pedestal(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int gc1034_init(struct v4l2_subdev *sd, u32 enable)
{
...
static int gc1034_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int gc1034_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int gc1034_set_fps(struct tx_isp_sensor *sensor, int fps)
{
	struct tx_isp_notify_argument arg;
...
static int gc1034_set_mode(struct tx_isp_sensor *sensor, int value)
{
	struct tx_isp_notify_argument arg;
	struct v4l2_subdev *sd = &sensor->sd;
...
static int gc1034_g_chip_ident(struct v4l2_subdev *sd,
		struct v4l2_dbg_chip_ident *chip)
{
	...
	if (reset_gpio != -1) {
		ret = gpio_request(reset_gpio,"gc1034_reset");
		if (!ret) {
	...
	if (pwdn_gpio != -1) {
		ret = gpio_request(pwdn_gpio,"gc1034_pwdn");
		if (!ret) {
	...
	if (ret) {
		v4l_err(client,
			"chip found @ 0x%x (%s) is not an gc1034 chip.\n",
			client->addr, client->adapter->name);
		return ret;
	}
	v4l_info(client, "gc1034 chip found @ 0x%02x (%s)\n",
		client->addr, client->adapter->name);

	return v4l2_chip_ident_i2c_client(client, chip, ident, 0);
}

static int gc1034_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}

static long gc1034_ops_private_ioctl(struct tx_isp_sensor *sensor, struct isp_private_ioctl *ctrl)
{
	struct v4l2_subdev *sd = &sensor->sd;
	long ret = 0;
	switch(ctrl->cmd) {
	case TX_ISP_PRIVATE_IOCTL_SENSOR_INT_TIME:
		ret = gc1034_set_integration_time(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_AGAIN:
		ret = gc1034_set_analog_gain(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_DGAIN:
		ret = gc1034_set_digital_gain(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_BLACK_LEVEL:
		ret = gc1034_get_black_pedestal(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_RESIZE:
		ret = gc1034_set_mode(sensor,ctrl->value);
		break;
	...
	case TX_ISP_PRIVATE_IOCTL_SENSOR_FPS:
		ret = gc1034_set_fps(sensor, ctrl->value);
		break;
	default:
		break;
	}
...
static long gc1034_ops_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct tx_isp_sensor *sensor =container_of(sd, struct tx_isp_sensor, sd);
	int ret;
	...
	case VIDIOC_ISP_PRIVATE_IOCTL:
		ret = gc1034_ops_private_ioctl(sensor, arg);
		break;
	default:
		return -1;
		break;
...
static int gc1034_g_register(struct v4l2_subdev *sd, struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	unsigned char val = 0;
	int ret;
...
static int gc1034_s_register(struct v4l2_subdev *sd, const struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static const struct v4l2_subdev_core_ops gc1034_core_ops = {
	.g_chip_ident = gc1034_g_chip_ident,
	.reset = gc1034_reset,
	.init = gc1034_init,
	.s_power = gc1034_s_power,
	.ioctl = gc1034_ops_ioctl,
#ifdef CONFIG_VIDEO_ADV_DEBUG
...
static const struct v4l2_subdev_video_ops gc1034_video_ops = {
	...
	.s_parm = gc1034_s_parm,
	.g_parm = gc1034_g_parm,
};

static const struct v4l2_subdev_ops gc1034_ops = {
		...
		const struct i2c_device_id *id)
{
	...
	gc1034_attr.dvp.gpio = sensor_gpio_func;
	 /*
		convert sensor-gain into isp-gain,
	 */
	gc1034_attr.max_again = log2_fixed_to_fixed(gc1034_attr.max_again, TX_ISP_GAIN_FIXED_POINT, LOG2_GAIN_SHIFT);
...
{
	return i2c_add_driver(&gc1034_driver);
}

static __exit void exit_sensor(void)
{
	i2c_del_driver(&gc1034_driver);
}
