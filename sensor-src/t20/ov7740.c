...
#define SENSOR_CHIP_ID 0x7740
#define SENSOR_BUS_TYPE TX_SENSOR_CONTROL_INTERFACE_I2C
#define SENSOR_I2C_ADDRESS 0x21
#define SENSOR_MAX_WIDTH 791
#define SENSOR_MAX_HEIGHT 252
...
static struct tx_isp_sensor_win_setting sensor_win_sizes[] = {
	/* 320*240 */
	{
		...
		.regs = sensor_init_regs_320_240_60fps,
	}
};
...
static int sensor_set_integration_time(struct v4l2_subdev *sd, int value)
{
	...
	sensor_write(sd, 0x0f, (unsigned char)((expo >> 8) & 0xff));
	if (ret < 0)
		return ret;
...
{
	sensor_write(sd, 0x00, (unsigned char)(value & 0xff));
	sensor_write(sd, 0x15, (unsigned char)((value >> 8)& 0x03));
	return 0;
}
...
static int sensor_set_fps(struct tx_isp_sensor *sensor, int fps)
{
	...
	if (newformat > (SENSOR_OUTPUT_MAX_FPS << 8) || newformat < (SENSOR_OUTPUT_MIN_FPS << 8))
		return -1;
	ret += sensor_read(sd, 0x2a, &tmp);
	hts = tmp;
	ret += sensor_read(sd, 0x29, &tmp);
	if (ret < 0)
		return -1;
	hts = (hts << 8) + tmp;
	/*vts = (pclk << 4) / (hts * (newformat >> 4));*/
	vts = pclk * (fps & 0xffff) / hts / ((fps & 0xffff0000) >> 16);
	ret = sensor_write(sd, 0x2b, (unsigned char)(vts & 0xff));
	ret += sensor_write(sd, 0x2c, (unsigned char)(vts >> 8));
	if (ret < 0)
		return -1;
