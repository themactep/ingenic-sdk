...
#define OV2735_REG_END		0xff
#define OV2735_REG_DELAY	0x00
...
struct again_lut ov2735_again_lut[] = {
	{0x10,	0},
	{0x11,	5731},
	{0x12,	11136},
	{0x13,	16248},
...
struct tx_isp_sensor_attribute ov2735_attr;

unsigned int ov2735_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
...
unsigned int ov2735_alloc_dgain(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_dgain)
{
	return isp_gain;
}
...
static struct regval_list ov2735_init_regs_1920_1080_25fps_dvp[] = {

	{0xfd, 0x00},
	/*{0x20, 0x00},*///soft reset enable, cause i2c error
	{OV2735_REG_DELAY, 0x05},
	{0xfd, 0x00},
	{0x2f, 0x10},
	{0x34, 0x00},
	{0x30, 0x15},
	...
	{OV2735_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list ov2735_init_regs_1920_1080_15fps_dvp[] = {
	{0xfd, 0x00},
	/* {0x20, 0x00},//soft reset enable, cause i2c error */
	{OV2735_REG_DELAY, 0x05},
	{0xfd, 0x00},
...
static enum v4l2_mbus_pixelcode ov2735_mbus_code[] = {
...
static struct regval_list ov2735_stream_on[] = {
	{0xfd, 0x00},
	{0x36, 0x00},
...
static struct regval_list ov2735_stream_off[] = {
	{0xfd, 0x00},
	{0x36, 0x01},
...
int ov2735_read(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *value)
{
...
static int ov2735_write(struct v4l2_subdev *sd, unsigned char reg,
			unsigned char value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int ov2735_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
	while (vals->reg_num != OV2735_REG_END) {
		if (vals->reg_num == OV2735_REG_DELAY) {
			...
			ret = ov2735_read(sd, vals->reg_num, &val);
			if (ret < 0)
				return ret;
			if (vals->reg_num == OV2735_PAGE_REG){
				val &= 0xf8;
				val |= (vals->value & 0x07);
				ret = ov2735_write(sd, vals->reg_num, val);
				ret = ov2735_read(sd, vals->reg_num, &val);
			}
		}
		vals++;
...
static int ov2735_write_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != OV2735_REG_END) {
		if (vals->reg_num == OV2735_REG_DELAY) {
			...
			ret = ov2735_write(sd, vals->reg_num, vals->value);
			if (ret < 0)
				return ret;
		}
		vals++;
...
static int ov2735_reset(struct v4l2_subdev *sd, u32 val)
{
	return 0;
}

static int ov2735_detect(struct v4l2_subdev *sd, unsigned int *ident)
{
	unsigned char v;
	int ret;
...
static int ov2735_set_integration_time(struct v4l2_subdev *sd, int value)
{
...
static int ov2735_set_analog_gain(struct v4l2_subdev *sd, int value)
{
	int ret = 0;
	ret = ov2735_write(sd, 0xfd, 0x01);
...
static int ov2735_set_digital_gain(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int ov2735_get_black_pedestal(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int ov2735_init(struct v4l2_subdev *sd, u32 enable)
{
	...
	if (ret)
		return ret;
	/*ret = ov2735_read_array(sd, wsize->regs);
	if (ret)
		return ret;*/
...
static int ov2735_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int ov2735_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int ov2735_set_fps(struct tx_isp_sensor *sensor, int fps)
{
...
static int ov2735_set_mode(struct tx_isp_sensor *sensor, int value)
{
	struct tx_isp_notify_argument arg;
	struct v4l2_subdev *sd = &sensor->sd;
...
static int ov2735_g_chip_ident(struct v4l2_subdev *sd,
		struct v4l2_dbg_chip_ident *chip)
{
	...
	if (ret) {
		v4l_err(client,
			"chip found @ 0x%x (%s) is not an ov2735 chip.\n",
			client->addr, client->adapter->name);
		return ret;
	}
	v4l_info(client, "ov2735 chip found @ 0x%02x (%s)\n",
		client->addr, client->adapter->name);
...
static long ov2735_ops_private_ioctl(struct tx_isp_sensor *sensor, struct isp_private_ioctl *ctrl)
{
	struct v4l2_subdev *sd = &sensor->sd;
	long ret = 0;
	switch(ctrl->cmd){
		case TX_ISP_PRIVATE_IOCTL_SENSOR_INT_TIME:
			ret = ov2735_set_integration_time(sd, ctrl->value);
			break;
		case TX_ISP_PRIVATE_IOCTL_SENSOR_AGAIN:
			ret = ov2735_set_analog_gain(sd, ctrl->value);
			break;
		case TX_ISP_PRIVATE_IOCTL_SENSOR_DGAIN:
			ret = ov2735_set_digital_gain(sd, ctrl->value);
			break;
		case TX_ISP_PRIVATE_IOCTL_SENSOR_BLACK_LEVEL:
			ret = ov2735_get_black_pedestal(sd, ctrl->value);
			break;
		case TX_ISP_PRIVATE_IOCTL_SENSOR_RESIZE:
			ret = ov2735_set_mode(sensor,ctrl->value);
			break;
		...
		case TX_ISP_PRIVATE_IOCTL_SENSOR_FPS:
			ret = ov2735_set_fps(sensor, ctrl->value);
			break;
		default:
...
static long ov2735_ops_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct tx_isp_sensor *sensor =container_of(sd, struct tx_isp_sensor, sd);
	int ret;
	switch(cmd){
		case VIDIOC_ISP_PRIVATE_IOCTL:
			ret = ov2735_ops_private_ioctl(sensor, arg);
			break;
		default:
			return -1;
			break;
...
static int ov2735_g_register(struct v4l2_subdev *sd, struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	unsigned char val = 0;
	int ret;
...
static int ov2735_s_register(struct v4l2_subdev *sd, const struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static const struct v4l2_subdev_video_ops ov2735_video_ops = {
	...
	.s_parm = ov2735_s_parm,
	.g_parm = ov2735_g_parm,
};

static const struct v4l2_subdev_ops ov2735_ops = {
		...
		const struct i2c_device_id *id)
{
...
#if 0
	ov2735_attr.dvp.gpio = sensor_gpio_func;

	switch(sensor_gpio_func){
		case DVP_PA_LOW_10BIT:
		case DVP_PA_HIGH_10BIT:
			mbus = ov2735_mbus_code[0];
			break;
		case DVP_PA_12BIT:
			mbus = ov2735_mbus_code[1];
			break;
		default:
			goto err_set_sensor_gpio;
	}

	for(i = 0; i < ARRAY_SIZE(ov2735_win_sizes); i++)
		ov2735_win_sizes[i].mbus_code = mbus;
	...
	v4l2_i2c_subdev_init(sd, client, &ov2735_ops);
...
{
	return i2c_add_driver(&ov2735_driver);
}
...
MODULE_DESCRIPTION("A low-level driver for OmniVision ov2735 sensors");
MODULE_LICENSE("GPL");
