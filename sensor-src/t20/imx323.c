...
int imx323_write(struct v4l2_subdev *sd, uint16_t reg,
		 unsigned char value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int imx323_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
static int imx323_write_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != SENSOR_REG_END) {
		if (vals->reg_num == SENSOR_REG_DELAY) {
...
static int imx323_reset(struct v4l2_subdev *sd, u32 val)
{
	return 0;
}

static int imx323_detect(struct v4l2_subdev *sd, unsigned int *ident)
{
	unsigned char v;
	int ret;
...
static int imx323_set_analog_gain(struct v4l2_subdev *sd, int value)
{
	int ret = 0;
...
static int imx323_set_digital_gain(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int imx323_get_black_pedestal(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int imx323_init(struct v4l2_subdev *sd, u32 enable)
{
...
static int imx323_s_stream(struct v4l2_subdev *sd, int enable)
{
	int ret = 0;
	if (enable) {
...
static int imx323_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int imx323_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}
...
static int imx323_g_chip_ident(struct v4l2_subdev *sd,
			       struct v4l2_dbg_chip_ident *chip)
{
	...
	int ret = ISP_SUCCESS;
	if (reset_gpio != -1) {
		ret = gpio_request(reset_gpio,"imx323_reset");
		if (!ret) {
	...
	if (pwdn_gpio != -1) {
		ret = gpio_request(pwdn_gpio,"imx323_pwdn");
		if (!ret) {
	...
	if (ret) {
		v4l_err(client,
			"chip found @ 0x%x (%s) is not an imx323 chip.\n",
			client->addr, client->adapter->name);
		return ret;
	}
	v4l_info(client, "imx323 chip found @ 0x%02x (%s)\n",
		 client->addr, client->adapter->name);
...
static long imx323_ops_private_ioctl(struct tx_isp_sensor *sensor, struct isp_private_ioctl *ctrl)
{
	struct v4l2_subdev *sd = &sensor->sd;
	long ret = 0;
	switch(ctrl->cmd) {
	case TX_ISP_PRIVATE_IOCTL_SENSOR_INT_TIME:
		ret = imx323_set_integration_time(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_AGAIN:
		ret = imx323_set_analog_gain(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_DGAIN:
		ret = imx323_set_digital_gain(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_BLACK_LEVEL:
		ret = imx323_get_black_pedestal(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_RESIZE:
		ret = imx323_set_mode(sensor,ctrl->value);
		break;
	...
	case TX_ISP_PRIVATE_IOCTL_SENSOR_FPS:
		ret = imx323_set_fps(sensor, ctrl->value);
		break;
	default:
		break;;
	}
...
static long imx323_ops_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct tx_isp_sensor *sensor =container_of(sd, struct tx_isp_sensor, sd);
	int ret;
	switch(cmd) {
	case VIDIOC_ISP_PRIVATE_IOCTL:
		ret = imx323_ops_private_ioctl(sensor, arg);
		break;
	default:
		return -1;
		break;
...
static int imx323_g_register(struct v4l2_subdev *sd, struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	unsigned char val = 0;
	int ret;
...
static int imx323_s_register(struct v4l2_subdev *sd, const struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static const struct v4l2_subdev_video_ops imx323_video_ops = {
	...
	.s_parm = imx323_s_parm,
	.g_parm = imx323_g_parm,
};

static const struct v4l2_subdev_ops imx323_ops = {
			...
			const struct i2c_device_id *id)
{
	...
	v4l2_i2c_subdev_init(sd, client, &imx323_ops);
...
{
	return i2c_add_driver(&imx323_driver);
}

static __exit void exit_sensor(void)
{
	i2c_del_driver(&imx323_driver);
}
