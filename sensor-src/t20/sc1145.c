...
#define SENSOR_NAME "sc1145"
#define SENSOR_CHIP_ID 0x1145
#define SENSOR_BUS_TYPE TX_SENSOR_CONTROL_INTERFACE_I2C
#define SENSOR_I2C_ADDRESS 0x30
#define SENSOR_MAX_WIDTH 1920
#define SENSOR_MAX_HEIGHT 900
#define SENSOR_CHIP_ID_H (0x11)
...
static int sensor_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		printk("sc1145 stream on\n");
	}
	else {
		ret = sensor_write_array(sd, sensor_stream_off);
		printk("sc1145 stream off\n");
	}
	return ret;
}
		...
		struct v4l2_dbg_chip_ident *chip)
{
	...
	if (ret) {
		v4l_err(client,
				"chip found @ 0x%x (%s) is not an sc1145 chip.\n",
				client->addr, client->adapter->name);
		return ret;
	}
	v4l_info(client, "sc1145 chip found @ 0x%02x (%s)\n",
			client->addr, client->adapter->name);
...
MODULE_DESCRIPTION("A low-level driver for OmniVision sc1145 sensors");
MODULE_LICENSE("GPL");
