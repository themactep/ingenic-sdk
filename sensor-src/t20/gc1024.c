...
unsigned int gc1024_gainone_to_reg(unsigned int gain_one, unsigned int *regs)
{
...
unsigned int gc1024_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	gain_one1 = gc1024_gainone_to_reg(gain_one, &regs);
	isp_gain1 = log2_fixed_to_fixed(gain_one1, TX_ISP_GAIN_FIXED_POINT, shift);
	*sensor_again = regs;
	return isp_gain1;
}
unsigned int gc1024_alloc_dgain(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_dgain)
{
	return isp_gain;
}
/*
...
struct tx_isp_sensor_attribute gc1024_attr={
	...
	.again_apply_delay = 2,
	.dgain_apply_delay = 2,
	.sensor_ctrl.alloc_again = gc1024_alloc_again,
	.sensor_ctrl.alloc_dgain = gc1024_alloc_dgain,
};

static struct regval_list gc1024_init_regs_1280_720[] = {
	/*SYS*/
	{0xfe,0x80},
	{0xfe,0x80},
	{0xfe,0x80},
...
/*
 * the order of the gc1024_win_sizes is [full_resolution, preview_resolution].
 */
...
static struct regval_list gc1024_stream_on[] = {
	{ 0xfe, 0x03},
	{ 0x10, 0x91},
	{ 0xfe, 0x00},
	{SENSOR_FLAG_END, 0x00},	/* END MARKER */
...
static int gc1024_write(struct v4l2_subdev *sd, unsigned char reg,
			unsigned char value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int gc1024_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
static int gc1024_reset(struct v4l2_subdev *sd, u32 val)
{
	return 0;
}

static int gc1024_detect(struct v4l2_subdev *sd, unsigned int *ident)
{
	unsigned char v;
	int ret;
	ret = gc1024_read(sd, 0xf0, &v);
...
static int gc1024_set_integration_time(struct v4l2_subdev *sd, int value)
{
	int ret = 0;
	ret = gc1024_write(sd, 0x4, value&0xff);
	if (ret < 0) {
		printk("gc1024_write error  %d\n" ,__LINE__);
		return ret;
	}
	ret = gc1024_write(sd, 0x3, (value&0x1f00)>>8);
	if (ret < 0) {
		printk("gc1024_write error  %d\n" ,__LINE__ );
		return ret;
	}
...
static int gc1024_set_analog_gain(struct v4l2_subdev *sd, int value)
{
	int ret = 0;
	ret = gc1024_write(sd, 0xb6, (value >> 12) & 0xf);
	if (ret < 0) {
		printk("gc1024_write error  %d" ,__LINE__ );
		return ret;
	}
	ret = gc1024_write(sd, 0xb1, (value >> 8) & 0xf);
	if (ret < 0) {
		printk("gc1024_write error  %d" ,__LINE__ );
		return ret;
	}
	ret = gc1024_write(sd, 0xb2, (value << 2) & 0xff);
	if (ret < 0) {
		printk("gc1024_write error  %d" ,__LINE__ );
		return ret;
	}
...
static int gc1024_set_digital_gain(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int gc1024_get_black_pedestal(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int gc1024_init(struct v4l2_subdev *sd, u32 enable)
{
...
static int gc1024_s_stream(struct v4l2_subdev *sd, int enable)
{
	int ret = 0;
	if (enable) {
		ret = gc1024_write_array(sd, gc1024_stream_on);
		pr_debug("gc1024 stream on\n");
	}
	else {
		ret = gc1024_write_array(sd, gc1024_stream_off);
		pr_debug("gc1024 stream off\n");
	}
	return ret;
}

static int gc1024_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int gc1024_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int gc1024_set_fps(struct tx_isp_sensor *sensor, int fps)
{
	struct tx_isp_notify_argument arg;
	...
	ret = gc1024_read(sd, 0x5, &tmp);
	hb = tmp;
	ret += gc1024_read(sd, 0x6, &tmp);
	hb = (hb << 8) + tmp;
	ret += gc1024_read(sd, 0xf, &tmp);
	win_width = tmp;
...
static int gc1024_set_mode(struct tx_isp_sensor *sensor, int value)
{
	struct tx_isp_notify_argument arg;
	struct v4l2_subdev *sd = &sensor->sd;
...
static int gc1024_g_chip_ident(struct v4l2_subdev *sd,
		struct v4l2_dbg_chip_ident *chip)
{
	...
	ret = gc1024_detect(sd, &ident);
	if (ret) {
		v4l_err(client,
			"chip found @ 0x%x (%s) is not an gc1024 chip.\n",
			client->addr, client->adapter->name);
		return ret;
	}
	v4l_info(client, "gc1024 chip found @ 0x%02x (%s)\n",
		client->addr, client->adapter->name);
	return v4l2_chip_ident_i2c_client(client, chip, ident, 0);
}

static int gc1024_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
static long gc1024_ops_private_ioctl(struct tx_isp_sensor *sensor, struct isp_private_ioctl *ctrl)
{
	struct v4l2_subdev *sd = &sensor->sd;
	long ret = 0;
	switch(ctrl->cmd) {
	...
	case TX_ISP_PRIVATE_IOCTL_SENSOR_AGAIN:
		ret = gc1024_set_analog_gain(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_DGAIN:
		ret = gc1024_set_digital_gain(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_BLACK_LEVEL:
		ret = gc1024_get_black_pedestal(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_RESIZE:
		ret = gc1024_set_mode(sensor,ctrl->value);
		break;
	...
	case TX_ISP_PRIVATE_IOCTL_SENSOR_FPS:
		ret = gc1024_set_fps(sensor, ctrl->value);
		break;
	default:
		break;
	}
...
static long gc1024_ops_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct tx_isp_sensor *sensor =container_of(sd, struct tx_isp_sensor, sd);
	int ret;
	switch(cmd) {
...
static int gc1024_g_register(struct v4l2_subdev *sd, struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	unsigned char val = 0;
	int ret;
	...
	if (!capable(CAP_SYS_ADMIN))
		return -EPERM;
	ret = gc1024_read(sd, reg->reg & 0xffff, &val);
	reg->val = val;
	reg->size = 2;
	return ret;
}

static int gc1024_s_register(struct v4l2_subdev *sd, const struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	if (!capable(CAP_SYS_ADMIN))
		return -EPERM;
	gc1024_write(sd, reg->reg & 0xffff, reg->val & 0xff);
	return 0;
}
#endif

static const struct v4l2_subdev_core_ops gc1024_core_ops = {
	.g_chip_ident = gc1024_g_chip_ident,
	.reset = gc1024_reset,
	.init = gc1024_init,
	.s_power = gc1024_s_power,
	.ioctl = gc1024_ops_ioctl,
#ifdef CONFIG_VIDEO_ADV_DEBUG
	.g_register = gc1024_g_register,
	.s_register = gc1024_s_register,
#endif
};

static const struct v4l2_subdev_video_ops gc1024_video_ops = {
	.s_stream = gc1024_s_stream,
	.s_parm = gc1024_s_parm,
	.g_parm = gc1024_g_parm,
};

static const struct v4l2_subdev_ops gc1024_ops = {
	.core = &gc1024_core_ops,
	.video = &gc1024_video_ops,
};

static int gc1024_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	...
	gc1024_attr.dvp.gpio = sensor_gpio_func;
	 /*
		convert sensor-gain into isp-gain,
	 */
	gc1024_attr.max_again = log2_fixed_to_fixed(gc1024_attr.max_again, TX_ISP_GAIN_FIXED_POINT, LOG2_GAIN_SHIFT);
	gc1024_attr.max_dgain = gc1024_attr.max_dgain;
	sd = &sensor->sd;
	video = &sensor->video;
	sensor->video.attr = &gc1024_attr;
...
static int gc1024_remove(struct i2c_client *client)
{
...
static const struct i2c_device_id gc1024_id[] = {
	{ "gc1024", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, gc1024_id);
