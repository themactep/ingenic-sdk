...
static int gc2023_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
static int gc2023_write_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != GC2023_FLAG_END) {
		if (vals->reg_num == GC2023_FLAG_DELAY) {
...
static int gc2023_reset(struct v4l2_subdev *sd, u32 val)
{
	return 0;
}

static int gc2023_detect(struct v4l2_subdev *sd, unsigned int *ident)
{
	unsigned char v;
	int ret;
...
static int gc2023_set_digital_gain(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int gc2023_get_black_pedestal(struct v4l2_subdev *sd, int value)
{
	return 0;
}

static int gc2023_init(struct v4l2_subdev *sd, u32 enable)
{
...
static int gc2023_s_stream(struct v4l2_subdev *sd, int enable)
{
	int ret = 0;
...
static int gc2023_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int gc2023_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	return 0;
}

static int gc2023_set_fps(struct tx_isp_sensor *sensor, int fps)
{
	struct tx_isp_notify_argument arg;
...
static int gc2023_set_mode(struct tx_isp_sensor *sensor, int value)
{
	struct tx_isp_notify_argument arg;
	struct v4l2_subdev *sd = &sensor->sd;
...
static int gc2023_g_chip_ident(struct v4l2_subdev *sd,
		struct v4l2_dbg_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		ret = gpio_request(reset_gpio,"gc2023_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = gpio_request(pwdn_gpio,"gc2023_pwdn");
		if(!ret){
	...
	if (ret) {
		v4l_err(client,
			"chip found @ 0x%x (%s) is not an gc2023 chip.\n",
			client->addr, client->adapter->name);
		return ret;
	}
	v4l_info(client, "gc2023 chip found @ 0x%02x (%s)\n",
		client->addr, client->adapter->name);
	return v4l2_chip_ident_i2c_client(client, chip, ident, 0);
}

static int gc2023_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
static long gc2023_ops_private_ioctl(struct tx_isp_sensor *sensor, struct isp_private_ioctl *ctrl)
{
	struct v4l2_subdev *sd = &sensor->sd;
	long ret = 0;
	switch(ctrl->cmd){
	case TX_ISP_PRIVATE_IOCTL_SENSOR_INT_TIME:
		ret = gc2023_set_integration_time(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_AGAIN:
		ret = gc2023_set_analog_gain(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_DGAIN:
		ret = gc2023_set_digital_gain(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_BLACK_LEVEL:
		ret = gc2023_get_black_pedestal(sd, ctrl->value);
		break;
	case TX_ISP_PRIVATE_IOCTL_SENSOR_RESIZE:
		ret = gc2023_set_mode(sensor,ctrl->value);
		break;
...
static long gc2023_ops_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct tx_isp_sensor *sensor =container_of(sd, struct tx_isp_sensor, sd);
	int ret;
	switch(cmd){
	case VIDIOC_ISP_PRIVATE_IOCTL:
		ret = gc2023_ops_private_ioctl(sensor, arg);
		break;
	default:
		return -1;
		break;
...
static int gc2023_g_register(struct v4l2_subdev *sd, struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	unsigned char val = 0;
	int ret;
...
static int gc2023_s_register(struct v4l2_subdev *sd, const struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static const struct v4l2_subdev_core_ops gc2023_core_ops = {
	.g_chip_ident = gc2023_g_chip_ident,
	.reset = gc2023_reset,
	.init = gc2023_init,
	.s_power = gc2023_s_power,
...
static const struct v4l2_subdev_video_ops gc2023_video_ops = {
	.s_stream = gc2023_s_stream,
	.s_parm = gc2023_s_parm,
	.g_parm = gc2023_g_parm,
};

static const struct v4l2_subdev_ops gc2023_ops = {
...
{
	return i2c_add_driver(&gc2023_driver);
}

static __exit void exit_gc2023(void)
{
	i2c_del_driver(&gc2023_driver);
}
