...
unsigned int imx335_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
        uint16_t again=(isp_gain*20)>>LOG2_GAIN_SHIFT;
        // Limit Max gain
        if(again>AGAIN_MAX_DB+DGAIN_MAX_DB) again=AGAIN_MAX_DB+DGAIN_MAX_DB;
...
struct tx_isp_mipi_bus imx335_mipi_linear = {
...
struct tx_isp_mipi_bus imx335_mipi_dol = {
        .mode = SENSOR_MIPI_SONY_MODE,
        .clk = 1188,
        .lans = 2,
        .settle_time_apative_en = 0,
...
struct tx_isp_sensor_attribute imx335_attr={
        ...
        .sensor_ctrl.alloc_again_short = imx335_alloc_again_short,
        .sensor_ctrl.alloc_dgain = imx335_alloc_dgain,
        .wdr_cache = 0,
};

static struct regval_list imx335_init_regs_2592_1944_15fps_imx335_mipi_dol[] = {
...
static struct tx_isp_sensor_win_setting imx335_win_sizes[] = {
        ...
        /* 1948*1109 [1]*/
        {
                ...
                .mbus_code      = TISP_VI_FMT_SRGGB10_1X10,
                .colorspace     = TISP_COLORSPACE_SRGB,
                .regs           = imx335_init_regs_2592_1944_15fps_imx335_mipi_dol,
        },
};
...
static int imx335_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;
        ...
        shs1 = rhs1 - (value << 2);
        ret = imx335_write(sd, 0x305C, (unsigned char)(shs1 & 0xff));
        ret += imx335_write(sd, 0x305D, (unsigned char)((shs1 >> 8) & 0xff));
        ret += imx335_write(sd, 0x305E, (unsigned char)((shs1 >> 16) & 0x3));
        return 0;
}
...
static int imx335_set_integration_time(struct tx_isp_subdev *sd, int value)
{
		...
		if (data_type == TX_SENSOR_DATA_TYPE_LINEAR)
		{
			...
			shs = vmax - value;
			ret = imx335_write(sd, 0x3058, (unsigned char)(shs & 0xff));
			ret += imx335_write(sd, 0x3059, (unsigned char)((shs >> 8) & 0xff));
			ret += imx335_write(sd, 0x305A, (unsigned char)((shs >> 16) & 0x0f));
		}
...
static int imx335_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;
        ret += imx335_write(sd, 0x30EA, (unsigned char)(value & 0xff));
		ret += imx335_write(sd, 0x30EB, (unsigned char)((value >> 8) & 0xff));
        if (ret < 0)
                return ret;
...
static int imx335_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
		...
		ret += imx335_write(sd, 0x30E9, (unsigned char)((value >> 8) & 0xff));
        if (ret < 0)
                return ret;
...
static int imx335_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	...
	ret = imx335_read(sd, 0x304E, &hreverse);
	ret += imx335_read(sd, 0x304F, &vreverse);
	switch(enable) {
	case 0:
		hreverse &= 0xFC;
		vreverse &= 0xFC;
	...
	ret += imx335_write(sd, 0x304E, hreverse);
	ret += imx335_write(sd, 0x304F, vreverse);
	ret += imx335_write(sd, 0x3081, reg_3081);
	ret += imx335_write(sd, 0x3083, reg_3083);
	ret += imx335_write(sd, 0x3074, reg_3074);
	ret += imx335_write(sd, 0x3075, reg_3075);

	return ret;
}

static int imx335_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
        ...
        ret = imx335_write(sd, 0x3000, 0x1);
        if(wdr_en == 1){
                memcpy((void*)(&(imx335_attr.mipi)),(void*)(&imx335_mipi_dol),sizeof(imx335_mipi_dol));
                data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
                wsize = &imx335_win_sizes[1];
                imx335_attr.wdr_cache = wdr_bufsize;
                imx335_attr.max_again = 404346;
                imx335_attr.max_again_short = 404346;
                imx335_attr.max_dgain = 0;
                imx335_attr.min_integration_time = 1;
                imx335_attr.min_integration_time_native = 1;
                imx335_attr.max_integration_time_native = 2339;
                imx335_attr.min_integration_time_short = 1;
                imx335_attr.max_integration_time_short = 152;
                imx335_attr.integration_time_limit = 2339;
                imx335_attr.total_width = 495;
                imx335_attr.total_height = 5000;
                imx335_attr.max_integration_time = 2339;
                imx335_attr.integration_time_apply_delay = 2;
                imx335_attr.again_apply_delay = 2;
                imx335_attr.dgain_apply_delay = 0;
				imx335_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
                imx335_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
                sensor->video.attr = &imx335_attr;
                ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
        } else if (wdr_en == 0){
                memcpy((void*)(&(imx335_attr.mipi)),(void*)(&imx335_mipi_linear),sizeof(imx335_mipi_linear));
                data_type = TX_SENSOR_DATA_TYPE_LINEAR;
                imx335_attr.data_type = data_type;
                ...
                imx335_attr.max_again_short = 404346;
                imx335_attr.max_dgain = 0;
                imx335_attr.min_integration_time = 4;
                imx335_attr.min_integration_time_native = 4;
...
static int imx335_set_wdr(struct tx_isp_subdev *sd, int wdr_en)
{
        int ret = 0;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
        ...
        case 0:
                wsize = &imx335_win_sizes[0];
                data_type = TX_SENSOR_DATA_TYPE_LINEAR;
                imx335_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
                imx335_attr.max_again = 404346;
                imx335_attr.max_again_short = 404346;
                imx335_attr.max_dgain = 0;
                imx335_attr.min_integration_time = 4;
...
static int imx335_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
        ...
        case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
                if(arg)
                        ret = imx335_set_integration_time_short(sd, sensor_val->value);
                break;
        ...
        case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
                if(arg)
                        ret = imx335_set_analog_gain_short(sd, sensor_val->value);
                break;
        ...
        case TX_ISP_EVENT_SENSOR_WDR:
                if(arg)
                        ret = imx335_set_wdr(sd, init->enable);
                break;
        case TX_ISP_EVENT_SENSOR_WDR_STOP:
                if(arg)
                        ret = imx335_set_wdr_stop(sd, init->enable);
                break;
        default:
                break;;
        }
