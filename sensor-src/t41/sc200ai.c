...
static int sensor_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sensor->video.attr->max_integration_time_native = (sensor->info.default_boot == 0) ? (2*vts -8) : (vts/2 - short_time/2 - 4);
	sensor->video.attr->integration_time_limit = (sensor->info.default_boot == 0) ? (2*vts -8) : (vts/2 - short_time/2 - 4);
	sensor->video.attr->total_height = vts;
	sensor->video.attr->max_integration_time = (sensor->info.default_boot == 0) ? (2*vts -8) : (vts/2 - short_time/2 - 4);
