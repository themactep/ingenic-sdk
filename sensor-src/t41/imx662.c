...
unsigned int imx662_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{

        return isp_gain;
}

unsigned int imx662_alloc_dgain(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_dgain)
{
        return 0;
}
...
struct tx_isp_sensor_attribute imx662_attr={
        ...
        .again_apply_delay = 2,
        .dgain_apply_delay = 0,
        .sensor_ctrl.alloc_again = imx662_alloc_again,
        .sensor_ctrl.alloc_again_short = imx662_alloc_again_short,
        .sensor_ctrl.alloc_dgain = imx662_alloc_dgain,
        .wdr_cache = 0,
...
static struct regval_list imx662_init_regs_960_540_30fps_mipi[] = {
...
int imx662_write(struct tx_isp_subdev *sd, uint16_t reg,
                 unsigned char value)
{
        struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
#if 0
static int imx662_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
        int ret;
        unsigned char val;
...
static int imx662_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
        return 0;
}

static int imx662_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
        unsigned char v;
        int ret;

        ret = imx662_read(sd, 0x4d1c, &v);
...
static int imx662_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;
        ...
        shs1 = rhs1 - value - 1;
        ret = imx662_write(sd, 0x3020, (unsigned char)(shs1 & 0xff));
        ret += imx662_write(sd, 0x3021, (unsigned char)((shs1 >> 8) & 0xff));
        ret += imx662_write(sd, 0x3022, (unsigned char)((shs1 >> 16) & 0x3));

        return 0;
}

static int imx662_set_integration_time(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;
        ...
        if(data_type == TX_SENSOR_DATA_TYPE_LINEAR){
                ...
                ret += imx662_write(sd, 0x3051, (unsigned char)((shs >> 8) & 0xff));
                ret += imx662_write(sd, 0x3052, (unsigned char)((shs >> 16) & 0x3));
        } else {
                //long frame use shs2
                vmax = imx662_attr.total_height;
                shs = vmax - value - 1;
                ret = imx662_write(sd, 0x3024, (unsigned char)(shs & 0xff));
                ret += imx662_write(sd, 0x3025, (unsigned char)((shs >> 8) & 0xff));
                ret += imx662_write(sd, 0x3026, (unsigned char)((shs >> 16) & 0x3));
        }

        if (0 != ret) {
                ISP_ERROR("err: imx662_write err\n");
                return ret;
        }
...
static int imx662_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;

        ret = imx662_write(sd, 0x30f2, (unsigned char)(value & 0xff));
        if (ret < 0)
                return ret;
...
static int imx662_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;
        ...
        if(data_type == TX_SENSOR_DATA_TYPE_LINEAR){
                ...
                ret += imx662_write(sd, 0x3014, (unsigned char)(value & 0xff));
        }
        if (ret < 0)
                return ret;
...
static int imx662_set_digital_gain(struct tx_isp_subdev *sd, int value)
{
        return 0;
}

static int imx662_get_black_pedestal(struct tx_isp_subdev *sd, int value)
{
        return 0;
}

static int imx662_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
...
static int imx662_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
        struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
        int ret = 0;
...
static int imx662_set_fps(struct tx_isp_subdev *sd, int fps)
{
        struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
        int ret = 0;
        unsigned int pclk = 0;
        ...
        hmax = value;
        ret += imx662_read(sd, 0x302d, &value);
        hmax = (value << 8) | hmax;

        vmax = pclk * (fps & 0xffff) / hmax / ((fps & 0xffff0000) >> 16);
        ret += imx662_write(sd, 0x3028, vmax & 0xff);
        ret += imx662_write(sd, 0x3029, (vmax >> 8) & 0xff);
        ret += imx662_write(sd, 0x302a, (vmax >> 16) & 0x0f);

        printk("hmax is 0x%x, vmax is 0x%x\n", hmax, vmax);

        /*record current integration time*/
        ret = imx662_read(sd, 0x3050, &value);
        shs = value;
        ret += imx662_read(sd, 0x3051, &value);
        shs = (value << 8) | shs;
        ret += imx662_read(sd, 0x3052, &value);
...
static int imx662_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
        ...
        ret = imx662_write(sd, 0x3000, 0x1);
        if(wdr_en == 1){
                memcpy((void*)(&(imx662_attr.mipi)),(void*)(&mipi_2dol_lcg),sizeof(mipi_2dol_lcg));
                data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
                wsize = &imx662_win_sizes[1];
                imx662_attr.data_type = data_type;
                imx662_attr.wdr_cache = wdr_bufsize;

                imx662_attr.max_again = 655360;//786432,
                imx662_attr.max_again_short = 655360;
                imx662_attr.max_dgain = 0;
                imx662_attr.min_integration_time = 1;
                imx662_attr.min_integration_time_native = 1;
                imx662_attr.max_integration_time_native = 1176;
                imx662_attr.min_integration_time_short = 1;
                imx662_attr.max_integration_time_short = 98;
                imx662_attr.integration_time_limit = 1176;
                imx662_attr.total_width = 2136;
                imx662_attr.total_height = 2781;
                imx662_attr.max_integration_time = 1176;
                imx662_attr.integration_time_apply_delay = 2;
                imx662_attr.again_apply_delay = 2;
                imx662_attr.dgain_apply_delay = 0;

                sensor->video.attr = &imx662_attr;
                ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
        } else if (wdr_en == 0){
                memcpy((void*)(&(imx662_attr.mipi)),(void*)(&mipi_linear),sizeof(mipi_linear));
                data_type = TX_SENSOR_DATA_TYPE_LINEAR;
                imx662_attr.data_type = data_type;
                ...
                imx662_attr.data_type = data_type;
                imx662_attr.wdr_cache = wdr_bufsize;
                imx662_attr.max_again = 655360;
                imx662_attr.max_again_short = 655360;
                imx662_attr.max_dgain = 0;
                imx662_attr.min_integration_time = 1;
                imx662_attr.min_integration_time_native = 1;
                imx662_attr.max_integration_time_native = 2496;
                imx662_attr.min_integration_time_short = 1;
                imx662_attr.max_integration_time_short = 98;
                imx662_attr.integration_time_limit = 2496;
                imx662_attr.total_width = 5280;
                imx662_attr.total_height = 1125;
                imx662_attr.max_integration_time = 2496;
                imx662_attr.integration_time_apply_delay = 2;
                imx662_attr.again_apply_delay = 2;
                imx662_attr.dgain_apply_delay = 0;
...
static int imx662_set_wdr(struct tx_isp_subdev *sd, int wdr_en)
{
        int ret = 0;
...
static int imx662_set_mode(struct tx_isp_subdev *sd, int value)
{
        struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
        int ret = ISP_SUCCESS;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
        ...
        case 0:
                wsize = &imx662_win_sizes[0];
                data_type = TX_SENSOR_DATA_TYPE_LINEAR;
                imx662_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
                imx662_attr.max_again = 655360;
                imx662_attr.max_again_short = 655360;
                imx662_attr.max_dgain = 0;
                imx662_attr.min_integration_time = 1;
                ...
                imx662_attr.max_integration_time_short = 98;
                imx662_attr.integration_time_limit = 2496;
                imx662_attr.total_width = 990;
                imx662_attr.total_height = 2500;
                imx662_attr.max_integration_time = 2496;
                imx662_attr.integration_time_apply_delay = 2;
                imx662_attr.again_apply_delay = 2;
                imx662_attr.dgain_apply_delay = 0;
        ...
        case 1:
                ...
                imx662_attr.min_integration_time = 1;
                imx662_attr.min_integration_time_native = 1;
                imx662_attr.max_integration_time_native = 3746;
                imx662_attr.min_integration_time_short = 1;
                imx662_attr.max_integration_time_short = 98;
                imx662_attr.integration_time_limit = 3746;
                imx662_attr.total_width = 660;
                imx662_attr.total_height = 3750;
                imx662_attr.max_integration_time = 3746;
                imx662_attr.integration_time_apply_delay = 2;
                imx662_attr.again_apply_delay = 2;
                imx662_attr.dgain_apply_delay = 0;
                memcpy((void*)(&(imx662_attr.mipi)),(void*)(&mipi_binning),sizeof(mipi_binning));
                break;
        case 2:
                wsize = &imx662_win_sizes[2];
                data_type = TX_SENSOR_DATA_TYPE_LINEAR;
                imx662_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
                imx662_attr.max_again = 655360;
                imx662_attr.max_again_short = 655360;
                ...
                imx662_attr.again_apply_delay = 2;
                imx662_attr.dgain_apply_delay = 0;
                memcpy((void*)(&(imx662_attr.mipi)),(void*)(&mipi_crop_720p),sizeof(mipi_crop_720p));
                break;
        case 3:
                wsize = &imx662_win_sizes[2];
                imx662_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
                data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
                imx662_attr.wdr_cache = wdr_bufsize;
                imx662_attr.max_again = 655360;
                imx662_attr.max_again_short = 655360;
...
static int imx662_g_chip_ident(struct tx_isp_subdev *sd,
...
static int imx662_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
        long ret = 0;
        ...
        case TX_ISP_EVENT_SENSOR_WDR:
                if(arg)
                        ret = imx662_set_wdr(sd, init->enable);
                break;
...
static int imx662_g_register(struct tx_isp_subdev *sd, struct tx_isp_dbg_register *reg)
{
        unsigned char val = 0;
        int len = 0;
        int ret = 0;
        ...
        if (!private_capable(CAP_SYS_ADMIN))
                return -EPERM;
        ret = imx662_read(sd, reg->reg & 0xffff, &val);
        reg->val = val;
        reg->size = 2;
...
static int imx662_s_register(struct tx_isp_subdev *sd, const struct tx_isp_dbg_register *reg)
{
        int len = 0;
        ...
        if (!private_capable(CAP_SYS_ADMIN))
                return -EPERM;
        imx662_write(sd, reg->reg & 0xffff, reg->val & 0xff);

        return 0;
}

static struct tx_isp_subdev_core_ops imx662_core_ops = {
        .g_chip_ident = imx662_g_chip_ident,
        .reset = imx662_reset,
        .init = imx662_init,
        .g_register = imx662_g_register,
        .s_register = imx662_s_register,
};

static struct tx_isp_subdev_video_ops imx662_video_ops = {
                        ...
                        const struct i2c_device_id *id)
{
        ...
        sensor->video.mbus.colorspace = wsize->colorspace;
        sensor->video.fps = wsize->fps;
        tx_isp_subdev_init(&sensor_platform_device, sd, &imx662_ops);
