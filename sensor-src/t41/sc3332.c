...
struct tx_isp_mipi_bus sc3332_mipi={
...
static struct regval_list sc3332_init_regs_2304_1296_30fps_mipi_2lane[] = {
	{0x0103,0x01},
    {0x36e9,0x80},
    {0x37f9,0x80},
    {0x301f,0x02},
...
static struct tx_isp_sensor_win_setting sc3332_win_sizes[] = {
	{
		...
		.regs 		= sc3332_init_regs_2304_1296_30fps_mipi_2lane,
	}
};
...
static int sc3332_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
       ...
       ret += sc3332_write(sd, 0x3e09, (unsigned char)((value & 0xff00) >> 8));
       if (ret < 0)
               return ret;
...
static int sc3332_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case 0:
		sclk = 0xaa0*15*2500; /* 1250 * 0XAA0 * 30 * 2 */
		max_fps = 15;
		break;
	default:
...
static int sc3332_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	...
	switch(enable) {
	case 0:
		sc3332_write(sd, 0x3221, val & 0x99);
		break;
	case 1:
		sc3332_write(sd, 0x3221, val | 0x06);
		break;
	case 2:
		sc3332_write(sd, 0x3221, val | 0x60);
		break;
	case 3:
		sc3332_write(sd, 0x3221, val | 0x66);
		break;
	}
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		memcpy(&(sc3332_attr.mipi), &sc3332_mipi, sizeof(sc3332_mipi));
		sc3332_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sc3332_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		sc3332_attr.max_integration_time_native = 0XAA0 -8;
		sc3332_attr.integration_time_limit = 0XAA0 -8;
		sc3332_attr.total_width = 2500;
		sc3332_attr.total_height = 0XAA0;
		sc3332_attr.max_integration_time = 0XAA0 -8;
		sc3332_attr.again =0;
		sc3332_attr.integration_time = 0x700;
		break;
	default:
...
static int sc3332_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = sc3332_set_vflip(sd, sensor_val->value);
		break;
	default:
		break;
	}
