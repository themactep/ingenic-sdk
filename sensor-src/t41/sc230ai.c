...
unsigned int sc230ai_alloc_integration_time(unsigned int it, unsigned char shift, unsigned int *sensor_it)
{
...
unsigned int sc230ai_alloc_integration_time_short(unsigned int it, unsigned char shift, unsigned int *sensor_it)
{
	unsigned int expo = it >> shift;
	unsigned int isp_it = it;
	*sensor_it = expo;
...
unsigned int sc230ai_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
        struct again_lut *lut = sc230ai_again_lut;
...
struct tx_isp_mipi_bus sc230ai_mipi_dol = {
...
struct tx_isp_mipi_bus sc230ai_mipi_linear={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 810,
	.lans = 2,
	.index = 0,
...
struct tx_isp_sensor_attribute sc230ai_attr={
	...
	.sensor_ctrl.alloc_again_short = sc230ai_alloc_again_short,
	.sensor_ctrl.alloc_dgain = sc230ai_alloc_dgain,
	.sensor_ctrl.alloc_integration_time_short = sc230ai_alloc_integration_time_short,
};
...
static struct regval_list sc230ai_init_regs_1920_1080_60fps_mipi[] = {
		...
		{0x36e9,0x53},
		{0x37f9,0x53},
		{SC230AI_REG_DELAY,0x10},
		{0x0100,0x01},
		{SC230AI_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list sc230ai_init_regs_1920_1080_30fps_mipi_dol[] = {
		{0x0103,0x01},
		{0x0100,0x00},
		{0x36e9,0x80},
		{0x37f9,0x80},
...
static struct tx_isp_sensor_win_setting sc230ai_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs		= sc230ai_init_regs_1920_1080_30fps_mipi_dol,
	},
};
...
int sc230ai_read(struct tx_isp_subdev *sd,  uint16_t reg,
		unsigned char *value)
{
...
int sc230ai_write(struct tx_isp_subdev *sd, uint16_t reg,
		 unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int sc230ai_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc230ai_write(sd, 0x3e09, (unsigned char)((again >> 8) & 0xff));
	ret += sc230ai_write(sd, 0x3e07, (unsigned char)(again & 0xff));
...
static int sc230ai_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;

	value = (value << 2) - 2;
	ret = sc230ai_write(sd,  0x3e04, (unsigned char)((value >> 4) & 0xff));
	ret = sc230ai_write(sd,  0x3e05, (unsigned char)(value & 0x0f) << 4);
...
static int sc230ai_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;

	ret += sc230ai_write(sd, 0x3e13, (unsigned char)((value >> 8) & 0xff));
	ret += sc230ai_write(sd, 0x3e11, (unsigned char)(value & 0xff));
...
static int sc230ai_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			pr_debug("sc230ai stream on\n");
			sensor->video.state = TX_ISP_MODULE_RUNNING;
		}
	} else {
		ret = sc230ai_write_array(sd, sc230ai_stream_off);
		pr_debug("sc230ai stream off\n");
	}
...
static int sc230ai_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	ret += sc230ai_read(sd, 0x320c, &val);
	hts = val;
	val = 0;
	ret += sc230ai_read(sd, 0x320d, &val);
	hts = ((hts << 8)| val);
	...
	sc230ai_write(sd, 0x320f, (unsigned char)(vts & 0xff));
	sc230ai_write(sd, 0x320e, (unsigned char)(vts >> 8));

	if (sensor->info.default_boot == 1)
	{
		short_time = aclk * (fps & 0xffff)/((fps & 0xffff0000) >> 16);
		sc230ai_write(sd, 0x3e24, (unsigned char)(short_time & 0xff));
		sc230ai_write(sd, 0x3e23, (unsigned char)(short_time >> 8));
	}
	...
	sensor->video.attr->max_integration_time_native = (sensor->info.default_boot == 0) ? (2*vts -9) : (vts/2 - short_time/2 - 4);
	sensor->video.attr->integration_time_limit = (sensor->info.default_boot == 0) ? (2*vts -9) : (vts/2 - short_time/2 - 4);
	sensor->video.attr->total_height = vts;
	sensor->video.attr->max_integration_time = (sensor->info.default_boot == 0) ? (2*vts -9) : (vts/2 - short_time/2 - 4);
...
static int sc230ai_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	...
	val = sc230ai_read(sd, 0x3221, &val);
	switch(enable) {
	case 0:
		val &= 0x99;
		break;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		memcpy(&sc230ai_attr.mipi, &sc230ai_mipi_linear, sizeof(sc230ai_mipi_linear));
		sc230ai_attr.mipi.clk = 810,
		sc230ai_attr.min_integration_time = 2;
		sc230ai_attr.min_integration_time_native = 2,
		sc230ai_attr.total_width = 2400;
		sc230ai_attr.total_height = 1125;
		sc230ai_attr.max_integration_time_native = 2241;   /* 1125*2-9 */
		sc230ai_attr.integration_time_limit = 2241;
		sc230ai_attr.max_integration_time = 2241;
		sc230ai_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sc230ai_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		printk("=================> linear is ok");
		break;
	case 1:
		sc230ai_attr.wdr_cache = wdr_bufsize;
		wsize = &sc230ai_win_sizes[1];
		memcpy(&sc230ai_attr.mipi, &sc230ai_mipi_dol, sizeof(sc230ai_mipi_dol));
		sc230ai_attr.mipi.clk = 810,
		sc230ai_attr.min_integration_time = 1;
		sc230ai_attr.min_integration_time_short = 1;
		sc230ai_attr.min_integration_time_native = 1,
		sc230ai_attr.total_width = 2400;
		sc230ai_attr.total_height = 2250;
		sc230ai_attr.max_integration_time_native = 1053; /* vts/2 -rhs/2-5 */
		sc230ai_attr.integration_time_limit = 1053;
		sc230ai_attr.max_integration_time = 1053;
		sc230ai_attr.max_integration_time_short = 64; /* rhs/2-4 */
		sc230ai_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
	...
	data_type = sc230ai_attr.data_type;
...
static int sc230ai_g_chip_ident(struct tx_isp_subdev *sd,
...
#if 1
static int sc230ai_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
	struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
	int ret = 0;

	ret = sc230ai_write(sd, 0x0103, 0x01);
	ret = sc230ai_write(sd, 0x0100, 0x00);
	if(wdr_en == 1){
		info->default_boot=1;
		memcpy(&sc230ai_attr.mipi, &sc230ai_mipi_dol, sizeof(sc230ai_mipi_dol));
		sc230ai_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		...
		sc230ai_attr.min_integration_time_native = 1,
		sc230ai_attr.total_width = 2400;
		sc230ai_attr.total_height = 2500;
		sc230ai_attr.max_integration_time_native = 1053;
		sc230ai_attr.integration_time_limit = 1053;
		sc230ai_attr.max_integration_time = 1053;
		sc230ai_attr.max_integration_time_short = 64;
		printk("\n-------------------------switch wdr ok ----------------------\n");
	}else if (wdr_en == 0){
		info->default_boot = 0;
		memcpy(&sc230ai_attr.mipi, &sc230ai_mipi_linear, sizeof(sc230ai_mipi_linear));
		...
		sc230ai_attr.total_width = 2400;
		sc230ai_attr.total_height = 1125;
		sc230ai_attr.max_integration_time_native = 2241;
		sc230ai_attr.integration_time_limit = 2241;
		sc230ai_attr.max_integration_time = 2241;
		printk("\n-------------------------switch linear ok ----------------------\n");
	}else{
...
static int sc230ai_set_wdr(struct tx_isp_subdev *sd, int wdr_en)
{
	int ret = 0;
//	printk("\n==========> set_wdr\n");
	private_gpio_direction_output(reset_gpio, 1);
...
static int sc230ai_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = sc230ai_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = sc230ai_set_analog_gain_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_WDR:
		if(arg)
			ret = sc230ai_set_wdr(sd, init->enable);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if(arg)
			ret = sc230ai_set_wdr_stop(sd, init->enable);
		break;
...
MODULE_DESCRIPTION("A low-level driver for sc230ai sensors");
MODULE_LICENSE("GPL");
