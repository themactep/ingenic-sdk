...
//    assert(m.getType() <= Matrix::kTranslate_Mask);
//    if (count > 0) {
...
//    assert(m.getType() <= (Matrix::kScale_Mask | Matrix::kTranslate_Mask));
//    if (count > 0) {
//        float tx = m.getTranslateX();
//        float ty = m.getTranslateY();
...
//    assert(m.getType() != Matrix::kPerspective_Mask);
//    float tx = m.getTranslateX();
//    float ty = m.getTranslateY();
//    float sx = m.getScaleX();
//    float sy = m.getScaleY();
...
void Matrix::RotTrans_xy(const Matrix& m, float sx, float sy, Point* pt) {
    assert((m.getType() & (kAffine_Mask | kPerspective_Mask)) == kAffine_Mask);
...
void Matrix::Rot_xy(const Matrix& m, float sx, float sy, Point* pt) {
    assert((m.getType() & (kAffine_Mask | kPerspective_Mask)) == kAffine_Mask);
    assert(0 == m.fMat[kMTransX]);
    assert(0 == m.fMat[kMTransY]);
...
void Matrix::ScaleTrans_xy(const Matrix& m, float sx, float sy, Point* pt) {
    assert((m.getType() & (kScale_Mask | kAffine_Mask | kPerspective_Mask)) == kScale_Mask);
...
void Matrix::Trans_xy(const Matrix& m, float sx, float sy, Point* pt) {
    assert(m.getType() == kTranslate_Mask);
...
void Matrix::dump() const {
    printf("| %f, %f, %f |\n", fMat[kMScaleX], fMat[kMSkewX], fMat[kMTransX]);
    printf("| %f, %f, %f |\n", fMat[kMSkewY], fMat[kMScaleY], fMat[kMTransY]);
    printf("| %f, %f, %f |\n", fMat[kMPersp0], fMat[kMPersp1], fMat[kMPersp2]);
}

} // namespace CV

int get_matrixs(float *matrix, float angel, uint32_t src_w, uint32_t src_h,
		uint32_t dst_w, uint32_t dst_h, uint8_t type)
{
	CV::Matrix trans;
	trans.setRotate(angel, (float)(src_w - 1) / 2, (float)(src_h - 1) / 2);
	trans.postScale((float)(dst_w) / (float)(src_w), (float)(dst_h) / (float)(src_h));
	trans.get9(matrix);
	if(type){
		float persp0 = 1.0 / ((rand() % 1000) +1);
		float persp1 = 1.0 / ((rand() % 1000) +1);
		matrix[6] = rand() % 2 ? persp0 : -persp0;
		matrix[7] = rand() % 2 ? persp1 : -persp1;
		matrix[8] = 1.0;
	}
