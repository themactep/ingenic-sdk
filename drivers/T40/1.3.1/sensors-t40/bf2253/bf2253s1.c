...
#define BF2253S1_CHIP_ID_H	(0x22)
#define BF2253S1_CHIP_ID_L	(0x53)
#define BF2253S1_REG_END		0xff
#define BF2253S1_REG_DELAY	0x00
#define BF2253S1_SUPPORT_30FPS_SCLK (66002400)
...
*         */
    ...
    // {0x7d,0x0e},
    //{0xe7,0x09}, //sync two 2253
	{BF2253S1_REG_END, 0x00},	/* END MARKER */
};
...
static int bf2253s1_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
	    ...
	    if (sensor->video.state == TX_ISP_MODULE_INIT){
            ...
            ISP_WARNING("bf2253s1 stream on\n");
		...
		ISP_WARNING("bf2253s1 stream off\n");
		sensor->video.state = TX_ISP_MODULE_DEINIT;
	}
...
static int bf2253s1_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sclk = BF2253S1_SUPPORT_30FPS_SCLK;
			       ...
			       struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("bf2253s1 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	ISP_WARNING("sensor driver version %s\n",SENSOR_VERSION);
	if(chip){
		memcpy(chip->name, "bf2253s1", sizeof("bf2253s1"));
		chip->ident = ident;
...
static __init int init_bf2253s1(void)
{
	return private_i2c_add_driver(&bf2253s1_driver);
}

static __exit void exit_bf2253s1(void)
{
	private_i2c_del_driver(&bf2253s1_driver);
}

module_init(init_bf2253s1);
module_exit(exit_bf2253s1);

MODULE_DESCRIPTION("A low-level driver for SmartSens bf2253s1 sensors");
MODULE_LICENSE("GPL");
