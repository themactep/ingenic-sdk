 ...
 *   0          1600*1200       20        mipi_1lane            linear
 *
 */
#define DEBUG
...
#define SC2356_CHIP_ID_H	(0xeb)
#define SC2356_CHIP_ID_L	(0x52)
#define SC2356_REG_END		0xffff
#define SC2356_REG_DELAY	0xfffe
...
unsigned int sc2356_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while(lut->gain <= sc2356_attr.max_again) {
		if(isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == sc2356_attr.max_again) && (isp_gain >= lut->gain)) {
...
struct tx_isp_sensor_attribute sc2356_attr={
	...
	.max_integration_time_native = 0X04E2-6, /* 1130*2-13 */
	.integration_time_limit = 0X04E2-6,
	.total_width = 0x776,
	.total_height = 0x04e2,
	.max_integration_time = 0X04E2-6,
...
static struct regval_list sc2356_init_regs_1920_1080_20fps_mipi[] = {
	{0x0103,0x01},
    {0x0100,0x00},
    {0x36e9,0x80},
    {0x36ea,0x0a},
    ...
    {0x4829,0x05},
    {0x0100,0x01},
	{SC2356_REG_DELAY,0x01},
	{SC2356_REG_END, 0x00},	/* END MARKER */
};
...
static struct tx_isp_sensor_win_setting sc2356_win_sizes[] = {
	{
		...
		.regs 		= sc2356_init_regs_1920_1080_20fps_mipi,
	},
};
struct tx_isp_sensor_win_setting *wsize = &sc2356_win_sizes[0];
...
int sc2356_read(struct tx_isp_subdev *sd, uint16_t reg, unsigned char *value)
{
...
int sc2356_write(struct tx_isp_subdev *sd, uint16_t reg, unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int sc2356_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
	while (vals->reg_num != SC2356_REG_END) {
		if (vals->reg_num == SC2356_REG_DELAY) {
...
static int sc2356_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc2356_write(sd, 0x3e02, (unsigned char)((it & 0x0f) << 4));
...
static int sc2356_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	...
	value *= 2;
	ret += sc2356_write(sd, 0x3e00, (unsigned char)((value >> 12) & 0xf));
	ret += sc2356_write(sd, 0x3e01, (unsigned char)((value >> 4) & 0xff));
	ret += sc2356_write(sd, 0x3e02, (unsigned char)((value & 0x0f) << 4));
...
static int sc2356_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc2356_write(sd, 0x3e07, (unsigned char)(value & 0xff));
	ret += sc2356_write(sd, 0x3e08, (unsigned char)(((value >> 8) & 0xff)));
	if (ret < 0)
		return ret;
...
static int sc2356_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sclk = 0x04e2*0x776*20;

	ret = sc2356_read(sd, 0x320c, &tmp);
	hts = tmp;
	ret += sc2356_read(sd, 0x320d, &tmp);
	if (0 != ret) {
	...
	ret += sc2356_write(sd, 0x320f, (unsigned char)(vts & 0xff));
	ret += sc2356_write(sd, 0x320e, (unsigned char)(vts >> 8));
	if (0 != ret) {
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		memcpy(&(sc2356_attr.mipi), &sc2356_attr.mipi, sizeof(sc2356_attr.mipi));
		sc2356_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sc2356_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		sc2356_attr.max_integration_time_native = 0x4e2-6;
		sc2356_attr.integration_time_limit = 0x4e2-6;
		sc2356_attr.total_width = 0x776;
		sc2356_attr.total_height = 0x4e2;
		sc2356_attr.max_integration_time = 0x4e2-6;
		sc2356_attr.again = 0;
		sc2356_attr.integration_time = 0x4e2-6;
		break;
	default:
			       ...
			       struct tx_isp_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"sc2356_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = private_gpio_request(pwdn_gpio,"sc2356_pwdn");
		if(!ret){
	...
	ISP_WARNING("sc2356 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
...
static int sc2356_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = -1;
...
//		break;
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = sc2356_set_vflip(sd, sensor_val->value);
		break;
	default:
		break;
	}
...
static int sc2356_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
MODULE_DESCRIPTION("A low-level driver for SmartSens sc2356 sensors");
MODULE_LICENSE("GPL");
