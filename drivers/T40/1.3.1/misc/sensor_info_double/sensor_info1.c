...
static unsigned i2c_adapter_nr = 3;
...
static int cim2_gpio = GPIO_PC(29);
module_param(cim2_gpio, int, S_IRUGO);
MODULE_PARM_DESC(cim2_gpio, "Cim1 GPIO NUM");
...
static int32_t process_one_adapter(struct device *dev, void *data)
{
	...
	if(cim2_gpio != -1){
		ret = gpio_request(cim2_gpio,"cim2");
		if(!ret){
			jzgpio_set_func((cim2_gpio / 32), GPIO_FUNC_1, 1 << (cim2_gpio % 32));
		}
	}

	for (i = 0; i < scnt; i++) {
		uint8_t idcnt = g_sinfo[i].id_cnt;
		g_sinfo[i].clk_name = "div_cim2";
                mclk = devm_clk_get(dev, g_sinfo[i].clk_name);
		if (IS_ERR(mclk)) {
...
#endif
	g_sinfo_proc = proc_mkdir("jz/sinfo1", 0);
	if (!g_sinfo_proc) {
		printk("err: jz_proc_mkdir failed\n");
	}
	proc_create_data("info1", S_IRUGO, g_sinfo_proc, &sinfo_proc_fops, NULL);
	/* i2c_for_each_dev(NULL, process_one_adapter); */
