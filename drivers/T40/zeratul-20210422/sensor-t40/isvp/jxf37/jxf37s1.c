...
static int reset_gpio = GPIO_PC(17);
...
//	sensor->mclk = devm_clk_get(&client->dev, "div_cim0");
	sensor->mclk = private_devm_clk_get(&client->dev, "div_cim0");
	set_sensor_mclk_function(0);
//	sensor->mclk = private_clk_get_rate("div_cim0");
	...
	printk("#####%s:%d#####\n", __func__, __LINE__);
	/* if(clk_prepare(clks[i]) < 0) { */
	/* 	printk("failed to prepare for clk\n"); */
	...
	if(data_type == TX_SENSOR_DATA_TYPE_LINEAR) {
		if(data_interface == TX_SENSOR_DATA_INTERFACE_DVP){
			...
			printk("#####%s:%d#####func is %d\n", __func__, __LINE__,sensor_gpio_func);
			...
			jxf37_attr.mipi.settle_time_apative_en = 1;
