...
static int jxf37_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	...
	//printk("PHY state 0x10054014 value is 0x%x\n", *(volatile u32*)0xb0054014);
		printk("jxf37 reg : 0x%x value : 0x%x\n",vals->reg_num, val);
		vals++;
	}
...
static int jxf37_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	/*expo*/
	if(it_last != expo){
		ret = jxf37_write(sd,  0x01, (unsigned char)(expo & 0xff));
		ret += jxf37_write(sd, 0x02, (unsigned char)((expo >> 8) & 0xff));
	}
	...
	if(ag_last != again){
		...
		ret += jxf37_write(sd, 0x00, (unsigned char)(again & 0x7f));
		...
		if(( ((ag_last < 0x10) && (again >= 0x10))
		   ...
		   || (ag_last == -1)){
			...
			if(ret < 0)
				return -1;
			if((val & 0x80) == 0){
				jxf37_write(sd, 0xc0, 0x2f);
				jxf37_write(sd, 0xc1, tmp1);
				jxf37_write(sd, 0xc2, 0x0c);
				jxf37_write(sd, 0xc3, tmp2);
				jxf37_write(sd, 0xc4, 0x82);
				jxf37_write(sd, 0xc5, tmp3);
...
static int jxf37_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		if(sensor->video.state == TX_ISP_MODULE_DEINIT){
			...
			//ret = jxf37_read_array(sd, wsize->regs);
		...
		pr_debug("jxf37 stream off\n");
			/*printk("jxf37 TX_ISP_MODULE_else\n");*/
	}
	/*printk("PHY state 0x10054014 value is 0x%x\n", *(volatile u32*)0xb0054014);*/
...
#else
	jxf37_write(sd, 0x22, (unsigned char)(vts & 0xff));
	jxf37_write(sd, 0x23, (unsigned char)(vts >> 8));
#endif
	if (0 != ret) {
...
static int jxf37_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
	...
	if(wdr_en == 1){
		...
		jxf37_attr.data_type = data_type;
		jxf37_attr.max_integration_time_native = 1500 - 4;
		jxf37_attr.integration_time_limit = 1500 - 4;
		jxf37_attr.total_width = 3456;
		jxf37_attr.total_height = 1500;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 1:
		...
		jxf37_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		data_type = TX_SENSOR_DATA_TYPE_LINEAR;
	...
	case 2:
		wsize = &jxf37_win_sizes[2];
		jxf37_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		memcpy((void*)(&(jxf37_attr.mipi)),(void*)(&jxf37_mipi_dol),sizeof(jxf37_mipi_dol));
		jxf37_attr.one_line_expr_in_us = 28;
		jxf37_attr.wdr_cache = wdr_bufsize;
	...
	case 3:
		...
		jxf37_attr.max_integration_time_native = 0x206 - 4;
		jxf37_attr.integration_time_limit = 0x206 - 4;
		jxf37_attr.total_width = 0x3e0;//992
		jxf37_attr.total_height = 0x206;//518
		jxf37_attr.max_integration_time = 0x206 - 4;
		jxf37_attr.mipi.clk = 360;
		jxf37_attr.mipi.settle_time_apative_en = 0;
	...
	case 4:
		...
		jxf37_attr.max_integration_time_native = 0x140 - 4;
		jxf37_attr.integration_time_limit = 0x140 - 4;
		jxf37_attr.total_width = 0x730;//1840
		jxf37_attr.total_height = 0x140;//320
		jxf37_attr.max_integration_time = 0x140 - 4;
		jxf37_attr.mipi.clk = 324;
		jxf37_attr.mipi.settle_time_apative_en = 0;
	...
	case TISP_SENSOR_VI_MIPI_CSI0:
		jxf37_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		jxf37_attr.mipi.index = 0;
		break;
	case TISP_SENSOR_VI_MIPI_CSI1:
		jxf37_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		jxf37_attr.mipi.index = 1;
		break;
	case TISP_SENSOR_VI_DVP:
		jxf37_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
		break;
	default:
		ISP_ERROR("Have no this MCLK Source!!!\n");
	}
...
static int jxf37_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	...
	ret += jxf37_write(sd, 0xc0, 0x12);
	ret += jxf37_write(sd, 0xc1, val);
	ret += jxf37_write(sd, 0xc2, 0x28);
	ret += jxf37_write(sd, 0xc3, vwinSt);
	ret = jxf37_read(sd, 0x1f, &valg);
	if(ret < 0)
		return -1;
	valg |= 0xc0; /*bit[7], register group write function,auto clean.bit[6] lanch immediately*/
	jxf37_write(sd, 0x1f, valg);
