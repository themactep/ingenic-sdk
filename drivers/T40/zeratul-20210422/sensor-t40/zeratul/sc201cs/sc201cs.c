...
#define SC201CS_SUPPORT_30FPS_SCLK (74250000)
#define SENSOR_OUTPUT_MAX_FPS 30
#define SENSOR_OUTPUT_MIN_FPS 5
#define SENSOR_VERSION	"H20210415a"
...
struct tx_isp_sensor_attribute sc201cs_attr={
	...
	.max_integration_time_native = 1500 - 5,
	.integration_time_limit = 1500 - 5,
	.total_width = 2200,
	.total_height = 1500,
	.max_integration_time = 1500 - 5,
...
static struct regval_list sc201cs_init_regs_1600_1200_25fps_mipi[] = {
        {0x0103,0x01},
        {0x0100,0x00},
        {0x36e9,0x80},
        {0x36ea,0x0f},
...
static struct tx_isp_sensor_win_setting sc201cs_win_sizes[] = {
	{
		...
		.regs 		= sc201cs_init_regs_1600_1200_25fps_mipi,
	},
};
struct tx_isp_sensor_win_setting *wsize1 = &sc201cs_win_sizes[0];
...
{return 0;
	...
	ret = sc201cs_write(sd, 0x3e09, (unsigned char)(again & 0xff));
	ret += sc201cs_write(sd, 0x3e08, (unsigned char)(((again >> 8) & 0xff)));
...
static int sc201cs_set_logic(struct tx_isp_subdev *sd, int value)
{
	...
	ret = sc201cs_read(sd, 0x3040, &reg0);
	if (0x40 == reg0) {
		if (gain_val < 0x740) {
			ret += sc201cs_write(sd, 0x363c, 0x0e);
		} else if (gain_val >= 0x740) {
			ret += sc201cs_write(sd, 0x363c, 0x07);
		}
	} else if (0x41 == reg0) {
		if (gain_val < 0x740) {
			ret += sc201cs_write(sd, 0x363c, 0x0f);
		} else if (gain_val >= 0x740) {
			ret += sc201cs_write(sd, 0x363c, 0x07);
		}
	} else {
	...
	if (gain_val >= 0xf60) { //6x
		ret += sc201cs_write(sd, 0x5799, 0x7);
		dpc_flag = 2;
	}
	else if (gain_val <= 0xf40) {//4x
		ret += sc201cs_write(sd, 0x5799, 0x00);
	}
	if (ret < 0)
		return ret;
...
static int sc201cs_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
	    if(sensor->video.state == TX_ISP_MODULE_DEINIT){
            ...
            ret = sc201cs_write_array(sd, wsize1->regs);
            if (ret)
                return ret;
#endif
...
static int sc201cs_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	...
	sensor->video.mbus.colorspace = wsize1->colorspace;
	sensor->video.fps = wsize1->fps;
	tx_isp_subdev_init(&sensor1_platform_device, sd, &sc201cs_ops);
...
{
	return sc201cs_win_sizes->width;
}

int get_sensor_height(void)
{
	return sc201cs_win_sizes->height;
}
...
{
	return private_i2c_del_driver(&sc201cs_driver);
}
#endif
#endif
