...
/*
 * the order of the jxh63ps1_win_sizes is [full_resolution, preview_resolution].
 */
...
int jxh63ps1_write(struct tx_isp_subdev *sd, unsigned char reg,
		unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int jxh63ps1_s_stream(struct tx_isp_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		ISP_WARNING("jxh63ps1 stream on\n");
		...
		ISP_WARNING("jxh63ps1 stream off\n");
	}
...
static int jxh63ps1_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	hts *= 2;
	if (0 != ret) {
		ISP_ERROR("err: jxh63ps1 read err\n");
		return ret;
	}
...
static int jxh63ps1_g_chip_ident(struct tx_isp_subdev *sd,
			      struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("jxh63ps1 chip found @ 0x%02x (%s) version %s\n", client->addr, client->adapter->name, SENSOR_VERSION);
	if(chip){
...
static const struct i2c_device_id jxh63ps1_id[] = {
	{ "jxh63ps1", 0 },
	{ }
};
...
module_exit(exit_jxh63ps1);

MODULE_DESCRIPTION("A low-level driver for galaxycore jxh63ps1 sensors");
MODULE_LICENSE("GPL");
