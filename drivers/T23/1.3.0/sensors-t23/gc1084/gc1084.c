...
#define GC1084_SUPPORT_30FPS_SCLK (2222 * 750 * 30)
...
static int gc1084_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sclk = GC1084_SUPPORT_30FPS_SCLK;
	...
	ret = gc1084_read(sd, 0x0d05, &tmp);
	hts = tmp;
	ret += gc1084_read(sd, 0x0d06, &tmp);
	if (0 != ret) {
	...
	ret += gc1084_write(sd, 0x0d41, (unsigned char)(vts >> 8));
