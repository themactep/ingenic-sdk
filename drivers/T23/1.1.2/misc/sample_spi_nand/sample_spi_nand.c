...
#include "sample_spi_nand.h"

#define DRVNAME "jz_spi_nand"
...
#define SIZE_NAND0  (0x100000 * 16)
#define SIZE_NAND1 0x800000
#define SIZE_NAND2 0x800000
...
static int jz_spinand_write(struct mtd_info *mtd, loff_t to, size_t len,size_t *retlen, u_char *buf)
{
    ...
    /*printk("%s[%d]: to = %lld, len = %d\n",__func__,__LINE__, to, len);*/
	/*if(len >= 21)*/
		/*printk("node_type = %d\n", buf[20]);*/
...
static int jz_spinand_read_oob(struct mtd_info *mtd,loff_t addr,struct mtd_oob_ops *ops)
{
    ...
    /*printk("%s[%d]: page = %d, colum = %d\n",__func__,__LINE__, page, column);*/
	flash = container_of(mtd, struct jz_spi_nandflash, mtd);
	mutex_lock(&flash->lock);
	spi_message_init(&message);
	memset(&transfer, 0, sizeof(transfer));
...
static void nand_chip_dump(struct nand_chip *chip)
{
	printk("chip_delay = %d\n", chip->chip_delay);
	printk("options = 0x%08x\n", chip->options);
	printk("bbt_options = 0x%08x\n", chip->bbt_options);
	...
	printk("pagebuf = %d\n", chip->pagebuf);
	printk("pagebuf_bitflips = 0x%08x\n", chip->pagebuf_bitflips);
	printk("subpagesize = %d\n", chip->subpagesize);
	printk("cellinfo = 0x%08x\n", chip->cellinfo);
...
static int __init jz_spi_nand_init(void)
{
	...
	master = spi_busnum_to_master(spinum);
	if(!master)
		printk("Please register spi%d master firstly!\n", spinum);

	jz_spi0_board_info.bus_num = spinum;

	d = bus_find_device_by_name(&spi_bus_type, NULL, "jz_spi_nand");
	if (d == NULL) {
		device = spi_new_device(master, &jz_spi0_board_info);
		if(!device)
			printk("Failed to register spi%d device!\n", spinum);
	}
