...
#define MOTOR_LEFT_FORWARD 0
#define MOTOR_LEFT_REVERSE 1
#define MOTOR_DOWN_FORWARD 0
#define MOTOR_DOWN_REVERSE 1
...
/* motor speed */
#define HRTIMER_SPEED 250
#define MOTOR_MAX_SPEED	900		/**< unit: beats per second */
#define MOTOR_MIN_SPEED	100

#define MS419XX_RESET_GPIO	GPIO_PB(26)
#define MS419XX_VDFZ_GPIO	GPIO_PB(25)
#define MS419XX_MAX_PSUM 8

#define MS419XX_STOP 0x3000
#define MS419XX_FORWARD 0x3408
#define MS419XX_REVERSE 0x3508
...
struct motor_driver {
	...
	int max_pos_irq;
	int min_pos_irq;
	int max_position;	/* It is right-top point when x is max and y is max.*/
	int cur_position;	/* It is left-bottom point when x is 0 and y is 0.*/
	int total_steps;
	int dst_steps;
	int cur_steps;
	char reset_min_pos;
	char reset_max_pos;
	/* ms419xx */
	unsigned short sport_prebuild;
	unsigned short sport_real;

	enum motor_direction move_dir;
	enum motor_direction move_dir_prebuild;
...
struct motor_device {
	...
	enum register_state reg_state;
	enum motor_ops_mode mode;

	enum motor_direction cruise_xdir;
	enum motor_direction cruise_ydir;

	struct motor_message msg;
	struct motors_steps dst_move;
	struct motors_steps cur_move;

	int run_step_irq;
	int flag;

	/* ms419xx */
	unsigned int vdfz_valid;
	unsigned int vdfz_state; // high or low
	unsigned int rtimer_cnt;
