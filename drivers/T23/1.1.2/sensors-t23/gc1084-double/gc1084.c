...
static struct regval_list gc1084_init_regs_1280_720_60fps_mipi[] = {
        {0x03fe, 0xf0},
        {0x03fe, 0xf0},
        {0x03fe, 0xf0},
        {0x03fe, 0x00},
...
static struct tx_isp_sensor_win_setting gc1084_win_sizes[] = {
	{
		...
		.regs 		= gc1084_init_regs_1280_720_60fps_mipi,
	},
};
...
static int gc1084_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sclk = GC1084_SUPPORT_50FPS_SCLK;

	ret = gc1084_read(sd, 0x320c, &tmp);
	hts = tmp;
	ret += gc1084_read(sd, 0x320d, &tmp);
	if (0 != ret) {
	...
	ret += gc1084_write(sd, 0x320f, (unsigned char)(vts & 0xff));
	ret += gc1084_write(sd, 0x320e, (unsigned char)(vts >> 8));
	if (0 != ret) {
...
static int gc1084_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	...
	// val = gc1084_read(sd, 0x022c, &val);
	//val1 = gc1084_read(sd, 0x0063, &val1);
