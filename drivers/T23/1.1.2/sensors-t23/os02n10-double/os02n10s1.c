...
#define SENSOR_VERSION  "H20231121a"
...
/*
 * the order of the os02n10s1_win_sizes is [full_resolution, preview_resolution].
 */
...
#ifdef SENSOR_I2C_REG_8BIT
int os02n10s1_read(struct tx_isp_subdev *sd, unsigned char reg,
                  unsigned char *value)
{
...
int os02n10s1_write(struct tx_isp_subdev *sd, unsigned char reg,
                   unsigned char value)
{
        struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int os02n10s1_read(struct tx_isp_subdev *sd, uint16_t reg,
                  unsigned char *value) {
        int ret;
        struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int os02n10s1_write(struct tx_isp_subdev *sd, uint16_t reg,
                   unsigned char value)
{
        struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int os02n10s1_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int os02n10s1_attr_check(struct tx_isp_subdev *sd)
{
	...
	if (IS_ERR(sensor->mclk)) {
        ...
        ret = os02n10s1_clk_set(sd, sclka, SENSOR_MCLK);
        if (ret) {
...
static int os02n10s1_g_chip_ident(struct tx_isp_subdev *sd,
...
static int os02n10s1_s_stream(struct tx_isp_subdev *sd, int enable)
{
        ...
        if (enable) {
                ...
                if (sensor->video.state == TX_ISP_MODULE_INIT) {
                        ...
                        pr_debug("os02n10s1 stream on\n");
                }

        } else {
                ret = os02n10s1_write_array(sd, os02n10s1_stream_off_mipi);
                sensor->video.state = TX_ISP_MODULE_INIT;
                pr_debug("os02n10s1 stream off\n");
        }
...
static int os02n10s1_probe(struct i2c_client *client,
...
static const struct i2c_device_id os02n10s1_id[] = {
        {"os02n10s1", 0},
        {}
};
