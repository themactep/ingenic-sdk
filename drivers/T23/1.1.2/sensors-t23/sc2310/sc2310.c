...
struct tx_isp_mipi_bus sc2310_mipi={
...
static struct regval_list sc2310_init_regs_1920_1080_30fps_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x36f9,0x80},
...
static struct tx_isp_sensor_win_setting sc2310_win_sizes[] = {
	/* resolution 2048*1536 @25fps max*/
	{
		...
		.regs 		= sc2310_init_regs_1920_1080_30fps_mipi,
	},
};
...
static int sc2310_set_expo(struct tx_isp_subdev *sd, int value)
{
	 int ret = 0;
     int it = (value & 0xffff);
     int again = (value & 0xffff0000) >> 16;

     ret += sc2310_write(sd, 0x3e01, (unsigned char)((it >> 4) & 0xff));
     ret += sc2310_write(sd, 0x3e02, (unsigned char)((it & 0x0f) << 4));

     ret += sc2310_write(sd, 0x3e08, (unsigned char)(((again >> 8) & 0xff)));
     ret += sc2310_write(sd, 0x3e09, (unsigned char)(again & 0xff));
     if (ret < 0)
         return ret;
...
static int sc2310_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc2310_write(sd, 0x3e01, (unsigned char)((value >> 4) & 0xff));
	ret += sc2310_write(sd, 0x3e02, (unsigned char)((value & 0x0f) << 4));
	if (ret < 0)
		return ret;
...
static int sc2310_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc2310_write(sd, 0x3e08, (unsigned char)(((value >> 8) & 0xff)));
	if (ret < 0)
		return ret;
...
static int sc2310_set_logic(struct tx_isp_subdev *sd, int value)
{
	return 0;
}
...
static int sc2310_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sclk = sc2310_SUPPORT_30FPS_SCLK;
...
static int sc2310_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = sc2310_set_expo(sd, *(int*)arg);
		break;
...
//			ret = sc2310_set_analog_gain(sd, *(int*)arg);
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = sc2310_set_vflip(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_LOGIC:
		if(arg)
			ret = sc2310_set_logic(sd, *(int*)arg);
		break;
	default:
		break;
	}
...
static int sc2310_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sc2310_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
	sc2310_attr.max_integration_time_native = 2244;
	sc2310_attr.integration_time_limit = 2244;
	sc2310_attr.total_width = 2200;
	sc2310_attr.total_height = 1125;
	sc2310_attr.max_integration_time = 2244;
	memcpy((void*)(&(sc2310_attr.mipi)),(void*)(&sc2310_mipi),sizeof(sc2310_mipi));
