				    ...
				    const char *buf, size_t count)
{
	...
	ingenic_tcu_set_period(timer_data->tcu_chn->cib.id, (u16)(period_ms * TIMER_FREQ / 1000));
				    ...
				    const char *buf, size_t count)
{
	...
	if (strncmp(buf, "1", 1) == 0) {
		tcu_enable_counter(timer_data->tcu_chn->cib.id); /* Enable TCU Channel */
		enable_irq(timer_data->irq); /* Enable Channel IRQ */
		tcu_set_counter(timer_data->tcu_chn->cib.id, 0); /* Clear Count */
		tcu_start_counter(timer_data->tcu_chn->cib.id); /* start Counter */
		dev_info(timer_data->dev, "timer enable\n");

	} else if (strncmp(buf, "0", 1) == 0) {
		tcu_stop_counter(timer_data->tcu_chn->cib.id); /* Stop Counter */
		disable_irq(timer_data->irq);/* Disable Channel IRQ */
		tcu_disable_counter(timer_data->tcu_chn->cib.id); /* Disable TCU Channel */
...
{
	struct sample_timer_data *timer_data = dev_get_drvdata(dev);
	u16 cnt_now = ingenic_tcu_get_count(timer_data->tcu_chn->cib.id);
...
static int sample_timer_probe(struct platform_device *pdev)
{
	...
	timer_data->tcu_chn = (struct ingenic_tcu_chn *)pdev->dev.platform_data;
	...
	ingenic_tcu_config(timer_data->tcu_chn);
