...
#define AUDIO_DRIVER_VERSION "V30-20250929a"
#define DEFAULT_EXCODEC_ADDR 0xff

#define INNER_CODEC_ONLY		0
#define EXTERNAL_CODEC_ONLY		1
#define INNER_MIC_EXTERNAL_SPK	2
#define INNER_SPK_EXTERNAL_MIC	3
...
static int stream_select = 2;//0是内放外录，1是内录外放 ,2内录内放 ，3外录外放
module_param(stream_select, int, S_IRUGO);
MODULE_PARM_DESC(stream_select, "external codec to spk:0->internal-codec spk 1->external-codec record");

static int samplerate = 0;
module_param(samplerate, int, S_IRUGO);
MODULE_PARM_DESC(samplerate, "max samplerate you use");

static int i2c_bus = 0x02;
...
static int codec_use = 0;
module_param(codec_use, int, S_IRUGO);
MODULE_PARM_DESC(codec_use, "0:only internal codec,1:only external codec,2:both");
...
static int mic_init(void *pipe)
{
	...
	if ((stream_select == 1) || (stream_select == 2))
		livingcodec = aic->incodec;
	else
		livingcodec = aic->excodec;
	if (NULL == livingcodec) {
...
static int set_codec_mic_datatype(struct audio_aic_device *aic, void *data)
{
	...
	//set samplerate
	if (CODEC_IS_0_LINES == livingcodec->pins || CODEC_IS_6_LINES == livingcodec->pins) {
#if defined(CONFIG_KERNEL_4_4_94) || defined(CONFIG_KERNEL_5_15)
		clk_set_rate(aic->mic_clock, data_type.sample_rate*256);
#endif
#ifdef CONFIG_KERNEL_3_10
		ret = clk_set_rate(aic->mic_clock, data_type.sample_rate*256);
		if (ret != 0) {
			ret = clk_set_parent(aic->mic_clock, clk_get(NULL, "mpll"));
			if (ret != 0) {
				clk_put(aic->mic_clock);
				audio_err_print("[Audio] mic clock set parent error !\n");
				return -1;
			}
			ret = clk_set_rate(aic->mic_clock, data_type.sample_rate*256);
			if (ret != 0) {
				clk_put(aic->mic_clock);
				audio_err_print("[Audio] mic clock set rate error !\n");
				return -1;
			}
		}
#endif
	} else if (CODEC_IS_4_LINES == livingcodec->pins) {
#if defined(CONFIG_KERNEL_4_4_94) || defined(CONFIG_KERNEL_5_15)
		clk_set_rate(aic->spk_clock, data_type.sample_rate*256);
#endif
#ifdef CONFIG_KERNEL_3_10
		ret = clk_set_rate(aic->spk_clock, data_type.sample_rate*256);
		if (ret != 0) {
			ret = clk_set_parent(aic->spk_clock, clk_get(NULL, "mpll"));
			if (ret != 0) {
	...
	//codec set data_type
	ret = livingcodec->record->set_datatype(&data_type);
...
static int set_codec_mic_protocol(struct audio_aic_device *aic, void *data)
{
	...
	ret = livingcodec->set_sign(protocol);
...
static int set_codec_mic_hpf_enable(struct audio_aic_device *aic, void *data)
{
	...
	if (NULL != livingcodec->record->set_mic_hpf_en)
		ret = livingcodec->record->set_mic_hpf_en(hp_en);
...
static int set_codec_mic_again(struct audio_aic_device *aic, void *data)
{
	...
	livingcodec->record->set_again(again.channel,again.gain[0]);
	gain_bk = again;
...
static int set_codec_mic_dgain(struct audio_aic_device *aic, void *data)
{
	...
	ret = livingcodec->record->set_dgain(dgain.channel,dgain.gain[0]);
...
static int set_codec_mic_alc_gain(struct audio_aic_device *aic, void *data)
{
	...
	if (livingcodec->record->set_alc_threshold)
		livingcodec->record->set_alc_threshold(alc.channel,alc.max_gain[0],alc.min_gain[0]);
...
static int set_codec_mic_mute(struct audio_aic_device *aic, void *data)
{
	...
	if(channel == MONO_LEFT){
		if(aic->codec_mic_info->lchannel_mute_en){
			ret = livingcodec->record->set_dgain(channel,aic->codec_mic_info->ldgain);
			aic->codec_mic_info->lchannel_mute_en = 0;
		}else{
			if (livingcodec->record->set_mute)
				ret = livingcodec->record->set_mute(channel);
			aic->codec_mic_info->lchannel_mute_en = 1;
		}
	}else{
		if(aic->codec_mic_info->rchannel_mute_en){
			ret = livingcodec->record->set_dgain(channel,aic->codec_mic_info->rdgain);
...
static int set_codec_mic_start(struct audio_aic_device *aic, void *data)
{
	...
	__i2s_enable(aic);
	ret = livingcodec->record->set_endpoint(CODEC_MIC_START);
...
static int set_codec_mic_stop(struct audio_aic_device *aic, void *data)
{
	...
	//mic stop
	ret = livingcodec->record->set_endpoint(CODEC_MIC_STOP);
	record_flag = 0;
	return ret;
}
...
static int set_codec_type(struct audio_aic_device *aic, void *data)
{
	int ret = 0;
	int codec = *(int *)data;
	if(codec_type_select == codec)
		return ret;

	if (NULL == aic->incodec || NULL == aic->excodec) {
		audio_err_print("if you want switch internal or external codec, you need enable both(codec_use=2)\n");
		return -1;
	}

	switch (codec_type_select) {
		case INNER_CODEC_ONLY:
			set_codec_mic_stop(aic,NULL);
			aic->incodec->set_power(CODEC_POWER_DOWN);
			aic->incodec->spk_gpio_free();
			break;
		case EXTERNAL_CODEC_ONLY:
			set_codec_mic_stop(aic,NULL);
			aic->excodec->set_power(CODEC_POWER_DOWN);
			aic->excodec->spk_gpio_free();
			break;
		case INNER_MIC_EXTERNAL_SPK:
			set_codec_mic_stop(aic,NULL);
			aic->incodec->set_power(CODEC_POWER_DOWN);
			aic->excodec->set_power(CODEC_POWER_DOWN);
			aic->excodec->spk_gpio_free();
			break;
		case INNER_SPK_EXTERNAL_MIC:
			set_codec_mic_stop(aic,NULL);
			aic->incodec->set_power(CODEC_POWER_DOWN);
			aic->excodec->set_power(CODEC_POWER_DOWN);
			aic->incodec->spk_gpio_free();
			break;
		default:
			audio_err_print("Invalid codec type: %d\n", codec_type_select);
			break;
	}


	if (INNER_CODEC_ONLY == codec || INNER_SPK_EXTERNAL_MIC == codec) {
		aic->incodec->spk_gpio_request();
	}else {
		aic->excodec->spk_gpio_request();
	}
	printk("the codec type is %d \n",codec);

	if (EXTERNAL_CODEC_ONLY == codec) {
		__i2s_only_external_codec(aic);

		if(CODEC_IS_MASTER_MODE == aic->excodec->mode) {
			//set codec master mode, AIC slave mode
			__i2s_external_slave_mic_clkset(aic);
			__i2s_external_slave_spk_clkset(aic);
		}else if(CODEC_IS_SLAVE_MODE == aic->excodec->mode) {
			__i2s_external_master_mic_clkset(aic);
			__i2s_external_master_spk_clkset(aic);
		}

		if(CODEC_IS_4_LINES == aic->excodec->pins) {
			__i2s_select_share_clk(aic);
		}
		else if(CODEC_IS_6_LINES == aic->excodec->pins||CODEC_IS_0_LINES == aic->excodec->pins) {
			__i2s_select_spilt_clk(aic);
		}

		if(DATA_RIGHT_JUSTIFIED_MODE == aic->excodec->transfer_protocol ||
				DATA_LEFT_JUSTIFIED_MODE == aic->excodec->transfer_protocol) {
			__i2s_select_msbjustified(aic);
			printk("excodec use msbjustified mode\n");
		}else {
			__i2s_select_i2s(aic);
			printk("excodec use i2s mode\n");
		}
		stream_select = 3;
		aic->excodec->set_power(CODEC_POWER_UP);
		printk("@@@@@ excodec codec power up@@@@@@\n");
	} else if(INNER_CODEC_ONLY == codec) {
		//aic: slave mode, codec master mode
		__i2s_only_internal_codec(aic);
		__i2s_internal_slave_mic_clkset(aic);
		__i2s_internal_slave_spk_clkset(aic);
		__i2s_select_share_clk(aic);
		__i2s_select_i2s(aic);
		printk("incodec use i2s mode\n");
		stream_select = 2;
		aic->incodec->set_power(CODEC_POWER_UP);
		printk("@@@@@ inner codec power up@@@@@@\n");
	}else if(INNER_MIC_EXTERNAL_SPK == codec) {
		__i2s_external_dac_codec(aic);//incodec录音，excodec放音
		__i2s_internal_slave_mic_clkset(aic);
		...
		if(DATA_RIGHT_JUSTIFIED_MODE == aic->excodec->transfer_protocol ||
				DATA_LEFT_JUSTIFIED_MODE == aic->excodec->transfer_protocol) {
			__i2s_select_msbjustified(aic);
			printk("incodec and excodec use msbjustified mode\n");
		}else {
			__i2s_select_i2s(aic);
			printk("incodec and excodec use i2s mode\n");
		}
		stream_select = 1;
		aic->incodec->set_power(CODEC_POWER_UP);
		aic->excodec->set_power(CODEC_POWER_UP);
		printk("@@@@@ incodec & excodec power up@@@@@@\n");
	}else if(INNER_SPK_EXTERNAL_MIC == codec) {
		__i2s_external_adc_codec(aic);//excodec录音，incodec放音
		__i2s_internal_slave_spk_clkset(aic);
...
static int spk_init(void *pipe)
{
	...
	if ((stream_select == 1) || (stream_select == 3))
		livingcodec = aic->excodec;
	else
		livingcodec = aic->incodec;
	if (NULL == livingcodec) {
...
static int set_codec_spk_datatype(struct audio_aic_device *aic, void *data)
{
	...
	if (ret != 0) {
		...
		if (ret != 0) {
			clk_put(aic->spk_clock);
			audio_err_print("[Audio] spk clock set parent error !\n");
			return -1;
		}
		ret = clk_set_rate(aic->spk_clock, data_type.sample_rate*256);
		if (ret != 0) {
			clk_put(aic->spk_clock);
			audio_err_print("[Audio] spk clock set rate error !\n");
			return -1;
		}
	}
#endif
	...
	//codec set data_type
	ret = livingcodec->playback->set_datatype(&data_type);
...
static int set_codec_spk_again(struct audio_aic_device *aic, void *data)
{
	...
	ret = livingcodec->playback->set_again(again.channel,again.gain[0]);
...
static int set_codec_spk_dgain(struct audio_aic_device *aic, void *data)
{
	...
	ret = livingcodec->playback->set_dgain(dgain.channel,dgain.gain[0]);
...
static int set_codec_spk_mute(struct audio_aic_device *aic, void *data)
{
	...
	if(channel == MONO_LEFT){
		if(aic->codec_spk_info->lchannel_mute_en){
			ret = livingcodec->playback->set_dgain(channel,aic->codec_spk_info->ldgain);
			aic->codec_spk_info->lchannel_mute_en = 0;
		}else{
			if (livingcodec->playback->set_mute)
				ret = livingcodec->playback->set_mute(channel);
			aic->codec_spk_info->lchannel_mute_en = 1;
		}
	}else{
		if(aic->codec_spk_info->rchannel_mute_en){
			ret = livingcodec->playback->set_dgain(channel,aic->codec_spk_info->rdgain);
...
static int set_codec_spk_start(struct audio_aic_device *aic, void *data)
{
	...
	__i2s_enable(aic);
	ret = livingcodec->playback->set_endpoint(CODEC_SPK_START);
	{
...
static int set_codec_spk_stop(struct audio_aic_device *aic, void *data)
{
	...
	ret = livingcodec->playback->set_endpoint(CODEC_SPK_STOP);
	play_flag = 0;
	return ret;
}
...
static int audio_aic_probe(struct platform_device *pdev)
{
...
#endif
#ifdef CONFIG_KERNEL_3_10
	ret = clk_set_rate(aic->spk_clock, DEFAULT_REPLAY_CLK);
	if (ret != 0) {
		ret = clk_set_parent(aic->spk_clock, clk_get(NULL, "mpll"));
		if (ret != 0) {
			audio_err_print("[Audio] spk clock set parent error !\n");
	...
	ret = clk_set_rate(aic->mic_clock, DEFAULT_RECORD_CLK);
	if (ret != 0) {
		ret = clk_set_parent(aic->mic_clock, clk_get(NULL, "mpll"));
		if (ret != 0) {
			audio_err_print("[Audio] mic clock set parent error !\n");
	...
	if(codec_use == 0) {
		...
		if(stream_select == 1) {
			...
			codec_type_select = INNER_MIC_EXTERNAL_SPK;
			aic->excodec->spk_gpio_request();
		}else if(stream_select == 0) {
			__i2s_external_adc_codec(aic);//excodec录音，incodec放音
			__i2s_internal_slave_spk_clkset(aic);
			...
			codec_type_select = INNER_SPK_EXTERNAL_MIC;
			aic->incodec->spk_gpio_request();
		}else if(stream_select == 2) {
			//aic: slave mode, codec master mode
			__i2s_only_internal_codec(aic);
			__i2s_internal_slave_mic_clkset(aic);
			__i2s_internal_slave_spk_clkset(aic);
			...
			__i2s_select_i2s(aic);
			printk("incodec use i2s mode\n");
			codec_type_select = INNER_CODEC_ONLY;
			aic->incodec->spk_gpio_request();
		}else if(stream_select == 3) {
			__i2s_only_external_codec(aic);
			...
			codec_type_select = EXTERNAL_CODEC_ONLY;
			aic->excodec->spk_gpio_request();
		}
...
static int __exit audio_aic_remove(struct platform_device *pdev)
{
	...
	if(codec_use == 0){
		if (aic->incodec) {
			inner_codec_release(aic->incodec);
			codec_device = pdev->dev.platform_data;
...
void set_suspend_config(void)
{
    ...
    if(record_flag){
        ...
        __i2s_enable_stereo(aic);
        __i2s_disable(aic);
        aic->incodec->record->set_endpoint(CODEC_MIC_STOP);
    }
    if(play_flag){
        __i2s_disable_transmit_dma(aic);
        __i2s_disable_replay(aic);
        ...
        msleep(2);
        __i2s_disable_replay(aic);
        aic->incodec->playback->set_endpoint(CODEC_SPK_STOP);
...
int set_resume_config(void)
{
	...
	if(codec_use == 0){
     ...
     if(record_flag){
         ...
         aic->incodec->record->set_datatype(&aic->codec_mic_info->data_type);
         //aic->livingcodec->set_sign(&aic->codec_mic_info->transfer_info);
         aic->incodec->record->set_again(MONO_LEFT,aic->codec_mic_info->lagain);
         __i2s_enable(aic);
         aic->incodec->record->set_endpoint(CODEC_MIC_START);
         __i2s_flush_rfifo(aic);
         __i2s_read_rfifo(aic);
         __i2s_enable_receive_dma(aic);
         __i2s_enable_record(aic);
     ...
     if(play_flag){
         ...
         __i2s_disable_signadj(aic);
         __i2s_disable_byteswap(aic);
         aic->incodec->playback->set_datatype(&aic->codec_spk_info->data_type);
         //aic->livingcodec->set_sign(&aic->codec_spk_info->transfer_info);
         aic->incodec->playback->set_again(MONO_LEFT,aic->codec_spk_info->lagain);
         aic->incodec->playback->set_endpoint(CODEC_SPK_START);
         __i2s_flush_tfifo(aic);
         __i2s_enable_transmit_dma(aic);
         __i2s_enable_replay(aic);
     }
