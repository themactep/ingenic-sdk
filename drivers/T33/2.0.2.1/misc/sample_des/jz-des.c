...
#define DES_DRIVER_VERSION "H20240801a"
...
static int jz_des_probe(struct platform_device *pdev)
{
	...
	printk("@@@@ des driver ok(version %s) @@@@@\n", DES_DRIVER_VERSION);
...
#ifdef CONFIG_KERNEL_5_15
	dma_free_noncoherent(&pdev->dev, JZ_DES_DMA_DATALEN,des_ope->dst_addr_v, des_ope->dst_addr_p,DMA_FROM_DEVICE);
#else
	dma_free_noncoherent(&pdev->dev, JZ_DES_DMA_DATALEN,des_ope->dst_addr_v, des_ope->dst_addr_p);
#endif
failed_alloc_dst:
#ifdef CONFIG_KERNEL_5_15
	dma_free_noncoherent(&pdev->dev, JZ_DES_DMA_DATALEN,des_ope->src_addr_v, des_ope->src_addr_p,DMA_TO_DEVICE);
#else
