...
{
	int i, dentry = 0, num_ext_entries = 0, len, name_len = 0;
		...
		for (i = 0; i < dentries_per_clu; i++, dentry++) {
			...
			if ((entry_type == TYPE_UNUSED) || (entry_type == TYPE_DELETED)) {
				...
				if ((entry_type == TYPE_FILE) || (entry_type == TYPE_DIR)) {
					...
					if (is_feasible_entry && name_len <= MAX_NAME_LENGTH) {
	...
	*/
	for (i = 2; i < es->num_entries; i++, ep++) {
		if (p_fs->fs_func->get_entry_type(ep) == TYPE_EXTEND)
			len = extract_uni_name_from_name_entry((NAME_DENTRY_T *)ep, uniname, i); //从给定文件名录中复制字符到uniname变量中,理论上文件名应有限制，但此函数可无限制直接写入uniname
		else
			goto out;
		uni_len += len;
		if(len != 15 || uni_len >= MAX_NAME_LENGTH)
				break;
		uniname += 15;
	}
