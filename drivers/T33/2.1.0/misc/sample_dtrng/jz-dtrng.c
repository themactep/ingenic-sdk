...
#define DTRNG_DRIVER_VERSION "H20240801a"
...
static struct proc_ops dtrng_devices_fileops = {
	.proc_read = dtrng_proc_read,
	.proc_write = dtrng_proc_write,
};
#else
...
static int jz_dtrng_probe(struct platform_device *pdev)
{
	...
	printk("@@@@ dtrng driver ok(version %s) @@@@@\n", DTRNG_DRIVER_VERSION);
	return 0;
failed_create_dtrng:
