...
static int isp_core_output_channel_init(struct tx_isp_core_device *core)
{
	...
	for(index = 0; index < core->num_chans; index++){
		...
		switch(index){
			case ISP_FR_VIDEO_CHANNEL:
				chan->max_width = TX_ISP_FR_CAHNNEL_MAX_WIDTH;
				chan->max_height = TX_ISP_FR_CAHNNEL_MAX_HEIGHT;
				chan->has_crop = true;
				chan->has_scaler = false;
				break;
			case ISP_DS1_VIDEO_CHANNEL:
				chan->max_width = TX_ISP_DS1_CAHNNEL_MAX_WIDTH;
				chan->max_height = TX_ISP_DS1_CAHNNEL_MAX_HEIGHT;
				chan->has_crop = true;
				chan->has_scaler = true;
				break;
			case ISP_DS2_VIDEO_CHANNEL:
				chan->max_width = TX_ISP_DS2_CAHNNEL_MAX_WIDTH;
				chan->max_height = TX_ISP_DS2_CAHNNEL_MAX_HEIGHT;
				chan->has_crop = true;
				chan->has_scaler = true;
				break;
			default:
