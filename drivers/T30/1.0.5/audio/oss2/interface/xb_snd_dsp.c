...
static int snd_reuqest_dma(struct dsp_pipe *dp, struct device *dev)
{
	...
	/* alloc memory */
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31))
...
#ifdef CONFIG_JZ_TS_DMIC
			if (dp->dp_dmic) {
				if(16000 == object->rate) {
					dp->dp_dmic->fragment_size = 4 * 16 * 10 * sizeof(short);
					ret = (int)ddata->dev_ioctl_2(ddata, MIC_SET_DMIC_SAMPLERATE, 16000);
				} else if(8000 == object->rate) {
					dp->dp_dmic->fragment_size = 4 * 8 * 10 * sizeof(short);
					ret = (int)ddata->dev_ioctl_2(ddata, MIC_SET_DMIC_SAMPLERATE, 8000);
				} else {
					snd_error_print("Dmic Don't support the samplerate:%d.\n", object->rate);
					ret = -EPERM;
					goto exit;
				}
				ret = (int)ddata->dev_ioctl_2(ddata, MIC_SET_DMIC_CHANNEL, 4);
...
static inline long dsp_ioctl_dmic_stream(struct file *file, struct dmic_stream *stream)
{
	...
	while(dmic_totalsize) {
		...
		if(object->datanode.dmic_aec){
			ret = copy_to_user(dmic_aecbuff, object->dmic_aecbuffer, dmic_size/4);
			if(ret){
				snd_error_print("failed to copy_to_user\n");
				ret = -EIO;
				break;
...
#elif (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31))
	fragsize = dp->fragment_size;
#else
	fragsize = dp->fragment_size;
#endif
	if(dp_dmic) dmic_fragsize = (dp_dmic->fragment_size)/4;
	mutex_lock(&dp->mutex);
...
static inline int dmic_copy_to_user(struct dsp_pipe *dp, struct task_node *node)
{
	...
	for(i = 0; i < dmic_cnt; i++)  {
		...
		if(node->dmic_aec) {
			...
			copy_to_user(node->dmic_aec, dmic_aec_frag->vaddr, size_dmic/4);
			memset(dmic_aec_frag->vaddr, 0, size_dmic/4);
			node->dmic_aec += size_dmic/4;
		}
