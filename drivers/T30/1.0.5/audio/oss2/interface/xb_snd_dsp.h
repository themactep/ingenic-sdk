...
struct dmic_stream {
	void __user *dmic_data;
	void __user *dmic_aec;
	unsigned int dmic_size;
};
#endif
...
struct dsp_route_object {
	...
	unsigned long long dmic_ioctrl_counter;
	unsigned long long dmic_iodata_counter;
	char *dmic_buffer;
	char *dmic_aecbuffer;
	volatile unsigned char dmic_aec_enable;
};
...
struct dsp_pipe {
	...
	struct dsp_pipe *dp_dmic;
	volatile unsigned char dmic_aec_enable;

	struct dsp_pipe *dp_aec;
};
...
struct dsp_endpoints {
    ...
    struct dsp_pipe *dmic_endpoint;
	struct dsp_pipe *aec_endpoint;
