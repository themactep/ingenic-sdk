...
#define SC2135_REG_END		0xffff
#define SC2135_REG_DELAY	0xfffe
#define SC2135_SUPPORT_PCLK_FPS_30 (81000*1000)
#define SC2135_SUPPORT_PCLK_FPS_15 (33750*1000)
...
static struct regval_list sc2135_init_regs_1920_1080_15fps[] = {
...
int sc2135_read(struct tx_isp_subdev *sd, uint16_t reg, unsigned char *value)
{
...
int sc2135_write(struct tx_isp_subdev *sd, uint16_t reg, unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int sc2135_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
...
static int sc2135_write_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != SC2135_REG_END) {
		if (vals->reg_num == SC2135_REG_DELAY) {
...
static int sc2135_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}

static int sc2135_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
	int ret;
	unsigned char v;
...
static int sc2135_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
...
static int sc2135_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
	...
	/* denoise logic */
	if (value < 0x20) {
		ret += sc2135_write(sd, 0x3630, 0xc4);
		ret += sc2135_write(sd, 0x3632, 0x20);
		...
		ret += sc2135_write(sd, 0x3632, 0x40);
...
static int sc2135_set_digital_gain(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int sc2135_get_black_pedestal(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int sc2135_init(struct tx_isp_subdev *sd, int enable)
{
	...
	case TX_SENSOR_MAX_FPS_25:
		wsize->fps = 25 << 16 | 1;
		wsize->regs = sc2135_init_regs_1920_1080_25fps;
		break;
	case TX_SENSOR_MAX_FPS_15:
		wsize->fps = 15 << 16 | 1;
		wsize->regs = sc2135_init_regs_1920_1080_15fps;
		break;
	default:
...
static int sc2135_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int sc2135_set_fps(struct tx_isp_subdev *sd, int fps)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	unsigned int pclk = 0;
	unsigned short hts;
	...
	switch (sensor_max_fps) {
	case TX_SENSOR_MAX_FPS_25:
		pclk = SC2135_SUPPORT_PCLK_FPS_30;
		max_fps = SENSOR_OUTPUT_MAX_FPS;
		break;
	case TX_SENSOR_MAX_FPS_15:
		pclk = SC2135_SUPPORT_PCLK_FPS_15;
...
static int sc2135_set_mode(struct tx_isp_subdev *sd, int value)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
...
static int sc2135_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
	char val = 0;
	val = enable ? 0x2 : 0;
	ret = sc2135_write(sd, 0x3220, val);
...
static int sc2135_g_chip_ident(struct tx_isp_subdev *sd,
		struct tx_isp_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"sc2135_reset");
		if(!ret){
	...
	if (pwdn_gpio != -1) {
		ret = private_gpio_request(pwdn_gpio, "sc2135_pwdn");
		if(!ret){
	...
	if (ret) {
		printk("chip found @ 0x%x (%s) is not an sc2135 chip.\n",
				client->addr, client->adapter->name);
		return ret;
	}
	printk("sc2135 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	if(chip){
		memcpy(chip->name, "sc2135", sizeof("sc2135"));
		chip->ident = ident;
		chip->revision = SENSOR_VERSION;
	}
	return 0;
}
static int sc2135_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	long ret = 0;
	...
	switch(cmd){
		case TX_ISP_EVENT_SENSOR_INT_TIME:
			if(arg)
				ret = sc2135_set_integration_time(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_AGAIN:
			if(arg)
				ret = sc2135_set_analog_gain(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_DGAIN:
			if(arg)
				ret = sc2135_set_digital_gain(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
			if(arg)
				ret = sc2135_get_black_pedestal(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_RESIZE:
			if(arg)
				ret = sc2135_set_mode(sd, *(int*)arg);
			break;
		...
		case TX_ISP_EVENT_SENSOR_FPS:
			if(arg)
				ret = sc2135_set_fps(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_VFLIP:
			if(arg)
				ret = sc2135_set_vflip(sd, *(int*)arg);
			break;
		default:
			break;;
	}
...
static int sc2135_g_register(struct tx_isp_subdev *sd, struct tx_isp_dbg_register *reg)
{
	unsigned char val = 0;
	int len = 0;
	int ret = 0;
...
static int sc2135_s_register(struct tx_isp_subdev *sd, const struct tx_isp_dbg_register *reg)
{
	int len = 0;
	len = strlen(sd->chip.name);
	if(len && strncmp(sd->chip.name, reg->name, len)){
...
static struct tx_isp_subdev_core_ops sc2135_core_ops = {
...
static struct tx_isp_subdev_video_ops sc2135_video_ops = {
	.s_stream = sc2135_s_stream,
};

static struct tx_isp_subdev_sensor_ops	sc2135_sensor_ops = {
	.ioctl	= sc2135_sensor_ops_ioctl,
};

static struct tx_isp_subdev_ops sc2135_ops = {
	.core = &sc2135_core_ops,
	.video = &sc2135_video_ops,
	.sensor = &sc2135_sensor_ops,
};
		...
		const struct i2c_device_id *id)
{
	...
	case TX_SENSOR_MAX_FPS_25:
		break;
	case TX_SENSOR_MAX_FPS_15:
		sc2135_attr.max_integration_time_native = 1121;
		sc2135_attr.integration_time_limit = 1121;
		sc2135_attr.total_width = 2000;
		sc2135_attr.total_height = 1125;
		sc2135_attr.max_integration_time = 1121;
		break;
	default:
		printk("Now we do not support this framerate!!!\n");
	}
	...
	sensor->video.mbus.colorspace = wsize->colorspace;
	sensor->video.fps = wsize->fps;
	tx_isp_subdev_init(&sensor_platform_device, sd, &sc2135_ops);
...
{
	private_i2c_del_driver(&sc2135_driver);
}
