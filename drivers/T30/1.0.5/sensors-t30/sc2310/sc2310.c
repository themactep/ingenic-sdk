		...
		struct tx_isp_chip_ident *chip)
{
	...
	if (ret) {
		printk("chip found @ 0x%x (%s) is not an sc2310 chip.\n",
				client->addr, client->adapter->name);
		return ret;
	}
	printk("sc2310 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	if(chip){
...
static __init int init_sc2310(void)
{
	...
	if(ret){
		printk("Failed to init sc2310 dirver.\n");
		return -1;
	}
