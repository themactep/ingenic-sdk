...
struct tx_isp_sensor_attribute tp2850_attr={
	...
	.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI,
        .mipi = {
                ...
                .mipi_sc.sensor_csi_fmt = TX_SENSOR_YUV422, //TX_SENSOR_YUV422,//RAW10
...
static int tp2850_init(struct tx_isp_subdev *sd, int enable)
{
	...
	int ret = 0;
    int fps_num = 15, fps_den = 1; /* fps = fps_num / fps_den */
	int32_t intt = 0, again = 0, dgain = 0;
...
static int tp2850_g_chip_ident(struct tx_isp_subdev *sd,struct tx_isp_chip_ident *chip)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int tp2850_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifndef SENSOR_WITHOUT_INIT//zjb	
	rate = clk_get_rate(clk_get_parent(sensor->mclk));
	printk("<0>clk_get_rate: rate=%ld\n",rate);
	if (((rate / 1000) % 27000) != 0) 
	{
		...
		printk("<0>private_clk_set_rate: %ld->27000000\n",clk_get_rate(sensor->mclk));
...
{
        return tp2850_win_sizes->width;
}

int get_sensor_height()
{
        return tp2850_win_sizes->height;
}
