...
static int jxh62_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case TX_SENSOR_DATA_INTERFACE_MIPI:
		...
		break;
	default:
		printk("jxh62 Do not support this inteface now.\n");
		ret = -1;
		break;
	} 
...
static int jxh62_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (data_interface == TX_SENSOR_DATA_INTERFACE_DVP){
        ...
        memcpy((void*)(&(jxh62_attr.dvp)),(void*)(&jxh62_dvp),sizeof(jxh62_dvp));
    } else if (data_interface == TX_SENSOR_DATA_INTERFACE_MIPI){
...
{
	return jxh62_win_sizes->width;
}

int get_sensor_height()
{
	return jxh62_win_sizes->height;
}
