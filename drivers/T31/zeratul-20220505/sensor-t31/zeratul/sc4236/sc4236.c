...
extern unsigned long g_mirror_flip;
...
struct tx_isp_sensor_attribute sc4236_attr={
	...
	.max_integration_time_native = 3196,
	.integration_time_limit = 3196,
	.total_width = 2500,
	.total_height = 3200,
	.max_integration_time = 3196,
...
static struct regval_list sc4236_init_regs_2048_1536_25fps_mipi_3m[] = {
	{0x0103, 0x01},
	{0x0100, 0x00},
	{0x301c, 0x78},
	{0x3640, 0x02},
	...
	{0x3366, 0x78},
	{0x33aa, 0x00},
	{0x320c, 0x09}, //hts 0x9c4 = 2500
	{0x320d, 0xc4},
	{0x3f04, 0x04},
	{0x3f05, 0xbe},
	{0x320e, 0x0c}, //vts 0x780 = 1920
	{0x320f, 0x80},
	{0x3235, 0x0c},
	{0x3236, 0x7e},
	{0x36e9, 0x02},
...
static struct tx_isp_sensor_win_setting sc4236_win_sizes[] = {
	/* 2048 * 1536 */
	{
		...
		.regs 		= sc4236_init_regs_2048_1536_25fps_mipi_3m,
	}
};

static enum v4l2_mbus_pixelcode sc4236_mbus_code[] = {
...
static int sc4236_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int sc4236_set_expo(struct tx_isp_subdev *sd, int value)
{
	int ret = -1;
	int it = (value & 0xffff) * 2;
	int again = (value & 0xffff0000) >> 16;

	ret += sc4236_write(sd, 0x3e00, (unsigned char)((it >> 12) & 0xf));
	ret += sc4236_write(sd, 0x3e01, (unsigned char)((it >> 4) & 0xff));
	ret += sc4236_write(sd, 0x3e02, (unsigned char)((it & 0x0f) << 4));

	if (value < 160) {
		ret += sc4236_write(sd, 0x330e, 0x40);
		ret += sc4236_write(sd, 0x3314, 0x14);
	}
	else if(value > 320){
		ret += sc4236_write(sd, 0x3314, 0x04);
		ret += sc4236_write(sd, 0x330e, 0x50);
	}

	ret = sc4236_write(sd, 0x3e09, (unsigned char)(again & 0xff));
	ret += sc4236_write(sd, 0x3e08, (unsigned char)((again >> 8 << 2) | 0x03));

	//printk("value is 0x%x,it is 0x %x, agian is 0x%x\n", value, it, again);
		if (again < 0x110) {
			ret += sc4236_write(sd,0x3812,0x00);
			ret += sc4236_write(sd,0x3301,0x1e);
			ret += sc4236_write(sd,0x3633,0x23);
			ret += sc4236_write(sd,0x3630,0x80);
			ret += sc4236_write(sd,0x3622,0xf6);
			ret += sc4236_write(sd,0x3812,0x30);
		}else if (again >= 0x110 && again < 0x310){
			ret += sc4236_write(sd,0x3812,0x00);
			ret += sc4236_write(sd,0x3301,0x50);
			ret += sc4236_write(sd,0x3633,0x23);
			ret += sc4236_write(sd,0x3630,0x80);
			ret += sc4236_write(sd,0x3622,0xf6);
			ret += sc4236_write(sd,0x3812,0x30);
			...
			ret += sc4236_write(sd,0x3812,0x00);
			ret += sc4236_write(sd,0x3301,0x50);
			ret += sc4236_write(sd,0x3633,0x43);
			ret += sc4236_write(sd,0x3630,0x82);
			ret += sc4236_write(sd,0x3622,0x16);
			ret += sc4236_write(sd,0x3812,0x30);
		}
		if (ret < 0)
			return ret;
...
static int sc4236_init(struct tx_isp_subdev *sd, int enable)
{
...
static int sc4236_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int sc4236_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = sc4236_set_expo(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		//if(arg)
		//	ret = sc4236_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		//if(arg)
		//	ret = sc4236_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = sc4236_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = sc4236_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = sc4236_set_mode(sd, *(int*)arg);
		break;
...
static int sc4236_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("probe ok ------->sc4236 build:%s %s\n",__DATE__,__TIME__);
...
{
	return sc4236_win_sizes->width;
}

int get_sensor_height()
{
	return sc4236_win_sizes->height;
}
...
int init_sensor(void)
{
	...
	if(ret){
		ISP_ERROR("Failed to init sc4236 dirver.\n");
		return -1;
	}
