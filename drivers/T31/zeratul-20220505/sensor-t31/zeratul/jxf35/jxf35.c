...
/* 1080p@25fps: insmod sensor_jxf35_t31.ko data_interface=1 sensor_max_fps=30  */
/* 1080p@60fps: insmod sensor_jxf35_t31.ko data_interface=1 sensor_max_fps=60  */
...
static int jxf35_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(data_type == TX_SENSOR_DATA_TYPE_LINEAR) {
		if((data_interface == TX_SENSOR_DATA_INTERFACE_MIPI) && (sensor_max_fps == TX_SENSOR_MAX_FPS_30)){
			...
			jxf35_attr.total_height = 0x546; //1125
...
int init_sensor(void)
{
	...
	if(ret){
		printk("Failed to init jxf35 dirver.\n");
		return -1;
	}
