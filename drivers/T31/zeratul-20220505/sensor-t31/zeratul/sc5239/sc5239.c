...
#define SENSOR_VERSION	"H20201026a"
...
static enum v4l2_mbus_pixelcode sc5239_mbus_code[] = {
...
static int sc5239_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static void xmi_testshowreg(struct tx_isp_subdev *sd)
{
	int ret=-1;
	unsigned char v;
	ret = sc5239_read(sd, 0x3e00, &v);
	printk("reg 0x0x3e00=0x%x.\r\n",v);

	ret = sc5239_read(sd, 0x3e01, &v);
	printk("reg 0x3e01=0x%x.\r\n",v);

	ret = sc5239_read(sd, 0x3e02, &v);
	printk("reg 0x3e02=0x%x.\r\n",v);

	ret = sc5239_read(sd, 0x3e09, &v);
	printk("reg 0x3e09=0x%x.\r\n",v);

	ret = sc5239_read(sd, 0x3e08, &v);
	printk("reg 0x3e08=0x%x.\r\n",v);
...
static int sc5239_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	//printk("xmi call sc5239_set_expo()\r\n");
	//xmi_testshowreg(sd);
	//return 0;
	if (value != expo_val) {
...
{
	int ret = 0;
	//printk("xmi call sc5239_set_integration_time()\r\n");
	//xmi_testshowreg(sd);
	//return 0;
...
{
	int ret = 0;
	//printk("xmi call sc5239_set_analog_gain()\r\n");
	//xmi_testshowreg(sd);
	//return 0;
...
static int sc5239_init(struct tx_isp_subdev *sd, int enable)
{
...
static int sc5239_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int sc5239_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	int ret = 0;
	//printk("xmit  set_fps %d.\r\n",fps);
	//printk("xmi call sc5239_set_fps()\r\n");
	//xmi_testshowreg(sd);
	//return 0;
...
static int sc5239_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = sc5239_set_expo(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		//if(arg)
		//	ret = sc5239_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		//if(arg)
		//	ret = sc5239_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = sc5239_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = sc5239_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = sc5239_set_mode(sd, *(int*)arg);
		break;
...
int init_sensor(void)
{
	...
	if(ret){
		ISP_ERROR("Failed to init sc5239 dirver.\n");
		return -1;
	}
