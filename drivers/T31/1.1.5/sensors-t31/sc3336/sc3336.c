...
struct tx_isp_mipi_bus sc3336_mipi_3m={
...
struct tx_isp_mipi_bus sc3336_mipi_1m={
    .mode = SENSOR_MIPI_OTHER_MODE,
    .clk = 510,
    .lans = 2,
    .settle_time_apative_en = 1,
...
static struct regval_list sc3336_init_regs_1152_648_30fps_binning_mipi[] = {
	/* cleaned_0x0e_SC3336_MIPI_24Minput_2Lane_10bit_255Mbps_1152x648_15fps_hsum_vbin */
	{0x0103, 0x01},
	{0x36e9, 0x80},
	{0x37f9, 0x80},
	{0x301f, 0x0e},
...
static int sc3336_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int sc3336_init(struct tx_isp_subdev *sd, int enable)
{
...
static int sc3336_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int sc3336_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case TX_SENSOR_RES_300:
		sclk = SC3336_SUPPORT_SCLK_RES_300;
		break;
...
static int sc3336_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = sc3336_set_expo(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
	//	if(arg)
	//		ret = sc3336_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
	//	if(arg)
	//		ret = sc3336_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = sc3336_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = sc3336_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = sc3336_set_mode(sd, *(int*)arg);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = sc3336_set_fps(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = sc3336_set_vflip(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_LOGIC:
		if(arg)
			ret = sc3336_set_logic(sd, *(int*)arg);
	default:
		break;
	}
...
static int sc3336_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(sensor_resolution == TX_SENSOR_RES_300){
		...
		memcpy((void*)(&(sc3336_attr.mipi)),(void*)(&sc3336_mipi_3m),sizeof(sc3336_mipi_3m));
		sc3336_attr.max_integration_time_native = 1632 - 8;
		sc3336_attr.integration_time_limit = 1632 - 8;
		sc3336_attr.total_width = 2500;
		sc3336_attr.total_height = 1632;
		sc3336_attr.max_integration_time = 1360 - 8;
	}else if(sensor_resolution == TX_SENSOR_RES_100){
		wsize = &sc3336_win_sizes[1];
		memcpy((void*)(&(sc3336_attr.mipi)),(void*)(&sc3336_mipi_1m),sizeof(sc3336_mipi_1m));
		sc3336_attr.max_integration_time_native = 1320 - 8;
		sc3336_attr.integration_time_limit = 1320 - 8;
		sc3336_attr.total_width = 2500;
