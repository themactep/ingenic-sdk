...
#define GC2063_SUPPORT_30FPS_SCLK (74250000)
#define GC2063_SUPPORT_15FPS_SCLK (49500000)
...
static struct regval_list gc2063_init_regs_1920_1080_15fps_mipi[] = {
	...
	{0x41, 0x05}, //04
	{0x42, 0xdc},/*vts for 25fps mipi*/ //65
	{0x9d, 0x10},
	{0xce, 0x7c},
	{0xd2, 0x41},
	{0xd3, 0xdc},
...
static struct regval_list gc2063_init_regs_1920_1080_25fps_mipi[] = {
	...
	{0x90, 0x00},/*use frame length to change fps*/
	{0x41, 0x05},
	{0x42, 0x46},/*0x546,vts for 25fps mipi; 0x465for 15fps; 0x49d for 14.28fps*/
	{0x9d, 0x10},
	{0xce, 0x7c},
	{0xd2, 0x41},
	{0xd3, 0xdc},
...
static struct regval_list gc2063_init_regs_1920_1080_25fps_dvp[] = {
	/****system****/
	{0xfe, 0x80},
	{0xfe, 0x80},
	{0xfe, 0x80},
...
static struct tx_isp_sensor_win_setting gc2063_win_sizes[] = {
	/* 1920*1080 */
	{
		...
		.regs 		= gc2063_init_regs_1920_1080_25fps_dvp,
	},
	{
		.width		= 1920,
		.height		= 1080,
...
struct tx_isp_sensor_win_setting *wsize = &gc2063_win_sizes[3];
...
static int it_last = 0;
static int again_last = -1;
static int gc2063_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	if(again_last != again) {
		...
		ret += gc2063_write(sd, 0xb2, val_lut[again].regb2);
...
static int gc2063_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case TX_SENSOR_MAX_FPS_25:
		wpclk = GC2063_SUPPORT_30FPS_SCLK;
		max_fps = SENSOR_OUTPUT_MAX_FPS;
		break;
	case TX_SENSOR_MAX_FPS_15:
		wpclk = GC2063_SUPPORT_15FPS_SCLK;
...
static int gc2063_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = gc2063_set_integration_time_short(sd, *(int*)arg);
		break;
	...
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = gc2063_set_analog_gain_short(sd, *(int*)arg);
		break;
...
static int gc2063_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if((data_interface == TX_SENSOR_DATA_INTERFACE_DVP) && (sensor_max_fps == TX_SENSOR_MAX_FPS_25)){
		...
		memcpy((void*)(&(gc2063_attr.dvp)),(void*)(&gc2063_dvp),sizeof(gc2063_dvp));
		gc2063_attr.max_integration_time_native = 1121;
		gc2063_attr.integration_time_limit = 1121;
		gc2063_attr.total_width = 2560;
		gc2063_attr.total_height = 1125;
		gc2063_attr.max_integration_time = 1121;
		...
		gc2063_attr.max_integration_time_native = 1121;
		gc2063_attr.integration_time_limit = 1121;
		gc2063_attr.total_width = 2200;
		gc2063_attr.total_height = 1500;
		gc2063_attr.max_integration_time = 1121;
		gc2063_attr.one_line_expr_in_us = 44;
	} else {
	...
	*/
	...
	gc2063_attr.max_again = 390624;//393216;444858;
	...
	pr_debug("probe ok ------->gc2063 build:%s %s\n", __DATE__, __TIME__);
	return 0;
...
int init_sensor(void)
{
	...
	if(ret){
		printk("Failed to init gc2035 dirver.\n");
		return -1;
	}
