...
#define JXF51_SUPPORT_30FPS_SCLK_DVP (86400000)
#define JXF51_SUPPORT_30FPS_SCLK_MIPI (72000000)
#define SENSOR_OUTPUT_MAX_FPS 30
#define SENSOR_OUTPUT_MIN_FPS 5
#define SENSOR_VERSION	"H20200420a"

#define SENSOR_RISCV_HF_MODE
#define SENSOR_WITHOUT_INIT

//extern uint32_t g_riscv_sensor_again;
//extern uint32_t g_riscv_sensor_inttime;
...
static struct regval_list jxf51_init_regs_1920_1080_30fps_dvp[] = {
#if 1
	{0x12, 0x60},
	{0x48, 0x85},
	{0x48, 0x05},
...
static struct tx_isp_sensor_win_setting jxf51_win_sizes[] = {
	/* 1920*1080 */
	{
		...
		.regs 		= jxf51_init_regs_1920_1080_30fps_dvp,
	},
	{
		.width		= 1536,
		.height		= 1536,
...
static struct tx_isp_sensor_win_setting *wsize = &jxf51_win_sizes[1];
...
static int jxf51_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case TX_SENSOR_DATA_INTERFACE_DVP:
		sclk = JXF51_SUPPORT_30FPS_SCLK_DVP;
		break;
	...
	default:
		ret = -1;
		printk("Now we do not support this data interface!!!\n");
	}
			      ...
			      struct tx_isp_chip_ident *chip)
{
	...
	if (ret) {
		printk("chip found @ 0x%x (%s) is not an jxf51 chip.\n",
	...
	ident = 0x0f51;
#endif
	printk("jxf51 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
...
#ifndef SENSOR_WITHOUT_INIT
	private_jzgpio_set_func(GPIO_PORT_A, GPIO_FUNC_1, 0x1 << 15);
...
{
	int ret = 0;
	printk("init_sensor jxf51\n");
	ret = private_driver_get_interface();
	if(ret){
		printk("Failed to init jxf51 dirver.\n");
		return -1;
	}
