...
int int_last = -1;
int again_last = -1;
static int imx307_set_expo(struct tx_isp_subdev *sd, int value)
{
    int ret = 0;
    int it = (value & 0xffff);
    ...
    //integration time
	if(it != int_last){
		//printk("===============================>write intt=0x%x 3020=0x%-02x 3021=0x%-02x 2022=0x%-02x\n", it, (unsigned char)(shs & 0xff),
		//		  (unsigned char)((shs >> 8) & 0xff), (unsigned char)((shs >> 16) & 0x3));
    ...
    //sensor analog gain
	if(gain != again_last){
		//printk("==================================>set_analog_gain 0x%x\n", (unsigned char)gain);
		ret += imx307_write(sd, 0x3014, (unsigned char)(gain & 0xff));
		gain = again_last;
	}
}
#if 1
static int imx307_set_integration_time(struct tx_isp_subdev *sd, int int_time)
{
	...
	vmax = imx307_attr.total_height;
	shs = vmax - int_time - 2;
    //printk("===============================>write intt=0x%x 3020=0x%-02x 3021=0x%-02x 2022=0x%-02x\n", int_time, (unsigned char)(shs & 0xff),
	//		  (unsigned char)((shs >> 8) & 0xff), (unsigned char)((shs >> 16) & 0x3));
	ret = imx307_write(sd, 0x3020, (unsigned char)(shs & 0xff));
	ret += imx307_write(sd, 0x3021, (unsigned char)((shs >> 8) & 0xff));
	ret += imx307_write(sd, 0x3022, (unsigned char)((shs >> 16) & 0x3));
...
static int imx307_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    case TX_ISP_EVENT_SENSOR_EXPO:
       if(arg)
           ret = imx307_set_expo(sd, *(int*)arg);
       break;
...
{
	return imx307_win_sizes->width;
}

int get_sensor_height()
{
	return imx307_win_sizes->height;
}
