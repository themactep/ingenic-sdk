...
#define OS02D20_SUPPORT_SCLK_30FPS (79200000)
#define OS02D20_SUPPORT_SCLK_60FPS (88800000)
#define OS02D20_SUPPORT_SCLK_HDR (88800000)
#define SENSOR_OUTPUT_MAX_FPS 60
#define SENSOR_OUTPUT_MIN_FPS 5
#define OS02D20_VTS_60_FPS  0x44f
#define OS02D20_VTS_30_FPS  0x7b5
#define OS02D20_VTS_30_FPS_HDR  0x44f
#define OS02D20_VB_60_FPS  0x0
#define OS02D20_VB_30_FPS  0x366
#define OS02D20_VB_30_FPS_HDR  0x0
#define SENSOR_VERSION	"H20220314aa"
...
unsigned int os02d20_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	struct again_lut *lut = os02d20_again_lut;
...
struct tx_isp_mipi_bus os02d20_mipi_hdr={
...
struct tx_isp_mipi_bus os02d20_mipi={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 396,
	.lans = 2,
...
struct tx_isp_sensor_attribute os02d20_attr={
	...
	.max_integration_time_native = 0x7b5 - 4,
	.integration_time_limit = 0x7b5 - 4,
	.total_width = 0x53a,
	.total_height = 0x7b5,
	.max_integration_time = 0x7b5 - 4,
	.integration_time_apply_delay = 2,
	.again_apply_delay = 2,
	.dgain_apply_delay = 0,
	.sensor_ctrl.alloc_again = os02d20_alloc_again,
	.sensor_ctrl.alloc_again_short = os02d20_alloc_again_short,
...
#else
	/*
	 * linear_30fps_mipiclk 792Mbps@2lane
	 * PCLK=79.2M, Timer_clk=79.2M, mipiclk=792Mbps/2lane ,Row_length=1338, Frame_length=1973
	 */
	{0xfd, 0x00},
	{0x36, 0x01},
	{0xfd, 0x00},
...
static struct regval_list os02d20_init_regs_1920_1080_hdr_30fps[] = {
	/*V18_HDR*/
	{0xfd, 0x00},
	{0x36, 0x01},
	{0xfd, 0x00},
...
static struct tx_isp_sensor_win_setting os02d20_win_sizes[] = {
	...
	/*[3] HDR 2M@30fps*/
	{
		...
		.mbus_code	= V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= os02d20_init_regs_1920_1080_hdr_30fps,
	},
};
...
static int os02d20_set_expo(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
	int it = (value & 0xffff);
	...
	ret += os02d20_write(sd, 0x04, (unsigned char)(it & 0xff));
	ret += os02d20_write(sd, 0x03, (unsigned char)(it >> 8) & 0xff);
	ret += os02d20_write(sd, 0x24, again);
...
static int os02d20_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	...
	ret = os02d20_write(sd, 0xfd, 0x01);
	ret += os02d20_write(sd, 0x30, (unsigned char)(expo & 0xff));
	ret += os02d20_write(sd, 0x2f, (unsigned char)((expo >> 8) & 0xff));
	ret += os02d20_write(sd, 0x01, 0x01);
	if (ret < 0)
		return ret;
...
static int os02d20_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
	...
	ret = os02d20_write(sd, 0xfd, 0x01);
	ret += os02d20_write(sd, 0x42, value);
	ret += os02d20_write(sd, 0x01, 0x01);
	if (ret < 0)
		return ret;
...
static int os02d20_init(struct tx_isp_subdev *sd, int enable)
{
	...
	printk(" ===================> os02d20 init wirte_array\n");
...
static int os02d20_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	if(data_type == TX_SENSOR_DATA_TYPE_LINEAR){
		...
		case TX_SENSOR_MAX_FPS_30:
			...
			sclk = OS02D20_SUPPORT_SCLK_30FPS;
			vts_init = OS02D20_VTS_30_FPS;
			vb_init = OS02D20_VB_30_FPS;
			break;
		case TX_SENSOR_MAX_FPS_60:
			max_fps = TX_SENSOR_MAX_FPS_60;
			sclk = OS02D20_SUPPORT_SCLK_60FPS;
			vts_init = OS02D20_VTS_60_FPS;
			vb_init = OS02D20_VB_60_FPS;
			break;
		...
		max_fps = TX_SENSOR_MAX_FPS_30;
		sclk = OS02D20_SUPPORT_SCLK_HDR >> 1;
		vts_init = OS02D20_VTS_30_FPS_HDR;
		vb_init = OS02D20_VB_30_FPS_HDR;
	}
	...
	//ISP_WARNING("%s %d hts:%x vts:%x vb:%x fps:%x\n", __func__, __LINE__, hts, vts, vb, fps);
...
static int os02d20_set_wdr(struct tx_isp_subdev *sd, int wdr_en)
{
	int ret = 0;

	//ret = os02d20_write(sd, 0x36, 0x01);
	...
	ret = os02d20_write_array(sd, wsize->regs);
	ret = os02d20_write(sd, 0x03, evl0);
	ret = os02d20_write(sd, 0x04, evl1);
	ret = os02d20_write(sd, 0x2f, evs0);
	ret = os02d20_write(sd, 0x30, evs1);
	ret = os02d20_write_array(sd, os02d20_stream_on);

	return 0;
}

static int os02d20_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
	...
	ret = os02d20_read(sd, 0x03, &evl0);
	ret = os02d20_read(sd, 0x04, &evl1);
	ret = os02d20_read(sd, 0x2f, &evs0);
	ret = os02d20_read(sd, 0x30, &evs1);

	ret = os02d20_write_array(sd, os02d20_stream_off);
	if(wdr_en == 1){
		wsize = &os02d20_win_sizes[3];
		...
		sensor->video.mbus.colorspace = wsize->colorspace;
		sensor->video.fps = wsize->fps;
		memcpy((void*)(&(os02d20_attr.mipi)),(void*)(&os02d20_mipi_hdr),sizeof(os02d20_mipi_hdr));
		data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		os02d20_attr.data_type = data_type;
		os02d20_attr.one_line_expr_in_us = 27;

		os02d20_attr.max_again = 259142;
		os02d20_attr.max_again_short = 259142;
		os02d20_attr.max_dgain = 0;
		os02d20_attr.min_integration_time = 2;
		os02d20_attr.min_integration_time_short = 2;
		os02d20_attr.max_integration_time_short = 69;//exposure ratio 16
		...
		os02d20_attr.wdr_cache = wdr_bufsize;
		os02d20_attr.mipi.mipi_sc.sensor_csi_fmt = TX_SENSOR_RAW10;
		...
		memcpy((void*)(&(os02d20_attr.mipi)),(void*)(&os02d20_mipi),sizeof(os02d20_mipi));
		data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		os02d20_attr.data_type = data_type;
		os02d20_attr.one_line_expr_in_us = 17;
		os02d20_attr.data_type = data_type;
		os02d20_attr.max_integration_time_native = 0x7b5 - 8;
		os02d20_attr.integration_time_limit = 0x7b5 - 8;
		os02d20_attr.total_width = 0x53a;
		os02d20_attr.total_height = 0x7b5;
		os02d20_attr.max_integration_time = 0x7b5 - 8;
		os02d20_attr.mipi.mipi_sc.sensor_csi_fmt = TX_SENSOR_RAW10;
...
static int os02d20_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = os02d20_set_expo(sd, *(int*)arg);
		break;
...
//			ret = os02d20_set_analog_gain(sd, *(int*)arg);
	...
	case TX_ISP_EVENT_SENSOR_WDR:
		if(arg)
			ret = os02d20_set_wdr(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if(arg)
			ret = os02d20_set_wdr_stop(sd, *(int*)arg);
		break;
	default:
		break;;
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if(data_type == TX_SENSOR_DATA_TYPE_LINEAR){
		...
		os02d20_attr.min_integration_time_short = 2;
		os02d20_attr.max_integration_time_short = 69;//exposure ratio 16
		os02d20_attr.max_integration_time_native = 0x454 - 4;
		os02d20_attr.integration_time_limit = 0x454 - 4;
		os02d20_attr.total_width = 0x53a;
		os02d20_attr.total_height = 0x454;
		os02d20_attr.max_integration_time = 0x454 - 4;
		os02d20_attr.wdr_cache = wdr_bufsize;
		os02d20_attr.mipi.mipi_sc.sensor_csi_fmt = TX_SENSOR_RAW10;
	}
	os02d20_attr.data_type = data_type;
	...
	printk("probe ok ------->os02d20 build:%s %s\n", __DATE__, __TIME__);
...
int init_sensor(void)
{
	...
	if(ret){
		printk("Failed to init os02d20 dirver.\n");
		return -1;
	}
