...
#define sc3335_SUPPORT_30FPS_SCLK (105000000)
#define sc3335_SUPPORT_60FPS_SCLK (102000000)
#define sc3335_SUPPORT_100FPS_SCLK (102000000)
#define SENSOR_OUTPUT_MAX_FPS 100
#define SENSOR_OUTPUT_MIN_FPS 5
#define SENSOR_VERSION	"H20210827a"
...
struct tx_isp_sensor_attribute sc3335_attr={
	...
	.max_integration_time_native = 1400*2 - 10,
	.integration_time_limit = 1400*2 - 10,
	.total_width = 2500,
	.total_height = 1400,
	.max_integration_time = 1400*2 - 10,
...
static struct regval_list sc3335_init_regs_640_360_100fps_mipi[] = {
    {0x0103, 0x01},
    {0x0100, 0x00},
    {0x36e9, 0x80},
    {0x36f9, 0x80},
...
static struct regval_list sc3335_init_regs_1152_648_60fps_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x36f9,0x80},
...
static struct tx_isp_sensor_win_setting sc3335_win_sizes[] = {
	...
	/* resolution 1152*648 @60fps max*/
	{
		...
		.mbus_code	= V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= sc3335_init_regs_1152_648_60fps_mipi,
	},
	{
		.width		= 640,
		.height		= 360,
		...
		.mbus_code	= V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= sc3335_init_regs_640_360_100fps_mipi,
	}
};
...
static int sc3335_write_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	...
	while (vals->reg_num != sc3335_REG_END) {
		if (vals->reg_num == sc3335_REG_DELAY) {
			...
			if (ret < 0){
				printk("=========================>write error read regnum    :0x%x regval:0x%x---\n",vals->reg_num, vals->value);
				return ret;
			}
		}
		vals++;
...
#if 1
	int i = 0;
	while (vals[i].reg_num != sc3335_REG_END) {
        if (vals[i].reg_num == sc3335_REG_DELAY) {
            private_msleep(vals[i].value);
        } else {
            ret = sc3335_write(sd, vals[i].reg_num, vals[i].value);
            if (ret < 0){
                printk("=========================>write error read regnum    :0x%-04x regval:0x%-02x---\n",vals->reg_num, vals->value);
                return ret;
            }
        }
        i++;
	...
	while (vals[i].reg_num != sc3335_REG_END) {
		sc3335_read(sd, vals[i].reg_num, &ucValue);
		printk("read regnum    :0x%-04x regval:0x%-02x---\n",vals[i].reg_num, ucValue);
		i++;
	}
#endif
	return 0;
...
static int sc3335_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case TX_SENSOR_MAX_FPS_60:
		sclk = sc3335_SUPPORT_60FPS_SCLK;
		max_fps = TX_SENSOR_MAX_FPS_60;
		break;
	case TX_SENSOR_MAX_FPS_100:
		sclk = sc3335_SUPPORT_100FPS_SCLK;
	...
	if (sclk == sc3335_SUPPORT_60FPS_SCLK) {
		hts = ((hts << 8) + tmp) << 1;
	}else {
		hts = ((hts << 8) + tmp) << 1;
	}
	...
	sensor->video.attr->max_integration_time_native = vts*2 - 10;
	sensor->video.attr->integration_time_limit = vts*2 - 10;
	sensor->video.attr->total_height = vts;
	sensor->video.attr->max_integration_time = vts*2 - 10;
...
static int sc3335_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	case TX_SENSOR_MAX_FPS_30:
		...
		sc3335_mipi.clk = 432;
		sc3335_mipi.image_twidth = 2304;
		sc3335_mipi.image_theight = 1296;
		memcpy((void*)(&(sc3335_attr.mipi)),(void*)(&sc3335_mipi),sizeof(sc3335_mipi));
		break;
	case TX_SENSOR_MAX_FPS_25:
		wsize = &sc3335_win_sizes[1];
		sc3335_attr.max_integration_time_native = 1382*2 - 10;
		sc3335_attr.integration_time_limit = 1382*2 - 10;
		sc3335_attr.total_width = 2200;
		sc3335_attr.total_height = 1382;
		sc3335_attr.max_integration_time = 1382*2 - 10;
		break;
	case TX_SENSOR_MAX_FPS_60:
		printk("probe 60fps\n");
		wsize = &sc3335_win_sizes[2];
		sc3335_mipi.clk = 510;
		sc3335_mipi.image_twidth = 1152;
		sc3335_mipi.image_theight = 648;

		sc3335_attr.max_integration_time_native = 666*2 - 10;
		sc3335_attr.integration_time_limit = 666*2 - 10;
		sc3335_attr.total_width = 2552;
		sc3335_attr.total_height = 666;
		sc3335_attr.max_integration_time = 666*2 - 10;
		memcpy((void*)(&(sc3335_attr.mipi)),(void*)(&sc3335_mipi),sizeof(sc3335_mipi));
		break;
	case TX_SENSOR_MAX_FPS_100:
		wsize = &sc3335_win_sizes[3];
		sc3335_mipi.clk = 255;
		sc3335_mipi.image_twidth = 640;
		sc3335_mipi.image_theight = 360;
		sc3335_attr.max_integration_time_native = 408*2 - 10;
		sc3335_attr.integration_time_limit = 408*2 - 10;
		sc3335_attr.total_width = 2500;
		sc3335_attr.total_height = 408;
		sc3335_attr.max_integration_time = 408* - 10;
		memcpy((void*)(&(sc3335_attr.mipi)),(void*)(&sc3335_mipi),sizeof(sc3335_mipi));
		break;
	default:
		ISP_ERROR("do not support max framerate %d in mipi mode\n",sensor_max_fps);
	...
	printk("probe ok ------->sc3335 max_fps:%d build:%s %s\n", sensor_max_fps, __DATE__, __TIME__);
...
{
	return sc3335_win_sizes->width;
}

int get_sensor_height()
{
	return sc3335_win_sizes->height;
}
