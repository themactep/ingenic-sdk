...
#define OS04C10_SUPPORT_SCLK_1M (101008000)
#define OS04C10_SUPPORT_SCLK_110FPS (101133010)
...
static struct regval_list os04c10_init_regs_2560_1440_30fps[] = {
	...
	{0x3807, 0xff},
	{0x3808, 0x0a},
	{0x3809, 0x00},/*4m x out size*/
	{0x380a, 0x05},
	{0x380b, 0xa0},/*4m y out size*/
	{0x380c, 0x08},
	{0x380d, 0x5c},
	{0x380e, 0x06},
	{0x380f, 0x26},
	{0x3811, 0x48},/*isp_x_win*/
	{0x3813, 0x2c},/*isp_y_win*/
	{0x3814, 0x01},
	{0x3815, 0x01},
	{0x3816, 0x01},
	{0x3817, 0x01},
...
static struct regval_list os04c10_init_regs_1280_720_60fps[] = {
	{0x0100, 0x00},
	{0x0103, 0x01},
	{0x0301, 0x84},
	{0x0303, 0x01},
...
static struct regval_list os04c10_init_regs_2448_1520_30fps[] = {
        {0x0100, 0x00},
        {0x0103, 0x01},
        {0x0301, 0x84},
        {0x0303, 0x01},
...
static struct regval_list os04c10_init_regs_1224_760_110fps[] = {
        {0x0100,0x00},
        {0x0103,0x01},
        {0x0301,0x84},
        {0x0303,0x01},
...
static struct tx_isp_sensor_win_setting os04c10_win_sizes[] = {
	...
	/* [4] 1M @50fps*/
	{
		...
		.mbus_code	= V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= os04c10_init_regs_1280_720_60fps,
	},
	/* [5] 3.7M @30fps*/
	{
		.width		= 2448,
		...
		.mbus_code	= V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= os04c10_init_regs_2448_1520_30fps,
	},
	/* [6] 1M @110fps*/
	{
		.width		= 1224,
		...
		.mbus_code	= V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= os04c10_init_regs_1224_760_110fps,
	},
};
...
static int os04c10_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	if(data_type == TX_SENSOR_DATA_TYPE_LINEAR){
		...
		} else if(sensor_resolution == SENSOR_RES_370){
				sclk = OS04C10_SUPPORT_SCLK;
				max_fps = TX_SENSOR_MAX_FPS_30;
		} else if(sensor_resolution == SENSOR_RES_200){
			switch (sensor_max_fps) {
			case TX_SENSOR_MAX_FPS_30:
				sclk = OS04C10_SUPPORT_SCLK;
				max_fps = TX_SENSOR_MAX_FPS_30;
		...
		} else if(sensor_resolution == SENSOR_RES_100){
			switch (sensor_max_fps) {
			    case TX_SENSOR_MAX_FPS_60:
			        sclk = OS04C10_SUPPORT_SCLK_1M;
			        max_fps = TX_SENSOR_MAX_FPS_60;
				break;
			    case TX_SENSOR_MAX_FPS_110:
			        sclk = OS04C10_SUPPORT_SCLK_110FPS;
			        max_fps = TX_SENSOR_MAX_FPS_110;
				break;
			default:
				ISP_ERROR("Now we do not support this framerate @1mp!!!\n");
                        }
		}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if(data_type == TX_SENSOR_DATA_TYPE_LINEAR){
		if(sensor_resolution == SENSOR_RES_400){
			...
			switch (sensor_max_fps) {
			    case TX_SENSOR_MAX_FPS_30:
				wsize = &os04c10_win_sizes[5];
				os04c10_attr.mipi.image_twidth = 2448;
				os04c10_attr.mipi.image_theight = 1520;
				os04c10_attr.max_integration_time_native = 0x626 - 8;
				os04c10_attr.integration_time_limit = 0x626 - 8;
				os04c10_attr.total_width = 0x85e * 2;
				os04c10_attr.total_height = 0x626;
				os04c10_attr.max_integration_time = 0x626 - 8;
				os04c10_attr.mipi.mipi_sc.sensor_csi_fmt = TX_SENSOR_RAW10;
				break;
			default:
				ISP_ERROR("Now we do not support this framerate @ 3.7MP!!!\n");
                        }
		} else if (sensor_resolution == SENSOR_RES_200){
			switch (sensor_max_fps) {
			case TX_SENSOR_MAX_FPS_60:
			...
			switch (sensor_max_fps) {
			case TX_SENSOR_MAX_FPS_60:
			    wsize = &os04c10_win_sizes[4];
			    os04c10_attr.mipi.image_twidth = 1280;
			    os04c10_attr.mipi.image_theight = 720;
			    os04c10_attr.max_integration_time_native = 0x626 - 8;
			    os04c10_attr.integration_time_limit = 0x626 - 8;
			    os04c10_attr.total_width = 0x42e * 2;
			    os04c10_attr.total_height = 0x626;
			    os04c10_attr.max_integration_time = 0x626 - 8;
			    os04c10_attr.mipi.mipi_sc.sensor_csi_fmt = TX_SENSOR_RAW10;
		    	    break;
			case TX_SENSOR_MAX_FPS_110:
			    wsize = &os04c10_win_sizes[6];
			    os04c10_attr.mipi.image_twidth = 1224;
			    os04c10_attr.mipi.image_theight = 760;
			    os04c10_attr.max_integration_time_native = 0x391 - 8;
			    os04c10_attr.integration_time_limit = 0x391 - 8;
			    os04c10_attr.total_width = 0x3ef * 2;
			    os04c10_attr.total_height = 0x391;
			    os04c10_attr.max_integration_time = 0x391 - 8;
			    os04c10_attr.mipi.mipi_sc.sensor_csi_fmt = TX_SENSOR_RAW10;
		    	    break;
			default:
				ISP_ERROR("Now we do not support this framerate @ 1MP!!!\n");
...
{
	return os04c10_win_sizes->width;
}

int get_sensor_height()
{
	return os04c10_win_sizes->height;
}
