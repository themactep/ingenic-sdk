...
static int gc2053_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	...
	pr_debug("probe ok ------->gc2053 build:%s %s\n", __DATE__, __TIME__);

#ifdef SENSOR_RISCV_HF_MODE
	gc2053_alloc_again(g_riscv_sensor_again, 0, &again);
	gc2053_set_analog_gain(sd, again);
	gc2053_set_integration_time(sd, g_riscv_sensor_inttime);
#endif
