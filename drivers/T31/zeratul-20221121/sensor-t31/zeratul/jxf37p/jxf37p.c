...
/* 1080p@25fps: insmod sensor_jxf37p_t31.ko data_type=0 data_interface=1 sensor_resolution=200  */
/* DOL 1080p@15fps: insmod sensor_jxf37p_t31.ko data_interface=1 data_type=2  */
...
static struct regval_list jxf37p_init_regs_1920_1080_30fps_mipi[] = {
	{0x12, 0x40},
	{0x48, 0x8A},
	{0x48, 0x0A},
	{0x0E, 0x19},
...
static int jxf37p_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	/*gain*/
	ret += jxf37p_write(sd, 0x00, (unsigned char)(again & 0x7f));
...
static int jxf37p_set_logic(struct tx_isp_subdev *sd, int value)
{
...
static int jxf37p_set_fps(struct tx_isp_subdev *sd, int fps)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
	unsigned int sclk = JXF37P_SUPPORT_30FPS_SCLK_MIPI;
	unsigned int hts = 0;
	unsigned int vts = 0;
	unsigned char val = 0;
...
#if 0
	jxf37p_write(sd, 0xc0, 0x22);
	jxf37p_write(sd, 0xc1, (unsigned char)(vts & 0xff));
	jxf37p_write(sd, 0xc2, 0x23);
	jxf37p_write(sd, 0xc3, (unsigned char)(vts >> 8));
	ret = jxf37p_read(sd, 0x1f, &val);
	pr_debug("before register 0x1f value : 0x%02x\n", val);
	if(ret < 0)
		return -1;
	val |= (1 << 7); //set bit[7],  register group write function,  auto clean
	jxf37p_write(sd, 0x1f, val);
	pr_debug("after register 0x1f value : 0x%02x\n", val);
#else
	jxf37p_write(sd, 0x22, (unsigned char)(vts & 0xff));
	jxf37p_write(sd, 0x23, (unsigned char)(vts >> 8));
#endif
	if (0 != ret) {
		...
		struct tx_isp_chip_ident *chip)
{
	...
	ISP_WARNING("jxf37p chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
...
static int jxf37p_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	switch(cmd){
		case TX_ISP_EVENT_SENSOR_LOGIC:
			if(arg)
				ret = jxf37p_set_logic(sd, *(int*)arg);
			break;
...
{
	i2c_del_driver(&jxf37p_driver);
}

//module_init(init_jxf37p);
//module_exit(exit_jxf37p);

//MODULE_DESCRIPTION("A low-level driver for Sonic jxf37p sensors");
//MODULE_LICENSE("GPL");
