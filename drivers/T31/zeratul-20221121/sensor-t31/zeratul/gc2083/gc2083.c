...
#define GC2083_SUPPORT_30FPS_SCLK (99000000)
#define SENSOR_OUTPUT_MAX_FPS 30
#define SENSOR_OUTPUT_MIN_FPS 5
#define SENSOR_VERSION	"H20220106a"
...
struct again_lut gc2083_again_lut[] = {
	//index 0xd0 0x155 0x410 0x411 0x412 0x413 0x414 0x415 0x416 0x417 0xb8  0xb9  0xdc1 ispgain
...
static struct regval_list gc2083_init_regs_1920_1080_30fps_mipi[] = {
	...
	{0x0067, 0x80}, //0xa0 //settle_ref_th
	{0x0054, 0xff}, //settle_sig_th
	{0x0055, 0x00}, //settle_sig_th
	{0x0056, 0x00}, //settle_colgain_th
	{0x0057, 0x20}, //settle_colgain_th
	{0x005a, 0xa0}, //settle_data_value
...
static int gc2083_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	if (temperature_value >= 0x60) {
		ret += gc2083_write(sd,0x0171,0x2a);
		ret += gc2083_write(sd,0x0172,0x30);
	} else if (temperature_value < 0x38) {
		ret += gc2083_write(sd,0x0171,0x40);
		ret += gc2083_write(sd,0x0172,0x40);
	}
	...
	/*set blc*/
	ret += gc2083_write(sd, 0x155, val_lut[again].exp_rate_darkc);
	ret += gc2083_write(sd, 0x410, val_lut[again].darks_g1);
	ret += gc2083_write(sd, 0x411, val_lut[again].darks_r1);
	ret += gc2083_write(sd, 0x412, val_lut[again].darks_b1);
	ret += gc2083_write(sd, 0x413, val_lut[again].darks_g2);
	ret += gc2083_write(sd, 0x414, val_lut[again].darkn_g1);
	ret += gc2083_write(sd, 0x415, val_lut[again].darkn_r1);
	ret += gc2083_write(sd, 0x416, val_lut[again].darkn_b1);
	ret += gc2083_write(sd, 0x417, val_lut[again].darkn_g2);
	if (0 != ret)
...
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	unsigned int wpclk = GC2083_SUPPORT_30FPS_SCLK;
	...
	ret += gc2083_read(sd, 0xd05, &tmp);
	hts = tmp;
	ret += gc2083_read(sd, 0xd06, &tmp);
	if(ret < 0)
		return -1;
	hts = ((hts << 8) + tmp) << 1;
	...
	ret += gc2083_write(sd, 0x31d, 0x2e);
	ret += gc2083_write(sd, 0x0d41, (unsigned char)((vts & 0xff00) >> 8));
...
static int gc2083_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	...
	ret += gc2083_write(sd, 0x0192, col_start);
...
static int gc2083_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	...
	printk("probe ok ------->gc2083 build:%s %s\n", __DATE__, __TIME__);
...
{
	return gc2083_win_sizes->width;
}

int get_sensor_height()
{
	return gc2083_win_sizes->height;
}
