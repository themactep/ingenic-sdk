...
static void motor_set_default(struct motor_device *mdev)
{
	...
	for(index = 0; index < HAS_MOTOR_CNT; index++){
		...
		if (motor->pdata->motor_st1_gpio)
			gpio_set_value(motor->pdata->motor_st1_gpio, 0);
		if (motor->pdata->motor_st2_gpio)
			gpio_set_value(motor->pdata->motor_st2_gpio, 0);
		if (motor->pdata->motor_st3_gpio)
			gpio_set_value(motor->pdata->motor_st3_gpio, 0);
		if (motor->pdata->motor_st4_gpio)
			gpio_set_value(motor->pdata->motor_st4_gpio, 0);
	}
	return;
}

static void motor_set_output(struct motor_device *mdev)
{
	int index = 0;
	struct motor_driver *motor = NULL;
	mdev->dev_state = MOTOR_OPS_STOP;
	for(index = 0; index < HAS_MOTOR_CNT; index++){
		...
		motor->state = MOTOR_OPS_STOP;
		if (motor->pdata->motor_st1_gpio)
			gpio_direction_output(motor->pdata->motor_st1_gpio, HORIZONTAL_GPIO_LEVEL);
		if (motor->pdata->motor_st2_gpio)
			gpio_direction_output(motor->pdata->motor_st2_gpio, HORIZONTAL_GPIO_LEVEL);
		if (motor->pdata->motor_st3_gpio)
			gpio_direction_output(motor->pdata->motor_st3_gpio, HORIZONTAL_GPIO_LEVEL);
		if (motor->pdata->motor_st4_gpio)
			gpio_direction_output(motor->pdata->motor_st4_gpio, HORIZONTAL_GPIO_LEVEL);
	}
	return;
}
...
static void motor_move_step(struct motor_device *mdev, int index)
{
	...
	if(motor->state != MOTOR_OPS_STOP){
		...
		if (motor->pdata->motor_st1_gpio)
			gpio_set_value(motor->pdata->motor_st1_gpio, step_8[step] & 0x8);
		if (motor->pdata->motor_st2_gpio)
			gpio_set_value(motor->pdata->motor_st2_gpio, step_8[step] & 0x4);
		if (motor->pdata->motor_st3_gpio)
			gpio_set_value(motor->pdata->motor_st3_gpio, step_8[step] & 0x2);
		if (motor->pdata->motor_st4_gpio)
			gpio_set_value(motor->pdata->motor_st4_gpio, step_8[step] & 0x1);
	}else{
		if (motor->pdata->motor_st1_gpio)
			gpio_set_value(motor->pdata->motor_st1_gpio, 0);
		if (motor->pdata->motor_st2_gpio)
