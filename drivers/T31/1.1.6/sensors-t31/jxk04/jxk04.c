...
static enum v4l2_mbus_pixelcode jxk04_mbus_code[] = {
...
static int jxk04_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int jxk04_init(struct tx_isp_subdev *sd, int enable)
{
...
static int jxk04_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int jxk04_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = -1;
	unsigned char val = 0x0;

	ret += jxk04_read(sd, 0x12, &val);
	if(enable & 0x2)
		val |= 0x10;
	else
		val &= 0xef;

	ret += jxk04_write(sd, 0x12, val);
	if(!ret)
		ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
...
static int jxk04_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	switch(cmd){
		...
		if(arg)
			ret = jxk04_set_expo(sd, *(int*)arg);
		break;
/*
		case TX_ISP_EVENT_SENSOR_INT_TIME:
			if(arg)
				ret = jxk04_set_integration_time(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_AGAIN:
			if(arg)
				ret = jxk04_set_analog_gain(sd, *(int*)arg);
			break;
*/
		case TX_ISP_EVENT_SENSOR_DGAIN:
			if(arg)
				ret = jxk04_set_digital_gain(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
			if(arg)
				ret = jxk04_get_black_pedestal(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_RESIZE:
			if(arg)
				ret = jxk04_set_mode(sd, *(int*)arg);
			break;
		...
		case TX_ISP_EVENT_SENSOR_FPS:
			if(arg)
				ret = jxk04_set_fps(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_VFLIP:
			if(arg)
				ret = jxk04_set_vflip(sd, *(int*)arg);
			break;
		default:
			break;
	}
...
static __init int init_jxk04(void)
{
	...
	if(ret){
		ISP_ERROR("Failed to init jxk04 dirver.\n");
		return -1;
	}
