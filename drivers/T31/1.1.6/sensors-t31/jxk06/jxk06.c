...
#define JXK06_SUPPORT_SCLK_4M	(86400000) //(172800000)
...
static struct regval_list jxk06_init_regs_2560_1920_15fps_mipi_4m[] = {
	{JXK06_REG_DELAY, 0x50},
	{JXK06_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list jxk06_init_regs_2560_1920_25fps_mipi_4m[] = {
	{0x12, 0x70},
	{0x48, 0x86},
	{0x48, 0x06},
	{0x0E, 0x11},
...
static struct tx_isp_sensor_win_setting jxk06_win_sizes[] = {
	/* 2560*1920 @15fps*/
	{
		...
		.regs 		= jxk06_init_regs_2560_1920_15fps_mipi_4m,
	},
	/* 2560*1920 @25fps*/
	{
		.width		= 2560,
		...
		.mbus_code	= V4L2_MBUS_FMT_SGBRG10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= jxk06_init_regs_2560_1920_25fps_mipi_4m,
	},
};

static enum v4l2_mbus_pixelcode jxk06_mbus_code[] = {
...
static int jxk06_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int jxk06_init(struct tx_isp_subdev *sd, int enable)
{
...
static int jxk06_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int jxk06_set_fps(struct tx_isp_subdev *sd, int fps)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;
	unsigned int sclk = JXK06_SUPPORT_SCLK_4M;
	unsigned int hts = 0;
	unsigned int vts = 0;
	...
	case TX_SENSOR_MAX_FPS_15:
		...
		sclk = JXK06_SUPPORT_SCLK_4M;
		break;
	case TX_SENSOR_MAX_FPS_30:
		max_fps = SENSOR_OUTPUT_MAX_FPS;
		sclk = JXK06_SUPPORT_SCLK_4M << 1;
		break;
	default:
		ISP_WARNING("jxk06 Do not support this max fps now.\n");
	}
	...
	hts = (hts | val) << 2;
	if (0 != ret) {
		ISP_WARNING("err: jxk06 read err\n");
		return ret;
	}
...
#endif
	if (0 != ret) {
		ISP_WARNING("err: jxk06_write err\n");
		return ret;
	}
		...
		struct tx_isp_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"jxk06_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = private_gpio_request(pwdn_gpio,"jxk06_pwdn");
		if(!ret){
...
static int jxk06_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	switch(cmd){
		...
		if(arg)
			ret = jxk06_set_expo(sd, *(int*)arg);
		break;
/*
		case TX_ISP_EVENT_SENSOR_INT_TIME:
			if(arg)
				ret = jxk06_set_integration_time(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_AGAIN:
			if(arg)
				ret = jxk06_set_analog_gain(sd, *(int*)arg);
			break;
*/
		case TX_ISP_EVENT_SENSOR_DGAIN:
			if(arg)
				ret = jxk06_set_digital_gain(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
			if(arg)
				ret = jxk06_get_black_pedestal(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_RESIZE:
			if(arg)
				ret = jxk06_set_mode(sd, *(int*)arg);
			break;
		...
		case TX_ISP_EVENT_SENSOR_FPS:
			if(arg)
				ret = jxk06_set_fps(sd, *(int*)arg);
			break;
		case TX_ISP_EVENT_SENSOR_VFLIP:
			if(arg)
				ret = jxk06_set_vflip(sd, *(int*)arg);
			break;
		default:
			break;
	}
...
static int jxk06_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	case TX_SENSOR_MAX_FPS_30:
		wsize=&jxk06_win_sizes[1];
#if 0
		jxk06_attr.max_integration_time_native = 2400 - 4;
		jxk06_attr.integration_time_limit = 2400 - 4;
		jxk06_attr.total_width = 2880;
		jxk06_attr.total_height = 2400;
		jxk06_attr.max_integration_time = 2400 - 4;
		jxk06_attr.one_line_expr_in_us = 17;
		jxk06_attr.mipi.mipi_sc.sensor_csi_fmt = TX_SENSOR_RAW10;
#endif
		break;
	default:
