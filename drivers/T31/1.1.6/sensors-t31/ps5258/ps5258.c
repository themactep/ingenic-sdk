...
static enum v4l2_mbus_pixelcode ps5258_mbus_code[] = {
...
static int ps5258_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int ps5258_init(struct tx_isp_subdev *sd, int enable)
{
	...
	ret = ps5258_read_array(sd, wsize->regs);
	if (ret)
		return ret;
...
static int ps5258_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
//		if(arg)
//			ret = ps5258_set_expo(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = ps5258_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = ps5258_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = ps5258_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = ps5258_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = ps5258_set_mode(sd, *(int*)arg);
		break;
