...
#define GC2023_REG_END		0xff
#define GC2023_REG_DELAY	0x00
#define GC2023_PAGE_REG	    0xfe
#define GC2023_SUPPORT_30FPS_SCLK (95424000)
...
unsigned int gc2023_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	gain_one1 = gc2023_gainone_to_reg(gain_one, &regs);
	isp_gain1 = private_log2_fixed_to_fixed(gain_one1, TX_ISP_GAIN_FIXED_POINT, shift);
	*sensor_again = regs;
	return isp_gain1;
}

unsigned int gc2023_alloc_dgain(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_dgain)
{
	return isp_gain;
}
...
struct tx_isp_sensor_attribute gc2023_attr={
	...
	.max_integration_time_native = 1344 - 4,
	.integration_time_limit = 1344 - 4,
	.total_width = 2840,
	.total_height = 1344,
	.max_integration_time = 1344- 4,
	.integration_time_apply_delay = 2,
	.again_apply_delay = 2,
	.dgain_apply_delay = 2,
	.sensor_ctrl.alloc_again = gc2023_alloc_again,
	.sensor_ctrl.alloc_dgain = gc2023_alloc_dgain,
	//	void priv; /* point to struct tx_isp_sensor_board_info */
};

static struct regval_list gc2023_init_regs_1920_1080_25fps_mipi[] = {
	/*SYSTEM  */
	{0xf2, 0x00},
	{0xf6, 0x00},
	{0xfc, 0x06},
	...
	{GC2023_REG_DELAY,0x10},
	{GC2023_REG_END, 0x00},	/* END MARKER */
};

/*
 * the order of the jxf23_win_sizes is [full_resolution, preview_resolution].
 */
static struct tx_isp_sensor_win_setting gc2023_win_sizes[] = {
	/* 1920*1080 */
	{
		.width		= 1920,
		.height		= 1080,
		...
		.regs 		= gc2023_init_regs_1920_1080_25fps_mipi,
	},
};

struct tx_isp_sensor_win_setting *wsize = &gc2023_win_sizes[0];

/*
 * the part of driver was fixed.
 */

static struct regval_list gc2023_stream_on[] = {
	{ 0xf2, 0x8f},
	{GC2023_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list gc2023_stream_off[] = {
	{ 0xf2, 0x80},
	{GC2023_REG_END, 0x00},	/* END MARKER */
};

int gc2023_read(struct tx_isp_subdev *sd, unsigned char reg,
		unsigned char *value)
{
...
static int gc2023_write(struct tx_isp_subdev *sd, unsigned char reg,
			unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int gc2023_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
	while (vals->reg_num != GC2023_REG_END) {
		if (vals->reg_num == GC2023_REG_DELAY) {
				msleep(vals->value);
		} else {
			ret = gc2023_read(sd, vals->reg_num, &val);
			if (ret < 0)
				return ret;
			if (vals->reg_num == GC2023_PAGE_REG){
				val &= 0xf8;
				val |= (vals->value & 0x07);
...
static int gc2023_write_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	while (vals->reg_num != GC2023_REG_END) {
		if (vals->reg_num == GC2023_REG_DELAY) {
...
static int gc2023_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}

static int gc2023_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
	unsigned char v;
	int ret;
...
static int gc2023_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
	...
	ret = gc2023_write(sd, 0x3, (value&0x1f00)>>8);
	if (ret < 0) {
		printk("gc2023_write error  %d\n" ,__LINE__ );
		return ret;
	}
...
static int gc2023_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
...
static int gc2023_set_digital_gain(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int gc2023_get_black_pedestal(struct tx_isp_subdev *sd, int value)
{
	return 0;
}

static int gc2023_init(struct tx_isp_subdev *sd, int enable)
{
	...
	ret = gc2023_write_array(sd, wsize->regs);
...
static int gc2023_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int gc2023_set_fps(struct tx_isp_subdev *sd, int fps)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	unsigned int pclk = 0;
	...
	case TX_SENSOR_MAX_FPS_25:
		pclk = GC2023_SUPPORT_30FPS_SCLK ;
	...
	if(ret < 0)
		return -1;
	printk("fps=%x,vts=%d,hts=%d,vb=%d,hb=%d,win_high=%d",fps,vts,hts,vb,hb,win_high);
...
static int gc2023_set_mode(struct tx_isp_subdev *sd, int value)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int gc2023_g_chip_ident(struct tx_isp_subdev *sd,
			       struct tx_isp_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"gc2023_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = private_gpio_request(pwdn_gpio,"gc2023_pwdn");
		if(!ret){
	...
	ret = gc2023_detect(sd, &ident);
	if (ret) {
		printk("chip found @ 0x%x (%s) is not an gc2023 chip.\n",
				client->addr, client->adapter->name);
		return ret;
	}
	printk("gc2023 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	if(chip){
		memcpy(chip->name, "gc2023", sizeof("gc2023"));
		chip->ident = ident;
...
static int gc2023_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	long ret = 0;
	...
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = gc2023_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = gc2023_set_analog_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = gc2023_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = gc2023_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = gc2023_set_mode(sd, *(int*)arg);
		break;
...
static int gc2023_g_register(struct tx_isp_subdev *sd, struct tx_isp_dbg_register *reg)
{
	unsigned char val = 0;
	int len = 0;
	int ret = 0;
	...
	if (!private_capable(CAP_SYS_ADMIN))
		return -EPERM;
	ret = gc2023_read(sd, reg->reg & 0xffff, &val);
	reg->val = val;
	reg->size = 2;
...
static int gc2023_s_register(struct tx_isp_subdev *sd, const struct tx_isp_dbg_register *reg)
{
	int len = 0;
	...
	if (!private_capable(CAP_SYS_ADMIN))
		return -EPERM;
	gc2023_write(sd, reg->reg & 0xffff, reg->val & 0xff);

	return 0;
}

static struct tx_isp_subdev_core_ops gc2023_core_ops = {
	.g_chip_ident = gc2023_g_chip_ident,
	.reset = gc2023_reset,
	.init = gc2023_init,
	/*.ioctl = gc2023_ops_ioctl,*/
	.g_register = gc2023_g_register,
	.s_register = gc2023_s_register,
};

static struct tx_isp_subdev_video_ops gc2023_video_ops = {
	.s_stream = gc2023_s_stream,
};

static struct tx_isp_subdev_sensor_ops	gc2023_sensor_ops = {
	.ioctl	= gc2023_sensor_ops_ioctl,
};

static struct tx_isp_subdev_ops gc2023_ops = {
	.core = &gc2023_core_ops,
	.video = &gc2023_video_ops,
	.sensor = &gc2023_sensor_ops,
};
...
static int gc2023_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	...
	sd = &sensor->sd;
	video = &sensor->video;
	sensor->video.attr = &gc2023_attr;
/*	gc2023_attr.expo_fs=1;  */
	...
	sensor->video.mbus.colorspace = wsize->colorspace;
	sensor->video.fps = wsize->fps;
	tx_isp_subdev_init(&sensor_platform_device, sd, &gc2023_ops);
...
static struct i2c_driver gc2023_driver = {
	.driver = {
		.owner	= THIS_MODULE,
		.name	= "gc2023",
	},
...
static __init int init_gc2023(void)
{
	int ret = 0;
	ret = private_driver_get_interface();
	if(ret){
		ISP_ERROR("Failed to init gc2023 dirver.\n");
		return -1;
	}
	return private_i2c_add_driver(&gc2023_driver);
}

static __exit void exit_gc2023(void)
{
	private_i2c_del_driver(&gc2023_driver);
}

module_init(init_gc2023);
module_exit(exit_gc2023);

MODULE_DESCRIPTION("A low-level driver for gc2023 sensors");
MODULE_LICENSE("GPL");
