...
int fast_wdr_mode=0;
static int data_type = TX_SENSOR_DATA_TYPE_LINEAR;//TX_SENSOR_DATA_TYPE_WDR_DOL;
...
static enum v4l2_mbus_pixelcode sc401ai_mbus_code[] = {
...
static int sc401ai_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sensor->video.attr->max_integration_time_native = (vts - 4)*2;
	sensor->video.attr->integration_time_limit = (vts - 4)*2;
	sensor->video.attr->total_height = vts;
	sensor->video.attr->max_integration_time = (vts - 4)*2;
	ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
...
int init_sensor(void)
{
	...
	if(ret){
		printk("Failed to init sc401ai dirver.\n");
		return -1;
	}
