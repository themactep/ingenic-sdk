...
#define SC200AI_SUPPORT_PCLK_FPS_60 (148500*1000)
...
struct again_lut sc200ai_again_c_lut[] = {
	{0x0340, 0},
	{0x0341, 1501},
	{0x0342, 2886},
	{0x0343, 4342},
...
#define lut_c_size ( sizeof(sc200ai_again_c_lut) / sizeof(struct again_lut ) )
...
#define dcg_gain (115700)
unsigned int sc200ai_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
    struct again_lut *lut = sc200ai_again_c_lut;
    int lut_last_idx = lut_c_size - 1;
    if(isp_gain >= lut[lut_last_idx].gain + dcg_gain)
    {
        *sensor_again = lut[lut_last_idx].value | 0x2000;
        return lut[lut_last_idx].gain + dcg_gain;
    }
    else if (isp_gain >= (dcg_gain)) {
		unsigned int again_tmp = isp_gain - dcg_gain;
		int i;
		for (i = 1; i < lut_c_size; i++) {
			if (again_tmp < lut[i].gain) {
                *sensor_again = lut[i-1].value | 0x2000;
                return lut[i-1].gain + dcg_gain; 
			}
		}
	} else {
		unsigned int again_tmp = isp_gain;
		int i;
		for (i = 1; i < lut_c_size; i++) {
			if (again_tmp < lut[i].gain) {
                *sensor_again = lut[i-1].value;
                return lut[i-1].gain; 
			}
		}
    }
...
static int sc200ai_reset(struct tx_isp_subdev *sd, int val)
{
	return 0;
}
...
static int sc200ai_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	if ((again >= 0x3f47) && dpc_flag) {
		ret += sc200ai_write(sd, 0x5799, 0x07);
		dpc_flag = false;
	} else if ((again <= 0x2f5f) && (!dpc_flag)) {
		ret += sc200ai_write(sd, 0x5799, 0x00);
		dpc_flag = true;
	}
	if (ret != 0) {
...
static int sc200ai_init(struct tx_isp_subdev *sd, int enable)
{
...
static int sc200ai_s_stream(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
...
static int sc200ai_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sclk = SC200AI_SUPPORT_PCLK_FPS_60;
...
static int sc200ai_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if(arg)
			ret = sc200ai_set_expo(sd, *(int*)arg);
		break;
	/*
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = sc200ai_set_integration_time(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = sc200ai_set_analog_gain(sd, *(int*)arg);
		break;
	*/
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = sc200ai_set_digital_gain(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = sc200ai_get_black_pedestal(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = sc200ai_set_mode(sd, *(int*)arg);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = sc200ai_set_fps(sd, *(int*)arg);
		break;
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = sc200ai_set_vflip(sd, *(int*)arg);
		break;
	default:
		break;
	}
...
static int sc200ai_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sc200ai_attr.max_again = 261403 + dcg_gain;
	sc200ai_attr.max_dgain = 0; //sc200ai_attr.max_dgain;
	sd = &sensor->sd;
	video = &sensor->video;
...
{
	return sc200ai_win_sizes->width;
}

int get_sensor_height()
{
	return sc200ai_win_sizes->height;
}
