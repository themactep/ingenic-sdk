...
#define JXH62_SUPPORT_PCLK_DVP (36000000)
#define JXH62_SUPPORT_PCLK_MIPI (43200000)
...
struct tx_isp_mipi_bus jxh62_mipi={
...
struct tx_isp_dvp_bus jxh62_dvp={
...
static struct regval_list jxh62_init_regs_1280_720_25fps_dvp[] = {
	{0x12,0x40},
	{0x0E,0x11},
	{0x0F,0x00},
	{0x10,0x18},
 ...
 */
static struct regval_list jxh62_init_regs_1280_720_25fps_mipi[] = {
	{0x12,0x40},
	{0x0E,0x11},
	{0x0F,0x09},
	{0x10,0x24},
...
static struct tx_isp_sensor_win_setting jxh62_win_sizes[] = {
	/* 1280*720 */
	{
		...
		.regs 		= jxh62_init_regs_1280_720_25fps_dvp,
	},
	{
		.width		= 1280,
		.height		= 720,
		...
		.mbus_code	= V4L2_MBUS_FMT_SBGGR10_1X10,
		.colorspace	= V4L2_COLORSPACE_SRGB,
		.regs 		= jxh62_init_regs_1280_720_25fps_mipi,
	},
};
...
static struct regval_list jxh62_stream_on_dvp[] = {
	{0x12, 0x00},
	{JXH62_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list jxh62_stream_off_dvp[] = {
	{0x12, 0x40},
	{JXH62_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list jxh62_stream_on_mipi[] = {
	{0x12, 0x00},
	{JXH62_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list jxh62_stream_off_mipi[] = {
	{0x12, 0x40},
	{JXH62_REG_END, 0x00},	/* END MARKER */
};
...
static int jxh62_s_stream(struct tx_isp_subdev *sd, int enable)
{
	...
	if (enable) {
		if (data_interface == TX_SENSOR_DATA_INTERFACE_DVP){
			ret = jxh62_write_array(sd, jxh62_stream_on_dvp);
		} else if (data_interface == TX_SENSOR_DATA_INTERFACE_MIPI){
			ret = jxh62_write_array(sd, jxh62_stream_on_mipi);

		}else{
			ISP_WARNING("Don't support this Sensor Data interface\n");
		}
		pr_debug("jxh62 stream on\n");
	}
	else {
		if (data_interface == TX_SENSOR_DATA_INTERFACE_DVP){
			ret = jxh62_write_array(sd, jxh62_stream_off_dvp);
		} else if (data_interface == TX_SENSOR_DATA_INTERFACE_MIPI){
			ret = jxh62_write_array(sd, jxh62_stream_off_mipi);
...
static int jxh62_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case TX_SENSOR_DATA_INTERFACE_DVP:
		sclk=JXH62_SUPPORT_PCLK_DVP;
		break;
	case TX_SENSOR_DATA_INTERFACE_MIPI:
		sclk=JXH62_SUPPORT_PCLK_MIPI;
		break;
	default:
			      ...
			      struct tx_isp_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		...
		if(!ret){
			...
			ISP_WARNING("gpio requrest fail %d\n",reset_gpio);
		}
	}
	if (pwdn_gpio != -1) {
	...
	if (ret) {
		ISP_WARNING("chip found @ 0x%x (%s) is not an jxh62 chip.\n",
		       client->addr, client->adapter->name);
		return ret;
	}
	ISP_WARNING("jxh62 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
	if(chip){
...
static int jxh62_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	if(IS_ERR_OR_NULL(sd)){
		ISP_WARNING("[%d]The pointer is invalid!\n", __LINE__);
		return -EINVAL;
	}
...
//            goto err_set_sensor_gpio;
	...
	for(i = 0; i < ARRAY_SIZE(jxh62_win_sizes); i++)
		jxh62_win_sizes[i].mbus_code = mbus;
            ...
            wsize = &jxh62_win_sizes[1];
            jxh62_attr.max_integration_time_native = 749,
            jxh62_attr.integration_time_limit = 749,
            jxh62_attr.total_width = 2304,
            jxh62_attr.total_height = 750,
            jxh62_attr.max_integration_time = 749,
            memcpy((void*)(&(jxh62_attr.mipi)),(void*)(&jxh62_mipi),sizeof(jxh62_mipi));
    } else{
     ...
     */
  	jxh62_attr.dbus_type = data_interface;
