...
static irqreturn_t jz_timer_thread_handle(int this_irq, void *dev_id)
{
	...
	if(mdev->reg_state == REGISTER_CHANGE){
		...
		if(motors[HORIZONTAL_MOTOR].move_dir == MOTOR_MOVE_STOP && motors[VERTICAL_MOTOR].move_dir == MOTOR_MOVE_STOP){
			#if defined(CONFIG_SOC_T40) || (defined(CONFIG_KERNEL_4_4_94) && defined(CONFIG_SOC_T41))
...
#if defined(CONFIG_SOC_T40) || (defined(CONFIG_KERNEL_4_4_94) && defined(CONFIG_SOC_T41))
	ingenic_tcu_set_prescale(mdev->tcu->cib.id,TCU_PRESCALE_64);
#else
...
static int motor_probe(struct platform_device *pdev)
{
	...
	ret = gpio_request(MS419XX_RESET_GPIO,"ms419xx_reset");
	if(ret){
		dev_err(&pdev->dev, "Failed to request reset gpio(%d)\n", MS419XX_RESET_GPIO);
		goto error_gpio;
	}
	gpio_direction_output(MS419XX_RESET_GPIO,1);
	msleep(10);
	gpio_direction_output(MS419XX_RESET_GPIO,0);
