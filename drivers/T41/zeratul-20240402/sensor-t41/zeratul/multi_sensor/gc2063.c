...
struct tx_isp_sensor_win_setting *wsize_two = &gc2063_win_sizes[0];
...
static int sensor_set_attr(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
	...
	sensor->video.vi_max_width = wsize_two->width;
	sensor->video.vi_max_height = wsize_two->height;
	sensor->video.mbus.width = wsize_two->width;
	sensor->video.mbus.height = wsize_two->height;
	sensor->video.mbus.code = wsize_two->mbus_code;
	sensor->video.mbus.field = TISP_FIELD_NONE;
	sensor->video.mbus.colorspace = wsize_two->colorspace;
	sensor->video.fps = wsize_two->fps;
	sensor->video.max_fps = wsize_two->fps;
...
static int gc2063_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	sensor_set_attr(sd, wsize_two);
	sensor->video.state = TX_ISP_MODULE_INIT;
	ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
	sensor->priv = wsize_two;
...
static int gc2063_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	printk("===================================\n");
	printk("------> [%s][%d]  enter SENSOR_WITHOUT_INIT gc2063 \n", __func__, __LINE__);
	printk("===================================\n");
			ret = gc2063_write_array(sd, wsize_two->regs);
			if (ret)
				return ret;
#endif
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	ISP_WARNING("\n====>[default_boot=%d] [resolution=%dx%d] [video_interface=%d] [MCLK=%d] \n", info->default_boot, wsize_two->width, wsize_two->height, info->video_interface, info->mclk);
...
struct platform_device sensor_platform_device_two = {
	.name = "gc2063",
	.id = -1,
	.dev = {
			...
			const struct i2c_device_id *id)
{
	...
	tx_isp_subdev_init(&sensor_platform_device_two, sd, &gc2063_ops);
...
char * get_sensor_name_two(void)
{
	return "gc2063";
}

int get_sensor_i2c_addr_two(void)
{
	return 0x37;
}

int get_sensor_width_two(void)
{
	return gc2063_win_sizes->width;
}

int get_sensor_height_two(void)
{
	return gc2063_win_sizes->height;
}

int get_sensor_wdr_mode_two(void)
{
	return 0;
}


int init_sensor_two(void)
{
	printk("-------------------------------------\n");
	printk("======> enter init_sensor_two \n");
	printk("-------------------------------------\n");
	return private_i2c_add_driver(&gc2063_driver);
}
struct add_sensor sensor_two = {
	.name = "gc2063",
	.sensor_init = init_sensor_two,
	.add_sensor_get_name = get_sensor_name_two,
	.add_sensor_get_i2c_addr = get_sensor_i2c_addr_two,
};
...
{
	printk("====== enter sensor_register_two use_num_sensor = %d ======\n", use_num_sensor);

    sensor_two.wdr_mode = 0;
	
	sensors[use_num_sensor] = &sensor_two;
	return 0;
}
void exit_sensor_two(void)
{
	i2c_del_driver(&gc2063_driver);
}
