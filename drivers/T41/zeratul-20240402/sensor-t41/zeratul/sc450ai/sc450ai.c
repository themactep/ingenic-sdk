...
#define SENSOR_VERSION	"H20230721a"
...
static int sc450ai_resume(struct tx_isp_subdev *sd)
{
	int ret = 0;
	...
	if (ret != 0) {
		ISP_ERROR("err: sc450ai write err %d\n",__LINE__);
		return ret;
	}

    /*resume fps*/
    if(resume_fps != 0){
        sc450ai_set_fps(sd, resume_fps);
    }
	return 0;
}
...
static int sc450ai_set_user_define(struct tx_isp_subdev *sd,void *sv)
{
...
static int sc450ai_pm_s_stream_ctrl(struct tx_isp_subdev *sd,void *sv)
{
    int ret = 0;
    tx_isp_sensor_user_data *data = (tx_isp_sensor_user_data*)sv;
    printk("%s[%d]: enter\n",__func__,__LINE__);
    ...
    if(data->dir){
        ...
        ret = sc450ai_write_array(sd, wsize->regs);
        printk("#123 sc450ai write array !!!!\n");
        if (ret < 0)
            return ret;
        //resume last attr
        ret = sc450ai_resume(sd);
        if(ret != 0){
            printk("sc450ai resume attr failed\n");
            return ret;
        }
        ret = sc450ai_write_array(sd, sc450ai_stream_on);
        ISP_WARNING("sc450ai stream on(power off)\n");
        ...
        ISP_WARNING("sc450ai stream off\n");
    }
    return 0;
}
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
    ...
    case 0:
        ...
        sc450ai_attr.total_height = 1872;
        sc450ai_attr.max_integration_time_native = 1868;   /* (1878*2-8)/2=1874 */
        sc450ai_attr.integration_time_limit = 1868;
        sc450ai_attr.max_integration_time = 1868;
...
static int sc450ai_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    case TX_ISP_EVENT_SENSOR_USER_DEF:
        if(arg)
            ret =  sc450ai_set_user_define(sd, arg);
        break;
...
{
    return sc450ai_win_sizes->width;
}

int get_sensor_height(void)
{
    return sc450ai_win_sizes->height;
}
