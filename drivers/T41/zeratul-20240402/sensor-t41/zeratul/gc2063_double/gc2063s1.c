...
#define GC2063s1_CHIP_ID_H	(0x20)
#define GC2063s1_CHIP_ID_L	(0x53)
#define GC2063s1_REG_END		0xff
#define GC2063s1_REG_DELAY	0x00
...
static struct regval_list gc2063s1_init_regs_1920_1080_30fps_dvp[] = {
	...
	{0xfe,0x00},
	{0x3e,0x40},
	{GC2063s1_REG_END, 0x00},/* END MARKER */
};
...
struct tx_isp_sensor_win_setting *wsize = &gc2063s1_win_sizes[1];
...
static int gc2063s1_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
	while (vals->reg_num != GC2063s1_REG_END) {
		if (vals->reg_num == GC2063s1_REG_DELAY) {
			msleep(vals->value);
		} else {
...
static int gc2063s1_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if(init->enable) {
		...
		if(sensor->video.state == TX_ISP_MODULE_RUNNING){
			...
			ISP_WARNING("gc2063s1 stream on\n");
		}
	}
	else {
		ret = gc2063s1_write_array(sd, gc2063s1_stream_off_mipi);
		ISP_WARNING("gc2063s1 stream off\n");
	}
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		memcpy(&(gc2063s1_attr.mipi), &gc2063s1_mipi, sizeof(gc2063s1_mipi));
		...
		gc2063s1_attr.again =0;
		gc2063s1_attr.integration_time = 0x8e01;
		gc2063s1_attr.one_line_expr_in_us = 28;

		break;
	case 1:
		wsize = &gc2063s1_win_sizes[0];
		memcpy(&(gc2063s1_attr.dvp), &gc2063s1_dvp, sizeof(gc2063s1_dvp));
		gc2063s1_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		...
		gc2063s1_attr.again =0;
		gc2063s1_attr.integration_time = 0xb201;
			       ...
			       struct tx_isp_chip_ident *chip)
{
	...
	ident = 0x2063;
#endif
	ISP_WARNING("gc2063s1 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
...
static int gc2063s1_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = gc2063s1_set_vflip(sd, sensor_val->value);
		break;
	default:
		break;
	}
...
{
	return gc2063s1_win_sizes->width;
}

int get_sensor_height(void)
{
	return gc2063s1_win_sizes->height;
}
...
{
	return gc2063_win_sizes->width;
}

int get_sensor1_height(void)
{
	return gc2063_win_sizes->height;
}
...
{
	return private_i2c_del_driver(&gc2063_driver);
}
#endif
#endif
