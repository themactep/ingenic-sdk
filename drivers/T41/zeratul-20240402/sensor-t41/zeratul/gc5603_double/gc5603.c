...
struct tx_isp_mipi_bus gc5603_mipi_dol = {
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 1152,
	.lans = 2,
	.settle_time_apative_en = 0,
...
static struct regval_list gc5603_init_regs_2880_1620_40fps_mipi[] = {
        {0x03fe, 0xf0},
        {0x03fe, 0x00},
        {0x03fe, 0x10},
        {0x03fe, 0x00},
...
static struct tx_isp_sensor_win_setting gc5603_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= gc5603_init_regs_2880_1620_40fps_mipi,
	},
};

struct tx_isp_sensor_win_setting *wsize = &gc5603_win_sizes[3];
...
static int gc5603_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	if(sensor->info.default_boot == 1){
		ret = gc5603_read(sd, 0x0200, &val);
		short_time = val << 8;
		ret += gc5603_read(sd, 0x0201, &val);
		short_time |= val;
	}
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		memcpy(&gc5603_attr.mipi, &gc5603_mipi_linear, sizeof(gc5603_mipi_linear));
	...
	case 1:
		...
		memcpy(&gc5603_attr.mipi, &gc5603_mipi_dol, sizeof(gc5603_mipi_dol));
		gc5603_attr.wdr_cache = wdr_bufsize;
	...
	case 3:
		...
		gc5603_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		memcpy(&gc5603_attr.mipi, &gc5603_mipi_linear, sizeof(gc5603_mipi_linear));
                gc5603_attr.mipi.clk = 1152;
		gc5603_attr.one_line_expr_in_us = 14;
		gc5603_attr.total_width = 3600;
	...
	default:
        ...
        gc5603_attr.fsync_attr.mode = fsync_mode;
        if (fsync_mode == TX_ISP_SENSOR_FSYNC_MODE_MS_REALTIME_MISPLACE) {
                gc5603_attr.total_height = gc5603_attr.total_height * 8 / 3;
                wsize->fps = ( (wsize->fps & 0xffff0000) * 3 | (wsize->fps & 0xffff) * 8 );
        }
        gc5603_attr.max_integration_time_native = gc5603_attr.total_height - 8;
        gc5603_attr.integration_time_limit = gc5603_attr.total_height - 8;
        gc5603_attr.max_integration_time = gc5603_attr.total_height - 8;
...
static int gc5603_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
	...
	if( wdr_en == 1){
		...
		gc5603_attr.max_integration_time = 2100 - 8;
		gc5603_attr.again = 0;
		gc5603_attr.integration_time = 0x6ce;
...
static int gc5603_fsync(struct tx_isp_subdev *sd, struct tx_isp_sensor_fsync *fsync)
{
        uint8_t val;
        uint16_t ret_val;
        ...
        case 0:
                ...
                case 2:
                        printk("=========>> [%s %d]\n", __func__, __LINE__);
                        gc5603_read(sd, 0x0341, &val);
                        gc5603_write(sd, 0x0341, val + 4);
                        gc5603_write(sd, 0x027f, 0x03);
                        gc5603_write(sd, 0x02f7, 0x01);
                        break;
                case 3:
                        printk("=========>> [%s %d]\n", __func__, __LINE__);
                        gc5603_read(sd, 0x0340, &val);
                        ret_val = val << 8;
                        gc5603_read(sd, 0x0341, &val);
                        ret_val |= val;
                        ret_val = (ret_val * 8 / 3) + 4;
                        gc5603_write(sd, 0x0340, ret_val >> 8);
                        gc5603_write(sd, 0x0341, ret_val & 0xff);
                        gc5603_write(sd, 0x027f, 0x03);
                        gc5603_write(sd, 0x02f7, 0x01);

                        /* step 13.9us */
                        gc5603_write(sd, 0x02f5, 0x09);
                        gc5603_write(sd, 0x02f6, 0x06);
                        break;
                }
                break;
        }
...
{
	return gc5603_win_sizes->width;
}

int get_sensor_height(void)
{
	return gc5603_win_sizes->height;
}

void init_sensor(void)
{
	private_i2c_add_driver(&gc5603_driver);
}
...
{
	return private_i2c_del_driver(&gc5603_driver);
}
#endif
#endif
