...
struct tx_isp_mipi_bus sc401ai_mipi={
...
struct tx_isp_mipi_bus sc401ai_mipi_15={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 420,
	.lans = 2,
	.settle_time_apative_en = 0,
...
struct tx_isp_mipi_bus sc401ai_27M_mipi={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 1188,
	.lans = 2,
	.settle_time_apative_en = 0,
...
static struct regval_list sc401ai_init_regs_2560_1440_15fps_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x36f9,0x80},
...
static struct regval_list sc401ai_init_regs_2560_1440_30fps_27M_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x36f9,0x80},
...
static struct tx_isp_sensor_win_setting sc401ai_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= sc401ai_init_regs_2560_1440_30fps_27M_mipi,
	},
};
...
static int sc401ai_resume_fps(struct tx_isp_subdev *sd,int fps)
{
...
static int sc401ai_resume(struct tx_isp_subdev *sd)
{
	int ret = 0;
	int it = (resume_expo_value & 0xffff);
	int again = (resume_expo_value & 0xffff0000) >> 16;
    /* resume expo */
	ret = sc401ai_write(sd, 0x3e00, (unsigned char)((it >> 12) & 0x0f));
    ...
    /*resume fps*/
    if(resume_fps != 0){
        sc401ai_resume_fps(sd, resume_fps);
    }
	return 0;
}
...
static int sc401ai_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	if ((again >= 0x3f47) && dpc_flag) {
		sc401ai_write(sd, 0x5799, 0x07);
		dpc_flag = false;
	} else if ((again <= 0x2f5f) && (!dpc_flag)) {
		sc401ai_write(sd, 0x5799, 0x00);
		dpc_flag = true;
	}
...
static int sc401ai_set_user_define(struct tx_isp_subdev *sd,void *sv)
{
...
static int sc401ai_pm_s_stream_ctrl(struct tx_isp_subdev *sd,void *sv)
{
    int ret = 0;
    struct tx_isp_sensor_user_data *data = (struct tx_isp_sensor_user_data*)sv;
    printk("%s[%d]: enter\n",__func__,__LINE__);
    ...
    if(data->cmd == TX_SENSOR_PM_RESUME){
        ...
        ret = sc401ai_write_array(sd, wsize->regs);
        if (ret < 0){
            printk("sc401ai resume setting failed\n");
            return ret;
        }
        ret = sc401ai_resume(sd);
        if(ret != 0){
            printk("sc401ai resume attr failed\n");
            return ret;
        }
        ret = sc401ai_write_array(sd, sc401ai_stream_on_mipi);
        if(ret != 0){
            printk("sc401ai streamon failed\n");
            return ret;
        }
        ISP_WARNING("sc401ai stream on(power off)\n");
#elif defined (SENSOR_STANDBY)
        ret = sc401ai_write_array(sd, sc401ai_stream_on_mipi);
        if (ret < 0)
            return ret;
        ISP_WARNING("sc401ai stream on(standby)\n");
#endif
        ...
        ISP_WARNING("sc401ai resume early process\n");
    }else{
        return -1;
    }
    return 0;
...
static int sc401ai_set_vflip(struct tx_isp_subdev *sd, int enable)
{
    ...
    val = sc401ai_read(sd, 0x3221, &val);
    switch(enable) {
    case 0:
        sc401ai_write(sd, 0x3221, val & 0x99);
        break;
    case 1:
        sc401ai_write(sd, 0x3221, val | 0x06);
        break;
    case 2:
        sc401ai_write(sd, 0x3221, val | 0x60);
        break;
    case 3:
        sc401ai_write(sd, 0x3221, val | 0x66);
        break;
    }
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
    ...
    case 0:
        ...
        sc401ai_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI,
            sc401ai_attr.max_integration_time_native = 5272;
        sc401ai_attr.integration_time_limit = 5272;
        sc401ai_attr.total_width = 3000;
        sc401ai_attr.total_height = 2460;
        sc401ai_attr.max_integration_time = 5272;
        sc401ai_attr.again =0x340;
        sc401ai_attr.integration_time = 0xb60;
        break;
    case 1:
        wsize = &sc401ai_win_sizes[1];
    ...
    case 2:
        ...
        memcpy(&(sc401ai_attr.mipi), &sc401ai_27M_mipi, sizeof(sc401ai_27M_mipi));
        sc401ai_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
        sc401ai_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI,
            sc401ai_attr.max_integration_time_native = 5272; /* 2 * 2640 - 8 */
        sc401ai_attr.integration_time_limit = 5272;
        sc401ai_attr.total_width = 3000;
        sc401ai_attr.total_height = 2640;
        sc401ai_attr.max_integration_time = 5272;
        sc401ai_attr.again =0x340;
        sc401ai_attr.integration_time = 0xb60;
        break;
    ...
    ISP_WARNING("\n====>[default_boot=%d] [resolution=%dx%d] [video_interface=%d] [MCLK=%d] Ctl[%d,%d]\n", info->default_boot, wsize->width, wsize->height,
                info->video_interface, info->mclk,reset_gpio,reset_gpio,pwdn_gpio);
...
static int sc401ai_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    case TX_ISP_EVENT_SENSOR_USER_DEF:
        if(arg)
            ret =  sc401ai_set_user_define(sd, arg);
        break;
                         ...
                         const struct i2c_device_id *id)
{
    ...
    pr_info("probe ok ----->sc401ai(%p %p, size%d)\n",sensor, &sensor->video, sizeof(*sensor));
...
{
    return sc401ai_win_sizes->width;
}

int get_sensor_height(void)
{
    return sc401ai_win_sizes->height;
}
