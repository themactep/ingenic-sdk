...
static int resume_again = 0;
static unsigned int resume_fps = 0;
static int gc8613_set_fps(struct tx_isp_subdev *sd, int fps);
static int gc8613_resume(struct tx_isp_subdev *sd)
{
    int ret = 0;
	int expo = (resume_expo_value & 0xffff);
...
static int gc8613_set_user_define(struct tx_isp_subdev *sd,void *sv)
{
...
static int gc8613_pm_s_stream_ctrl(struct tx_isp_subdev *sd,void *sv)
{
    int ret = 0;
    tx_isp_sensor_user_data *data = (tx_isp_sensor_user_data*)sv;
    printk("%s[%d]: enter\n",__func__,__LINE__);
    ...
    if(data->dir){
        ...
        if (ret < 0)
            return ret;
        //resume last attr
        ret = gc8613_resume(sd);
        if(ret != 0){
            printk("gc8613 resume attr failed\n");
            return ret;
        }
        ret = gc8613_write_array(sd, gc8613_stream_on);
        ISP_WARNING("gc8613 stream on(power off)\n");
#elif defined (SENSOR_STANDBY)
        ret = gc8613_write_array(sd, gc8613_stream_on);
        ISP_WARNING("gc8613 stream on(standby)\n");
#endif
    }else{
        ret = gc8613_write_array(sd, gc8613_stream_off);
        ISP_WARNING("gc8613 stream off\n");
    }
    return 0;
}
...
static int gc8613_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    case TX_ISP_EVENT_SENSOR_USER_DEF:
        if(arg)
            ret =  gc8613_set_user_define(sd, arg);
        break;
    case TX_ISP_EVENT_PM_S_STREAM_CTRL:
        if(arg)
            ret =  gc8613_pm_s_stream_ctrl(sd, arg);
        break;
	default:
		break;
	}
...
{
    return gc8613_win_sizes->width;
}

int get_sensor_height(void)
{
    return gc8613_win_sizes->height;
}
