...
#define SC830AI_SUPPORT_LINEAR_30FPS_SCLK     141750000   /*  2100 * 2250 * 30  */
#define SC830AI_SUPPORT_LINEAR_60FPS_SCLK     182250000   /*  2025 * 1500 * 60  */
#define SC830AI_SUPPORT_LINEAR_1080P_60FPS_SCLK    141750000	/* 2100 * 1125 *60 */
#define SC830AI_SUPPORT_WDR_30FPS_SCLK   	   141750000   /*  2100 * 2250 * 30  */
#define SC830AI_SUPPORT_WDR_60FPS_SCLK        182250000   /*  2025 * 1500 * 60  */
...
unsigned int sc830ai_alloc_integration_time(unsigned int it, unsigned char shift, unsigned int *sensor_it)
{
...
unsigned int sc830ai_alloc_integration_time_short(unsigned int it, unsigned char shift, unsigned int *sensor_it)
{
	unsigned int expo = it >> shift;
	unsigned int isp_it = it;
	*sensor_it = expo;
...
unsigned int sc830ai_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
        struct again_lut *lut = sc830ai_again_lut;
...
struct tx_isp_mipi_bus sc830ai_mipi_30fps_linear={
...
struct tx_isp_mipi_bus sc830ai_mipi_60fps_linear={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 1215,
	.lans = 2,
	.index = 0,
...
struct tx_isp_mipi_bus sc830ai_mipi_60fps_1080P_linear={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 708,
	.lans = 2,
	.index = 0,
...
struct tx_isp_mipi_bus sc830ai_mipi_30fps_dol = {
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 709,
	.lans = 2,
...
struct tx_isp_mipi_bus sc830ai_mipi_60fps_dol = {
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 1012,
	.lans = 2,
	.settle_time_apative_en = 1,
...
struct tx_isp_sensor_attribute sc830ai_attr={
	...
	.sensor_ctrl.alloc_again_short = sc830ai_alloc_again_short,
	.sensor_ctrl.alloc_dgain = sc830ai_alloc_dgain,
	.sensor_ctrl.alloc_integration_time_short = sc830ai_alloc_integration_time_short,
};
...
static struct regval_list sc830ai_init_regs_2560_1440_60fps_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
...
static struct regval_list sc830ai_init_regs_1920_1080_60fps_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
...
static struct regval_list sc830ai_init_regs_1920_1080_30fps_mipi_dol[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
...
static struct regval_list sc830ai_init_regs_1280_720_60fps_mipi_dol[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
...
static struct tx_isp_sensor_win_setting sc830ai_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= sc830ai_init_regs_2560_1440_60fps_mipi,
	},
	{
		.width		= 1920,
		.height		= 1080,
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= sc830ai_init_regs_1920_1080_60fps_mipi,
	},
	{
		.width		= 1920,
		.height		= 1080,
		...
		.regs		= sc830ai_init_regs_1920_1080_30fps_mipi_dol,
	},
	{
		.width		= 1280,
		.height		= 720,
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.mbus_code	= TISP_COLORSPACE_SRGB,
		.regs		= sc830ai_init_regs_1280_720_60fps_mipi_dol,
	},
};
...
static int sc830ai_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret = sc830ai_write(sd,  0x3e00, (unsigned char)((it >> 12) & 0x0f));
...
static int sc830ai_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc830ai_write(sd, 0x3e02, (unsigned char)((it & 0x0f) << 4));
	if (ret < 0) {
		ISP_ERROR("sc830ai_write error  %d\n" ,__LINE__ );
		return ret;
	}
...
static int sc830ai_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;
...
//	printk("\n==============> short_time = 0x%x\n", value);
	...
	ret = sc830ai_write(sd,  0x3e22, (unsigned char)((value >> 12) & 0x0f));
	ret = sc830ai_write(sd,  0x3e04, (unsigned char)((value >> 4) & 0xff));
	ret = sc830ai_write(sd,  0x3e05, (unsigned char)(value & 0x0f) << 4);
...
static int sc830ai_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc830ai_write(sd, 0x3e09, (unsigned char)((value >> 8) & 0xff));
	ret += sc830ai_write(sd, 0x3e07, (unsigned char)(value & 0xff));

	if (ret < 0) {
		ISP_ERROR("sc830ai_write error  %d" ,__LINE__ );
		return ret;
	}
...
static int sc830ai_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;
        ret += sc830ai_write(sd, 0x3e13, (unsigned char)((value >> 8) & 0xff));
		ret += sc830ai_write(sd, 0x3e11, (unsigned char)(value & 0xff));
        if (ret < 0)
                return ret;
...
static int sc830ai_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			...
			pr_debug("sc830ai stream on\n");
			sensor->video.state = TX_ISP_MODULE_RUNNING;
		}
	} else {
		ret = sc830ai_write_array(sd, sc830ai_stream_off);
		pr_debug("sc830ai stream off\n");
	}
...
static int sc830ai_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case 0:
		sclk = SC830AI_SUPPORT_LINEAR_30FPS_SCLK;
		sensor_max_fps = TX_SENSOR_MAX_FPS_30;
		break;
	case 1:
		sclk = SC830AI_SUPPORT_LINEAR_60FPS_SCLK;
		sensor_max_fps = TX_SENSOR_MAX_FPS_60;
		break;
	case 2:
		sclk = SC830AI_SUPPORT_LINEAR_1080P_60FPS_SCLK;
		sensor_max_fps = TX_SENSOR_MAX_FPS_60;
		break;
	case 3:
		sclk = SC830AI_SUPPORT_WDR_30FPS_SCLK;
		sensor_max_fps = TX_SENSOR_MAX_FPS_30;
		break;
	case 4:
		sclk = SC830AI_SUPPORT_WDR_60FPS_SCLK;
		sensor_max_fps = TX_SENSOR_MAX_FPS_60;
		break;
	default:
	...
	sc830ai_write(sd, 0x320f, (unsigned char)(vts & 0xff));
	sc830ai_write(sd, 0x320e, (unsigned char)(vts >> 8));

	if(sensor->info.default_boot >= 2){
	sc830ai_read(sd, 0x3e23, &val);
	short_time = val << 8;
	sc830ai_read(sd, 0x3e24, &val);
	short_time |= val;
	}
	...
	sensor->video.attr->max_integration_time_native = (sensor->info.default_boot <= 1) ? (2*vts -10) : (vts/2 - short_time/2 - 9);
	sensor->video.attr->integration_time_limit = (sensor->info.default_boot <= 1) ? (2*vts -10) : (vts/2 - short_time/2 - 9);
	sensor->video.attr->total_height = vts;
	sensor->video.attr->max_integration_time = (sensor->info.default_boot <= 1) ? (2*vts -10) : (vts/2 - short_time/2 - 9);
	sensor->video.attr->max_integration_time = (sensor->info.default_boot <= 1) ? (2*vts -10) : (vts/2 - short_time/2 - 9);
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		memcpy(&sc830ai_attr.mipi, &sc830ai_mipi_30fps_linear, sizeof(sc830ai_mipi_30fps_linear));
		sc830ai_attr.mipi.clk = 1135;
		sc830ai_attr.min_integration_time = 4;
		sc830ai_attr.min_integration_time_native = 4;
		sc830ai_attr.total_width = 2100;
		sc830ai_attr.total_height = 2250;
		sc830ai_attr.max_integration_time_native = 4483;
		sc830ai_attr.integration_time_limit = 4483;
		sc830ai_attr.max_integration_time = 4483;
	...
	case 1:
		...
		memcpy(&sc830ai_attr.mipi, &sc830ai_mipi_60fps_linear, sizeof(sc830ai_mipi_60fps_linear));
		sc830ai_attr.mipi.clk = 1215;
		sc830ai_attr.min_integration_time = 4;
		sc830ai_attr.min_integration_time_native = 4,
		sc830ai_attr.total_width = 2025;
		sc830ai_attr.total_height = 1500;
		sc830ai_attr.max_integration_time_native = 2983;
		sc830ai_attr.integration_time_limit = 2983;
		sc830ai_attr.max_integration_time = 2983;
		sc830ai_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sc830ai_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		printk("=================> linear is ok\n");
		break;
	case 2:
		wsize = &sc830ai_win_sizes[2];
		memcpy(&sc830ai_attr.mipi, &sc830ai_mipi_60fps_1080P_linear, sizeof(sc830ai_mipi_60fps_1080P_linear));
		sc830ai_attr.mipi.clk = 708,
		sc830ai_attr.min_integration_time = 4;
		sc830ai_attr.min_integration_time_native = 4,
		sc830ai_attr.total_width = 2100;
		sc830ai_attr.total_height = 1125;
		sc830ai_attr.max_integration_time_native = 2233;
		sc830ai_attr.integration_time_limit = 2233;
		sc830ai_attr.max_integration_time = 2233;
		sc830ai_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sc830ai_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		printk("=================> linear is ok\n");
		break;
	case 3:
		...
		sc830ai_attr.wdr_cache = wdr_bufsize;
		wsize = &sc830ai_win_sizes[3];
		memcpy(&sc830ai_attr.mipi, &sc830ai_mipi_30fps_dol, sizeof(sc830ai_mipi_30fps_dol));
		sc830ai_attr.mipi.clk = 709,
		sc830ai_attr.min_integration_time = 1;
		sc830ai_attr.min_integration_time_native = 1,
		sc830ai_attr.min_integration_time_short = 1;
		sc830ai_attr.total_width = 2100;
		sc830ai_attr.total_height = 2250;
		sc830ai_attr.max_integration_time_native = 1044; /* vts/2-short/2-9 */
		sc830ai_attr.integration_time_limit = 1044;      /* 1125-72-9= 1044*/
		sc830ai_attr.max_integration_time = 1044;
		sc830ai_attr.max_integration_time_short = 64; /* short/2-8 */
		sc830ai_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		printk("=================> 30fps_hdr is ok\n");
		break;
	case 4:
		sc830ai_attr.wdr_cache = wdr_bufsize;
		wsize = &sc830ai_win_sizes[4];
		memcpy(&sc830ai_attr.mipi, &sc830ai_mipi_60fps_dol, sizeof(sc830ai_mipi_60fps_dol));
		sc830ai_attr.mipi.clk = 1012,
		sc830ai_attr.min_integration_time = 1;
		sc830ai_attr.min_integration_time_native = 1,
		sc830ai_attr.min_integration_time_short = 1;
		sc830ai_attr.total_width = 2025;
		sc830ai_attr.total_height = 1500;
		sc830ai_attr.max_integration_time_native = 656; /* vts/2-short/2-9 */
		sc830ai_attr.integration_time_limit = 656;
		sc830ai_attr.max_integration_time = 656;
		sc830ai_attr.max_integration_time_short = 77; /* short/2-8 */
		sc830ai_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		printk("=================> 30fps_hdr is ok\n");
		break;
	default:
	...
	data_type = sc830ai_attr.data_type;
...
#if 1
static int sc830ai_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
	struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
	int ret = 0;

	ret = sc830ai_write(sd, 0x0103, 0x1);
	...
	if(wdr_en == 1){
		...
		if(info->default_boot == 2)
		{
			...
			sc830ai_attr.total_width = 2100;
			sc830ai_attr.total_height = 2250;
			sc830ai_attr.max_integration_time_native = 1044;
			sc830ai_attr.integration_time_limit = 1044;
			sc830ai_attr.max_integration_time = 1044;
			sc830ai_attr.max_integration_time_short = 64;
			printk("\n-------------------------switch wdr@30fps ok ----------------------\n");
		}
		else if(info->default_boot == 3)
		{
			...
			sc830ai_attr.total_width = 2025;
			sc830ai_attr.total_height = 1500;
			sc830ai_attr.max_integration_time_native = 656;
			sc830ai_attr.integration_time_limit = 656;
			sc830ai_attr.max_integration_time = 656;
			sc830ai_attr.max_integration_time_short = 77;
			printk("\n-------------------------switch wdr@60fps ok ----------------------\n");
		}
	}
	else if (wdr_en == 0){
...
static int sc830ai_set_wdr(struct tx_isp_subdev *sd, int wdr_en)
{
	int ret = 0;
//	printk("\n==========> set_wdr\n");
	private_gpio_direction_output(reset_gpio, 1);
...
static int sc830ai_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = sc830ai_set_analog_gain_short(sd, sensor_val->value);
		break;
	...
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = sc830ai_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_WDR:
		if(arg)
			ret = sc830ai_set_wdr(sd, init->enable);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if(arg)
			ret = sc830ai_set_wdr_stop(sd, init->enable);
		break;
...
static int sc830ai_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
MODULE_DESCRIPTION("A low-level driver for sc830ai sensors");
MODULE_LICENSE("GPL");
