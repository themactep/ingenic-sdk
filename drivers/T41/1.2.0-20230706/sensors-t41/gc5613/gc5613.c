...
#define GC5613_CHIP_ID_H	(0x56)
#define GC5613_CHIP_ID_L	(0x03)
#define GC5613_REG_END		0xffff
#define GC5613_REG_DELAY	0x0000
...
unsigned int gc5613_alloc_integration_time(unsigned int it, unsigned char shift, unsigned int *sensor_it)
{
...
unsigned int gc5613_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while(lut->gain <= gc5613_attr.max_again) {
		if(isp_gain == 0) {
			*sensor_again = 0;
			return lut[0].gain;
			...
			*sensor_again = (lut - 1)->index;
			...
			if((lut->gain == gc5613_attr.max_again) && (isp_gain >= lut->gain)) {
...
struct tx_isp_mipi_bus gc5613_mipi = {
...
static struct regval_list gc5613_init_regs_2816_1584_30fps_mipi[] = {
	{0x03fe,0xf0},
	{0x03fe,0x00},
	{0x03fe,0x10},
	{0x03fe,0x00},
	...
	{0x0115,0x10},
	{0x0100,0x09},
	{GC5613_REG_DELAY, 0x14},/*for frame struct to take effect*/
	{0x0a70,0x00},
	{0x0080,0x02},
	{0x0a67,0x00},
{GC5613_REG_END, 0x00},	/* END MARKER */
};
...
static struct tx_isp_sensor_win_setting gc5613_win_sizes[] = {
	/* [0] 2816*1584 @ max 30fps*/
	{
		...
		.regs 		= gc5613_init_regs_2816_1584_30fps_mipi,
	}
};

struct tx_isp_sensor_win_setting *wsize = &gc5613_win_sizes[0];

static struct regval_list gc5613_stream_on[] = {
	{GC5613_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list gc5613_stream_off[] = {
	{GC5613_REG_END, 0x00},	/* END MARKER */
};
...
static int gc5613_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
	while (vals->reg_num != GC5613_REG_END) {
		if (vals->reg_num == GC5613_REG_DELAY) {
...
static int gc5613_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
	...
	if (ret < 0)
		return ret;
	if (v != GC5613_CHIP_ID_H)
		return -ENODEV;
	ret = gc5613_read(sd, 0x03f1, &v);
	ISP_WARNING("-----%s: %d ret = %d, v = 0x%02x\n", __func__, __LINE__, ret,v);
	if (ret < 0)
		return ret;
	if (v != GC5613_CHIP_ID_L)
		return -ENODEV;
	*ident = (*ident << 8) | v;
...
static int gc5613_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret = gc5613_write(sd, 0x0203, expo & 0xff);
	ret += gc5613_write(sd, 0x0202, expo >> 8);
...
static int gc5613_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	ret += gc5613_write(sd, 0x031d ,0x2d);
	ret += gc5613_write(sd, 0x0614, val_lut[value].reg614);
	ret += gc5613_write(sd, 0x0615, val_lut[value].reg615);
	ret += gc5613_write(sd, 0x0225, val_lut[value].reg225);
	ret += gc5613_write(sd, 0x031d, 0x28);

	ret += gc5613_write(sd, 0x1467, val_lut[value].reg1467);
	ret += gc5613_write(sd, 0x1468, val_lut[value].reg1468);
	ret += gc5613_write(sd, 0x00b8, val_lut[value].regb8);
	ret += gc5613_write(sd, 0x00b9, val_lut[value].regb9);
	if (ret < 0)
		ISP_ERROR("gc5613_write error  %d\n" ,__LINE__ );
		...
		if (sensor->video.state == TX_ISP_MODULE_DEINIT){
			...
			if (ret)
                ...
                if(sensor->video.state == TX_ISP_MODULE_RUNNING){
                        ret = gc5613_write_array(sd, gc5613_stream_on);
                        ISP_WARNING("gc5613 stream on\n");
                }
	} else {
		ret = gc5613_write_array(sd, gc5613_stream_off);
...
static int gc5613_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	ret += gc5613_read(sd, 0x0342, &tmp);
	hts = tmp & 0x0f;
	ret += gc5613_read(sd, 0x0343, &tmp);
	if(ret < 0)
		return -1;
	hts = ((hts << 8) | tmp) << 1;
	vts = sclk * (fps & 0xffff) / hts / ((fps & 0xffff0000) >> 16);
	ret = gc5613_write(sd, 0x0340, (unsigned char)((vts & 0x3f00) >> 8));
	ret += gc5613_write(sd, 0x0341, (unsigned char)(vts & 0xff));
	if(ret < 0)
		return -1;
...
static int gc5613_set_hvflip(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
	uint8_t val, val1;

	/* 2'b01: mirror; 2'b10:flip*/
	val = gc5613_read(sd, 0x022c, &val);
	val1 = gc5613_read(sd, 0x0063, &val1);

	/* 2'b01 mirror; 2'b10 flip; 2'b11 mirror &flip */
	switch(enable){
		case 0:
			gc5613_write(sd, 0x022c, val & 0x00); /*normal*/
			gc5613_write(sd, 0x0063, val1 & 0x00);
			break;
		case 1:
			gc5613_write(sd, 0x022c, val | 0x01); /*mirror*/
			gc5613_write(sd, 0x0063, val1 | 0x11);
			break;
		case 2:
			gc5613_write(sd, 0x022c, val | 0x02); /*filp*/
			gc5613_write(sd, 0x0063, val1 | 0x22);
			break;
		case 3:
			gc5613_write(sd, 0x022c, val | 0x03); /*mirror & filp*/
			gc5613_write(sd, 0x0063, val1 | 0x33);
			break;
	}
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		memcpy(&gc5613_attr.mipi, &gc5613_mipi, sizeof(gc5613_mipi));
		gc5613_attr.one_line_expr_in_us = 19;
		gc5613_attr.total_width = 2400;
		gc5613_attr.total_height = 1750;
		gc5613_attr.max_integration_time_native = 1750 - 8;
		gc5613_attr.integration_time_limit = 1750 - 8;
		gc5613_attr.max_integration_time = 1750 - 8;
		gc5613_attr.again = 0;
		gc5613_attr.integration_time = 0x10;
		break;
	default:
		...
		struct tx_isp_chip_ident *chip)
{
	...
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"gc5613_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = private_gpio_request(pwdn_gpio,"gc5613_pwdn");
		if(!ret){
...
*/
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = gc5613_set_hvflip(sd, sensor_val->value);
		break;
	default:
		break;
	}
...
static int gc5613_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
MODULE_DESCRIPTION("A low-level driver for gc5613 sensors");
MODULE_LICENSE("GPL");
