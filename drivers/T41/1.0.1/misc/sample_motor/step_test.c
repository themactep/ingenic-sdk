...
{
	unsigned int coordinate[MOTOR_NUMBER];	 //电机当前绝对坐标
	unsigned int target_coord[MOTOR_NUMBER]; //目标坐标
...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		printf("\nMOTOR_STOP:0 MOTOR_STEPS:2 MOTOR_COORDINATE:3 MOTOR_GET_STATUS:4 MOTOR_SET_SPEED:5 MOTOR_RESET:6 Exit:q\n");
		printf("Choose:");
		scanf(" %c", &ch);
		switch (ch)
		{
