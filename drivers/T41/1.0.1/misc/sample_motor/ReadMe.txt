...
2.部分参数说明：
  驱动三个入口参数，默认值如下：
   ...
   maxstep[n] = MOTORn_MAX_STEP; //电机0运动最大步数范围
...
6.建议
  ...
  如果没有两个电机不同速度的需求，建议档位 MOTOR_GEAR和gear[n]都设为1，使用 stMotorSpeed.speed 参数控制速度，这样可以减少定时器中断的频率。
...
8. 使用step_test程序调试 MOTORn_MAX_STEP 参数：
  ...
  3）第2)步得到的旋转方向即是“正方向”，断电将两电机向“负方向”拧到终点，即(0,0)坐标。
  4）先给一个较小的 MOTORn_MAX_STEP 参数，使用 MOTOR_RESET 命令让电机转动，等到电机转到(maxstep[0],maxstep[1])坐标，观察是否转到实际终点坐标。
     若没有转到则增大 MOTORn_MAX_STEP；若提前转到，发生堵转，则减小MOTORn_MAX_STEP。
