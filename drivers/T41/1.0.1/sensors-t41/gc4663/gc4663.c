...
#define SENSOR_VERSION	"H20220806a"
...
unsigned int gc4663_alloc_integration_time(unsigned int it, unsigned char shift, unsigned int *sensor_it)
{
	...
	if(data_type == TX_SENSOR_DATA_TYPE_WDR_DOL){
		if (expo % 2 == 0)
			expo = expo - 1;
		if(expo < gc4663_attr.min_integration_time)
			expo = 3;
	}
	isp_it = expo << shift;
#endif
...
unsigned int gc4663_alloc_integration_time_short(unsigned int it, unsigned char shift, unsigned int *sensor_it)
{
	...
	if (expo % 2 == 0)
		expo = expo - 1;
	if(expo < gc4663_attr.min_integration_time_short)
		expo = 3;
	isp_it = expo << shift;
	expo = (expo - 1) / 2;
	if(expo < 0)
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		gc4663_attr.max_integration_time_short = 4;
		break;
	case 1:
		...
		gc4663_attr.min_integration_time = 5;
                gc4663_attr.min_integration_time_short = 5;
		gc4663_attr.max_again_short = 131072;
	...
	if (((rate / 1000) % 27000) != 0) {
		ret = clk_set_parent(sclka, clk_get(NULL, "vpll"));
		sclka = private_devm_clk_get(&client->dev, "vpll");
		if (IS_ERR(sclka)) {
	...
	sensor->video.min_fps = SENSOR_OUTPUT_MIN_FPS;
...
{
	private_i2c_del_driver(&gc4663_driver);
}
