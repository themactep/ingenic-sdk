...
#define SENSOR_VERSION	"H20220317a"
#define DISCON_CLOCK
...
struct tx_isp_mipi_bus jxf23_mipi_1lane = {
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 216,
	.lans = 1,
	.index = 0,
...
struct tx_isp_mipi_bus jxf23_mipi_1lane1 = {
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 144,
	.lans = 1,
	.index = 0,
...
static struct regval_list jxf23_init_regs_1920_1080_15fps_mipi_1lane[] = {
        {0x0E, 0x11},
        {0x0F, 0x14},
        {0x10, 0x40},
        {0x11, 0x80},
...
static struct regval_list jxf23_init_regs_1928_1088_3fps_mipi_1lane[] = {
        {0x0E, 0x11},
        {0x0F, 0x14},
        {0x10, 0x40},
        {0x11, 0x80},
...
static struct tx_isp_sensor_win_setting jxf23_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= jxf23_init_regs_1920_1080_15fps_mipi_1lane,
	},
	{
		.width		= 1920,
		.height		= 1080,
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= jxf23_init_regs_1928_1088_3fps_mipi_1lane,
	}
};
struct tx_isp_sensor_win_setting *wsize = &jxf23_win_sizes[6];
...
static int jxf23_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	return 0;
}
...
static int jxf23_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
...
static int jxf23_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = 0;

	printk("[ %s:%d ] enable is %d\n", __func__, __LINE__, init->enable);
	if (init->enable) {
		...
		ISP_WARNING("jxf23 stream off0000\n");
		if(data_interface == TX_SENSOR_DATA_INTERFACE_DVP){
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	printk("boot is %d, csi is %d, rest is %d, pwdn is %d\n", info->default_boot, info->video_interface, info->rst_gpio, info->pwdn_gpio);
	...
	case 3:
		...
		break;
        case 5:
		wsize = &jxf23_win_sizes[5];
		jxf23_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		memcpy((void*)(&(jxf23_attr.mipi)),(void*)(&jxf23_mipi_1lane),sizeof(jxf23_mipi_1lane));
                break;
        case 6:
		wsize = &jxf23_win_sizes[6];
		jxf23_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		memcpy((void*)(&(jxf23_attr.mipi)),(void*)(&jxf23_mipi_1lane1),sizeof(jxf23_mipi_1lane1));
                break;
	default:
	...
	case TISP_SENSOR_VI_MIPI_CSI0:
		...
		jxf23_attr.mipi.index = 0;
		break;
	case TISP_SENSOR_VI_MIPI_CSI1:
		jxf23_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		jxf23_attr.mipi.index = 1;
		break;
...
static int jxf23_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if(arg)
			ret = jxf23_set_integration_time(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = jxf23_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if(arg)
			ret = jxf23_set_analog_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = jxf23_set_analog_gain_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if(arg)
			ret = jxf23_set_digital_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if(arg)
			ret = jxf23_get_black_pedestal(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if(arg)
			ret = jxf23_set_mode(sd, sensor_val->value);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if(arg)
			ret = jxf23_set_fps(sd, sensor_val->value);
		break;
	default:
		break;
	}
