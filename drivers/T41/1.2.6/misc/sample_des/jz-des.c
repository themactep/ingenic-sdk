...
static bool dma_chan_tx_filter(struct dma_chan *chan, void *filter_param)
{
    struct des_operation *des = (struct des_operation*)filter_param;
#if (defined(CONFIG_KERNEL_4_4_94) || defined(CONFIG_KERNEL_6_1))
    return des->dma_tx_type == (int)chan->private;
#else
    return des->dma_type == (int)chan->private;
#endif
}

static bool dma_chan_rx_filter(struct dma_chan *chan, void *filter_param)
{
    struct des_operation *des = (struct des_operation*)filter_param;
#if (defined(CONFIG_KERNEL_4_4_94) || defined(CONFIG_KERNEL_6_1))
    return des->dma_rx_type == (int)chan->private;
#else
    return des->dma_type == (int)chan->private;
#endif
}
...
static int des_dma_prepare(struct des_operation *des)
{
    ...
    des->dma_chan_tx = dma_request_channel(mask_tx,dma_chan_tx_filter,(void *)des);
    if(!des->dma_chan_tx)
    {
    ...
    des->dma_chan_rx = dma_request_channel(mask_rx,dma_chan_rx_filter,(void *)des);
    if(!des->dma_chan_rx)
    {
    ...
    des->dma_config_tx.src_maxburst = 8;
    des->dma_config_tx.dst_maxburst = 8;
    ...
    des->dma_config_rx.src_maxburst = 8;
    des->dma_config_rx.dst_maxburst = 8;
...
static int des_dma_operation(struct des_operation *des)
{
    ...
    while(offset < para->datalen)
    {
        ...
        //DMA sync
        dma_sync_single_for_device(des->dev, (dma_addr_t)des->dst_addr_p, len, DMA_FROM_DEVICE);
        dma_sync_single_for_device(des->dev, (dma_addr_t)des->src_addr_p, len, DMA_TO_DEVICE);
...
static int jz_des_probe(struct platform_device *pdev)
{
...
#if (defined(CONFIG_KERNEL_4_4_94) || defined(CONFIG_KERNEL_6_1))
	des_ope->dma_tx_type = INGENIC_DMA_REQ_DES_TX;
	des_ope->dma_rx_type = INGENIC_DMA_REQ_DES_RX;
#else
    ...
    //dma
    des_ope->src_addr_v = dma_alloc_coherent(&pdev->dev,
                                JZ_DES_DMA_DATALEN,
                                &des_ope->src_addr_p,
                                GFP_KERNEL | GFP_DMA);
    ...
    des_ope->dst_addr_v = dma_alloc_coherent(&pdev->dev,
				        JZ_DES_DMA_DATALEN,
				        &des_ope->dst_addr_p,
				        GFP_KERNEL | GFP_DMA);
...
#if (defined(CONFIG_KERNEL_4_4_94) || defined(CONFIG_KERNEL_6_1))
    des_ope->clk = clk_get(des_ope->dev,"gate_des");
    if (IS_ERR(des_ope->clk))
    {
	    dev_err(&pdev->dev, "des clk get failed!\n");
	    ret = -EINVAL;
	    goto failed_clk;
    }
    clk_prepare_enable(des_ope->clk);
#else
...
failed_config_dma_channel:
    dma_free_coherent(&pdev->dev, JZ_DES_DMA_DATALEN,des_ope->dst_addr_v, des_ope->dst_addr_p);
failed_alloc_dst:
    dma_free_coherent(&pdev->dev, JZ_DES_DMA_DATALEN,des_ope->src_addr_v, des_ope->src_addr_p);
failed_alloc_src:
...
static int jz_des_remove(struct platform_device *pdev)
{
    ...
    misc_deregister(&des_ope->des_dev);
#if (defined(CONFIG_KERNEL_4_4_94) || defined(CONFIG_KERNEL_6_1))
    clk_disable_unprepare(des_ope->clk);
    devm_clk_put(&pdev->dev, des_ope->clk);
#else
    clk_disable(des_ope->clk);
    clk_put(des_ope->clk);
#endif
    dma_free_coherent(&pdev->dev, JZ_DES_DMA_DATALEN, des_ope->dst_addr_v, des_ope->dst_addr_p);
    dma_free_coherent(&pdev->dev, JZ_DES_DMA_DATALEN, des_ope->src_addr_v, des_ope->src_addr_p);
...
static void  platform_des_release(struct device *dev)
{
	return ;
}
...
#if (defined(CONFIG_KERNEL_4_4_94) || defined(CONFIG_KERNEL_6_1))
	[1] = {
		.start  = INGENIC_DMA_REQ_DES_TX,
		.end    = INGENIC_DMA_REQ_DES_TX,
		.flags  = IORESOURCE_DMA,
	},
	[2] = {
		.start  = INGENIC_DMA_REQ_DES_RX,
		.end    = INGENIC_DMA_REQ_DES_RX,
		.flags  =  IORESOURCE_DMA,
	},
#else
    [1] = {
