...
int main(int argc, char** argv)
{
	...
	strncpy(lzma_conf.src1_name, "kernel.7z.lzma", 20);
	strncpy(lzma_conf.dst1_name, "decompress_uImage", 20);
	lzma_conf.dst1_size = 32*1024*1024;
	lzma_conf.src1_size = sta.st_size;

	ioctl(fd, IOCTL_LZMA1_START, &lzma_conf);
