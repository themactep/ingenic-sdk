...
#include "host/T41/dmic/as-dmic/as-dmic.h"
...
#define AUDIO_DRIVER_VERSION "H20240805a"
...
static void dsp_workqueue_handle(struct work_struct *work)
{
	...
	if(dump_time_stamp) {
		if(amic_route && amic_route->state == AUDIO_BUSY_STATE) {
			printk("amic: index = %d, fragment->time_stamp.tv_sec=%ld, fragment->time_stamp.tv_usec = %ld\n",amic_route->manage.new_dma_tracer,amic_route->manage.fragments[amic_route->manage.new_dma_tracer].time_stamp.tv_sec,amic_route->manage.fragments[amic_route->manage.new_dma_tracer].time_stamp.tv_usec);
		}
		if(ao_route && ao_route->state == AUDIO_BUSY_STATE) {
			printk("spk: index = %d, fragment->time_stamp.tv_sec=%ld, fragment->time_stamp.tv_usec = %ld\n",ao_route->manage.new_dma_tracer,ao_route->manage.fragments[ao_route->manage.new_dma_tracer].time_stamp.tv_sec,ao_route->manage.fragments[ao_route->manage.new_dma_tracer].time_stamp.tv_usec);
		}
		if(dmic_route && dmic_route->state == AUDIO_BUSY_STATE) {
			printk("dmic: index = %d, fragment->time_stamp.tv_sec=%ld, fragment->time_stamp.tv_usec = %ld\n",dmic_route->manage.new_dma_tracer,dmic_route->manage.fragments[dmic_route->manage.new_dma_tracer].time_stamp.tv_sec,dmic_route->manage.fragments[dmic_route->manage.new_dma_tracer].time_stamp.tv_usec);
		}
	}
#if 1
...
#ifdef CONFIG_KERNEL_6_1
		copy_to_user((__user void*)(stream.timestamp), &fragment->time_stamp,sizeof(struct __kernel_old_timeval));
#else
		copy_to_user((__user void*)(stream.timestamp), &fragment->time_stamp,sizeof(struct timeval));
#endif
		fragment->priv = NULL;
		i++;
...
static long dsp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case AMIC_AI_SET_CODEC_TYPE:
			copy_from_user(&codec, (__user void*)arg, sizeof(codec));
		...
		case DMIC_AI_GET_VOLUME:
			...
			break;
		default:
			audio_warn_print("SOUDND ERROR:ioctl command %x is not supported\n", cmd);
			ret = -1;
			goto EXIT_IOCTRL;
	}
EXIT_IOCTRL:
