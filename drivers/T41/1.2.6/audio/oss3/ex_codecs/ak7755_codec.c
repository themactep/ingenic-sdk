...
static int ak7755_spk_gpio_request(void)
{
	if(spk_gpio != -1) {
...
static int ak7755_spk_gpio_free(void)
{
	if (spk_gpio != -1) {
...
static int ak7755_record_set_again(int channel,int again)
{
	/*******************************************
...
static int ak7755_record_set_dgain(int channel,int dgain)
{
	/*******************************************************
	...
	if (channel == MONO_LEFT) {
		/* left channel */
		ak7755_reg_set(AK7755_D5_ADC_DVOLUME_SETTING1, 0, 7, dgain);
	} else if (channel == MONO_RIGHT) {
		/* right channel */
		ak7755_reg_set(AK7755_D6_ADC_DVOLUME_SETTING2, 0, 7, dgain);
	}
	return AUDIO_SUCCESS;
}
...
static int ak7755_playback_set_again(int channel ,int again)
{
	/*************************************************************
	...
	if (again < 0) again = 0;
	if (again > 0xf) again = 0xf;
	ak7755_reg_update_bits(AK7755_D4_LO1_LO2_VOLUME_SETTING, 0x0F,again);
	return AUDIO_SUCCESS;
}

static int ak7755_playback_set_dgain(int channel, int dgain)
{
	 /******************************************************
	...
	ak7755_reg_update_bits(AK7755_D8_DAC_DVOLUME_SETTING1, 0xFF,dgain);
	return AUDIO_SUCCESS;
}
...
static struct codec_attributes ak7755_attrs = {
	...
	.spk_gpio_request = ak7755_spk_gpio_request,
	.spk_gpio_free = ak7755_spk_gpio_free,
};
