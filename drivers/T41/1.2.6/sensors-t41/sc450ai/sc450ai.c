 ...
 *   1          2688*1512       20      mipi_2lane           linear
...
#define SC450AI_CHIP_ID_H	(0xbd)
#define SC450AI_CHIP_ID_L	(0x2f)
#define SC450AI_REG_END		0xffff
#define SC450AI_REG_DELAY	0xfffe
#define SENSOR_OUTPUT_MIN_FPS 5
#define SENSOR_VERSION	"H20241127a"
...
unsigned int sc450ai_alloc_integration_time(unsigned int it, unsigned char shift, unsigned int *sensor_it)
{
...
unsigned int sc450ai_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while(lut->gain <= sc450ai_attr.max_again) {
		if(isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == sc450ai_attr.max_again) && (isp_gain >= lut->gain)) {
				*sensor_again = lut->value;
				return lut->gain;
			}
		}
...
struct tx_isp_mipi_bus sc450ai_mipi_linear={
...
struct tx_isp_mipi_bus sc450ai_mipi={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 720,
	.lans = 2,
	.index = 0,
...
struct tx_isp_mipi_bus sc450ai_mipi_1440P_linear={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 1080,
	.lans = 2,
	.index = 0,
...
static struct regval_list sc450ai_init_regs_2592_1520_25fps_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x36f9,0x80},
	...
	{SC450AI_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list sc450ai_init_regs_2688_1512_20fps_mipi[] = {
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x36f9,0x80},
	...
	{0x3209,0x80},
	{0x320a,0x05},
	{0x320b,0xf0},//hts = 0x2ee = 750
	{0x320c,0x02},//
	{0x320d,0xee},//hts = 0x618 = 1560
	{0x320e,0x06},//
	{0x320f,0x18},
	{0x3210,0x00},
	{0x3211,0x08},
...
static struct regval_list sc450ai_init_regs_2560_1440_30fps_mipi[] = {
	...
	{0x3209,0x00},
	{0x320a,0x05},
	{0x320b,0xa0},//hts = 0x480 = 1152
	{0x320c,0x04},//
	{0x320d,0x80},//vts = 0x9c4 = 2500
	{0x320e,0x09},//
	{0x320f,0xc4},
	{0x3210,0x00},
	{0x3211,0x08},
...
static struct tx_isp_sensor_win_setting sc450ai_win_sizes[] = {
	{
		...
		.regs 		= sc450ai_init_regs_2592_1520_25fps_mipi,
	},
	{
		.width		= 2688,
		.height		= 1512,
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= sc450ai_init_regs_2688_1512_20fps_mipi,
	},
	{
		.width		= 2560,
		.height		= 1440,
...
struct tx_isp_sensor_win_setting *wsize = &sc450ai_win_sizes[0];

/*
 * the part of driver was fixed.
 */

static struct regval_list sc450ai_stream_on[] = {
	{SC450AI_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list sc450ai_stream_off[] = {
	{SC450AI_REG_END, 0x00},	/* END MARKER */
};
...
static int sc450ai_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
	int ret;
	unsigned char val;
	while (vals->reg_num != SC450AI_REG_END) {
		if (vals->reg_num == SC450AI_REG_DELAY) {
...
static int sc450ai_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	it <<= 1;
	ret = sc450ai_write(sd,  0x3e00, (unsigned char)((it >> 12) & 0x0f));
	ret += sc450ai_write(sd, 0x3e01, (unsigned char)((it >> 4) & 0xff));
	ret += sc450ai_write(sd, 0x3e02, (unsigned char)((it & 0x0f) << 4));
...
static int sc450ai_set_integration_time(struct tx_isp_subdev *sd, int value)
{
	...
	ret = sc450ai_write(sd,  0x3e00, (unsigned char)((value >> 12) & 0x0f));
	ret += sc450ai_write(sd, 0x3e01, (unsigned char)((value >> 4) & 0xff));
	ret += sc450ai_write(sd, 0x3e02, (unsigned char)((value & 0x0f) << 4));
...
static int sc450ai_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	...
	if (init->enable) {
		...
		if (sensor->video.state == TX_ISP_MODULE_INIT) {
			ret = sc450ai_write_array(sd, sc450ai_stream_on);
			sensor->video.state = TX_ISP_MODULE_RUNNING;
			pr_debug("sc450ai stream on\n");
			sensor->video.state = TX_ISP_MODULE_RUNNING;
		}
	} else {
		ret = sc450ai_write_array(sd, sc450ai_stream_off);
...
static int sc450ai_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
	uint8_t val;
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	...
	val = sc450ai_read(sd, 0x3221, &val);
	switch(enable) {
	case 0:
		val &= 0x99;
		break;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		memcpy(&sc450ai_attr.mipi, &sc450ai_mipi_linear, sizeof(sc450ai_mipi_linear));
		sc450ai_attr.min_integration_time = 1;
		sc450ai_attr.min_integration_time_native = 1,
		sc450ai_attr.total_width = 750;
		sc450ai_attr.total_height = 1878;
		sc450ai_attr.max_integration_time_native = 1874;   /* (1878*2-8)/2=1874 */
		sc450ai_attr.integration_time_limit = 1874;
		sc450ai_attr.max_integration_time = 1874;
	...
	case 1:
		...
		memcpy(&sc450ai_attr.mipi, &sc450ai_mipi, sizeof(sc450ai_mipi));
		sc450ai_attr.min_integration_time = 1;
		sc450ai_attr.min_integration_time_native = 1,
		sc450ai_attr.total_width = 750;
		sc450ai_attr.total_height = 1560;
		sc450ai_attr.max_integration_time_native = 1556;    /* (1560*2-8)/2 = 1556 */
		sc450ai_attr.integration_time_limit = 1556;
		sc450ai_attr.max_integration_time = 1556;
		sc450ai_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sc450ai_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		break;
	case 2:
		wsize = &sc450ai_win_sizes[2];
		memcpy(&sc450ai_attr.mipi, &sc450ai_mipi_1440P_linear, sizeof(sc450ai_mipi_1440P_linear));
		sc450ai_attr.min_integration_time = 1;
		sc450ai_attr.min_integration_time_native = 1,
		sc450ai_attr.total_width = 1152;
		sc450ai_attr.total_height = 2500;
		sc450ai_attr.max_integration_time_native = 2496;   /* (2500*2-8)/2 = 2496 */
		sc450ai_attr.integration_time_limit = 2496;
		sc450ai_attr.max_integration_time = 2496;
		sc450ai_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sc450ai_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		break;
	default:
...
static int sc450ai_g_chip_ident(struct tx_isp_subdev *sd,struct tx_isp_chip_ident *chip)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
	...
	if(reset_gpio != -1){
		ret = private_gpio_request(reset_gpio,"sc450ai_reset");
		if(!ret){
	...
	if(pwdn_gpio != -1){
		ret = private_gpio_request(pwdn_gpio,"sc450ai_pwdn");
		if(!ret){
...
static int sc450ai_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_VFLIP:
		if(arg)
			ret = sc450ai_set_vflip(sd, sensor_val->value);
		break;
	default:
		break;
	}
...
static int sc450ai_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
MODULE_DESCRIPTION("A low-level driver for sc450ai sensors");
MODULE_LICENSE("GPL");
