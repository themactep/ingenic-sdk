 ...
 *   2          1920*1080       30        mipi_2lane           hdr
 *   3          1920*1080       20        mipi_2lane           hdr
 *   4          1280*720        20        mipi_2lane           linear
 */
...
struct tx_isp_mipi_bus sc233a_mipi_dol_30fps = {
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 810,
	.lans = 2,
	.settle_time_apative_en = 0,
...
struct tx_isp_mipi_bus sc233a_mipi_dol_20fps = {
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 810,
	.lans = 2,
	.settle_time_apative_en = 0,
...
struct tx_isp_mipi_bus sc233a_mipi_720p={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 405,
	.lans = 2,
	.settle_time_apative_en = 0,
...
static struct regval_list sc233a_init_regs_1920_1080_30fps_mipi_dol[] = {
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
	{0x301f,0x90},
...
static struct regval_list sc233a_init_regs_1920_1080_20fps_mipi_dol[] = {
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
	{0x301f,0x90},
...
static struct regval_list sc233a_init_regs_1280_720_20fps_mipi[] = {
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
	{0x301f,0x85},
...
static struct tx_isp_sensor_win_setting sc233a_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs		= sc233a_init_regs_1920_1080_30fps_mipi_dol,
	},
	{
		.width		= 1920,
		.height		= 1080,
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs		= sc233a_init_regs_1920_1080_20fps_mipi_dol,
	},
	{
		.width		= 1280,
		.height		= 720,
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs		= sc233a_init_regs_1280_720_20fps_mipi,
	},
...
static int sc233a_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
       int ret = 0;
       ret += sc233a_write(sd, 0x3e11, (unsigned char)(value & 0xff));
       ret += sc233a_write(sd, 0x3e13, (unsigned char)((value & 0xff00) >> 8));
       if (ret < 0)
               return ret;
...
static int sc233a_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	case 1:
		...
		break;
	case 2:
		sclk = 0x9c4 * 0x870 * 30 * 2;
		sensor_max_fps = 30;
		break;
	case 3:
		sclk = 0xea6 * 0x870 * 20 * 2;
		sensor_max_fps = 20;
		break;
	case 4:
		sclk = 0x753 * 0x870 * 20 * 2;
		sensor_max_fps = 20;
		break;
	default:
	...
	sensor->video.attr->max_integration_time_native = (sensor->info.default_boot == 1||sensor->info.default_boot == 4) ? (2*vts -10) : (2*vts - short_time - 18);
	sensor->video.attr->integration_time_limit = (sensor->info.default_boot == 1||sensor->info.default_boot == 4) ? (2*vts -10) : (2*vts - short_time - 18);
	sensor->video.attr->total_height = vts;
	sensor->video.attr->max_integration_time = (sensor->info.default_boot == 1||sensor->info.default_boot == 4) ? (2*vts -10) : (2*vts - short_time - 18);
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 1:
		...
		sc233a_attr.min_integration_time = 5;
		sc233a_attr.min_integration_time_short = 5;
    ...
    case 2:
		sc233a_attr.wdr_cache = wdr_bufsize;
		wsize = &sc233a_win_sizes[2];
		memcpy(&sc233a_attr.mipi, &sc233a_mipi_dol_30fps, sizeof(sc233a_mipi_dol_30fps));
		sc233a_attr.mipi.clk = 810,
		sc233a_attr.min_integration_time = 5;
		sc233a_attr.min_integration_time_short = 5;
		sc233a_attr.total_width = 0x870;
		sc233a_attr.total_height = 0x9c4;
		sc233a_attr.max_integration_time_native = 0x9c4-150-7; /* 2400-146-7 */
		sc233a_attr.integration_time_limit = 0x9c4-150-7;
		sc233a_attr.max_integration_time = 0x9c4-150-7;
		sc233a_attr.max_integration_time_short = 150; /* 146-6 */
		sc233a_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		printk("=================> 30fps_hdr is ok");
		break;
    case 3:
		sc233a_attr.wdr_cache = wdr_bufsize;
		wsize = &sc233a_win_sizes[3];
		memcpy(&sc233a_attr.mipi, &sc233a_mipi_dol_20fps, sizeof(sc233a_mipi_dol_20fps));
		sc233a_attr.mipi.clk = 810,
		sc233a_attr.min_integration_time = 5;
		sc233a_attr.min_integration_time_short = 5;
		sc233a_attr.total_width = 0x870;
		sc233a_attr.total_height = 0xea6;
		sc233a_attr.max_integration_time_native = 0xea6-150-7; /* 2400-146-7 */
		sc233a_attr.integration_time_limit = 0xea6-150-7;
		sc233a_attr.max_integration_time = 0xea6-150-7;
		sc233a_attr.max_integration_time_short = 150; /* 146-6 */
		sc233a_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		printk("=================> 30fps_hdr is ok");
		break;
	case 4:
		wsize = &sc233a_win_sizes[4];
		memcpy(&sc233a_attr.mipi, &sc233a_mipi_720p, sizeof(sc233a_mipi_720p));
		sc233a_attr.min_integration_time = 3;
		sc233a_attr.mipi.clk = 405,
		sc233a_attr.total_width = 0x870;
		sc233a_attr.total_height = 0x753;
		sc233a_attr.max_integration_time_native = 0x753*2-10;   /* 1440*2-10 */
		sc233a_attr.integration_time_limit = 0x753*2-10;
		sc233a_attr.max_integration_time = 0x753*2-10;
		sc233a_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sc233a_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		printk("=================> 720p linear is ok");
		break;
        	default:
...
static int sc233a_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
	...
	if(wdr_en == 1  && info->default_boot == 1){
		info->default_boot=1;
		memcpy(&sc233a_attr.mipi, &sc233a_mipi_dol, sizeof(sc233a_mipi_dol));
		sc233a_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		sc233a_attr.wdr_cache = wdr_bufsize;
		...
		printk("\n-------------------------switch wdr@25fps ok ----------------------\n");
	}else if(wdr_en == 1&& info->default_boot == 2 ){
		info->default_boot=2;
		memcpy(&sc233a_attr.mipi, &sc233a_mipi_dol_30fps, sizeof(sc233a_mipi_dol_30fps));
		sc233a_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		sc233a_attr.wdr_cache = wdr_bufsize;
		...
		sc233a_attr.max_integration_time = 0x9c4-150-7;
		sc233a_attr.again = 0;
		sc233a_attr.max_integration_time_short = 150;
		printk("\n-------------------------switch wdr@30fps ok ----------------------\n");
	}else if (wdr_en == 1 && info->default_boot == 3){
		info->default_boot=3;
		memcpy(&sc233a_attr.mipi, &sc233a_mipi_dol_30fps, sizeof(sc233a_mipi_dol_30fps));
		sc233a_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
		sc233a_attr.wdr_cache = wdr_bufsize;
...
static int sc233a_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = sc233a_set_analog_gain_short(sd, sensor_val->value);
		break;
