 ...
 *   2          1920*1056       60        mipi_2lane           linear    AISP
 *   3          1928*1088       60        mipi_2lane           linear    AISP
 */
...
#define SENSOR_VERSION	"H20240313a"
...
struct tx_isp_mipi_bus sc200ai_mipi_1928_1088_linear={
	.mode = SENSOR_MIPI_OTHER_MODE,
	.clk = 792,
	.lans = 2,
	.index = 0,
...
static struct regval_list sc200ai_init_regs_1928_1088_60fps_mipi[] = {
        {0x0103,0x01},
        {0x0100,0x00},
        {0x36e9,0x80},
        {0x36f9,0x80},
...
static struct tx_isp_sensor_win_setting sc200ai_win_sizes[] = {
	...
	},
	{
		...
		.mbus_code	= TISP_VI_FMT_SBGGR10_1X10,
		.colorspace	= TISP_COLORSPACE_SRGB,
		.regs 		= sc200ai_init_regs_1928_1088_60fps_mipi,
	},
};
...
static int sc200ai_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	if (data_type == TX_SENSOR_DATA_TYPE_LINEAR) it = (it << 1) - 1;
	else if (data_type == TX_SENSOR_DATA_TYPE_WDR_DOL) it = (it << 2) + 1;
...
static int sc200ai_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	sensor->video.attr->max_integration_time_native = (sensor->info.default_boot == 1) ? (vts/2 - short_time/2 - 4) : (vts - 4);
	sensor->video.attr->integration_time_limit = (sensor->info.default_boot == 1) ? (vts/2 - short_time/2 - 4) : (vts - 4);
	sensor->video.attr->total_height = vts;
	sensor->video.attr->max_integration_time = (sensor->info.default_boot == 1) ? (vts/2 - short_time/2 - 4) : (vts - 4);
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
	...
	case 0:
		...
		sc200ai_attr.max_integration_time_native = 1196;   /* 1200-4 */
		sc200ai_attr.integration_time_limit = 1196;
		sc200ai_attr.max_integration_time = 1196;
		sc200ai_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sc200ai_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		printk("=================> 1920*1080 linear is ok");
		break;
	case 1:
	...
	case 2:
		...
		sc200ai_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
		sc200ai_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		printk("=================> 1920*1056 linear is ok");
		break;
	case 3:
		wsize = &sc200ai_win_sizes[3];
		memcpy(&sc200ai_attr.mipi, &sc200ai_mipi_1928_1088_linear, sizeof(sc200ai_mipi_1928_1088_linear));
		sc200ai_attr.mipi.clk = 792,
		sc200ai_attr.min_integration_time = 1;
		sc200ai_attr.min_integration_time_native = 1,
		sc200ai_attr.total_width = 2200;
...
static int sc200ai_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
	...
	if(wdr_en == 1){
		...
		sc200ai_attr.total_width = 2200;
		sc200ai_attr.total_height = 1200;
		sc200ai_attr.max_integration_time_native = 1196;
		sc200ai_attr.integration_time_limit = 1196;
		sc200ai_attr.max_integration_time = 1196;
		printk("\n-------------------------switch linear ok ----------------------\n");
	}else{
