 ...
 *   2          2560*1440       30        mipi_2lane           linear    AISP
 */
#define DEBUG
...
#define SENSOR_VERSION  "H20240314a"
 ...
 */
struct tx_isp_sensor_attribute imx664_attr;

unsigned int imx664_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
        uint16_t again=0;
...
unsigned int imx664_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
        uint16_t again=0;
        uint32_t hcg = 166528;//5.82x
        uint32_t hcg_thr = 196608;//20x 196608;//8x
...
unsigned int imx664_alloc_dgain(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_dgain)
{
        return 0;
}

struct tx_isp_mipi_bus imx664_mipi_linear = {
...
struct tx_isp_mipi_bus imx664_mipi_dol = {
        .mode = SENSOR_MIPI_SONY_MODE,
        .clk = 1782,
        .lans = 2,
        .settle_time_apative_en = 0,
...
struct tx_isp_mipi_bus imx664_mipi_1440P_linear = {
        .mode = SENSOR_MIPI_SONY_MODE,
        .clk = 1188,
        .lans = 2,
        .settle_time_apative_en = 0,
...
struct tx_isp_sensor_attribute imx664_attr={
        ...
        .again_apply_delay = 2,
        .dgain_apply_delay = 0,
        .sensor_ctrl.alloc_again = imx664_alloc_again,
        .sensor_ctrl.alloc_again_short = imx664_alloc_again_short,
        .sensor_ctrl.alloc_dgain = imx664_alloc_dgain,
        .wdr_cache = 0,
};

static struct regval_list imx664_init_regs_2688_1520_30fps_mipi[] = {
	{0x3000,0x01},  // STANDBY
	{0x3001,0x00},  // REGHOLD
	{0x3002,0x01},  // XMSTA
...
static struct regval_list imx664_init_regs_2560_1440_30fps_mipi[] = {
        {0x3000,0x01}, //STANDBY
        {0x3001,0x00}, //REGHOLD
        {0x3002,0x01}, //XMSTA
        {0x3014,0x01}, //INCK_SEL[3:0]
...
/*
 * the order of the imx664_win_sizes is [full_resolution, preview_resolution].
 */
static struct tx_isp_sensor_win_setting imx664_win_sizes[] = {
        ...
        },
        {
                ...
                .mbus_code      = TISP_VI_FMT_SRGGB12_1X12,
                .colorspace     = TISP_COLORSPACE_SRGB,
                .regs           = imx664_init_regs_2560_1440_30fps_mipi,
        },
};

static struct tx_isp_sensor_win_setting *wsize = &imx664_win_sizes[0];
...
int imx664_write(struct tx_isp_subdev *sd, uint16_t reg,
                 unsigned char value)
{
        struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
#if 0
static int imx664_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
        int ret;
        unsigned char val;
...
static int imx664_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
        return 0;
}

static int imx664_detect(struct tx_isp_subdev *sd, unsigned int *ident)
{
#if 1
       unsigned char v;
       int ret;

       ret = imx664_write(sd, 0x3000, 0x00);
       private_msleep(40);
       ret = imx664_read(sd, 0x4D1D, &v);
...
static int imx664_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;
        ...
        shs1 = rhs1 - (value << 2);
        ret = imx664_write(sd, 0x3054, (unsigned char)(shs1 & 0xff));
        ret += imx664_write(sd, 0x3055, (unsigned char)((shs1 >> 8) & 0xff));
        ret += imx664_write(sd, 0x3056, (unsigned char)((shs1 >> 16) & 0x3));

        return 0;
}

static int imx664_set_integration_time(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;
        ...
        {
        	vmax = imx664_attr.total_height;
        	shs = vmax - value;
        	ret = imx664_write(sd, 0x3050, (unsigned char)(shs & 0xff));
        	ret += imx664_write(sd, 0x3051, (unsigned char)((shs >> 8) & 0xff));
        	ret += imx664_write(sd, 0x3052, (unsigned char)((shs >> 16) & 0x0f));
...
static int imx664_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;
...
static int imx664_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
        int ret = 0;
...
static int imx664_set_digital_gain(struct tx_isp_subdev *sd, int value)
{
        return 0;
}

static int imx664_get_black_pedestal(struct tx_isp_subdev *sd, int value)
{
        return 0;
}

static int imx664_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
...
static int imx664_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
        struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
        int ret = 0;
        ...
        if (init->enable) {
                ...
                sensor->video.state = TX_ISP_MODULE_INIT;
                pr_debug("imx664 stream off\n");
        }

        return ret;
}

static int imx664_set_fps(struct tx_isp_subdev *sd, int fps)
{
        struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
        unsigned int sclk = 0;
        unsigned int aclk = 0;
        ...
        ret += imx664_read(sd, 0x302D, &val);
        hts = val;
        ret += imx664_read(sd, 0x302C, &val);
        hts = (hts << 8) + val;
        if (0 != ret) {
                ISP_ERROR("err: imx664 read err\n");
                return -1;
        }

        vts = sclk * (fps & 0xffff) / hts / ((fps & 0xffff0000) >> 16);
        ret += imx664_write(sd, 0x3001, 0x01);
        ret = imx664_write(sd, 0x302A, (unsigned char)((vts & 0xf0000) >> 16));
        ret += imx664_write(sd, 0x3029, (unsigned char)((vts & 0xff00) >> 8));
        ret += imx664_write(sd, 0x3028, (unsigned char)(vts & 0xff));
        ret += imx664_write(sd, 0x3001, 0x00);
        if (0 != ret) {
                ISP_ERROR("err: imx664_write err\n");
                return ret;
        }
        ...
        if((sensor->info.default_boot) != 1){
            ...
            ret += imx664_write(sd, 0x3060, (rhs1 & 0xff));
...
static int imx664_set_vflip(struct tx_isp_subdev *sd, int enable)
{
	int ret = 0;
	uint8_t h_val;
	uint8_t v_val;

	ret = imx664_read(sd, 0x3020, &h_val);
	ret = imx664_read(sd, 0x3021, &v_val);
	switch(enable) {
	case 0:
		h_val &= 0xFE;
		v_val &= 0xFE;
	...
	ret = imx664_write(sd, 0x3021, v_val);

	return ret;
}

static int imx664_set_wdr_stop(struct tx_isp_subdev *sd, int wdr_en)
{
        ...
        if(wdr_en == 1){
                ...
                imx664_attr.wdr_cache = wdr_bufsize;
                imx664_attr.max_dgain = 0;
                imx664_attr.min_integration_time = 1;
                imx664_attr.min_integration_time_native = 1;
                imx664_attr.max_integration_time_native = 1157;
                imx664_attr.min_integration_time_short = 1;
                imx664_attr.max_integration_time_short = 75;
                imx664_attr.integration_time_limit = 1157;
                imx664_attr.total_width = 750;
                imx664_attr.total_height = 2474;
                imx664_attr.max_integration_time = 1157;
                imx664_attr.integration_time_apply_delay = 2;
                imx664_attr.again_apply_delay = 2;
                imx664_attr.dgain_apply_delay = 0;
                imx664_attr.data_type = TX_SENSOR_DATA_TYPE_WDR_DOL;
                ...
                imx664_attr.wdr_cache = wdr_bufsize;
                imx664_attr.max_dgain = 0;
                imx664_attr.min_integration_time = 2;
                imx664_attr.min_integration_time_native = 2;
                imx664_attr.max_integration_time_native = 1650 - 8;
                imx664_attr.integration_time_limit = 1650 - 8;
                imx664_attr.total_width = 1500;
                imx664_attr.total_height = 1650;
                imx664_attr.max_integration_time = 1650 - 8;
                imx664_attr.integration_time_apply_delay = 2;
                imx664_attr.again_apply_delay = 2;
                imx664_attr.dgain_apply_delay = 0;
                imx664_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;
                imx664_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
                sensor->video.attr = &imx664_attr;
                info->default_boot = 0;
                ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SYNC_SENSOR_ATTR, &sensor->video);
...
static int imx664_set_wdr(struct tx_isp_subdev *sd, int wdr_en)
{
        int ret = 0;
...
static int imx664_set_mode(struct tx_isp_subdev *sd, int value)
{
        struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
        int ret = ISP_SUCCESS;
...
static int sensor_attr_check(struct tx_isp_subdev *sd)
{
        ...
        case 2:
                ...
                imx664_attr.again_apply_delay = 2;
                imx664_attr.dgain_apply_delay = 0;
                memcpy((void*)(&(imx664_attr.mipi)),(void*)(&imx664_mipi_1440P_linear),sizeof(imx664_mipi_1440P_linear));
                break;
        default:
...
static int imx664_g_chip_ident(struct tx_isp_subdev *sd,
                               struct tx_isp_chip_ident *chip)
{
        ...
        ret = imx664_detect(sd, &ident);
        if (ret) {
                ISP_ERROR("chip found @ 0x%x (%s) is not an imx664 chip.\n",
...
static int imx664_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
        long ret = 0;
...
static int imx664_g_register(struct tx_isp_subdev *sd, struct tx_isp_dbg_register *reg)
{
        unsigned char val = 0;
        int len = 0;
        int ret = 0;
        ...
        if (!private_capable(CAP_SYS_ADMIN))
                return -EPERM;
        ret = imx664_read(sd, reg->reg & 0xffff, &val);
        reg->val = val;
        reg->size = 2;
...
static int imx664_s_register(struct tx_isp_subdev *sd, const struct tx_isp_dbg_register *reg)
{
        int len = 0;
        ...
        if (!private_capable(CAP_SYS_ADMIN))
                return -EPERM;
        imx664_write(sd, reg->reg & 0xffff, reg->val & 0xff);

        return 0;
}

static struct tx_isp_subdev_core_ops imx664_core_ops = {
        .g_chip_ident = imx664_g_chip_ident,
        .reset = imx664_reset,
        .init = imx664_init,
        .g_register = imx664_g_register,
        .s_register = imx664_s_register,
};

static struct tx_isp_subdev_video_ops imx664_video_ops = {
        .s_stream = imx664_s_stream,
};
                        ...
                        const struct i2c_device_id *id)
{
        ...
        tx_isp_subdev_init(&sensor_platform_device, sd, &imx664_ops);
        tx_isp_set_subdevdata(sd, client);
        tx_isp_set_subdev_hostdata(sd, sensor);
        private_i2c_set_clientdata(client, sd);

        pr_debug("probe ok ------->imx664\n");
...
static const struct i2c_device_id imx664_id[] = {
        { "imx664", 0 },
        { }
};
MODULE_DEVICE_TABLE(i2c, imx664_id);
