...
#define AUDIO_DRIVER_VERSION "H20210526a"
		...
		if(route && route->state == AUDIO_BUSY_STATE){
			if(route->index != AUDIO_ROUTE_DMIC_ID){
				...
				if(route->index == AUDIO_ROUTE_AMIC_ID){
					getrawmonotonic(&ts);
					route->manage.fragments[index].time_stamp = ts;
					if(index ==  0){
						route->manage.fragments[route->manage.fragment_cnt-1].time_stamp = ts;
					}else{
						route->manage.fragments[index-1].time_stamp = ts;
					}
...
//		printk("dsp_get_mic_stream: fragment->time_stamp.tv_sec=%ld,fragment->time_stamp.tv_usec = %ld\n",fragment->time_stamp.tv_sec,fragment->time_stamp.tv_usec);
		copy_to_user((__user void*)(stream.timestamp), &fragment->time_stamp,sizeof(struct timespec));
		fragment->priv = NULL;
		i++;
