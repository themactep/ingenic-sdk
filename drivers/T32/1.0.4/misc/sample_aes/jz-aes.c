...
#define AES_DRIVER_VERSION "H20240801a"
...
static int aes_start_encrypt_processing(struct aes_operation *aes)
{
	...
	while(offset < para->datalen){
		...
		dma_sync_single_for_device(aes->dev, (dma_addr_t)aes->dst_addr_p, len, DMA_FROM_DEVICE);
		dma_sync_single_for_device(aes->dev, (dma_addr_t)aes->src_addr_p, len, DMA_TO_DEVICE);
...
static int jz_aes_init(struct file *file, struct aes_operation *aes)
{
	...
	if((para->enworkmode == IN_UNF_CIPHER_WORK_MODE_ENC_ECB ) || (para->enworkmode == IN_UNF_CIPHER_WORK_MODE_DEC_ECB)){
		if(para->enworkmode == IN_UNF_CIPHER_WORK_MODE_ENC_ECB){
			...
			aes_reg_write(aes, AES_ASCR, 3 << 28 | 1<<0 | 1<<8 | 2<<6);
		}else{
			/* Enable AES DMA, Decrypts, ECB mode */
			aes_reg_write(aes, AES_ASCR, 3 << 28 | 1<<0 | 1<<4 | 1<<8 | 2 << 6);
		}
	}else{
		if(para->enworkmode == IN_UNF_CIPHER_WORK_MODE_ENC_CBC){
			/* Enable AES DMA, Encrypts, CBC mode */
			aes_reg_write(aes, AES_ASCR, 3 << 28 | 1<<8 | 1<<5 | 1<<0 | 2 << 6);
		}else{
			/* Enable AES DMA, Decrypts, CBC mode */
			aes_reg_write(aes, AES_ASCR, 3 << 28 | 1<<8 | 1<<5 | 1<<4 | 1<<0 | 2 << 6);
		}
		for(i = 0; i < 4; i++)
...
{
	int ret = 0;
	if((aes->para.enworkmode == IN_UNF_CIPHER_WORK_MODE_ENC_ECB) || (aes->para.enworkmode == IN_UNF_CIPHER_WORK_MODE_ENC_CBC)){
