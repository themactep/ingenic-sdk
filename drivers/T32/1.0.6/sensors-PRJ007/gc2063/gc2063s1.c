...
unsigned int gc2063s1_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	while (lut->gain <= gc2063s1_attr.again.max) {
		if (isp_gain == 0) {
			...
			*sensor_again = (lut - 1)->index;
			...
			if ((lut->gain == gc2063s1_attr.again.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_WDR_2_FRAME
unsigned int gc2063s1_alloc_again_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
#ifndef SENSOR_TEST
#ifdef SENSOR_AGAIN_TABLE
	/* Analog gain table */
	struct again_lut *lut = gc2063s1_again_lut;
	while(lut->gain <= gc2063s1_attr.again_short.max) {
		if(isp_gain == 0) {
			*sensor_again = 0;
			return 0;
		}
			...
			*sensor_again = (lut - 1)->value;
			...
			if((lut->gain == gc2063s1_attr.again_short.max) && (isp_gain >= lut->gain)) {
...
#ifdef SENSOR_HCG
unsigned int gc2063s1_alloc_hcg(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
unsigned int gc2063s1_alloc_hcg_short(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_hcg)
{
...
struct tx_isp_mipi_bus gc2063s1_mipi_linear = {
...
#ifdef SENSOR_WDR_2_FRAME
	.sensor_ctrl.alloc_again_short = gc2063s1_alloc_again_short,
#endif /* SENSOR_WDR_2_FRAME */
...
/*
 * the order of the gc2063s1_win_sizes is [full_resolution, preview_resolution].
 */
...
static struct tx_isp_sensor_win_setting *wsize = &gc2063s1_win_sizes[0];
...
#ifdef SENSOR_I2C_REG_8BIT
int gc2063s1_read(struct tx_isp_subdev *sd, unsigned char reg,
		unsigned char *value)
{
...
int gc2063s1_write(struct tx_isp_subdev *sd, unsigned char reg,
		 unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int gc2063s1_read(struct tx_isp_subdev *sd, uint16_t reg,
		unsigned char *value) {
	int ret;
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
int gc2063s1_write(struct tx_isp_subdev *sd, uint16_t reg,
		 unsigned char value)
{
	struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static int gc2063s1_clk_set(struct tx_isp_subdev *sd, struct clk *sclka, int mclk)
{
...
static int gc2063s1_attr_set(struct tx_isp_subdev *sd, struct tx_isp_sensor_win_setting *wise)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int gc2063s1_setting_select(struct tx_isp_subdev *sd, int deboot)
{
	int ret = ISP_SUCCESS;
	...
	case 0:
		...
		gc2063s1_attr.data_type = TX_SENSOR_DATA_TYPE_LINEAR;

		gc2063s1_attr.again.value = 65536;
		gc2063s1_attr.again.max = 444864;
		gc2063s1_attr.again.min = 0;
		gc2063s1_attr.again.apply_delay = 2;

		gc2063s1_attr.integration_time.value = 0x7b8;
		gc2063s1_attr.integration_time.max = 1181 - 8;
		gc2063s1_attr.integration_time.min = 1;
		gc2063s1_attr.integration_time.apply_delay = 2;

		gc2063s1_attr.total_width = 1100;
		gc2063s1_attr.total_height = 1181;

		gc2063s1_attr.expo_fs = TX_SENSOR_EXPO_FRAME_START;

#ifdef SENSOR_HCG
		gc2063s1_attr.hcg.base_gain = ;
		gc2063s1_attr.hcg.apply_delay = ;
#endif /* SENSOR_HCG */

#ifdef SENSOR_WDR_2_FRAME
		gc2063s1_attr.again_short.value = ;
		gc2063s1_attr.again_short.max = ;
		gc2063s1_attr.again_short.min = ;
		gc2063s1_attr.again_short.apply_delay = ;

		gc2063s1_attr.integration_time_short.value = ;
		gc2063s1_attr.integration_time_short.max = ;
		gc2063s1_attr.integration_time_short.min = ;
		gc2063s1_attr.integration_time_short.apply_delay = ;

		gc2063s1_attr.wdr_cache = wdr_line * gc2063s1_attr.total_width;

#ifdef SENSOR_HCG
		gc2063s1_attr.hcg_short.base_gain = ;
		gc2063s1_attr.hcg_short.apply_delay = ;
#endif /* SENSOR_HCG */
#endif /* SENSOR_WDR_2_FRAME */

		memcpy((void *)(&(gc2063s1_attr.mipi)), (void *)(&gc2063s1_mipi_linear), sizeof(gc2063s1_attr.mipi));
		break;
	case 1:
...
static int gc2063s1_attr_check(struct tx_isp_subdev *sd)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	...
	gc2063s1_setting_select(sd, info->default_boot);

	switch (info->video_interface) {
	case TISP_SENSOR_VI_MIPI_CSI0:
		gc2063s1_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		gc2063s1_attr.mipi.index = 0;
		break;
	case TISP_SENSOR_VI_MIPI_CSI1:
		gc2063s1_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_MIPI;
		gc2063s1_attr.mipi.index = 1;
		break;
	case TISP_SENSOR_VI_DVP:
		gc2063s1_attr.dbus_type = TX_SENSOR_DATA_INTERFACE_DVP;
		break;
	default:
	...
	ret = gc2063s1_clk_set(sd, sclka, SENSOR_MCLK);
	if (ret) {
...
static int gc2063s1_g_chip_ident(struct tx_isp_subdev *sd,
			       struct tx_isp_chip_ident *chip)
{
	...
	gc2063s1_attr_check(sd);
#ifndef ZRT_SENSOR_WITHOUT_INIT
	if (info->rst_gpio != -1) {
		ret = private_gpio_request(info->rst_gpio, "gc2063s1_reset");
		if (!ret) {
	...
	if (info->pwdn_gpio != -1) {
		ret = private_gpio_request(info->pwdn_gpio, "gc2063s1_pwdn");
		if (!ret) {
...
static int gc2063s1_reset(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	return 0;
}

static int gc2063s1_init(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
	...
	if (!init->enable) {
		...
		sensor->video.fps.max = wsize->fps;
		sensor->video.fps.apply_delay = 1;
		ret = gc2063s1_attr_set(sd, wsize);
		sensor->video.state = TX_ISP_MODULE_DEINIT;
	}
...
static int gc2063s1_fsync(struct tx_isp_subdev *sd, struct tx_isp_frame_sync *sync)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
	uint8_t val;
	...
	gc2063s1_write(sd, 0x42, (ret_val & 0xff) + 8);
	gc2063s1_write(sd, 0xfe, 0x00);
	gc2063s1_write(sd, 0x7f, 0x09);
	gc2063s1_write(sd, 0x82, 0x01);
	gc2063s1_write(sd, 0x83, 0x7f);
	gc2063s1_write(sd, 0x84, 0x04);
#else
	/* slave */
	gc2063s1_read(sd, 0x41, &val);
	ret_val = val << 8;
...
static int gc2063s1_s_stream(struct tx_isp_subdev *sd, struct tx_isp_initarg *init)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	int ret = ISP_SUCCESS;
...
static int gc2063s1_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret += gc2063s1_write(sd, 0xb8, val_lut[again].regb8);
	ret += gc2063s1_write(sd, 0xb9, val_lut[again].regb9);
...
static int gc2063s1_set_fps(struct tx_isp_subdev *sd, int fps)
{
	...
	ret += gc2063s1_read(sd, 0x05, &val);
	hts = val << 8;
	val = 0;
	ret += gc2063s1_read(sd, 0x06, &val);
	hts |= val;
	if (0 != ret) {
		ISP_ERROR("err: gc2063s1 read err\n");
		return ret;
	}

	vts = sclk * (fps & 0xffff) / hts / ((fps & 0xffff0000) >> 16);

	gc2063s1_write(sd, 0x41, (unsigned char) (vts >> 8));
	gc2063s1_write(sd, 0x42, (unsigned char) (vts & 0xff));

	if (0 != ret) {
		ISP_ERROR("err: gc2063s1_write err\n");
		return ret;
	}
...
static int gc2063s1_set_hvflip(struct tx_isp_subdev *sd, void *arg)
{
	...
	ret += gc2063s1_write(sd, 0x17, val);
...
#ifdef SENSOR_PM
static int gc2063s1_pm(struct tx_isp_subdev *sd, void *arg)
{
	int ret = ISP_SUCCESS;
...
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
static int gc2063s1_set_expo_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
#else
static int gc2063s1_set_analog_gain_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int gc2063s1_set_integration_time_short(struct tx_isp_subdev *sd, int value)
{
	int ret = ISP_SUCCESS;
...
static int gc2063s1_set_wdr_stop(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = tx_isp_get_subdev_hostdata(sd);
	struct tx_isp_sensor_register_info *info = &sensor->info;
	...
	if (par->wdr_en == 1) {
		...
		gc2063s1_setting_select(sd, 1);
		gc2063s1_attr_set(sd, wsize);
	} else if (par->wdr_en == 0) {
		if (par->boot == -1) {
			info->default_boot = 0;
		}
		gc2063s1_setting_select(sd, 0);
		gc2063s1_attr_set(sd, wsize);
	} else {
...
static int gc2063s1_set_wdr(struct tx_isp_subdev *sd, void *arg)
{
	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	struct tx_isp_wdr_par *par = (struct tx_isp_wdr_par *)arg;
	struct tx_isp_sensor_register_info *info = &sensor->info;
...
static int gc2063s1_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_EXPO:
		if (arg)
			ret = gc2063s1_set_expo(sd, sensor_val->value);
		break;
#else
	case TX_ISP_EVENT_SENSOR_INT_TIME:
		if (arg)
			ret = gc2063s1_set_integration_time(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN:
		if (arg)
			ret = gc2063s1_set_analog_gain(sd, sensor_val->value);
		break;
#endif /* SENSOR_EXPO */
	case TX_ISP_EVENT_SENSOR_DGAIN:
		if (arg)
			ret = gc2063s1_set_digital_gain(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_BLACK_LEVEL:
		if (arg)
			ret = gc2063s1_get_black_pedestal(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_RESIZE:
		if (arg)
			ret = gc2063s1_set_mode(sd, sensor_val->value);
		break;
	...
	case TX_ISP_EVENT_SENSOR_FPS:
		if (arg)
			ret = gc2063s1_set_fps(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_HVFLIP:
		if(arg)
			ret = gc2063s1_set_hvflip(sd, (void *)sensor_val->value);
		break;
#ifdef SENSOR_PM
	case TX_ISP_EVENT_PM_S_STREAM_CTRL:
		if(arg)
			ret =  gc2063s1_pm(sd, arg);
		break;
#endif /* SENSOR_PM */
#ifdef SENSOR_WDR_2_FRAME
#ifdef SENSOR_EXPO
	case TX_ISP_EVENT_SENSOR_EXPO_SHORT:
		if (arg)
			ret = gc2063s1_set_expo_short(sd, sensor_val->value);
		break;
#else
	case TX_ISP_EVENT_SENSOR_INT_TIME_SHORT:
		if(arg)
			ret = gc2063s1_set_integration_time_short(sd, sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_AGAIN_SHORT:
		if(arg)
			ret = gc2063s1_set_analog_gain_short(sd, sensor_val->value);
		break;
#endif /* SENSOR_EXPO */
	case TX_ISP_EVENT_SENSOR_WDR:
		if(arg)
			ret = gc2063s1_set_wdr(sd, (void *)sensor_val->value);
		break;
	case TX_ISP_EVENT_SENSOR_WDR_STOP:
		if(arg)
			ret = gc2063s1_set_wdr_stop(sd, (void *)sensor_val->value);
		break;
#endif /* SENSOR_WDR_2_FRAME */
	default:
		break;
...
static int gc2063s1_probe(struct i2c_client *client,
...
static const struct i2c_device_id gc2063s1_id[] = {
	{"gc2063s1", 0},
	{}
};
...
#ifdef CONFIG_ZERATUL
static int init_first_gc2063s1(void) {
	return private_i2c_add_driver(&gc2063s1_driver);
}

static void exit_first_gc2063s1(void) {
	private_i2c_del_driver(&gc2063s1_driver);
}
...
struct tx_isp_sensor_fast_attr sensor1 = {
	...
	.wdr = 0,
#endif /* SENSOR_WDR_2_FRAME */
	.init_sensor = init_first_gc2063s1,
	.exit_sensor = exit_first_gc2063s1
};
#endif  /* CONFIG_ZERATUL */

MODULE_DESCRIPTION("A gc2063s1 low-level CIS driver");
MODULE_LICENSE("GPL");
