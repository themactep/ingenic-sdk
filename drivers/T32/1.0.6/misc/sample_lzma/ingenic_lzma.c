...
static int work_lzma(char *input_name, off_t in_file_size, char *output_name,char channel)
{
	...
	file_fp = (unsigned char *)__get_free_pages(GFP_KERNEL, get_order(in_file_size));
	//file_fp = kzalloc(in_file_size,GFP_KERNEL);
	if (file_fp <= 0)
	{
	...
	// printk("inputfile:%s,outputfile:%s,filesize:%ld,chan:%d,getorder:%d\n",input_name,output_name,in_file_size,channel,get_order(in_file_size));
	in_fp = filp_open(input_name, O_RDWR | O_CREAT , 0644);
	if (in_fp == NULL)
	{
	...
	// printk("filesize:%lld,pos:%lld\n",vfs_llseek(in_fp, 0, SEEK_END),in_fp->f_pos);
	vfs_llseek(in_fp, 0, SEEK_SET);

	fs = get_fs();
	set_fs(KERNEL_DS);

	dma_cache_sync(NULL, file_fp, (1<<get_order(in_file_size))*PAGE_SIZE, DMA_BIDIRECTIONAL);
	...
	file_fp_out = (unsigned char *)__get_free_pages(GFP_KERNEL, OUT_SIZE);
	if (file_fp_out <= 0)
	{
		free_pages((unsigned long)file_fp, OUT_SIZE);
	...
	while (lzma_readl(LZMA_CTRL,channel) & 0x00000001){
		if(cnt++ == 0){
			printk("idata0:%#x,idata1:%#x",lzma_readl(LZMA_RDATAH0_DBG,channel),lzma_readl(LZMA_RDATAL0_DBG,channel));
			printk("iaddr0:%#x,iaddr1:%#x",lzma_readl(LZMA_RADDR0_DBG,channel),lzma_readl(LZMA_RADDR1_DBG,channel));
		}
	}
	...
	dma_cache_sync(NULL, file_fp_out, (0x1 << OUT_SIZE) * PAGE_SIZE, DMA_BIDIRECTIONAL);
	...
	free_pages((unsigned long)file_fp, get_order(in_file_size));
	free_pages((unsigned long)file_fp_out, OUT_SIZE);
#ifdef FPGA_TEST
